<template>
  <div id="Readarea">{{province}} {{city}} {{area}}</div>
</template>
<script>
  export default{
    props: ['areaId','areaData','startAreaData'],
    data(){
      return {
        session: sessionStorage.getItem('session'),
        province: null,
        city: null,
        area: null,
        provinceData:[
          {
            "provinceID": 1,
            "divisionCode": "110000",
            "provinceName": "北京市"
          },
          {
            "provinceID": 2,
            "divisionCode": "120000",
            "provinceName": "天津市"
          },
          {
            "provinceID": 3,
            "divisionCode": "130000",
            "provinceName": "河北省"
          },
          {
            "provinceID": 4,
            "divisionCode": "140000",
            "provinceName": "山西省"
          },
          {
            "provinceID": 5,
            "divisionCode": "150000",
            "provinceName": "内蒙古自治区"
          },
          {
            "provinceID": 6,
            "divisionCode": "210000",
            "provinceName": "辽宁省"
          },
          {
            "provinceID": 7,
            "divisionCode": "220000",
            "provinceName": "吉林省"
          },
          {
            "provinceID": 8,
            "divisionCode": "230000",
            "provinceName": "黑龙江省"
          },
          {
            "provinceID": 9,
            "divisionCode": "310000",
            "provinceName": "上海市"
          },
          {
            "provinceID": 10,
            "divisionCode": "320000",
            "provinceName": "江苏省"
          },
          {
            "provinceID": 11,
            "divisionCode": "330000",
            "provinceName": "浙江省"
          },
          {
            "provinceID": 12,
            "divisionCode": "340000",
            "provinceName": "安徽省"
          },
          {
            "provinceID": 13,
            "divisionCode": "350000",
            "provinceName": "福建省"
          },
          {
            "provinceID": 14,
            "divisionCode": "360000",
            "provinceName": "江西省"
          },
          {
            "provinceID": 15,
            "divisionCode": "370000",
            "provinceName": "山东省"
          },
          {
            "provinceID": 16,
            "divisionCode": "410000",
            "provinceName": "河南省"
          },
          {
            "provinceID": 17,
            "divisionCode": "420000",
            "provinceName": "湖北省"
          },
          {
            "provinceID": 18,
            "divisionCode": "430000",
            "provinceName": "湖南省"
          },
          {
            "provinceID": 19,
            "divisionCode": "440000",
            "provinceName": "广东省"
          },
          {
            "provinceID": 20,
            "divisionCode": "450000",
            "provinceName": "广西壮族自治区"
          },
          {
            "provinceID": 21,
            "divisionCode": "460000",
            "provinceName": "海南省"
          },
          {
            "provinceID": 22,
            "divisionCode": "500000",
            "provinceName": "重庆市"
          },
          {
            "provinceID": 23,
            "divisionCode": "510000",
            "provinceName": "四川省"
          },
          {
            "provinceID": 24,
            "divisionCode": "520000",
            "provinceName": "贵州省"
          },
          {
            "provinceID": 25,
            "divisionCode": "530000",
            "provinceName": "云南省"
          },
          {
            "provinceID": 26,
            "divisionCode": "540000",
            "provinceName": "西藏自治区"
          },
          {
            "provinceID": 27,
            "divisionCode": "610000",
            "provinceName": "陕西省"
          },
          {
            "provinceID": 28,
            "divisionCode": "620000",
            "provinceName": "甘肃省"
          },
          {
            "provinceID": 29,
            "divisionCode": "630000",
            "provinceName": "青海省"
          },
          {
            "provinceID": 30,
            "divisionCode": "640000",
            "provinceName": "宁夏回族自治区"
          },
          {
            "provinceID": 31,
            "divisionCode": "650000",
            "provinceName": "新疆维吾尔自治区"
          },
          {
            "provinceID": 32,
            "divisionCode": "710000",
            "provinceName": "台湾省"
          },
          {
            "provinceID": 33,
            "divisionCode": "810000",
            "provinceName": "香港特别行政区"
          },
          {
            "provinceID": 34,
            "divisionCode": "820000",
            "provinceName": "澳门特别行政区"
          }
        ],
        cityData:[
          {
            "cityID": 1,
            "provinceID": 1,
            "divisionCode": "110100",
            "cityName": "北京市"
          },
          {
            "cityID": 2,
            "provinceID": 2,
            "divisionCode": "120100",
            "cityName": "天津市"
          },
          {
            "cityID": 3,
            "provinceID": 3,
            "divisionCode": "130100",
            "cityName": "石家庄市"
          },
          {
            "cityID": 4,
            "provinceID": 3,
            "divisionCode": "130200",
            "cityName": "唐山市"
          },
          {
            "cityID": 5,
            "provinceID": 3,
            "divisionCode": "130300",
            "cityName": "秦皇岛市"
          },
          {
            "cityID": 6,
            "provinceID": 3,
            "divisionCode": "130400",
            "cityName": "邯郸市"
          },
          {
            "cityID": 7,
            "provinceID": 3,
            "divisionCode": "130500",
            "cityName": "邢台市"
          },
          {
            "cityID": 8,
            "provinceID": 3,
            "divisionCode": "130600",
            "cityName": "保定市"
          },
          {
            "cityID": 9,
            "provinceID": 3,
            "divisionCode": "130700",
            "cityName": "张家口市"
          },
          {
            "cityID": 10,
            "provinceID": 3,
            "divisionCode": "130800",
            "cityName": "承德市"
          },
          {
            "cityID": 11,
            "provinceID": 3,
            "divisionCode": "130900",
            "cityName": "沧州市"
          },
          {
            "cityID": 12,
            "provinceID": 3,
            "divisionCode": "131000",
            "cityName": "廊坊市"
          },
          {
            "cityID": 13,
            "provinceID": 3,
            "divisionCode": "131100",
            "cityName": "衡水市"
          },
          {
            "cityID": 14,
            "provinceID": 3,
            "divisionCode": "139000",
            "cityName": "省直辖县级行政区划"
          },
          {
            "cityID": 15,
            "provinceID": 4,
            "divisionCode": "140100",
            "cityName": "太原市"
          },
          {
            "cityID": 16,
            "provinceID": 4,
            "divisionCode": "140200",
            "cityName": "大同市"
          },
          {
            "cityID": 17,
            "provinceID": 4,
            "divisionCode": "140300",
            "cityName": "阳泉市"
          },
          {
            "cityID": 18,
            "provinceID": 4,
            "divisionCode": "140400",
            "cityName": "长治市"
          },
          {
            "cityID": 19,
            "provinceID": 4,
            "divisionCode": "140500",
            "cityName": "晋城市"
          },
          {
            "cityID": 20,
            "provinceID": 4,
            "divisionCode": "140600",
            "cityName": "朔州市"
          },
          {
            "cityID": 21,
            "provinceID": 4,
            "divisionCode": "140700",
            "cityName": "晋中市"
          },
          {
            "cityID": 22,
            "provinceID": 4,
            "divisionCode": "140800",
            "cityName": "运城市"
          },
          {
            "cityID": 23,
            "provinceID": 4,
            "divisionCode": "140900",
            "cityName": "忻州市"
          },
          {
            "cityID": 24,
            "provinceID": 4,
            "divisionCode": "141000",
            "cityName": "临汾市"
          },
          {
            "cityID": 25,
            "provinceID": 4,
            "divisionCode": "141100",
            "cityName": "吕梁市"
          },
          {
            "cityID": 26,
            "provinceID": 5,
            "divisionCode": "150100",
            "cityName": "呼和浩特市"
          },
          {
            "cityID": 27,
            "provinceID": 5,
            "divisionCode": "150200",
            "cityName": "包头市"
          },
          {
            "cityID": 28,
            "provinceID": 5,
            "divisionCode": "150300",
            "cityName": "乌海市"
          },
          {
            "cityID": 29,
            "provinceID": 5,
            "divisionCode": "150400",
            "cityName": "赤峰市"
          },
          {
            "cityID": 30,
            "provinceID": 5,
            "divisionCode": "150500",
            "cityName": "通辽市"
          },
          {
            "cityID": 31,
            "provinceID": 5,
            "divisionCode": "150600",
            "cityName": "鄂尔多斯市"
          },
          {
            "cityID": 32,
            "provinceID": 5,
            "divisionCode": "150700",
            "cityName": "呼伦贝尔市"
          },
          {
            "cityID": 33,
            "provinceID": 5,
            "divisionCode": "150800",
            "cityName": "巴彦淖尔市"
          },
          {
            "cityID": 34,
            "provinceID": 5,
            "divisionCode": "150900",
            "cityName": "乌兰察布市"
          },
          {
            "cityID": 35,
            "provinceID": 5,
            "divisionCode": "152200",
            "cityName": "兴安盟"
          },
          {
            "cityID": 36,
            "provinceID": 5,
            "divisionCode": "152500",
            "cityName": "锡林郭勒盟"
          },
          {
            "cityID": 37,
            "provinceID": 5,
            "divisionCode": "152900",
            "cityName": "阿拉善盟"
          },
          {
            "cityID": 38,
            "provinceID": 6,
            "divisionCode": "210100",
            "cityName": "沈阳市"
          },
          {
            "cityID": 39,
            "provinceID": 6,
            "divisionCode": "210200",
            "cityName": "大连市"
          },
          {
            "cityID": 40,
            "provinceID": 6,
            "divisionCode": "210300",
            "cityName": "鞍山市"
          },
          {
            "cityID": 41,
            "provinceID": 6,
            "divisionCode": "210400",
            "cityName": "抚顺市"
          },
          {
            "cityID": 42,
            "provinceID": 6,
            "divisionCode": "210500",
            "cityName": "本溪市"
          },
          {
            "cityID": 43,
            "provinceID": 6,
            "divisionCode": "210600",
            "cityName": "丹东市"
          },
          {
            "cityID": 44,
            "provinceID": 6,
            "divisionCode": "210700",
            "cityName": "锦州市"
          },
          {
            "cityID": 45,
            "provinceID": 6,
            "divisionCode": "210800",
            "cityName": "营口市"
          },
          {
            "cityID": 46,
            "provinceID": 6,
            "divisionCode": "210900",
            "cityName": "阜新市"
          },
          {
            "cityID": 47,
            "provinceID": 6,
            "divisionCode": "211000",
            "cityName": "辽阳市"
          },
          {
            "cityID": 48,
            "provinceID": 6,
            "divisionCode": "211100",
            "cityName": "盘锦市"
          },
          {
            "cityID": 49,
            "provinceID": 6,
            "divisionCode": "211200",
            "cityName": "铁岭市"
          },
          {
            "cityID": 50,
            "provinceID": 6,
            "divisionCode": "211300",
            "cityName": "朝阳市"
          },
          {
            "cityID": 51,
            "provinceID": 6,
            "divisionCode": "211400",
            "cityName": "葫芦岛市"
          },
          {
            "cityID": 52,
            "provinceID": 7,
            "divisionCode": "220100",
            "cityName": "长春市"
          },
          {
            "cityID": 53,
            "provinceID": 7,
            "divisionCode": "220200",
            "cityName": "吉林市"
          },
          {
            "cityID": 54,
            "provinceID": 7,
            "divisionCode": "220300",
            "cityName": "四平市"
          },
          {
            "cityID": 55,
            "provinceID": 7,
            "divisionCode": "220400",
            "cityName": "辽源市"
          },
          {
            "cityID": 56,
            "provinceID": 7,
            "divisionCode": "220500",
            "cityName": "通化市"
          },
          {
            "cityID": 57,
            "provinceID": 7,
            "divisionCode": "220600",
            "cityName": "白山市"
          },
          {
            "cityID": 58,
            "provinceID": 7,
            "divisionCode": "220700",
            "cityName": "松原市"
          },
          {
            "cityID": 59,
            "provinceID": 7,
            "divisionCode": "220800",
            "cityName": "白城市"
          },
          {
            "cityID": 60,
            "provinceID": 7,
            "divisionCode": "222400",
            "cityName": "延边朝鲜族自治州"
          },
          {
            "cityID": 61,
            "provinceID": 8,
            "divisionCode": "230100",
            "cityName": "哈尔滨市"
          },
          {
            "cityID": 62,
            "provinceID": 8,
            "divisionCode": "230200",
            "cityName": "齐齐哈尔市"
          },
          {
            "cityID": 63,
            "provinceID": 8,
            "divisionCode": "230300",
            "cityName": "鸡西市"
          },
          {
            "cityID": 64,
            "provinceID": 8,
            "divisionCode": "230400",
            "cityName": "鹤岗市"
          },
          {
            "cityID": 65,
            "provinceID": 8,
            "divisionCode": "230500",
            "cityName": "双鸭山市"
          },
          {
            "cityID": 66,
            "provinceID": 8,
            "divisionCode": "230600",
            "cityName": "大庆市"
          },
          {
            "cityID": 67,
            "provinceID": 8,
            "divisionCode": "230700",
            "cityName": "伊春市"
          },
          {
            "cityID": 68,
            "provinceID": 8,
            "divisionCode": "230800",
            "cityName": "佳木斯市"
          },
          {
            "cityID": 69,
            "provinceID": 8,
            "divisionCode": "230900",
            "cityName": "七台河市"
          },
          {
            "cityID": 70,
            "provinceID": 8,
            "divisionCode": "231000",
            "cityName": "牡丹江市"
          },
          {
            "cityID": 71,
            "provinceID": 8,
            "divisionCode": "231100",
            "cityName": "黑河市"
          },
          {
            "cityID": 72,
            "provinceID": 8,
            "divisionCode": "231200",
            "cityName": "绥化市"
          },
          {
            "cityID": 73,
            "provinceID": 8,
            "divisionCode": "232700",
            "cityName": "大兴安岭地区"
          },
          {
            "cityID": 74,
            "provinceID": 9,
            "divisionCode": "310100",
            "cityName": "市辖区"
          },
          {
            "cityID": 75,
            "provinceID": 10,
            "divisionCode": "320100",
            "cityName": "南京市"
          },
          {
            "cityID": 76,
            "provinceID": 10,
            "divisionCode": "320200",
            "cityName": "无锡市"
          },
          {
            "cityID": 77,
            "provinceID": 10,
            "divisionCode": "320300",
            "cityName": "徐州市"
          },
          {
            "cityID": 78,
            "provinceID": 10,
            "divisionCode": "320400",
            "cityName": "常州市"
          },
          {
            "cityID": 79,
            "provinceID": 10,
            "divisionCode": "320500",
            "cityName": "苏州市"
          },
          {
            "cityID": 80,
            "provinceID": 10,
            "divisionCode": "320600",
            "cityName": "南通市"
          },
          {
            "cityID": 81,
            "provinceID": 10,
            "divisionCode": "320700",
            "cityName": "连云港市"
          },
          {
            "cityID": 82,
            "provinceID": 10,
            "divisionCode": "320800",
            "cityName": "淮安市"
          },
          {
            "cityID": 83,
            "provinceID": 10,
            "divisionCode": "320900",
            "cityName": "盐城市"
          },
          {
            "cityID": 84,
            "provinceID": 10,
            "divisionCode": "321000",
            "cityName": "扬州市"
          },
          {
            "cityID": 85,
            "provinceID": 10,
            "divisionCode": "321100",
            "cityName": "镇江市"
          },
          {
            "cityID": 86,
            "provinceID": 10,
            "divisionCode": "321200",
            "cityName": "泰州市"
          },
          {
            "cityID": 87,
            "provinceID": 10,
            "divisionCode": "321300",
            "cityName": "宿迁市"
          },
          {
            "cityID": 88,
            "provinceID": 11,
            "divisionCode": "330100",
            "cityName": "杭州市"
          },
          {
            "cityID": 89,
            "provinceID": 11,
            "divisionCode": "330200",
            "cityName": "宁波市"
          },
          {
            "cityID": 90,
            "provinceID": 11,
            "divisionCode": "330300",
            "cityName": "温州市"
          },
          {
            "cityID": 91,
            "provinceID": 11,
            "divisionCode": "330400",
            "cityName": "嘉兴市"
          },
          {
            "cityID": 92,
            "provinceID": 11,
            "divisionCode": "330500",
            "cityName": "湖州市"
          },
          {
            "cityID": 93,
            "provinceID": 11,
            "divisionCode": "330600",
            "cityName": "绍兴市"
          },
          {
            "cityID": 94,
            "provinceID": 11,
            "divisionCode": "330700",
            "cityName": "金华市"
          },
          {
            "cityID": 95,
            "provinceID": 11,
            "divisionCode": "330800",
            "cityName": "衢州市"
          },
          {
            "cityID": 96,
            "provinceID": 11,
            "divisionCode": "330900",
            "cityName": "舟山市"
          },
          {
            "cityID": 97,
            "provinceID": 11,
            "divisionCode": "331000",
            "cityName": "台州市"
          },
          {
            "cityID": 98,
            "provinceID": 11,
            "divisionCode": "331100",
            "cityName": "丽水市"
          },
          {
            "cityID": 99,
            "provinceID": 12,
            "divisionCode": "340100",
            "cityName": "合肥市"
          },
          {
            "cityID": 100,
            "provinceID": 12,
            "divisionCode": "340200",
            "cityName": "芜湖市"
          },
          {
            "cityID": 101,
            "provinceID": 12,
            "divisionCode": "340300",
            "cityName": "蚌埠市"
          },
          {
            "cityID": 102,
            "provinceID": 12,
            "divisionCode": "340400",
            "cityName": "淮南市"
          },
          {
            "cityID": 103,
            "provinceID": 12,
            "divisionCode": "340500",
            "cityName": "马鞍山市"
          },
          {
            "cityID": 104,
            "provinceID": 12,
            "divisionCode": "340600",
            "cityName": "淮北市"
          },
          {
            "cityID": 105,
            "provinceID": 12,
            "divisionCode": "340700",
            "cityName": "铜陵市"
          },
          {
            "cityID": 106,
            "provinceID": 12,
            "divisionCode": "340800",
            "cityName": "安庆市"
          },
          {
            "cityID": 107,
            "provinceID": 12,
            "divisionCode": "341000",
            "cityName": "黄山市"
          },
          {
            "cityID": 108,
            "provinceID": 12,
            "divisionCode": "341100",
            "cityName": "滁州市"
          },
          {
            "cityID": 109,
            "provinceID": 12,
            "divisionCode": "341200",
            "cityName": "阜阳市"
          },
          {
            "cityID": 110,
            "provinceID": 12,
            "divisionCode": "341300",
            "cityName": "宿州市"
          },
          {
            "cityID": 111,
            "provinceID": 12,
            "divisionCode": "341500",
            "cityName": "六安市"
          },
          {
            "cityID": 112,
            "provinceID": 12,
            "divisionCode": "341600",
            "cityName": "亳州市"
          },
          {
            "cityID": 113,
            "provinceID": 12,
            "divisionCode": "341700",
            "cityName": "池州市"
          },
          {
            "cityID": 114,
            "provinceID": 12,
            "divisionCode": "341800",
            "cityName": "宣城市"
          },
          {
            "cityID": 115,
            "provinceID": 13,
            "divisionCode": "350100",
            "cityName": "福州市"
          },
          {
            "cityID": 116,
            "provinceID": 13,
            "divisionCode": "350200",
            "cityName": "厦门市"
          },
          {
            "cityID": 117,
            "provinceID": 13,
            "divisionCode": "350300",
            "cityName": "莆田市"
          },
          {
            "cityID": 118,
            "provinceID": 13,
            "divisionCode": "350400",
            "cityName": "三明市"
          },
          {
            "cityID": 119,
            "provinceID": 13,
            "divisionCode": "350500",
            "cityName": "泉州市"
          },
          {
            "cityID": 120,
            "provinceID": 13,
            "divisionCode": "350600",
            "cityName": "漳州市"
          },
          {
            "cityID": 121,
            "provinceID": 13,
            "divisionCode": "350700",
            "cityName": "南平市"
          },
          {
            "cityID": 122,
            "provinceID": 13,
            "divisionCode": "350800",
            "cityName": "龙岩市"
          },
          {
            "cityID": 123,
            "provinceID": 13,
            "divisionCode": "350900",
            "cityName": "宁德市"
          },
          {
            "cityID": 124,
            "provinceID": 14,
            "divisionCode": "360100",
            "cityName": "南昌市"
          },
          {
            "cityID": 125,
            "provinceID": 14,
            "divisionCode": "360200",
            "cityName": "景德镇市"
          },
          {
            "cityID": 126,
            "provinceID": 14,
            "divisionCode": "360300",
            "cityName": "萍乡市"
          },
          {
            "cityID": 127,
            "provinceID": 14,
            "divisionCode": "360400",
            "cityName": "九江市"
          },
          {
            "cityID": 128,
            "provinceID": 14,
            "divisionCode": "360500",
            "cityName": "新余市"
          },
          {
            "cityID": 129,
            "provinceID": 14,
            "divisionCode": "360600",
            "cityName": "鹰潭市"
          },
          {
            "cityID": 130,
            "provinceID": 14,
            "divisionCode": "360700",
            "cityName": "赣州市"
          },
          {
            "cityID": 131,
            "provinceID": 14,
            "divisionCode": "360800",
            "cityName": "吉安市"
          },
          {
            "cityID": 132,
            "provinceID": 14,
            "divisionCode": "360900",
            "cityName": "宜春市"
          },
          {
            "cityID": 133,
            "provinceID": 14,
            "divisionCode": "361000",
            "cityName": "抚州市"
          },
          {
            "cityID": 134,
            "provinceID": 14,
            "divisionCode": "361100",
            "cityName": "上饶市"
          },
          {
            "cityID": 135,
            "provinceID": 15,
            "divisionCode": "370100",
            "cityName": "济南市"
          },
          {
            "cityID": 136,
            "provinceID": 15,
            "divisionCode": "370200",
            "cityName": "青岛市"
          },
          {
            "cityID": 137,
            "provinceID": 15,
            "divisionCode": "370300",
            "cityName": "淄博市"
          },
          {
            "cityID": 138,
            "provinceID": 15,
            "divisionCode": "370400",
            "cityName": "枣庄市"
          },
          {
            "cityID": 139,
            "provinceID": 15,
            "divisionCode": "370500",
            "cityName": "东营市"
          },
          {
            "cityID": 140,
            "provinceID": 15,
            "divisionCode": "370600",
            "cityName": "烟台市"
          },
          {
            "cityID": 141,
            "provinceID": 15,
            "divisionCode": "370700",
            "cityName": "潍坊市"
          },
          {
            "cityID": 142,
            "provinceID": 15,
            "divisionCode": "370800",
            "cityName": "济宁市"
          },
          {
            "cityID": 143,
            "provinceID": 15,
            "divisionCode": "370900",
            "cityName": "泰安市"
          },
          {
            "cityID": 144,
            "provinceID": 15,
            "divisionCode": "371000",
            "cityName": "威海市"
          },
          {
            "cityID": 145,
            "provinceID": 15,
            "divisionCode": "371100",
            "cityName": "日照市"
          },
          {
            "cityID": 146,
            "provinceID": 15,
            "divisionCode": "371200",
            "cityName": "莱芜市"
          },
          {
            "cityID": 147,
            "provinceID": 15,
            "divisionCode": "371300",
            "cityName": "临沂市"
          },
          {
            "cityID": 148,
            "provinceID": 15,
            "divisionCode": "371400",
            "cityName": "德州市"
          },
          {
            "cityID": 149,
            "provinceID": 15,
            "divisionCode": "371500",
            "cityName": "聊城市"
          },
          {
            "cityID": 150,
            "provinceID": 15,
            "divisionCode": "371600",
            "cityName": "滨州市"
          },
          {
            "cityID": 151,
            "provinceID": 15,
            "divisionCode": "371700",
            "cityName": "菏泽市"
          },
          {
            "cityID": 152,
            "provinceID": 16,
            "divisionCode": "410100",
            "cityName": "郑州市"
          },
          {
            "cityID": 153,
            "provinceID": 16,
            "divisionCode": "410200",
            "cityName": "开封市"
          },
          {
            "cityID": 154,
            "provinceID": 16,
            "divisionCode": "410300",
            "cityName": "洛阳市"
          },
          {
            "cityID": 155,
            "provinceID": 16,
            "divisionCode": "410400",
            "cityName": "平顶山市"
          },
          {
            "cityID": 156,
            "provinceID": 16,
            "divisionCode": "410500",
            "cityName": "安阳市"
          },
          {
            "cityID": 157,
            "provinceID": 16,
            "divisionCode": "410600",
            "cityName": "鹤壁市"
          },
          {
            "cityID": 158,
            "provinceID": 16,
            "divisionCode": "410700",
            "cityName": "新乡市"
          },
          {
            "cityID": 159,
            "provinceID": 16,
            "divisionCode": "410800",
            "cityName": "焦作市"
          },
          {
            "cityID": 160,
            "provinceID": 16,
            "divisionCode": "410900",
            "cityName": "濮阳市"
          },
          {
            "cityID": 161,
            "provinceID": 16,
            "divisionCode": "411000",
            "cityName": "许昌市"
          },
          {
            "cityID": 162,
            "provinceID": 16,
            "divisionCode": "411100",
            "cityName": "漯河市"
          },
          {
            "cityID": 163,
            "provinceID": 16,
            "divisionCode": "411200",
            "cityName": "三门峡市"
          },
          {
            "cityID": 164,
            "provinceID": 16,
            "divisionCode": "411300",
            "cityName": "南阳市"
          },
          {
            "cityID": 165,
            "provinceID": 16,
            "divisionCode": "411400",
            "cityName": "商丘市"
          },
          {
            "cityID": 166,
            "provinceID": 16,
            "divisionCode": "411500",
            "cityName": "信阳市"
          },
          {
            "cityID": 167,
            "provinceID": 16,
            "divisionCode": "411600",
            "cityName": "周口市"
          },
          {
            "cityID": 168,
            "provinceID": 16,
            "divisionCode": "411700",
            "cityName": "驻马店市"
          },
          {
            "cityID": 169,
            "provinceID": 16,
            "divisionCode": "419000",
            "cityName": "省直辖县级行政区划"
          },
          {
            "cityID": 170,
            "provinceID": 17,
            "divisionCode": "420100",
            "cityName": "武汉市"
          },
          {
            "cityID": 171,
            "provinceID": 17,
            "divisionCode": "420200",
            "cityName": "黄石市"
          },
          {
            "cityID": 172,
            "provinceID": 17,
            "divisionCode": "420300",
            "cityName": "十堰市"
          },
          {
            "cityID": 173,
            "provinceID": 17,
            "divisionCode": "420500",
            "cityName": "宜昌市"
          },
          {
            "cityID": 174,
            "provinceID": 17,
            "divisionCode": "420600",
            "cityName": "襄阳市"
          },
          {
            "cityID": 175,
            "provinceID": 17,
            "divisionCode": "420700",
            "cityName": "鄂州市"
          },
          {
            "cityID": 176,
            "provinceID": 17,
            "divisionCode": "420800",
            "cityName": "荆门市"
          },
          {
            "cityID": 177,
            "provinceID": 17,
            "divisionCode": "420900",
            "cityName": "孝感市"
          },
          {
            "cityID": 178,
            "provinceID": 17,
            "divisionCode": "421000",
            "cityName": "荆州市"
          },
          {
            "cityID": 179,
            "provinceID": 17,
            "divisionCode": "421100",
            "cityName": "黄冈市"
          },
          {
            "cityID": 180,
            "provinceID": 17,
            "divisionCode": "421200",
            "cityName": "咸宁市"
          },
          {
            "cityID": 181,
            "provinceID": 17,
            "divisionCode": "421300",
            "cityName": "随州市"
          },
          {
            "cityID": 182,
            "provinceID": 17,
            "divisionCode": "422800",
            "cityName": "恩施土家族苗族自治州"
          },
          {
            "cityID": 183,
            "provinceID": 17,
            "divisionCode": "429000",
            "cityName": "省直辖县级行政区划"
          },
          {
            "cityID": 184,
            "provinceID": 18,
            "divisionCode": "430100",
            "cityName": "长沙市"
          },
          {
            "cityID": 185,
            "provinceID": 18,
            "divisionCode": "430200",
            "cityName": "株洲市"
          },
          {
            "cityID": 186,
            "provinceID": 18,
            "divisionCode": "430300",
            "cityName": "湘潭市"
          },
          {
            "cityID": 187,
            "provinceID": 18,
            "divisionCode": "430400",
            "cityName": "衡阳市"
          },
          {
            "cityID": 188,
            "provinceID": 18,
            "divisionCode": "430500",
            "cityName": "邵阳市"
          },
          {
            "cityID": 189,
            "provinceID": 18,
            "divisionCode": "430600",
            "cityName": "岳阳市"
          },
          {
            "cityID": 190,
            "provinceID": 18,
            "divisionCode": "430700",
            "cityName": "常德市"
          },
          {
            "cityID": 191,
            "provinceID": 18,
            "divisionCode": "430800",
            "cityName": "张家界市"
          },
          {
            "cityID": 192,
            "provinceID": 18,
            "divisionCode": "430900",
            "cityName": "益阳市"
          },
          {
            "cityID": 193,
            "provinceID": 18,
            "divisionCode": "431000",
            "cityName": "郴州市"
          },
          {
            "cityID": 194,
            "provinceID": 18,
            "divisionCode": "431100",
            "cityName": "永州市"
          },
          {
            "cityID": 195,
            "provinceID": 18,
            "divisionCode": "431200",
            "cityName": "怀化市"
          },
          {
            "cityID": 196,
            "provinceID": 18,
            "divisionCode": "431300",
            "cityName": "娄底市"
          },
          {
            "cityID": 197,
            "provinceID": 18,
            "divisionCode": "433100",
            "cityName": "湘西土家族苗族自治州"
          },
          {
            "cityID": 198,
            "provinceID": 19,
            "divisionCode": "440100",
            "cityName": "广州市"
          },
          {
            "cityID": 199,
            "provinceID": 19,
            "divisionCode": "440200",
            "cityName": "韶关市"
          },
          {
            "cityID": 200,
            "provinceID": 19,
            "divisionCode": "440300",
            "cityName": "深圳市"
          },
          {
            "cityID": 201,
            "provinceID": 19,
            "divisionCode": "440400",
            "cityName": "珠海市"
          },
          {
            "cityID": 202,
            "provinceID": 19,
            "divisionCode": "440500",
            "cityName": "汕头市"
          },
          {
            "cityID": 203,
            "provinceID": 19,
            "divisionCode": "440600",
            "cityName": "佛山市"
          },
          {
            "cityID": 204,
            "provinceID": 19,
            "divisionCode": "440700",
            "cityName": "江门市"
          },
          {
            "cityID": 205,
            "provinceID": 19,
            "divisionCode": "440800",
            "cityName": "湛江市"
          },
          {
            "cityID": 206,
            "provinceID": 19,
            "divisionCode": "440900",
            "cityName": "茂名市"
          },
          {
            "cityID": 207,
            "provinceID": 19,
            "divisionCode": "441200",
            "cityName": "肇庆市"
          },
          {
            "cityID": 208,
            "provinceID": 19,
            "divisionCode": "441300",
            "cityName": "惠州市"
          },
          {
            "cityID": 209,
            "provinceID": 19,
            "divisionCode": "441400",
            "cityName": "梅州市"
          },
          {
            "cityID": 210,
            "provinceID": 19,
            "divisionCode": "441500",
            "cityName": "汕尾市"
          },
          {
            "cityID": 211,
            "provinceID": 19,
            "divisionCode": "441600",
            "cityName": "河源市"
          },
          {
            "cityID": 212,
            "provinceID": 19,
            "divisionCode": "441700",
            "cityName": "阳江市"
          },
          {
            "cityID": 213,
            "provinceID": 19,
            "divisionCode": "441800",
            "cityName": "清远市"
          },
          {
            "cityID": 214,
            "provinceID": 19,
            "divisionCode": "441900",
            "cityName": "东莞市"
          },
          {
            "cityID": 215,
            "provinceID": 19,
            "divisionCode": "442000",
            "cityName": "中山市"
          },
          {
            "cityID": 216,
            "provinceID": 19,
            "divisionCode": "445100",
            "cityName": "潮州市"
          },
          {
            "cityID": 217,
            "provinceID": 19,
            "divisionCode": "445200",
            "cityName": "揭阳市"
          },
          {
            "cityID": 218,
            "provinceID": 19,
            "divisionCode": "445300",
            "cityName": "云浮市"
          },
          {
            "cityID": 219,
            "provinceID": 20,
            "divisionCode": "450100",
            "cityName": "南宁市"
          },
          {
            "cityID": 220,
            "provinceID": 20,
            "divisionCode": "450200",
            "cityName": "柳州市"
          },
          {
            "cityID": 221,
            "provinceID": 20,
            "divisionCode": "450300",
            "cityName": "桂林市"
          },
          {
            "cityID": 222,
            "provinceID": 20,
            "divisionCode": "450400",
            "cityName": "梧州市"
          },
          {
            "cityID": 223,
            "provinceID": 20,
            "divisionCode": "450500",
            "cityName": "北海市"
          },
          {
            "cityID": 224,
            "provinceID": 20,
            "divisionCode": "450600",
            "cityName": "防城港市"
          },
          {
            "cityID": 225,
            "provinceID": 20,
            "divisionCode": "450700",
            "cityName": "钦州市"
          },
          {
            "cityID": 226,
            "provinceID": 20,
            "divisionCode": "450800",
            "cityName": "贵港市"
          },
          {
            "cityID": 227,
            "provinceID": 20,
            "divisionCode": "450900",
            "cityName": "玉林市"
          },
          {
            "cityID": 228,
            "provinceID": 20,
            "divisionCode": "451000",
            "cityName": "百色市"
          },
          {
            "cityID": 229,
            "provinceID": 20,
            "divisionCode": "451100",
            "cityName": "贺州市"
          },
          {
            "cityID": 230,
            "provinceID": 20,
            "divisionCode": "451200",
            "cityName": "河池市"
          },
          {
            "cityID": 231,
            "provinceID": 20,
            "divisionCode": "451300",
            "cityName": "来宾市"
          },
          {
            "cityID": 232,
            "provinceID": 20,
            "divisionCode": "451400",
            "cityName": "崇左市"
          },
          {
            "cityID": 233,
            "provinceID": 21,
            "divisionCode": "460100",
            "cityName": "海口市"
          },
          {
            "cityID": 234,
            "provinceID": 21,
            "divisionCode": "460200",
            "cityName": "三亚市"
          },
          {
            "cityID": 235,
            "provinceID": 21,
            "divisionCode": "460300",
            "cityName": "三沙市"
          },
          {
            "cityID": 236,
            "provinceID": 21,
            "divisionCode": "460400",
            "cityName": "儋州市"
          },
          {
            "cityID": 237,
            "provinceID": 21,
            "divisionCode": "469000",
            "cityName": "省直辖县级行政区划"
          },
          {
            "cityID": 238,
            "provinceID": 22,
            "divisionCode": "500100",
            "cityName": "市辖区"
          },
          {
            "cityID": 239,
            "provinceID": 22,
            "divisionCode": "500200",
            "cityName": "县"
          },
          {
            "cityID": 240,
            "provinceID": 23,
            "divisionCode": "510100",
            "cityName": "成都市"
          },
          {
            "cityID": 241,
            "provinceID": 23,
            "divisionCode": "510300",
            "cityName": "自贡市"
          },
          {
            "cityID": 242,
            "provinceID": 23,
            "divisionCode": "510400",
            "cityName": "攀枝花市"
          },
          {
            "cityID": 243,
            "provinceID": 23,
            "divisionCode": "510500",
            "cityName": "泸州市"
          },
          {
            "cityID": 244,
            "provinceID": 23,
            "divisionCode": "510600",
            "cityName": "德阳市"
          },
          {
            "cityID": 245,
            "provinceID": 23,
            "divisionCode": "510700",
            "cityName": "绵阳市"
          },
          {
            "cityID": 246,
            "provinceID": 23,
            "divisionCode": "510800",
            "cityName": "广元市"
          },
          {
            "cityID": 247,
            "provinceID": 23,
            "divisionCode": "510900",
            "cityName": "遂宁市"
          },
          {
            "cityID": 248,
            "provinceID": 23,
            "divisionCode": "511000",
            "cityName": "内江市"
          },
          {
            "cityID": 249,
            "provinceID": 23,
            "divisionCode": "511100",
            "cityName": "乐山市"
          },
          {
            "cityID": 250,
            "provinceID": 23,
            "divisionCode": "511300",
            "cityName": "南充市"
          },
          {
            "cityID": 251,
            "provinceID": 23,
            "divisionCode": "511400",
            "cityName": "眉山市"
          },
          {
            "cityID": 252,
            "provinceID": 23,
            "divisionCode": "511500",
            "cityName": "宜宾市"
          },
          {
            "cityID": 253,
            "provinceID": 23,
            "divisionCode": "511600",
            "cityName": "广安市"
          },
          {
            "cityID": 254,
            "provinceID": 23,
            "divisionCode": "511700",
            "cityName": "达州市"
          },
          {
            "cityID": 255,
            "provinceID": 23,
            "divisionCode": "511800",
            "cityName": "雅安市"
          },
          {
            "cityID": 256,
            "provinceID": 23,
            "divisionCode": "511900",
            "cityName": "巴中市"
          },
          {
            "cityID": 257,
            "provinceID": 23,
            "divisionCode": "512000",
            "cityName": "资阳市"
          },
          {
            "cityID": 258,
            "provinceID": 23,
            "divisionCode": "513200",
            "cityName": "阿坝藏族羌族自治州"
          },
          {
            "cityID": 259,
            "provinceID": 23,
            "divisionCode": "513300",
            "cityName": "甘孜藏族自治州"
          },
          {
            "cityID": 260,
            "provinceID": 23,
            "divisionCode": "513400",
            "cityName": "凉山彝族自治州"
          },
          {
            "cityID": 261,
            "provinceID": 24,
            "divisionCode": "520100",
            "cityName": "贵阳市"
          },
          {
            "cityID": 262,
            "provinceID": 24,
            "divisionCode": "520200",
            "cityName": "六盘水市"
          },
          {
            "cityID": 263,
            "provinceID": 24,
            "divisionCode": "520300",
            "cityName": "遵义市"
          },
          {
            "cityID": 264,
            "provinceID": 24,
            "divisionCode": "520400",
            "cityName": "安顺市"
          },
          {
            "cityID": 265,
            "provinceID": 24,
            "divisionCode": "520500",
            "cityName": "毕节市"
          },
          {
            "cityID": 266,
            "provinceID": 24,
            "divisionCode": "520600",
            "cityName": "铜仁市"
          },
          {
            "cityID": 267,
            "provinceID": 24,
            "divisionCode": "522300",
            "cityName": "黔西南布依族苗族自治州"
          },
          {
            "cityID": 268,
            "provinceID": 24,
            "divisionCode": "522600",
            "cityName": "黔东南苗族侗族自治州"
          },
          {
            "cityID": 269,
            "provinceID": 24,
            "divisionCode": "522700",
            "cityName": "黔南布依族苗族自治州"
          },
          {
            "cityID": 270,
            "provinceID": 25,
            "divisionCode": "530100",
            "cityName": "昆明市"
          },
          {
            "cityID": 271,
            "provinceID": 25,
            "divisionCode": "530300",
            "cityName": "曲靖市"
          },
          {
            "cityID": 272,
            "provinceID": 25,
            "divisionCode": "530400",
            "cityName": "玉溪市"
          },
          {
            "cityID": 273,
            "provinceID": 25,
            "divisionCode": "530500",
            "cityName": "保山市"
          },
          {
            "cityID": 274,
            "provinceID": 25,
            "divisionCode": "530600",
            "cityName": "昭通市"
          },
          {
            "cityID": 275,
            "provinceID": 25,
            "divisionCode": "530700",
            "cityName": "丽江市"
          },
          {
            "cityID": 276,
            "provinceID": 25,
            "divisionCode": "530800",
            "cityName": "普洱市"
          },
          {
            "cityID": 277,
            "provinceID": 25,
            "divisionCode": "530900",
            "cityName": "临沧市"
          },
          {
            "cityID": 278,
            "provinceID": 25,
            "divisionCode": "532300",
            "cityName": "楚雄彝族自治州"
          },
          {
            "cityID": 279,
            "provinceID": 25,
            "divisionCode": "532500",
            "cityName": "红河哈尼族彝族自治州"
          },
          {
            "cityID": 280,
            "provinceID": 25,
            "divisionCode": "532600",
            "cityName": "文山壮族苗族自治州"
          },
          {
            "cityID": 281,
            "provinceID": 25,
            "divisionCode": "532800",
            "cityName": "西双版纳傣族自治州"
          },
          {
            "cityID": 282,
            "provinceID": 25,
            "divisionCode": "532900",
            "cityName": "大理白族自治州"
          },
          {
            "cityID": 283,
            "provinceID": 25,
            "divisionCode": "533100",
            "cityName": "德宏傣族景颇族自治州"
          },
          {
            "cityID": 284,
            "provinceID": 25,
            "divisionCode": "533300",
            "cityName": "怒江傈僳族自治州"
          },
          {
            "cityID": 285,
            "provinceID": 25,
            "divisionCode": "533400",
            "cityName": "迪庆藏族自治州"
          },
          {
            "cityID": 286,
            "provinceID": 26,
            "divisionCode": "540100",
            "cityName": "拉萨市"
          },
          {
            "cityID": 287,
            "provinceID": 26,
            "divisionCode": "540200",
            "cityName": "日喀则市"
          },
          {
            "cityID": 288,
            "provinceID": 26,
            "divisionCode": "540300",
            "cityName": "昌都市"
          },
          {
            "cityID": 289,
            "provinceID": 26,
            "divisionCode": "540400",
            "cityName": "林芝市"
          },
          {
            "cityID": 290,
            "provinceID": 26,
            "divisionCode": "540500",
            "cityName": "山南市"
          },
          {
            "cityID": 291,
            "provinceID": 26,
            "divisionCode": "542400",
            "cityName": "那曲地区"
          },
          {
            "cityID": 292,
            "provinceID": 26,
            "divisionCode": "542500",
            "cityName": "阿里地区"
          },
          {
            "cityID": 293,
            "provinceID": 27,
            "divisionCode": "610100",
            "cityName": "西安市"
          },
          {
            "cityID": 294,
            "provinceID": 27,
            "divisionCode": "610200",
            "cityName": "铜川市"
          },
          {
            "cityID": 295,
            "provinceID": 27,
            "divisionCode": "610300",
            "cityName": "宝鸡市"
          },
          {
            "cityID": 296,
            "provinceID": 27,
            "divisionCode": "610400",
            "cityName": "咸阳市"
          },
          {
            "cityID": 297,
            "provinceID": 27,
            "divisionCode": "610500",
            "cityName": "渭南市"
          },
          {
            "cityID": 298,
            "provinceID": 27,
            "divisionCode": "610600",
            "cityName": "延安市"
          },
          {
            "cityID": 299,
            "provinceID": 27,
            "divisionCode": "610700",
            "cityName": "汉中市"
          },
          {
            "cityID": 300,
            "provinceID": 27,
            "divisionCode": "610800",
            "cityName": "榆林市"
          },
          {
            "cityID": 301,
            "provinceID": 27,
            "divisionCode": "610900",
            "cityName": "安康市"
          },
          {
            "cityID": 302,
            "provinceID": 27,
            "divisionCode": "611000",
            "cityName": "商洛市"
          },
          {
            "cityID": 303,
            "provinceID": 28,
            "divisionCode": "620100",
            "cityName": "兰州市"
          },
          {
            "cityID": 304,
            "provinceID": 28,
            "divisionCode": "620200",
            "cityName": "嘉峪关市"
          },
          {
            "cityID": 305,
            "provinceID": 28,
            "divisionCode": "620300",
            "cityName": "金昌市"
          },
          {
            "cityID": 306,
            "provinceID": 28,
            "divisionCode": "620400",
            "cityName": "白银市"
          },
          {
            "cityID": 307,
            "provinceID": 28,
            "divisionCode": "620500",
            "cityName": "天水市"
          },
          {
            "cityID": 308,
            "provinceID": 28,
            "divisionCode": "620600",
            "cityName": "武威市"
          },
          {
            "cityID": 309,
            "provinceID": 28,
            "divisionCode": "620700",
            "cityName": "张掖市"
          },
          {
            "cityID": 310,
            "provinceID": 28,
            "divisionCode": "620800",
            "cityName": "平凉市"
          },
          {
            "cityID": 311,
            "provinceID": 28,
            "divisionCode": "620900",
            "cityName": "酒泉市"
          },
          {
            "cityID": 312,
            "provinceID": 28,
            "divisionCode": "621000",
            "cityName": "庆阳市"
          },
          {
            "cityID": 313,
            "provinceID": 28,
            "divisionCode": "621100",
            "cityName": "定西市"
          },
          {
            "cityID": 314,
            "provinceID": 28,
            "divisionCode": "621200",
            "cityName": "陇南市"
          },
          {
            "cityID": 315,
            "provinceID": 28,
            "divisionCode": "622900",
            "cityName": "临夏回族自治州"
          },
          {
            "cityID": 316,
            "provinceID": 28,
            "divisionCode": "623000",
            "cityName": "甘南藏族自治州"
          },
          {
            "cityID": 317,
            "provinceID": 29,
            "divisionCode": "630100",
            "cityName": "西宁市"
          },
          {
            "cityID": 318,
            "provinceID": 29,
            "divisionCode": "630200",
            "cityName": "海东市"
          },
          {
            "cityID": 319,
            "provinceID": 29,
            "divisionCode": "632200",
            "cityName": "海北藏族自治州"
          },
          {
            "cityID": 320,
            "provinceID": 29,
            "divisionCode": "632300",
            "cityName": "黄南藏族自治州"
          },
          {
            "cityID": 321,
            "provinceID": 29,
            "divisionCode": "632500",
            "cityName": "海南藏族自治州"
          },
          {
            "cityID": 322,
            "provinceID": 29,
            "divisionCode": "632600",
            "cityName": "果洛藏族自治州"
          },
          {
            "cityID": 323,
            "provinceID": 29,
            "divisionCode": "632700",
            "cityName": "玉树藏族自治州"
          },
          {
            "cityID": 324,
            "provinceID": 29,
            "divisionCode": "632800",
            "cityName": "海西蒙古族藏族自治州"
          },
          {
            "cityID": 325,
            "provinceID": 30,
            "divisionCode": "640100",
            "cityName": "银川市"
          },
          {
            "cityID": 326,
            "provinceID": 30,
            "divisionCode": "640200",
            "cityName": "石嘴山市"
          },
          {
            "cityID": 327,
            "provinceID": 30,
            "divisionCode": "640300",
            "cityName": "吴忠市"
          },
          {
            "cityID": 328,
            "provinceID": 30,
            "divisionCode": "640400",
            "cityName": "固原市"
          },
          {
            "cityID": 329,
            "provinceID": 30,
            "divisionCode": "640500",
            "cityName": "中卫市"
          },
          {
            "cityID": 330,
            "provinceID": 31,
            "divisionCode": "650100",
            "cityName": "乌鲁木齐市"
          },
          {
            "cityID": 331,
            "provinceID": 31,
            "divisionCode": "650200",
            "cityName": "克拉玛依市"
          },
          {
            "cityID": 332,
            "provinceID": 31,
            "divisionCode": "650400",
            "cityName": "吐鲁番市"
          },
          {
            "cityID": 333,
            "provinceID": 31,
            "divisionCode": "650500",
            "cityName": "哈密市"
          },
          {
            "cityID": 334,
            "provinceID": 31,
            "divisionCode": "652300",
            "cityName": "昌吉回族自治州"
          },
          {
            "cityID": 335,
            "provinceID": 31,
            "divisionCode": "652700",
            "cityName": "博尔塔拉蒙古自治州"
          },
          {
            "cityID": 336,
            "provinceID": 31,
            "divisionCode": "652800",
            "cityName": "巴音郭楞蒙古自治州"
          },
          {
            "cityID": 337,
            "provinceID": 31,
            "divisionCode": "652900",
            "cityName": "阿克苏地区"
          },
          {
            "cityID": 338,
            "provinceID": 31,
            "divisionCode": "653000",
            "cityName": "克孜勒苏柯尔克孜自治州"
          },
          {
            "cityID": 339,
            "provinceID": 31,
            "divisionCode": "653100",
            "cityName": "喀什地区"
          },
          {
            "cityID": 340,
            "provinceID": 31,
            "divisionCode": "653200",
            "cityName": "和田地区"
          },
          {
            "cityID": 341,
            "provinceID": 31,
            "divisionCode": "654000",
            "cityName": "伊犁哈萨克自治州"
          },
          {
            "cityID": 342,
            "provinceID": 31,
            "divisionCode": "654200",
            "cityName": "塔城地区"
          },
          {
            "cityID": 343,
            "provinceID": 31,
            "divisionCode": "654300",
            "cityName": "阿勒泰地区"
          },
          {
            "cityID": 344,
            "provinceID": 31,
            "divisionCode": "659000",
            "cityName": "自治区直辖县级行政区划"
          }
        ],
        dirtrictData:[
          {
            "districtID": 1,
            "cityID": 1,
            "divisionCode": "110101",
            "districtName": "东城区"
          },
          {
            "districtID": 2,
            "cityID": 1,
            "divisionCode": "110102",
            "districtName": "西城区"
          },
          {
            "districtID": 3,
            "cityID": 1,
            "divisionCode": "110105",
            "districtName": "朝阳区"
          },
          {
            "districtID": 4,
            "cityID": 1,
            "divisionCode": "110106",
            "districtName": "丰台区"
          },
          {
            "districtID": 5,
            "cityID": 1,
            "divisionCode": "110107",
            "districtName": "石景山区"
          },
          {
            "districtID": 6,
            "cityID": 1,
            "divisionCode": "110108",
            "districtName": "海淀区"
          },
          {
            "districtID": 7,
            "cityID": 1,
            "divisionCode": "110109",
            "districtName": "门头沟区"
          },
          {
            "districtID": 8,
            "cityID": 1,
            "divisionCode": "110111",
            "districtName": "房山区"
          },
          {
            "districtID": 9,
            "cityID": 1,
            "divisionCode": "110112",
            "districtName": "通州区"
          },
          {
            "districtID": 10,
            "cityID": 1,
            "divisionCode": "110113",
            "districtName": "顺义区"
          },
          {
            "districtID": 11,
            "cityID": 1,
            "divisionCode": "110114",
            "districtName": "昌平区"
          },
          {
            "districtID": 12,
            "cityID": 1,
            "divisionCode": "110115",
            "districtName": "大兴区"
          },
          {
            "districtID": 13,
            "cityID": 1,
            "divisionCode": "110116",
            "districtName": "怀柔区"
          },
          {
            "districtID": 14,
            "cityID": 1,
            "divisionCode": "110117",
            "districtName": "平谷区"
          },
          {
            "districtID": 15,
            "cityID": 1,
            "divisionCode": "110118",
            "districtName": "密云区"
          },
          {
            "districtID": 16,
            "cityID": 1,
            "divisionCode": "110119",
            "districtName": "延庆区"
          },
          {
            "districtID": 17,
            "cityID": 2,
            "divisionCode": "120101",
            "districtName": "和平区"
          },
          {
            "districtID": 18,
            "cityID": 2,
            "divisionCode": "120102",
            "districtName": "河东区"
          },
          {
            "districtID": 19,
            "cityID": 2,
            "divisionCode": "120103",
            "districtName": "河西区"
          },
          {
            "districtID": 20,
            "cityID": 2,
            "divisionCode": "120104",
            "districtName": "南开区"
          },
          {
            "districtID": 21,
            "cityID": 2,
            "divisionCode": "120105",
            "districtName": "河北区"
          },
          {
            "districtID": 22,
            "cityID": 2,
            "divisionCode": "120106",
            "districtName": "红桥区"
          },
          {
            "districtID": 23,
            "cityID": 2,
            "divisionCode": "120110",
            "districtName": "东丽区"
          },
          {
            "districtID": 24,
            "cityID": 2,
            "divisionCode": "120111",
            "districtName": "西青区"
          },
          {
            "districtID": 25,
            "cityID": 2,
            "divisionCode": "120112",
            "districtName": "津南区"
          },
          {
            "districtID": 26,
            "cityID": 2,
            "divisionCode": "120113",
            "districtName": "北辰区"
          },
          {
            "districtID": 27,
            "cityID": 2,
            "divisionCode": "120114",
            "districtName": "武清区"
          },
          {
            "districtID": 28,
            "cityID": 2,
            "divisionCode": "120115",
            "districtName": "宝坻区"
          },
          {
            "districtID": 29,
            "cityID": 2,
            "divisionCode": "120116",
            "districtName": "滨海新区"
          },
          {
            "districtID": 30,
            "cityID": 2,
            "divisionCode": "120117",
            "districtName": "宁河区"
          },
          {
            "districtID": 31,
            "cityID": 2,
            "divisionCode": "120118",
            "districtName": "静海区"
          },
          {
            "districtID": 32,
            "cityID": 2,
            "divisionCode": "120119",
            "districtName": "蓟州区"
          },
          {
            "districtID": 33,
            "cityID": 3,
            "divisionCode": "130101",
            "districtName": "市辖区"
          },
          {
            "districtID": 34,
            "cityID": 3,
            "divisionCode": "130102",
            "districtName": "长安区"
          },
          {
            "districtID": 35,
            "cityID": 3,
            "divisionCode": "130104",
            "districtName": "桥西区"
          },
          {
            "districtID": 36,
            "cityID": 3,
            "divisionCode": "130105",
            "districtName": "新华区"
          },
          {
            "districtID": 37,
            "cityID": 3,
            "divisionCode": "130107",
            "districtName": "井陉矿区"
          },
          {
            "districtID": 38,
            "cityID": 3,
            "divisionCode": "130108",
            "districtName": "裕华区"
          },
          {
            "districtID": 39,
            "cityID": 3,
            "divisionCode": "130109",
            "districtName": "藁城区"
          },
          {
            "districtID": 40,
            "cityID": 3,
            "divisionCode": "130110",
            "districtName": "鹿泉区"
          },
          {
            "districtID": 41,
            "cityID": 3,
            "divisionCode": "130111",
            "districtName": "栾城区"
          },
          {
            "districtID": 42,
            "cityID": 3,
            "divisionCode": "130121",
            "districtName": "井陉县"
          },
          {
            "districtID": 43,
            "cityID": 3,
            "divisionCode": "130123",
            "districtName": "正定县"
          },
          {
            "districtID": 44,
            "cityID": 3,
            "divisionCode": "130125",
            "districtName": "行唐县"
          },
          {
            "districtID": 45,
            "cityID": 3,
            "divisionCode": "130126",
            "districtName": "灵寿县"
          },
          {
            "districtID": 46,
            "cityID": 3,
            "divisionCode": "130127",
            "districtName": "高邑县"
          },
          {
            "districtID": 47,
            "cityID": 3,
            "divisionCode": "130128",
            "districtName": "深泽县"
          },
          {
            "districtID": 48,
            "cityID": 3,
            "divisionCode": "130129",
            "districtName": "赞皇县"
          },
          {
            "districtID": 49,
            "cityID": 3,
            "divisionCode": "130130",
            "districtName": "无极县"
          },
          {
            "districtID": 50,
            "cityID": 3,
            "divisionCode": "130131",
            "districtName": "平山县"
          },
          {
            "districtID": 51,
            "cityID": 3,
            "divisionCode": "130132",
            "districtName": "元氏县"
          },
          {
            "districtID": 52,
            "cityID": 3,
            "divisionCode": "130133",
            "districtName": "赵县"
          },
          {
            "districtID": 53,
            "cityID": 3,
            "divisionCode": "130183",
            "districtName": "晋州市"
          },
          {
            "districtID": 54,
            "cityID": 3,
            "divisionCode": "130184",
            "districtName": "新乐市"
          },
          {
            "districtID": 55,
            "cityID": 4,
            "divisionCode": "130201",
            "districtName": "市辖区"
          },
          {
            "districtID": 56,
            "cityID": 4,
            "divisionCode": "130202",
            "districtName": "路南区"
          },
          {
            "districtID": 57,
            "cityID": 4,
            "divisionCode": "130203",
            "districtName": "路北区"
          },
          {
            "districtID": 58,
            "cityID": 4,
            "divisionCode": "130204",
            "districtName": "古冶区"
          },
          {
            "districtID": 59,
            "cityID": 4,
            "divisionCode": "130205",
            "districtName": "开平区"
          },
          {
            "districtID": 60,
            "cityID": 4,
            "divisionCode": "130207",
            "districtName": "丰南区"
          },
          {
            "districtID": 61,
            "cityID": 4,
            "divisionCode": "130208",
            "districtName": "丰润区"
          },
          {
            "districtID": 62,
            "cityID": 4,
            "divisionCode": "130209",
            "districtName": "曹妃甸区"
          },
          {
            "districtID": 63,
            "cityID": 4,
            "divisionCode": "130223",
            "districtName": "滦县"
          },
          {
            "districtID": 64,
            "cityID": 4,
            "divisionCode": "130224",
            "districtName": "滦南县"
          },
          {
            "districtID": 65,
            "cityID": 4,
            "divisionCode": "130225",
            "districtName": "乐亭县"
          },
          {
            "districtID": 66,
            "cityID": 4,
            "divisionCode": "130227",
            "districtName": "迁西县"
          },
          {
            "districtID": 67,
            "cityID": 4,
            "divisionCode": "130229",
            "districtName": "玉田县"
          },
          {
            "districtID": 68,
            "cityID": 4,
            "divisionCode": "130281",
            "districtName": "遵化市"
          },
          {
            "districtID": 69,
            "cityID": 4,
            "divisionCode": "130283",
            "districtName": "迁安市"
          },
          {
            "districtID": 70,
            "cityID": 5,
            "divisionCode": "130301",
            "districtName": "市辖区"
          },
          {
            "districtID": 71,
            "cityID": 5,
            "divisionCode": "130302",
            "districtName": "海港区"
          },
          {
            "districtID": 72,
            "cityID": 5,
            "divisionCode": "130303",
            "districtName": "山海关区"
          },
          {
            "districtID": 73,
            "cityID": 5,
            "divisionCode": "130304",
            "districtName": "北戴河区"
          },
          {
            "districtID": 74,
            "cityID": 5,
            "divisionCode": "130306",
            "districtName": "抚宁区"
          },
          {
            "districtID": 75,
            "cityID": 5,
            "divisionCode": "130321",
            "districtName": "青龙满族自治县"
          },
          {
            "districtID": 76,
            "cityID": 5,
            "divisionCode": "130322",
            "districtName": "昌黎县"
          },
          {
            "districtID": 77,
            "cityID": 5,
            "divisionCode": "130324",
            "districtName": "卢龙县"
          },
          {
            "districtID": 78,
            "cityID": 6,
            "divisionCode": "130401",
            "districtName": "市辖区"
          },
          {
            "districtID": 79,
            "cityID": 6,
            "divisionCode": "130402",
            "districtName": "邯山区"
          },
          {
            "districtID": 80,
            "cityID": 6,
            "divisionCode": "130403",
            "districtName": "丛台区"
          },
          {
            "districtID": 81,
            "cityID": 6,
            "divisionCode": "130404",
            "districtName": "复兴区"
          },
          {
            "districtID": 82,
            "cityID": 6,
            "divisionCode": "130406",
            "districtName": "峰峰矿区"
          },
          {
            "districtID": 83,
            "cityID": 6,
            "divisionCode": "130421",
            "districtName": "邯郸县"
          },
          {
            "districtID": 84,
            "cityID": 6,
            "divisionCode": "130423",
            "districtName": "临漳县"
          },
          {
            "districtID": 85,
            "cityID": 6,
            "divisionCode": "130424",
            "districtName": "成安县"
          },
          {
            "districtID": 86,
            "cityID": 6,
            "divisionCode": "130425",
            "districtName": "大名县"
          },
          {
            "districtID": 87,
            "cityID": 6,
            "divisionCode": "130426",
            "districtName": "涉县"
          },
          {
            "districtID": 88,
            "cityID": 6,
            "divisionCode": "130427",
            "districtName": "磁县"
          },
          {
            "districtID": 89,
            "cityID": 6,
            "divisionCode": "130428",
            "districtName": "肥乡县"
          },
          {
            "districtID": 90,
            "cityID": 6,
            "divisionCode": "130429",
            "districtName": "永年县"
          },
          {
            "districtID": 91,
            "cityID": 6,
            "divisionCode": "130430",
            "districtName": "邱县"
          },
          {
            "districtID": 92,
            "cityID": 6,
            "divisionCode": "130431",
            "districtName": "鸡泽县"
          },
          {
            "districtID": 93,
            "cityID": 6,
            "divisionCode": "130432",
            "districtName": "广平县"
          },
          {
            "districtID": 94,
            "cityID": 6,
            "divisionCode": "130433",
            "districtName": "馆陶县"
          },
          {
            "districtID": 95,
            "cityID": 6,
            "divisionCode": "130434",
            "districtName": "魏县"
          },
          {
            "districtID": 96,
            "cityID": 6,
            "divisionCode": "130435",
            "districtName": "曲周县"
          },
          {
            "districtID": 97,
            "cityID": 6,
            "divisionCode": "130481",
            "districtName": "武安市"
          },
          {
            "districtID": 98,
            "cityID": 7,
            "divisionCode": "130501",
            "districtName": "市辖区"
          },
          {
            "districtID": 99,
            "cityID": 7,
            "divisionCode": "130502",
            "districtName": "桥东区"
          },
          {
            "districtID": 100,
            "cityID": 7,
            "divisionCode": "130503",
            "districtName": "桥西区"
          },
          {
            "districtID": 101,
            "cityID": 7,
            "divisionCode": "130521",
            "districtName": "邢台县"
          },
          {
            "districtID": 102,
            "cityID": 7,
            "divisionCode": "130522",
            "districtName": "临城县"
          },
          {
            "districtID": 103,
            "cityID": 7,
            "divisionCode": "130523",
            "districtName": "内丘县"
          },
          {
            "districtID": 104,
            "cityID": 7,
            "divisionCode": "130524",
            "districtName": "柏乡县"
          },
          {
            "districtID": 105,
            "cityID": 7,
            "divisionCode": "130525",
            "districtName": "隆尧县"
          },
          {
            "districtID": 106,
            "cityID": 7,
            "divisionCode": "130526",
            "districtName": "任县"
          },
          {
            "districtID": 107,
            "cityID": 7,
            "divisionCode": "130527",
            "districtName": "南和县"
          },
          {
            "districtID": 108,
            "cityID": 7,
            "divisionCode": "130528",
            "districtName": "宁晋县"
          },
          {
            "districtID": 109,
            "cityID": 7,
            "divisionCode": "130529",
            "districtName": "巨鹿县"
          },
          {
            "districtID": 110,
            "cityID": 7,
            "divisionCode": "130530",
            "districtName": "新河县"
          },
          {
            "districtID": 111,
            "cityID": 7,
            "divisionCode": "130531",
            "districtName": "广宗县"
          },
          {
            "districtID": 112,
            "cityID": 7,
            "divisionCode": "130532",
            "districtName": "平乡县"
          },
          {
            "districtID": 113,
            "cityID": 7,
            "divisionCode": "130533",
            "districtName": "威县"
          },
          {
            "districtID": 114,
            "cityID": 7,
            "divisionCode": "130534",
            "districtName": "清河县"
          },
          {
            "districtID": 115,
            "cityID": 7,
            "divisionCode": "130535",
            "districtName": "临西县"
          },
          {
            "districtID": 116,
            "cityID": 7,
            "divisionCode": "130581",
            "districtName": "南宫市"
          },
          {
            "districtID": 117,
            "cityID": 7,
            "divisionCode": "130582",
            "districtName": "沙河市"
          },
          {
            "districtID": 118,
            "cityID": 8,
            "divisionCode": "130601",
            "districtName": "市辖区"
          },
          {
            "districtID": 119,
            "cityID": 8,
            "divisionCode": "130602",
            "districtName": "竞秀区"
          },
          {
            "districtID": 120,
            "cityID": 8,
            "divisionCode": "130606",
            "districtName": "莲池区"
          },
          {
            "districtID": 121,
            "cityID": 8,
            "divisionCode": "130607",
            "districtName": "满城区"
          },
          {
            "districtID": 122,
            "cityID": 8,
            "divisionCode": "130608",
            "districtName": "清苑区"
          },
          {
            "districtID": 123,
            "cityID": 8,
            "divisionCode": "130609",
            "districtName": "徐水区"
          },
          {
            "districtID": 124,
            "cityID": 8,
            "divisionCode": "130623",
            "districtName": "涞水县"
          },
          {
            "districtID": 125,
            "cityID": 8,
            "divisionCode": "130624",
            "districtName": "阜平县"
          },
          {
            "districtID": 126,
            "cityID": 8,
            "divisionCode": "130626",
            "districtName": "定兴县"
          },
          {
            "districtID": 127,
            "cityID": 8,
            "divisionCode": "130627",
            "districtName": "唐县"
          },
          {
            "districtID": 128,
            "cityID": 8,
            "divisionCode": "130628",
            "districtName": "高阳县"
          },
          {
            "districtID": 129,
            "cityID": 8,
            "divisionCode": "130629",
            "districtName": "容城县"
          },
          {
            "districtID": 130,
            "cityID": 8,
            "divisionCode": "130630",
            "districtName": "涞源县"
          },
          {
            "districtID": 131,
            "cityID": 8,
            "divisionCode": "130631",
            "districtName": "望都县"
          },
          {
            "districtID": 132,
            "cityID": 8,
            "divisionCode": "130632",
            "districtName": "安新县"
          },
          {
            "districtID": 133,
            "cityID": 8,
            "divisionCode": "130633",
            "districtName": "易县"
          },
          {
            "districtID": 134,
            "cityID": 8,
            "divisionCode": "130634",
            "districtName": "曲阳县"
          },
          {
            "districtID": 135,
            "cityID": 8,
            "divisionCode": "130635",
            "districtName": "蠡县"
          },
          {
            "districtID": 136,
            "cityID": 8,
            "divisionCode": "130636",
            "districtName": "顺平县"
          },
          {
            "districtID": 137,
            "cityID": 8,
            "divisionCode": "130637",
            "districtName": "博野县"
          },
          {
            "districtID": 138,
            "cityID": 8,
            "divisionCode": "130638",
            "districtName": "雄县"
          },
          {
            "districtID": 139,
            "cityID": 8,
            "divisionCode": "130681",
            "districtName": "涿州市"
          },
          {
            "districtID": 140,
            "cityID": 8,
            "divisionCode": "130683",
            "districtName": "安国市"
          },
          {
            "districtID": 141,
            "cityID": 8,
            "divisionCode": "130684",
            "districtName": "高碑店市"
          },
          {
            "districtID": 142,
            "cityID": 9,
            "divisionCode": "130701",
            "districtName": "市辖区"
          },
          {
            "districtID": 143,
            "cityID": 9,
            "divisionCode": "130702",
            "districtName": "桥东区"
          },
          {
            "districtID": 144,
            "cityID": 9,
            "divisionCode": "130703",
            "districtName": "桥西区"
          },
          {
            "districtID": 145,
            "cityID": 9,
            "divisionCode": "130705",
            "districtName": "宣化区"
          },
          {
            "districtID": 146,
            "cityID": 9,
            "divisionCode": "130706",
            "districtName": "下花园区"
          },
          {
            "districtID": 147,
            "cityID": 9,
            "divisionCode": "130708",
            "districtName": "万全区"
          },
          {
            "districtID": 148,
            "cityID": 9,
            "divisionCode": "130709",
            "districtName": "崇礼区"
          },
          {
            "districtID": 149,
            "cityID": 9,
            "divisionCode": "130722",
            "districtName": "张北县"
          },
          {
            "districtID": 150,
            "cityID": 9,
            "divisionCode": "130723",
            "districtName": "康保县"
          },
          {
            "districtID": 151,
            "cityID": 9,
            "divisionCode": "130724",
            "districtName": "沽源县"
          },
          {
            "districtID": 152,
            "cityID": 9,
            "divisionCode": "130725",
            "districtName": "尚义县"
          },
          {
            "districtID": 153,
            "cityID": 9,
            "divisionCode": "130726",
            "districtName": "蔚县"
          },
          {
            "districtID": 154,
            "cityID": 9,
            "divisionCode": "130727",
            "districtName": "阳原县"
          },
          {
            "districtID": 155,
            "cityID": 9,
            "divisionCode": "130728",
            "districtName": "怀安县"
          },
          {
            "districtID": 156,
            "cityID": 9,
            "divisionCode": "130730",
            "districtName": "怀来县"
          },
          {
            "districtID": 157,
            "cityID": 9,
            "divisionCode": "130731",
            "districtName": "涿鹿县"
          },
          {
            "districtID": 158,
            "cityID": 9,
            "divisionCode": "130732",
            "districtName": "赤城县"
          },
          {
            "districtID": 159,
            "cityID": 10,
            "divisionCode": "130801",
            "districtName": "市辖区"
          },
          {
            "districtID": 160,
            "cityID": 10,
            "divisionCode": "130802",
            "districtName": "双桥区"
          },
          {
            "districtID": 161,
            "cityID": 10,
            "divisionCode": "130803",
            "districtName": "双滦区"
          },
          {
            "districtID": 162,
            "cityID": 10,
            "divisionCode": "130804",
            "districtName": "鹰手营子矿区"
          },
          {
            "districtID": 163,
            "cityID": 10,
            "divisionCode": "130821",
            "districtName": "承德县"
          },
          {
            "districtID": 164,
            "cityID": 10,
            "divisionCode": "130822",
            "districtName": "兴隆县"
          },
          {
            "districtID": 165,
            "cityID": 10,
            "divisionCode": "130823",
            "districtName": "平泉县"
          },
          {
            "districtID": 166,
            "cityID": 10,
            "divisionCode": "130824",
            "districtName": "滦平县"
          },
          {
            "districtID": 167,
            "cityID": 10,
            "divisionCode": "130825",
            "districtName": "隆化县"
          },
          {
            "districtID": 168,
            "cityID": 10,
            "divisionCode": "130826",
            "districtName": "丰宁满族自治县"
          },
          {
            "districtID": 169,
            "cityID": 10,
            "divisionCode": "130827",
            "districtName": "宽城满族自治县"
          },
          {
            "districtID": 170,
            "cityID": 10,
            "divisionCode": "130828",
            "districtName": "围场满族蒙古族自治县"
          },
          {
            "districtID": 171,
            "cityID": 11,
            "divisionCode": "130901",
            "districtName": "市辖区"
          },
          {
            "districtID": 172,
            "cityID": 11,
            "divisionCode": "130902",
            "districtName": "新华区"
          },
          {
            "districtID": 173,
            "cityID": 11,
            "divisionCode": "130903",
            "districtName": "运河区"
          },
          {
            "districtID": 174,
            "cityID": 11,
            "divisionCode": "130921",
            "districtName": "沧县"
          },
          {
            "districtID": 175,
            "cityID": 11,
            "divisionCode": "130922",
            "districtName": "青县"
          },
          {
            "districtID": 176,
            "cityID": 11,
            "divisionCode": "130923",
            "districtName": "东光县"
          },
          {
            "districtID": 177,
            "cityID": 11,
            "divisionCode": "130924",
            "districtName": "海兴县"
          },
          {
            "districtID": 178,
            "cityID": 11,
            "divisionCode": "130925",
            "districtName": "盐山县"
          },
          {
            "districtID": 179,
            "cityID": 11,
            "divisionCode": "130926",
            "districtName": "肃宁县"
          },
          {
            "districtID": 180,
            "cityID": 11,
            "divisionCode": "130927",
            "districtName": "南皮县"
          },
          {
            "districtID": 181,
            "cityID": 11,
            "divisionCode": "130928",
            "districtName": "吴桥县"
          },
          {
            "districtID": 182,
            "cityID": 11,
            "divisionCode": "130929",
            "districtName": "献县"
          },
          {
            "districtID": 183,
            "cityID": 11,
            "divisionCode": "130930",
            "districtName": "孟村回族自治县"
          },
          {
            "districtID": 184,
            "cityID": 11,
            "divisionCode": "130981",
            "districtName": "泊头市"
          },
          {
            "districtID": 185,
            "cityID": 11,
            "divisionCode": "130982",
            "districtName": "任丘市"
          },
          {
            "districtID": 186,
            "cityID": 11,
            "divisionCode": "130983",
            "districtName": "黄骅市"
          },
          {
            "districtID": 187,
            "cityID": 11,
            "divisionCode": "130984",
            "districtName": "河间市"
          },
          {
            "districtID": 188,
            "cityID": 12,
            "divisionCode": "131001",
            "districtName": "市辖区"
          },
          {
            "districtID": 189,
            "cityID": 12,
            "divisionCode": "131002",
            "districtName": "安次区"
          },
          {
            "districtID": 190,
            "cityID": 12,
            "divisionCode": "131003",
            "districtName": "广阳区"
          },
          {
            "districtID": 191,
            "cityID": 12,
            "divisionCode": "131022",
            "districtName": "固安县"
          },
          {
            "districtID": 192,
            "cityID": 12,
            "divisionCode": "131023",
            "districtName": "永清县"
          },
          {
            "districtID": 193,
            "cityID": 12,
            "divisionCode": "131024",
            "districtName": "香河县"
          },
          {
            "districtID": 194,
            "cityID": 12,
            "divisionCode": "131025",
            "districtName": "大城县"
          },
          {
            "districtID": 195,
            "cityID": 12,
            "divisionCode": "131026",
            "districtName": "文安县"
          },
          {
            "districtID": 196,
            "cityID": 12,
            "divisionCode": "131028",
            "districtName": "大厂回族自治县"
          },
          {
            "districtID": 197,
            "cityID": 12,
            "divisionCode": "131081",
            "districtName": "霸州市"
          },
          {
            "districtID": 198,
            "cityID": 12,
            "divisionCode": "131082",
            "districtName": "三河市"
          },
          {
            "districtID": 199,
            "cityID": 13,
            "divisionCode": "131101",
            "districtName": "市辖区"
          },
          {
            "districtID": 200,
            "cityID": 13,
            "divisionCode": "131102",
            "districtName": "桃城区"
          },
          {
            "districtID": 201,
            "cityID": 13,
            "divisionCode": "131103",
            "districtName": "冀州区"
          },
          {
            "districtID": 202,
            "cityID": 13,
            "divisionCode": "131121",
            "districtName": "枣强县"
          },
          {
            "districtID": 203,
            "cityID": 13,
            "divisionCode": "131122",
            "districtName": "武邑县"
          },
          {
            "districtID": 204,
            "cityID": 13,
            "divisionCode": "131123",
            "districtName": "武强县"
          },
          {
            "districtID": 205,
            "cityID": 13,
            "divisionCode": "131124",
            "districtName": "饶阳县"
          },
          {
            "districtID": 206,
            "cityID": 13,
            "divisionCode": "131125",
            "districtName": "安平县"
          },
          {
            "districtID": 207,
            "cityID": 13,
            "divisionCode": "131126",
            "districtName": "故城县"
          },
          {
            "districtID": 208,
            "cityID": 13,
            "divisionCode": "131127",
            "districtName": "景县"
          },
          {
            "districtID": 209,
            "cityID": 13,
            "divisionCode": "131128",
            "districtName": "阜城县"
          },
          {
            "districtID": 210,
            "cityID": 13,
            "divisionCode": "131182",
            "districtName": "深州市"
          },
          {
            "districtID": 211,
            "cityID": 14,
            "divisionCode": "139001",
            "districtName": "定州市"
          },
          {
            "districtID": 212,
            "cityID": 14,
            "divisionCode": "139002",
            "districtName": "辛集市"
          },
          {
            "districtID": 213,
            "cityID": 15,
            "divisionCode": "140101",
            "districtName": "市辖区"
          },
          {
            "districtID": 214,
            "cityID": 15,
            "divisionCode": "140105",
            "districtName": "小店区"
          },
          {
            "districtID": 215,
            "cityID": 15,
            "divisionCode": "140106",
            "districtName": "迎泽区"
          },
          {
            "districtID": 216,
            "cityID": 15,
            "divisionCode": "140107",
            "districtName": "杏花岭区"
          },
          {
            "districtID": 217,
            "cityID": 15,
            "divisionCode": "140108",
            "districtName": "尖草坪区"
          },
          {
            "districtID": 218,
            "cityID": 15,
            "divisionCode": "140109",
            "districtName": "万柏林区"
          },
          {
            "districtID": 219,
            "cityID": 15,
            "divisionCode": "140110",
            "districtName": "晋源区"
          },
          {
            "districtID": 220,
            "cityID": 15,
            "divisionCode": "140121",
            "districtName": "清徐县"
          },
          {
            "districtID": 221,
            "cityID": 15,
            "divisionCode": "140122",
            "districtName": "阳曲县"
          },
          {
            "districtID": 222,
            "cityID": 15,
            "divisionCode": "140123",
            "districtName": "娄烦县"
          },
          {
            "districtID": 223,
            "cityID": 15,
            "divisionCode": "140181",
            "districtName": "古交市"
          },
          {
            "districtID": 224,
            "cityID": 16,
            "divisionCode": "140201",
            "districtName": "市辖区"
          },
          {
            "districtID": 225,
            "cityID": 16,
            "divisionCode": "140202",
            "districtName": "城区"
          },
          {
            "districtID": 226,
            "cityID": 16,
            "divisionCode": "140203",
            "districtName": "矿区"
          },
          {
            "districtID": 227,
            "cityID": 16,
            "divisionCode": "140211",
            "districtName": "南郊区"
          },
          {
            "districtID": 228,
            "cityID": 16,
            "divisionCode": "140212",
            "districtName": "新荣区"
          },
          {
            "districtID": 229,
            "cityID": 16,
            "divisionCode": "140221",
            "districtName": "阳高县"
          },
          {
            "districtID": 230,
            "cityID": 16,
            "divisionCode": "140222",
            "districtName": "天镇县"
          },
          {
            "districtID": 231,
            "cityID": 16,
            "divisionCode": "140223",
            "districtName": "广灵县"
          },
          {
            "districtID": 232,
            "cityID": 16,
            "divisionCode": "140224",
            "districtName": "灵丘县"
          },
          {
            "districtID": 233,
            "cityID": 16,
            "divisionCode": "140225",
            "districtName": "浑源县"
          },
          {
            "districtID": 234,
            "cityID": 16,
            "divisionCode": "140226",
            "districtName": "左云县"
          },
          {
            "districtID": 235,
            "cityID": 16,
            "divisionCode": "140227",
            "districtName": "大同县"
          },
          {
            "districtID": 236,
            "cityID": 17,
            "divisionCode": "140301",
            "districtName": "市辖区"
          },
          {
            "districtID": 237,
            "cityID": 17,
            "divisionCode": "140302",
            "districtName": "城区"
          },
          {
            "districtID": 238,
            "cityID": 17,
            "divisionCode": "140303",
            "districtName": "矿区"
          },
          {
            "districtID": 239,
            "cityID": 17,
            "divisionCode": "140311",
            "districtName": "郊区"
          },
          {
            "districtID": 240,
            "cityID": 17,
            "divisionCode": "140321",
            "districtName": "平定县"
          },
          {
            "districtID": 241,
            "cityID": 17,
            "divisionCode": "140322",
            "districtName": "盂县"
          },
          {
            "districtID": 242,
            "cityID": 18,
            "divisionCode": "140401",
            "districtName": "市辖区"
          },
          {
            "districtID": 243,
            "cityID": 18,
            "divisionCode": "140402",
            "districtName": "城区"
          },
          {
            "districtID": 244,
            "cityID": 18,
            "divisionCode": "140411",
            "districtName": "郊区"
          },
          {
            "districtID": 245,
            "cityID": 18,
            "divisionCode": "140421",
            "districtName": "长治县"
          },
          {
            "districtID": 246,
            "cityID": 18,
            "divisionCode": "140423",
            "districtName": "襄垣县"
          },
          {
            "districtID": 247,
            "cityID": 18,
            "divisionCode": "140424",
            "districtName": "屯留县"
          },
          {
            "districtID": 248,
            "cityID": 18,
            "divisionCode": "140425",
            "districtName": "平顺县"
          },
          {
            "districtID": 249,
            "cityID": 18,
            "divisionCode": "140426",
            "districtName": "黎城县"
          },
          {
            "districtID": 250,
            "cityID": 18,
            "divisionCode": "140427",
            "districtName": "壶关县"
          },
          {
            "districtID": 251,
            "cityID": 18,
            "divisionCode": "140428",
            "districtName": "长子县"
          },
          {
            "districtID": 252,
            "cityID": 18,
            "divisionCode": "140429",
            "districtName": "武乡县"
          },
          {
            "districtID": 253,
            "cityID": 18,
            "divisionCode": "140430",
            "districtName": "沁县"
          },
          {
            "districtID": 254,
            "cityID": 18,
            "divisionCode": "140431",
            "districtName": "沁源县"
          },
          {
            "districtID": 255,
            "cityID": 18,
            "divisionCode": "140481",
            "districtName": "潞城市"
          },
          {
            "districtID": 256,
            "cityID": 19,
            "divisionCode": "140501",
            "districtName": "市辖区"
          },
          {
            "districtID": 257,
            "cityID": 19,
            "divisionCode": "140502",
            "districtName": "城区"
          },
          {
            "districtID": 258,
            "cityID": 19,
            "divisionCode": "140521",
            "districtName": "沁水县"
          },
          {
            "districtID": 259,
            "cityID": 19,
            "divisionCode": "140522",
            "districtName": "阳城县"
          },
          {
            "districtID": 260,
            "cityID": 19,
            "divisionCode": "140524",
            "districtName": "陵川县"
          },
          {
            "districtID": 261,
            "cityID": 19,
            "divisionCode": "140525",
            "districtName": "泽州县"
          },
          {
            "districtID": 262,
            "cityID": 19,
            "divisionCode": "140581",
            "districtName": "高平市"
          },
          {
            "districtID": 263,
            "cityID": 20,
            "divisionCode": "140601",
            "districtName": "市辖区"
          },
          {
            "districtID": 264,
            "cityID": 20,
            "divisionCode": "140602",
            "districtName": "朔城区"
          },
          {
            "districtID": 265,
            "cityID": 20,
            "divisionCode": "140603",
            "districtName": "平鲁区"
          },
          {
            "districtID": 266,
            "cityID": 20,
            "divisionCode": "140621",
            "districtName": "山阴县"
          },
          {
            "districtID": 267,
            "cityID": 20,
            "divisionCode": "140622",
            "districtName": "应县"
          },
          {
            "districtID": 268,
            "cityID": 20,
            "divisionCode": "140623",
            "districtName": "右玉县"
          },
          {
            "districtID": 269,
            "cityID": 20,
            "divisionCode": "140624",
            "districtName": "怀仁县"
          },
          {
            "districtID": 270,
            "cityID": 21,
            "divisionCode": "140701",
            "districtName": "市辖区"
          },
          {
            "districtID": 271,
            "cityID": 21,
            "divisionCode": "140702",
            "districtName": "榆次区"
          },
          {
            "districtID": 272,
            "cityID": 21,
            "divisionCode": "140721",
            "districtName": "榆社县"
          },
          {
            "districtID": 273,
            "cityID": 21,
            "divisionCode": "140722",
            "districtName": "左权县"
          },
          {
            "districtID": 274,
            "cityID": 21,
            "divisionCode": "140723",
            "districtName": "和顺县"
          },
          {
            "districtID": 275,
            "cityID": 21,
            "divisionCode": "140724",
            "districtName": "昔阳县"
          },
          {
            "districtID": 276,
            "cityID": 21,
            "divisionCode": "140725",
            "districtName": "寿阳县"
          },
          {
            "districtID": 277,
            "cityID": 21,
            "divisionCode": "140726",
            "districtName": "太谷县"
          },
          {
            "districtID": 278,
            "cityID": 21,
            "divisionCode": "140727",
            "districtName": "祁县"
          },
          {
            "districtID": 279,
            "cityID": 21,
            "divisionCode": "140728",
            "districtName": "平遥县"
          },
          {
            "districtID": 280,
            "cityID": 21,
            "divisionCode": "140729",
            "districtName": "灵石县"
          },
          {
            "districtID": 281,
            "cityID": 21,
            "divisionCode": "140781",
            "districtName": "介休市"
          },
          {
            "districtID": 282,
            "cityID": 22,
            "divisionCode": "140801",
            "districtName": "市辖区"
          },
          {
            "districtID": 283,
            "cityID": 22,
            "divisionCode": "140802",
            "districtName": "盐湖区"
          },
          {
            "districtID": 284,
            "cityID": 22,
            "divisionCode": "140821",
            "districtName": "临猗县"
          },
          {
            "districtID": 285,
            "cityID": 22,
            "divisionCode": "140822",
            "districtName": "万荣县"
          },
          {
            "districtID": 286,
            "cityID": 22,
            "divisionCode": "140823",
            "districtName": "闻喜县"
          },
          {
            "districtID": 287,
            "cityID": 22,
            "divisionCode": "140824",
            "districtName": "稷山县"
          },
          {
            "districtID": 288,
            "cityID": 22,
            "divisionCode": "140825",
            "districtName": "新绛县"
          },
          {
            "districtID": 289,
            "cityID": 22,
            "divisionCode": "140826",
            "districtName": "绛县"
          },
          {
            "districtID": 290,
            "cityID": 22,
            "divisionCode": "140827",
            "districtName": "垣曲县"
          },
          {
            "districtID": 291,
            "cityID": 22,
            "divisionCode": "140828",
            "districtName": "夏县"
          },
          {
            "districtID": 292,
            "cityID": 22,
            "divisionCode": "140829",
            "districtName": "平陆县"
          },
          {
            "districtID": 293,
            "cityID": 22,
            "divisionCode": "140830",
            "districtName": "芮城县"
          },
          {
            "districtID": 294,
            "cityID": 22,
            "divisionCode": "140881",
            "districtName": "永济市"
          },
          {
            "districtID": 295,
            "cityID": 22,
            "divisionCode": "140882",
            "districtName": "河津市"
          },
          {
            "districtID": 296,
            "cityID": 23,
            "divisionCode": "140901",
            "districtName": "市辖区"
          },
          {
            "districtID": 297,
            "cityID": 23,
            "divisionCode": "140902",
            "districtName": "忻府区"
          },
          {
            "districtID": 298,
            "cityID": 23,
            "divisionCode": "140921",
            "districtName": "定襄县"
          },
          {
            "districtID": 299,
            "cityID": 23,
            "divisionCode": "140922",
            "districtName": "五台县"
          },
          {
            "districtID": 300,
            "cityID": 23,
            "divisionCode": "140923",
            "districtName": "代县"
          },
          {
            "districtID": 301,
            "cityID": 23,
            "divisionCode": "140924",
            "districtName": "繁峙县"
          },
          {
            "districtID": 302,
            "cityID": 23,
            "divisionCode": "140925",
            "districtName": "宁武县"
          },
          {
            "districtID": 303,
            "cityID": 23,
            "divisionCode": "140926",
            "districtName": "静乐县"
          },
          {
            "districtID": 304,
            "cityID": 23,
            "divisionCode": "140927",
            "districtName": "神池县"
          },
          {
            "districtID": 305,
            "cityID": 23,
            "divisionCode": "140928",
            "districtName": "五寨县"
          },
          {
            "districtID": 306,
            "cityID": 23,
            "divisionCode": "140929",
            "districtName": "岢岚县"
          },
          {
            "districtID": 307,
            "cityID": 23,
            "divisionCode": "140930",
            "districtName": "河曲县"
          },
          {
            "districtID": 308,
            "cityID": 23,
            "divisionCode": "140931",
            "districtName": "保德县"
          },
          {
            "districtID": 309,
            "cityID": 23,
            "divisionCode": "140932",
            "districtName": "偏关县"
          },
          {
            "districtID": 310,
            "cityID": 23,
            "divisionCode": "140981",
            "districtName": "原平市"
          },
          {
            "districtID": 311,
            "cityID": 24,
            "divisionCode": "141001",
            "districtName": "市辖区"
          },
          {
            "districtID": 312,
            "cityID": 24,
            "divisionCode": "141002",
            "districtName": "尧都区"
          },
          {
            "districtID": 313,
            "cityID": 24,
            "divisionCode": "141021",
            "districtName": "曲沃县"
          },
          {
            "districtID": 314,
            "cityID": 24,
            "divisionCode": "141022",
            "districtName": "翼城县"
          },
          {
            "districtID": 315,
            "cityID": 24,
            "divisionCode": "141023",
            "districtName": "襄汾县"
          },
          {
            "districtID": 316,
            "cityID": 24,
            "divisionCode": "141024",
            "districtName": "洪洞县"
          },
          {
            "districtID": 317,
            "cityID": 24,
            "divisionCode": "141025",
            "districtName": "古县"
          },
          {
            "districtID": 318,
            "cityID": 24,
            "divisionCode": "141026",
            "districtName": "安泽县"
          },
          {
            "districtID": 319,
            "cityID": 24,
            "divisionCode": "141027",
            "districtName": "浮山县"
          },
          {
            "districtID": 320,
            "cityID": 24,
            "divisionCode": "141028",
            "districtName": "吉县"
          },
          {
            "districtID": 321,
            "cityID": 24,
            "divisionCode": "141029",
            "districtName": "乡宁县"
          },
          {
            "districtID": 322,
            "cityID": 24,
            "divisionCode": "141030",
            "districtName": "大宁县"
          },
          {
            "districtID": 323,
            "cityID": 24,
            "divisionCode": "141031",
            "districtName": "隰县"
          },
          {
            "districtID": 324,
            "cityID": 24,
            "divisionCode": "141032",
            "districtName": "永和县"
          },
          {
            "districtID": 325,
            "cityID": 24,
            "divisionCode": "141033",
            "districtName": "蒲县"
          },
          {
            "districtID": 326,
            "cityID": 24,
            "divisionCode": "141034",
            "districtName": "汾西县"
          },
          {
            "districtID": 327,
            "cityID": 24,
            "divisionCode": "141081",
            "districtName": "侯马市"
          },
          {
            "districtID": 328,
            "cityID": 24,
            "divisionCode": "141082",
            "districtName": "霍州市"
          },
          {
            "districtID": 329,
            "cityID": 25,
            "divisionCode": "141101",
            "districtName": "市辖区"
          },
          {
            "districtID": 330,
            "cityID": 25,
            "divisionCode": "141102",
            "districtName": "离石区"
          },
          {
            "districtID": 331,
            "cityID": 25,
            "divisionCode": "141121",
            "districtName": "文水县"
          },
          {
            "districtID": 332,
            "cityID": 25,
            "divisionCode": "141122",
            "districtName": "交城县"
          },
          {
            "districtID": 333,
            "cityID": 25,
            "divisionCode": "141123",
            "districtName": "兴县"
          },
          {
            "districtID": 334,
            "cityID": 25,
            "divisionCode": "141124",
            "districtName": "临县"
          },
          {
            "districtID": 335,
            "cityID": 25,
            "divisionCode": "141125",
            "districtName": "柳林县"
          },
          {
            "districtID": 336,
            "cityID": 25,
            "divisionCode": "141126",
            "districtName": "石楼县"
          },
          {
            "districtID": 337,
            "cityID": 25,
            "divisionCode": "141127",
            "districtName": "岚县"
          },
          {
            "districtID": 338,
            "cityID": 25,
            "divisionCode": "141128",
            "districtName": "方山县"
          },
          {
            "districtID": 339,
            "cityID": 25,
            "divisionCode": "141129",
            "districtName": "中阳县"
          },
          {
            "districtID": 340,
            "cityID": 25,
            "divisionCode": "141130",
            "districtName": "交口县"
          },
          {
            "districtID": 341,
            "cityID": 25,
            "divisionCode": "141181",
            "districtName": "孝义市"
          },
          {
            "districtID": 342,
            "cityID": 25,
            "divisionCode": "141182",
            "districtName": "汾阳市"
          },
          {
            "districtID": 343,
            "cityID": 26,
            "divisionCode": "150101",
            "districtName": "市辖区"
          },
          {
            "districtID": 344,
            "cityID": 26,
            "divisionCode": "150102",
            "districtName": "新城区"
          },
          {
            "districtID": 345,
            "cityID": 26,
            "divisionCode": "150103",
            "districtName": "回民区"
          },
          {
            "districtID": 346,
            "cityID": 26,
            "divisionCode": "150104",
            "districtName": "玉泉区"
          },
          {
            "districtID": 347,
            "cityID": 26,
            "divisionCode": "150105",
            "districtName": "赛罕区"
          },
          {
            "districtID": 348,
            "cityID": 26,
            "divisionCode": "150121",
            "districtName": "土默特左旗"
          },
          {
            "districtID": 349,
            "cityID": 26,
            "divisionCode": "150122",
            "districtName": "托克托县"
          },
          {
            "districtID": 350,
            "cityID": 26,
            "divisionCode": "150123",
            "districtName": "和林格尔县"
          },
          {
            "districtID": 351,
            "cityID": 26,
            "divisionCode": "150124",
            "districtName": "清水河县"
          },
          {
            "districtID": 352,
            "cityID": 26,
            "divisionCode": "150125",
            "districtName": "武川县"
          },
          {
            "districtID": 353,
            "cityID": 27,
            "divisionCode": "150201",
            "districtName": "市辖区"
          },
          {
            "districtID": 354,
            "cityID": 27,
            "divisionCode": "150202",
            "districtName": "东河区"
          },
          {
            "districtID": 355,
            "cityID": 27,
            "divisionCode": "150203",
            "districtName": "昆都仑区"
          },
          {
            "districtID": 356,
            "cityID": 27,
            "divisionCode": "150204",
            "districtName": "青山区"
          },
          {
            "districtID": 357,
            "cityID": 27,
            "divisionCode": "150205",
            "districtName": "石拐区"
          },
          {
            "districtID": 358,
            "cityID": 27,
            "divisionCode": "150206",
            "districtName": "白云鄂博矿区"
          },
          {
            "districtID": 359,
            "cityID": 27,
            "divisionCode": "150207",
            "districtName": "九原区"
          },
          {
            "districtID": 360,
            "cityID": 27,
            "divisionCode": "150221",
            "districtName": "土默特右旗"
          },
          {
            "districtID": 361,
            "cityID": 27,
            "divisionCode": "150222",
            "districtName": "固阳县"
          },
          {
            "districtID": 362,
            "cityID": 27,
            "divisionCode": "150223",
            "districtName": "达尔罕茂明安联合旗"
          },
          {
            "districtID": 363,
            "cityID": 28,
            "divisionCode": "150301",
            "districtName": "市辖区"
          },
          {
            "districtID": 364,
            "cityID": 28,
            "divisionCode": "150302",
            "districtName": "海勃湾区"
          },
          {
            "districtID": 365,
            "cityID": 28,
            "divisionCode": "150303",
            "districtName": "海南区"
          },
          {
            "districtID": 366,
            "cityID": 28,
            "divisionCode": "150304",
            "districtName": "乌达区"
          },
          {
            "districtID": 367,
            "cityID": 29,
            "divisionCode": "150401",
            "districtName": "市辖区"
          },
          {
            "districtID": 368,
            "cityID": 29,
            "divisionCode": "150402",
            "districtName": "红山区"
          },
          {
            "districtID": 369,
            "cityID": 29,
            "divisionCode": "150403",
            "districtName": "元宝山区"
          },
          {
            "districtID": 370,
            "cityID": 29,
            "divisionCode": "150404",
            "districtName": "松山区"
          },
          {
            "districtID": 371,
            "cityID": 29,
            "divisionCode": "150421",
            "districtName": "阿鲁科尔沁旗"
          },
          {
            "districtID": 372,
            "cityID": 29,
            "divisionCode": "150422",
            "districtName": "巴林左旗"
          },
          {
            "districtID": 373,
            "cityID": 29,
            "divisionCode": "150423",
            "districtName": "巴林右旗"
          },
          {
            "districtID": 374,
            "cityID": 29,
            "divisionCode": "150424",
            "districtName": "林西县"
          },
          {
            "districtID": 375,
            "cityID": 29,
            "divisionCode": "150425",
            "districtName": "克什克腾旗"
          },
          {
            "districtID": 376,
            "cityID": 29,
            "divisionCode": "150426",
            "districtName": "翁牛特旗"
          },
          {
            "districtID": 377,
            "cityID": 29,
            "divisionCode": "150428",
            "districtName": "喀喇沁旗"
          },
          {
            "districtID": 378,
            "cityID": 29,
            "divisionCode": "150429",
            "districtName": "宁城县"
          },
          {
            "districtID": 379,
            "cityID": 29,
            "divisionCode": "150430",
            "districtName": "敖汉旗"
          },
          {
            "districtID": 380,
            "cityID": 30,
            "divisionCode": "150501",
            "districtName": "市辖区"
          },
          {
            "districtID": 381,
            "cityID": 30,
            "divisionCode": "150502",
            "districtName": "科尔沁区"
          },
          {
            "districtID": 382,
            "cityID": 30,
            "divisionCode": "150521",
            "districtName": "科尔沁左翼中旗"
          },
          {
            "districtID": 383,
            "cityID": 30,
            "divisionCode": "150522",
            "districtName": "科尔沁左翼后旗"
          },
          {
            "districtID": 384,
            "cityID": 30,
            "divisionCode": "150523",
            "districtName": "开鲁县"
          },
          {
            "districtID": 385,
            "cityID": 30,
            "divisionCode": "150524",
            "districtName": "库伦旗"
          },
          {
            "districtID": 386,
            "cityID": 30,
            "divisionCode": "150525",
            "districtName": "奈曼旗"
          },
          {
            "districtID": 387,
            "cityID": 30,
            "divisionCode": "150526",
            "districtName": "扎鲁特旗"
          },
          {
            "districtID": 388,
            "cityID": 30,
            "divisionCode": "150581",
            "districtName": "霍林郭勒市"
          },
          {
            "districtID": 389,
            "cityID": 31,
            "divisionCode": "150601",
            "districtName": "市辖区"
          },
          {
            "districtID": 390,
            "cityID": 31,
            "divisionCode": "150602",
            "districtName": "东胜区"
          },
          {
            "districtID": 391,
            "cityID": 31,
            "divisionCode": "150603",
            "districtName": "康巴什区"
          },
          {
            "districtID": 392,
            "cityID": 31,
            "divisionCode": "150621",
            "districtName": "达拉特旗"
          },
          {
            "districtID": 393,
            "cityID": 31,
            "divisionCode": "150622",
            "districtName": "准格尔旗"
          },
          {
            "districtID": 394,
            "cityID": 31,
            "divisionCode": "150623",
            "districtName": "鄂托克前旗"
          },
          {
            "districtID": 395,
            "cityID": 31,
            "divisionCode": "150624",
            "districtName": "鄂托克旗"
          },
          {
            "districtID": 396,
            "cityID": 31,
            "divisionCode": "150625",
            "districtName": "杭锦旗"
          },
          {
            "districtID": 397,
            "cityID": 31,
            "divisionCode": "150626",
            "districtName": "乌审旗"
          },
          {
            "districtID": 398,
            "cityID": 31,
            "divisionCode": "150627",
            "districtName": "伊金霍洛旗"
          },
          {
            "districtID": 399,
            "cityID": 32,
            "divisionCode": "150701",
            "districtName": "市辖区"
          },
          {
            "districtID": 400,
            "cityID": 32,
            "divisionCode": "150702",
            "districtName": "海拉尔区"
          },
          {
            "districtID": 401,
            "cityID": 32,
            "divisionCode": "150703",
            "districtName": "扎赉诺尔区"
          },
          {
            "districtID": 402,
            "cityID": 32,
            "divisionCode": "150721",
            "districtName": "阿荣旗"
          },
          {
            "districtID": 403,
            "cityID": 32,
            "divisionCode": "150722",
            "districtName": "莫力达瓦达斡尔族自治旗"
          },
          {
            "districtID": 404,
            "cityID": 32,
            "divisionCode": "150723",
            "districtName": "鄂伦春自治旗"
          },
          {
            "districtID": 405,
            "cityID": 32,
            "divisionCode": "150724",
            "districtName": "鄂温克族自治旗"
          },
          {
            "districtID": 406,
            "cityID": 32,
            "divisionCode": "150725",
            "districtName": "陈巴尔虎旗"
          },
          {
            "districtID": 407,
            "cityID": 32,
            "divisionCode": "150726",
            "districtName": "新巴尔虎左旗"
          },
          {
            "districtID": 408,
            "cityID": 32,
            "divisionCode": "150727",
            "districtName": "新巴尔虎右旗"
          },
          {
            "districtID": 409,
            "cityID": 32,
            "divisionCode": "150781",
            "districtName": "满洲里市"
          },
          {
            "districtID": 410,
            "cityID": 32,
            "divisionCode": "150782",
            "districtName": "牙克石市"
          },
          {
            "districtID": 411,
            "cityID": 32,
            "divisionCode": "150783",
            "districtName": "扎兰屯市"
          },
          {
            "districtID": 412,
            "cityID": 32,
            "divisionCode": "150784",
            "districtName": "额尔古纳市"
          },
          {
            "districtID": 413,
            "cityID": 32,
            "divisionCode": "150785",
            "districtName": "根河市"
          },
          {
            "districtID": 414,
            "cityID": 33,
            "divisionCode": "150801",
            "districtName": "市辖区"
          },
          {
            "districtID": 415,
            "cityID": 33,
            "divisionCode": "150802",
            "districtName": "临河区"
          },
          {
            "districtID": 416,
            "cityID": 33,
            "divisionCode": "150821",
            "districtName": "五原县"
          },
          {
            "districtID": 417,
            "cityID": 33,
            "divisionCode": "150822",
            "districtName": "磴口县"
          },
          {
            "districtID": 418,
            "cityID": 33,
            "divisionCode": "150823",
            "districtName": "乌拉特前旗"
          },
          {
            "districtID": 419,
            "cityID": 33,
            "divisionCode": "150824",
            "districtName": "乌拉特中旗"
          },
          {
            "districtID": 420,
            "cityID": 33,
            "divisionCode": "150825",
            "districtName": "乌拉特后旗"
          },
          {
            "districtID": 421,
            "cityID": 33,
            "divisionCode": "150826",
            "districtName": "杭锦后旗"
          },
          {
            "districtID": 422,
            "cityID": 34,
            "divisionCode": "150901",
            "districtName": "市辖区"
          },
          {
            "districtID": 423,
            "cityID": 34,
            "divisionCode": "150902",
            "districtName": "集宁区"
          },
          {
            "districtID": 424,
            "cityID": 34,
            "divisionCode": "150921",
            "districtName": "卓资县"
          },
          {
            "districtID": 425,
            "cityID": 34,
            "divisionCode": "150922",
            "districtName": "化德县"
          },
          {
            "districtID": 426,
            "cityID": 34,
            "divisionCode": "150923",
            "districtName": "商都县"
          },
          {
            "districtID": 427,
            "cityID": 34,
            "divisionCode": "150924",
            "districtName": "兴和县"
          },
          {
            "districtID": 428,
            "cityID": 34,
            "divisionCode": "150925",
            "districtName": "凉城县"
          },
          {
            "districtID": 429,
            "cityID": 34,
            "divisionCode": "150926",
            "districtName": "察哈尔右翼前旗"
          },
          {
            "districtID": 430,
            "cityID": 34,
            "divisionCode": "150927",
            "districtName": "察哈尔右翼中旗"
          },
          {
            "districtID": 431,
            "cityID": 34,
            "divisionCode": "150928",
            "districtName": "察哈尔右翼后旗"
          },
          {
            "districtID": 432,
            "cityID": 34,
            "divisionCode": "150929",
            "districtName": "四子王旗"
          },
          {
            "districtID": 433,
            "cityID": 34,
            "divisionCode": "150981",
            "districtName": "丰镇市"
          },
          {
            "districtID": 434,
            "cityID": 35,
            "divisionCode": "152201",
            "districtName": "乌兰浩特市"
          },
          {
            "districtID": 435,
            "cityID": 35,
            "divisionCode": "152202",
            "districtName": "阿尔山市"
          },
          {
            "districtID": 436,
            "cityID": 35,
            "divisionCode": "152221",
            "districtName": "科尔沁右翼前旗"
          },
          {
            "districtID": 437,
            "cityID": 35,
            "divisionCode": "152222",
            "districtName": "科尔沁右翼中旗"
          },
          {
            "districtID": 438,
            "cityID": 35,
            "divisionCode": "152223",
            "districtName": "扎赉特旗"
          },
          {
            "districtID": 439,
            "cityID": 35,
            "divisionCode": "152224",
            "districtName": "突泉县"
          },
          {
            "districtID": 440,
            "cityID": 36,
            "divisionCode": "152501",
            "districtName": "二连浩特市"
          },
          {
            "districtID": 441,
            "cityID": 36,
            "divisionCode": "152502",
            "districtName": "锡林浩特市"
          },
          {
            "districtID": 442,
            "cityID": 36,
            "divisionCode": "152522",
            "districtName": "阿巴嘎旗"
          },
          {
            "districtID": 443,
            "cityID": 36,
            "divisionCode": "152523",
            "districtName": "苏尼特左旗"
          },
          {
            "districtID": 444,
            "cityID": 36,
            "divisionCode": "152524",
            "districtName": "苏尼特右旗"
          },
          {
            "districtID": 445,
            "cityID": 36,
            "divisionCode": "152525",
            "districtName": "东乌珠穆沁旗"
          },
          {
            "districtID": 446,
            "cityID": 36,
            "divisionCode": "152526",
            "districtName": "西乌珠穆沁旗"
          },
          {
            "districtID": 447,
            "cityID": 36,
            "divisionCode": "152527",
            "districtName": "太仆寺旗"
          },
          {
            "districtID": 448,
            "cityID": 36,
            "divisionCode": "152528",
            "districtName": "镶黄旗"
          },
          {
            "districtID": 449,
            "cityID": 36,
            "divisionCode": "152529",
            "districtName": "正镶白旗"
          },
          {
            "districtID": 450,
            "cityID": 36,
            "divisionCode": "152530",
            "districtName": "正蓝旗"
          },
          {
            "districtID": 451,
            "cityID": 36,
            "divisionCode": "152531",
            "districtName": "多伦县"
          },
          {
            "districtID": 452,
            "cityID": 37,
            "divisionCode": "152921",
            "districtName": "阿拉善左旗"
          },
          {
            "districtID": 453,
            "cityID": 37,
            "divisionCode": "152922",
            "districtName": "阿拉善右旗"
          },
          {
            "districtID": 454,
            "cityID": 37,
            "divisionCode": "152923",
            "districtName": "额济纳旗"
          },
          {
            "districtID": 455,
            "cityID": 38,
            "divisionCode": "210101",
            "districtName": "市辖区"
          },
          {
            "districtID": 456,
            "cityID": 38,
            "divisionCode": "210102",
            "districtName": "和平区"
          },
          {
            "districtID": 457,
            "cityID": 38,
            "divisionCode": "210103",
            "districtName": "沈河区"
          },
          {
            "districtID": 458,
            "cityID": 38,
            "divisionCode": "210104",
            "districtName": "大东区"
          },
          {
            "districtID": 459,
            "cityID": 38,
            "divisionCode": "210105",
            "districtName": "皇姑区"
          },
          {
            "districtID": 460,
            "cityID": 38,
            "divisionCode": "210106",
            "districtName": "铁西区"
          },
          {
            "districtID": 461,
            "cityID": 38,
            "divisionCode": "210111",
            "districtName": "苏家屯区"
          },
          {
            "districtID": 462,
            "cityID": 38,
            "divisionCode": "210112",
            "districtName": "浑南区"
          },
          {
            "districtID": 463,
            "cityID": 38,
            "divisionCode": "210113",
            "districtName": "沈北新区"
          },
          {
            "districtID": 464,
            "cityID": 38,
            "divisionCode": "210114",
            "districtName": "于洪区"
          },
          {
            "districtID": 465,
            "cityID": 38,
            "divisionCode": "210115",
            "districtName": "辽中区"
          },
          {
            "districtID": 466,
            "cityID": 38,
            "divisionCode": "210123",
            "districtName": "康平县"
          },
          {
            "districtID": 467,
            "cityID": 38,
            "divisionCode": "210124",
            "districtName": "法库县"
          },
          {
            "districtID": 468,
            "cityID": 38,
            "divisionCode": "210181",
            "districtName": "新民市"
          },
          {
            "districtID": 469,
            "cityID": 39,
            "divisionCode": "210201",
            "districtName": "市辖区"
          },
          {
            "districtID": 470,
            "cityID": 39,
            "divisionCode": "210202",
            "districtName": "中山区"
          },
          {
            "districtID": 471,
            "cityID": 39,
            "divisionCode": "210203",
            "districtName": "西岗区"
          },
          {
            "districtID": 472,
            "cityID": 39,
            "divisionCode": "210204",
            "districtName": "沙河口区"
          },
          {
            "districtID": 473,
            "cityID": 39,
            "divisionCode": "210211",
            "districtName": "甘井子区"
          },
          {
            "districtID": 474,
            "cityID": 39,
            "divisionCode": "210212",
            "districtName": "旅顺口区"
          },
          {
            "districtID": 475,
            "cityID": 39,
            "divisionCode": "210213",
            "districtName": "金州区"
          },
          {
            "districtID": 476,
            "cityID": 39,
            "divisionCode": "210214",
            "districtName": "普兰店区"
          },
          {
            "districtID": 477,
            "cityID": 39,
            "divisionCode": "210224",
            "districtName": "长海县"
          },
          {
            "districtID": 478,
            "cityID": 39,
            "divisionCode": "210281",
            "districtName": "瓦房店市"
          },
          {
            "districtID": 479,
            "cityID": 39,
            "divisionCode": "210283",
            "districtName": "庄河市"
          },
          {
            "districtID": 480,
            "cityID": 40,
            "divisionCode": "210301",
            "districtName": "市辖区"
          },
          {
            "districtID": 481,
            "cityID": 40,
            "divisionCode": "210302",
            "districtName": "铁东区"
          },
          {
            "districtID": 482,
            "cityID": 40,
            "divisionCode": "210303",
            "districtName": "铁西区"
          },
          {
            "districtID": 483,
            "cityID": 40,
            "divisionCode": "210304",
            "districtName": "立山区"
          },
          {
            "districtID": 484,
            "cityID": 40,
            "divisionCode": "210311",
            "districtName": "千山区"
          },
          {
            "districtID": 485,
            "cityID": 40,
            "divisionCode": "210321",
            "districtName": "台安县"
          },
          {
            "districtID": 486,
            "cityID": 40,
            "divisionCode": "210323",
            "districtName": "岫岩满族自治县"
          },
          {
            "districtID": 487,
            "cityID": 40,
            "divisionCode": "210381",
            "districtName": "海城市"
          },
          {
            "districtID": 488,
            "cityID": 41,
            "divisionCode": "210401",
            "districtName": "市辖区"
          },
          {
            "districtID": 489,
            "cityID": 41,
            "divisionCode": "210402",
            "districtName": "新抚区"
          },
          {
            "districtID": 490,
            "cityID": 41,
            "divisionCode": "210403",
            "districtName": "东洲区"
          },
          {
            "districtID": 491,
            "cityID": 41,
            "divisionCode": "210404",
            "districtName": "望花区"
          },
          {
            "districtID": 492,
            "cityID": 41,
            "divisionCode": "210411",
            "districtName": "顺城区"
          },
          {
            "districtID": 493,
            "cityID": 41,
            "divisionCode": "210421",
            "districtName": "抚顺县"
          },
          {
            "districtID": 494,
            "cityID": 41,
            "divisionCode": "210422",
            "districtName": "新宾满族自治县"
          },
          {
            "districtID": 495,
            "cityID": 41,
            "divisionCode": "210423",
            "districtName": "清原满族自治县"
          },
          {
            "districtID": 496,
            "cityID": 42,
            "divisionCode": "210501",
            "districtName": "市辖区"
          },
          {
            "districtID": 497,
            "cityID": 42,
            "divisionCode": "210502",
            "districtName": "平山区"
          },
          {
            "districtID": 498,
            "cityID": 42,
            "divisionCode": "210503",
            "districtName": "溪湖区"
          },
          {
            "districtID": 499,
            "cityID": 42,
            "divisionCode": "210504",
            "districtName": "明山区"
          },
          {
            "districtID": 500,
            "cityID": 42,
            "divisionCode": "210505",
            "districtName": "南芬区"
          },
          {
            "districtID": 501,
            "cityID": 42,
            "divisionCode": "210521",
            "districtName": "本溪满族自治县"
          },
          {
            "districtID": 502,
            "cityID": 42,
            "divisionCode": "210522",
            "districtName": "桓仁满族自治县"
          },
          {
            "districtID": 503,
            "cityID": 43,
            "divisionCode": "210601",
            "districtName": "市辖区"
          },
          {
            "districtID": 504,
            "cityID": 43,
            "divisionCode": "210602",
            "districtName": "元宝区"
          },
          {
            "districtID": 505,
            "cityID": 43,
            "divisionCode": "210603",
            "districtName": "振兴区"
          },
          {
            "districtID": 506,
            "cityID": 43,
            "divisionCode": "210604",
            "districtName": "振安区"
          },
          {
            "districtID": 507,
            "cityID": 43,
            "divisionCode": "210624",
            "districtName": "宽甸满族自治县"
          },
          {
            "districtID": 508,
            "cityID": 43,
            "divisionCode": "210681",
            "districtName": "东港市"
          },
          {
            "districtID": 509,
            "cityID": 43,
            "divisionCode": "210682",
            "districtName": "凤城市"
          },
          {
            "districtID": 510,
            "cityID": 44,
            "divisionCode": "210701",
            "districtName": "市辖区"
          },
          {
            "districtID": 511,
            "cityID": 44,
            "divisionCode": "210702",
            "districtName": "古塔区"
          },
          {
            "districtID": 512,
            "cityID": 44,
            "divisionCode": "210703",
            "districtName": "凌河区"
          },
          {
            "districtID": 513,
            "cityID": 44,
            "divisionCode": "210711",
            "districtName": "太和区"
          },
          {
            "districtID": 514,
            "cityID": 44,
            "divisionCode": "210726",
            "districtName": "黑山县"
          },
          {
            "districtID": 515,
            "cityID": 44,
            "divisionCode": "210727",
            "districtName": "义县"
          },
          {
            "districtID": 516,
            "cityID": 44,
            "divisionCode": "210781",
            "districtName": "凌海市"
          },
          {
            "districtID": 517,
            "cityID": 44,
            "divisionCode": "210782",
            "districtName": "北镇市"
          },
          {
            "districtID": 518,
            "cityID": 45,
            "divisionCode": "210801",
            "districtName": "市辖区"
          },
          {
            "districtID": 519,
            "cityID": 45,
            "divisionCode": "210802",
            "districtName": "站前区"
          },
          {
            "districtID": 520,
            "cityID": 45,
            "divisionCode": "210803",
            "districtName": "西市区"
          },
          {
            "districtID": 521,
            "cityID": 45,
            "divisionCode": "210804",
            "districtName": "鲅鱼圈区"
          },
          {
            "districtID": 522,
            "cityID": 45,
            "divisionCode": "210811",
            "districtName": "老边区"
          },
          {
            "districtID": 523,
            "cityID": 45,
            "divisionCode": "210881",
            "districtName": "盖州市"
          },
          {
            "districtID": 524,
            "cityID": 45,
            "divisionCode": "210882",
            "districtName": "大石桥市"
          },
          {
            "districtID": 525,
            "cityID": 46,
            "divisionCode": "210901",
            "districtName": "市辖区"
          },
          {
            "districtID": 526,
            "cityID": 46,
            "divisionCode": "210902",
            "districtName": "海州区"
          },
          {
            "districtID": 527,
            "cityID": 46,
            "divisionCode": "210903",
            "districtName": "新邱区"
          },
          {
            "districtID": 528,
            "cityID": 46,
            "divisionCode": "210904",
            "districtName": "太平区"
          },
          {
            "districtID": 529,
            "cityID": 46,
            "divisionCode": "210905",
            "districtName": "清河门区"
          },
          {
            "districtID": 530,
            "cityID": 46,
            "divisionCode": "210911",
            "districtName": "细河区"
          },
          {
            "districtID": 531,
            "cityID": 46,
            "divisionCode": "210921",
            "districtName": "阜新蒙古族自治县"
          },
          {
            "districtID": 532,
            "cityID": 46,
            "divisionCode": "210922",
            "districtName": "彰武县"
          },
          {
            "districtID": 533,
            "cityID": 47,
            "divisionCode": "211001",
            "districtName": "市辖区"
          },
          {
            "districtID": 534,
            "cityID": 47,
            "divisionCode": "211002",
            "districtName": "白塔区"
          },
          {
            "districtID": 535,
            "cityID": 47,
            "divisionCode": "211003",
            "districtName": "文圣区"
          },
          {
            "districtID": 536,
            "cityID": 47,
            "divisionCode": "211004",
            "districtName": "宏伟区"
          },
          {
            "districtID": 537,
            "cityID": 47,
            "divisionCode": "211005",
            "districtName": "弓长岭区"
          },
          {
            "districtID": 538,
            "cityID": 47,
            "divisionCode": "211011",
            "districtName": "太子河区"
          },
          {
            "districtID": 539,
            "cityID": 47,
            "divisionCode": "211021",
            "districtName": "辽阳县"
          },
          {
            "districtID": 540,
            "cityID": 47,
            "divisionCode": "211081",
            "districtName": "灯塔市"
          },
          {
            "districtID": 541,
            "cityID": 48,
            "divisionCode": "211101",
            "districtName": "市辖区"
          },
          {
            "districtID": 542,
            "cityID": 48,
            "divisionCode": "211102",
            "districtName": "双台子区"
          },
          {
            "districtID": 543,
            "cityID": 48,
            "divisionCode": "211103",
            "districtName": "兴隆台区"
          },
          {
            "districtID": 544,
            "cityID": 48,
            "divisionCode": "211104",
            "districtName": "大洼区"
          },
          {
            "districtID": 545,
            "cityID": 48,
            "divisionCode": "211122",
            "districtName": "盘山县"
          },
          {
            "districtID": 546,
            "cityID": 49,
            "divisionCode": "211201",
            "districtName": "市辖区"
          },
          {
            "districtID": 547,
            "cityID": 49,
            "divisionCode": "211202",
            "districtName": "银州区"
          },
          {
            "districtID": 548,
            "cityID": 49,
            "divisionCode": "211204",
            "districtName": "清河区"
          },
          {
            "districtID": 549,
            "cityID": 49,
            "divisionCode": "211221",
            "districtName": "铁岭县"
          },
          {
            "districtID": 550,
            "cityID": 49,
            "divisionCode": "211223",
            "districtName": "西丰县"
          },
          {
            "districtID": 551,
            "cityID": 49,
            "divisionCode": "211224",
            "districtName": "昌图县"
          },
          {
            "districtID": 552,
            "cityID": 49,
            "divisionCode": "211281",
            "districtName": "调兵山市"
          },
          {
            "districtID": 553,
            "cityID": 49,
            "divisionCode": "211282",
            "districtName": "开原市"
          },
          {
            "districtID": 554,
            "cityID": 50,
            "divisionCode": "211301",
            "districtName": "市辖区"
          },
          {
            "districtID": 555,
            "cityID": 50,
            "divisionCode": "211302",
            "districtName": "双塔区"
          },
          {
            "districtID": 556,
            "cityID": 50,
            "divisionCode": "211303",
            "districtName": "龙城区"
          },
          {
            "districtID": 557,
            "cityID": 50,
            "divisionCode": "211321",
            "districtName": "朝阳县"
          },
          {
            "districtID": 558,
            "cityID": 50,
            "divisionCode": "211322",
            "districtName": "建平县"
          },
          {
            "districtID": 559,
            "cityID": 50,
            "divisionCode": "211324",
            "districtName": "喀喇沁左翼蒙古族自治县"
          },
          {
            "districtID": 560,
            "cityID": 50,
            "divisionCode": "211381",
            "districtName": "北票市"
          },
          {
            "districtID": 561,
            "cityID": 50,
            "divisionCode": "211382",
            "districtName": "凌源市"
          },
          {
            "districtID": 562,
            "cityID": 51,
            "divisionCode": "211401",
            "districtName": "市辖区"
          },
          {
            "districtID": 563,
            "cityID": 51,
            "divisionCode": "211402",
            "districtName": "连山区"
          },
          {
            "districtID": 564,
            "cityID": 51,
            "divisionCode": "211403",
            "districtName": "龙港区"
          },
          {
            "districtID": 565,
            "cityID": 51,
            "divisionCode": "211404",
            "districtName": "南票区"
          },
          {
            "districtID": 566,
            "cityID": 51,
            "divisionCode": "211421",
            "districtName": "绥中县"
          },
          {
            "districtID": 567,
            "cityID": 51,
            "divisionCode": "211422",
            "districtName": "建昌县"
          },
          {
            "districtID": 568,
            "cityID": 51,
            "divisionCode": "211481",
            "districtName": "兴城市"
          },
          {
            "districtID": 569,
            "cityID": 52,
            "divisionCode": "220101",
            "districtName": "市辖区"
          },
          {
            "districtID": 570,
            "cityID": 52,
            "divisionCode": "220102",
            "districtName": "南关区"
          },
          {
            "districtID": 571,
            "cityID": 52,
            "divisionCode": "220103",
            "districtName": "宽城区"
          },
          {
            "districtID": 572,
            "cityID": 52,
            "divisionCode": "220104",
            "districtName": "朝阳区"
          },
          {
            "districtID": 573,
            "cityID": 52,
            "divisionCode": "220105",
            "districtName": "二道区"
          },
          {
            "districtID": 574,
            "cityID": 52,
            "divisionCode": "220106",
            "districtName": "绿园区"
          },
          {
            "districtID": 575,
            "cityID": 52,
            "divisionCode": "220112",
            "districtName": "双阳区"
          },
          {
            "districtID": 576,
            "cityID": 52,
            "divisionCode": "220113",
            "districtName": "九台区"
          },
          {
            "districtID": 577,
            "cityID": 52,
            "divisionCode": "220122",
            "districtName": "农安县"
          },
          {
            "districtID": 578,
            "cityID": 52,
            "divisionCode": "220182",
            "districtName": "榆树市"
          },
          {
            "districtID": 579,
            "cityID": 52,
            "divisionCode": "220183",
            "districtName": "德惠市"
          },
          {
            "districtID": 580,
            "cityID": 53,
            "divisionCode": "220201",
            "districtName": "市辖区"
          },
          {
            "districtID": 581,
            "cityID": 53,
            "divisionCode": "220202",
            "districtName": "昌邑区"
          },
          {
            "districtID": 582,
            "cityID": 53,
            "divisionCode": "220203",
            "districtName": "龙潭区"
          },
          {
            "districtID": 583,
            "cityID": 53,
            "divisionCode": "220204",
            "districtName": "船营区"
          },
          {
            "districtID": 584,
            "cityID": 53,
            "divisionCode": "220211",
            "districtName": "丰满区"
          },
          {
            "districtID": 585,
            "cityID": 53,
            "divisionCode": "220221",
            "districtName": "永吉县"
          },
          {
            "districtID": 586,
            "cityID": 53,
            "divisionCode": "220281",
            "districtName": "蛟河市"
          },
          {
            "districtID": 587,
            "cityID": 53,
            "divisionCode": "220282",
            "districtName": "桦甸市"
          },
          {
            "districtID": 588,
            "cityID": 53,
            "divisionCode": "220283",
            "districtName": "舒兰市"
          },
          {
            "districtID": 589,
            "cityID": 53,
            "divisionCode": "220284",
            "districtName": "磐石市"
          },
          {
            "districtID": 590,
            "cityID": 54,
            "divisionCode": "220301",
            "districtName": "市辖区"
          },
          {
            "districtID": 591,
            "cityID": 54,
            "divisionCode": "220302",
            "districtName": "铁西区"
          },
          {
            "districtID": 592,
            "cityID": 54,
            "divisionCode": "220303",
            "districtName": "铁东区"
          },
          {
            "districtID": 593,
            "cityID": 54,
            "divisionCode": "220322",
            "districtName": "梨树县"
          },
          {
            "districtID": 594,
            "cityID": 54,
            "divisionCode": "220323",
            "districtName": "伊通满族自治县"
          },
          {
            "districtID": 595,
            "cityID": 54,
            "divisionCode": "220381",
            "districtName": "公主岭市"
          },
          {
            "districtID": 596,
            "cityID": 54,
            "divisionCode": "220382",
            "districtName": "双辽市"
          },
          {
            "districtID": 597,
            "cityID": 55,
            "divisionCode": "220401",
            "districtName": "市辖区"
          },
          {
            "districtID": 598,
            "cityID": 55,
            "divisionCode": "220402",
            "districtName": "龙山区"
          },
          {
            "districtID": 599,
            "cityID": 55,
            "divisionCode": "220403",
            "districtName": "西安区"
          },
          {
            "districtID": 600,
            "cityID": 55,
            "divisionCode": "220421",
            "districtName": "东丰县"
          },
          {
            "districtID": 601,
            "cityID": 55,
            "divisionCode": "220422",
            "districtName": "东辽县"
          },
          {
            "districtID": 602,
            "cityID": 56,
            "divisionCode": "220501",
            "districtName": "市辖区"
          },
          {
            "districtID": 603,
            "cityID": 56,
            "divisionCode": "220502",
            "districtName": "东昌区"
          },
          {
            "districtID": 604,
            "cityID": 56,
            "divisionCode": "220503",
            "districtName": "二道江区"
          },
          {
            "districtID": 605,
            "cityID": 56,
            "divisionCode": "220521",
            "districtName": "通化县"
          },
          {
            "districtID": 606,
            "cityID": 56,
            "divisionCode": "220523",
            "districtName": "辉南县"
          },
          {
            "districtID": 607,
            "cityID": 56,
            "divisionCode": "220524",
            "districtName": "柳河县"
          },
          {
            "districtID": 608,
            "cityID": 56,
            "divisionCode": "220581",
            "districtName": "梅河口市"
          },
          {
            "districtID": 609,
            "cityID": 56,
            "divisionCode": "220582",
            "districtName": "集安市"
          },
          {
            "districtID": 610,
            "cityID": 57,
            "divisionCode": "220601",
            "districtName": "市辖区"
          },
          {
            "districtID": 611,
            "cityID": 57,
            "divisionCode": "220602",
            "districtName": "浑江区"
          },
          {
            "districtID": 612,
            "cityID": 57,
            "divisionCode": "220605",
            "districtName": "江源区"
          },
          {
            "districtID": 613,
            "cityID": 57,
            "divisionCode": "220621",
            "districtName": "抚松县"
          },
          {
            "districtID": 614,
            "cityID": 57,
            "divisionCode": "220622",
            "districtName": "靖宇县"
          },
          {
            "districtID": 615,
            "cityID": 57,
            "divisionCode": "220623",
            "districtName": "长白朝鲜族自治县"
          },
          {
            "districtID": 616,
            "cityID": 57,
            "divisionCode": "220681",
            "districtName": "临江市"
          },
          {
            "districtID": 617,
            "cityID": 58,
            "divisionCode": "220701",
            "districtName": "市辖区"
          },
          {
            "districtID": 618,
            "cityID": 58,
            "divisionCode": "220702",
            "districtName": "宁江区"
          },
          {
            "districtID": 619,
            "cityID": 58,
            "divisionCode": "220721",
            "districtName": "前郭尔罗斯蒙古族自治县"
          },
          {
            "districtID": 620,
            "cityID": 58,
            "divisionCode": "220722",
            "districtName": "长岭县"
          },
          {
            "districtID": 621,
            "cityID": 58,
            "divisionCode": "220723",
            "districtName": "乾安县"
          },
          {
            "districtID": 622,
            "cityID": 58,
            "divisionCode": "220781",
            "districtName": "扶余市"
          },
          {
            "districtID": 623,
            "cityID": 59,
            "divisionCode": "220801",
            "districtName": "市辖区"
          },
          {
            "districtID": 624,
            "cityID": 59,
            "divisionCode": "220802",
            "districtName": "洮北区"
          },
          {
            "districtID": 625,
            "cityID": 59,
            "divisionCode": "220821",
            "districtName": "镇赉县"
          },
          {
            "districtID": 626,
            "cityID": 59,
            "divisionCode": "220822",
            "districtName": "通榆县"
          },
          {
            "districtID": 627,
            "cityID": 59,
            "divisionCode": "220881",
            "districtName": "洮南市"
          },
          {
            "districtID": 628,
            "cityID": 59,
            "divisionCode": "220882",
            "districtName": "大安市"
          },
          {
            "districtID": 629,
            "cityID": 60,
            "divisionCode": "222401",
            "districtName": "延吉市"
          },
          {
            "districtID": 630,
            "cityID": 60,
            "divisionCode": "222402",
            "districtName": "图们市"
          },
          {
            "districtID": 631,
            "cityID": 60,
            "divisionCode": "222403",
            "districtName": "敦化市"
          },
          {
            "districtID": 632,
            "cityID": 60,
            "divisionCode": "222404",
            "districtName": "珲春市"
          },
          {
            "districtID": 633,
            "cityID": 60,
            "divisionCode": "222405",
            "districtName": "龙井市"
          },
          {
            "districtID": 634,
            "cityID": 60,
            "divisionCode": "222406",
            "districtName": "和龙市"
          },
          {
            "districtID": 635,
            "cityID": 60,
            "divisionCode": "222424",
            "districtName": "汪清县"
          },
          {
            "districtID": 636,
            "cityID": 60,
            "divisionCode": "222426",
            "districtName": "安图县"
          },
          {
            "districtID": 637,
            "cityID": 61,
            "divisionCode": "230101",
            "districtName": "市辖区"
          },
          {
            "districtID": 638,
            "cityID": 61,
            "divisionCode": "230102",
            "districtName": "道里区"
          },
          {
            "districtID": 639,
            "cityID": 61,
            "divisionCode": "230103",
            "districtName": "南岗区"
          },
          {
            "districtID": 640,
            "cityID": 61,
            "divisionCode": "230104",
            "districtName": "道外区"
          },
          {
            "districtID": 641,
            "cityID": 61,
            "divisionCode": "230108",
            "districtName": "平房区"
          },
          {
            "districtID": 642,
            "cityID": 61,
            "divisionCode": "230109",
            "districtName": "松北区"
          },
          {
            "districtID": 643,
            "cityID": 61,
            "divisionCode": "230110",
            "districtName": "香坊区"
          },
          {
            "districtID": 644,
            "cityID": 61,
            "divisionCode": "230111",
            "districtName": "呼兰区"
          },
          {
            "districtID": 645,
            "cityID": 61,
            "divisionCode": "230112",
            "districtName": "阿城区"
          },
          {
            "districtID": 646,
            "cityID": 61,
            "divisionCode": "230113",
            "districtName": "双城区"
          },
          {
            "districtID": 647,
            "cityID": 61,
            "divisionCode": "230123",
            "districtName": "依兰县"
          },
          {
            "districtID": 648,
            "cityID": 61,
            "divisionCode": "230124",
            "districtName": "方正县"
          },
          {
            "districtID": 649,
            "cityID": 61,
            "divisionCode": "230125",
            "districtName": "宾县"
          },
          {
            "districtID": 650,
            "cityID": 61,
            "divisionCode": "230126",
            "districtName": "巴彦县"
          },
          {
            "districtID": 651,
            "cityID": 61,
            "divisionCode": "230127",
            "districtName": "木兰县"
          },
          {
            "districtID": 652,
            "cityID": 61,
            "divisionCode": "230128",
            "districtName": "通河县"
          },
          {
            "districtID": 653,
            "cityID": 61,
            "divisionCode": "230129",
            "districtName": "延寿县"
          },
          {
            "districtID": 654,
            "cityID": 61,
            "divisionCode": "230183",
            "districtName": "尚志市"
          },
          {
            "districtID": 655,
            "cityID": 61,
            "divisionCode": "230184",
            "districtName": "五常市"
          },
          {
            "districtID": 656,
            "cityID": 62,
            "divisionCode": "230201",
            "districtName": "市辖区"
          },
          {
            "districtID": 657,
            "cityID": 62,
            "divisionCode": "230202",
            "districtName": "龙沙区"
          },
          {
            "districtID": 658,
            "cityID": 62,
            "divisionCode": "230203",
            "districtName": "建华区"
          },
          {
            "districtID": 659,
            "cityID": 62,
            "divisionCode": "230204",
            "districtName": "铁锋区"
          },
          {
            "districtID": 660,
            "cityID": 62,
            "divisionCode": "230205",
            "districtName": "昂昂溪区"
          },
          {
            "districtID": 661,
            "cityID": 62,
            "divisionCode": "230206",
            "districtName": "富拉尔基区"
          },
          {
            "districtID": 662,
            "cityID": 62,
            "divisionCode": "230207",
            "districtName": "碾子山区"
          },
          {
            "districtID": 663,
            "cityID": 62,
            "divisionCode": "230208",
            "districtName": "梅里斯达斡尔族区"
          },
          {
            "districtID": 664,
            "cityID": 62,
            "divisionCode": "230221",
            "districtName": "龙江县"
          },
          {
            "districtID": 665,
            "cityID": 62,
            "divisionCode": "230223",
            "districtName": "依安县"
          },
          {
            "districtID": 666,
            "cityID": 62,
            "divisionCode": "230224",
            "districtName": "泰来县"
          },
          {
            "districtID": 667,
            "cityID": 62,
            "divisionCode": "230225",
            "districtName": "甘南县"
          },
          {
            "districtID": 668,
            "cityID": 62,
            "divisionCode": "230227",
            "districtName": "富裕县"
          },
          {
            "districtID": 669,
            "cityID": 62,
            "divisionCode": "230229",
            "districtName": "克山县"
          },
          {
            "districtID": 670,
            "cityID": 62,
            "divisionCode": "230230",
            "districtName": "克东县"
          },
          {
            "districtID": 671,
            "cityID": 62,
            "divisionCode": "230231",
            "districtName": "拜泉县"
          },
          {
            "districtID": 672,
            "cityID": 62,
            "divisionCode": "230281",
            "districtName": "讷河市"
          },
          {
            "districtID": 673,
            "cityID": 63,
            "divisionCode": "230301",
            "districtName": "市辖区"
          },
          {
            "districtID": 674,
            "cityID": 63,
            "divisionCode": "230302",
            "districtName": "鸡冠区"
          },
          {
            "districtID": 675,
            "cityID": 63,
            "divisionCode": "230303",
            "districtName": "恒山区"
          },
          {
            "districtID": 676,
            "cityID": 63,
            "divisionCode": "230304",
            "districtName": "滴道区"
          },
          {
            "districtID": 677,
            "cityID": 63,
            "divisionCode": "230305",
            "districtName": "梨树区"
          },
          {
            "districtID": 678,
            "cityID": 63,
            "divisionCode": "230306",
            "districtName": "城子河区"
          },
          {
            "districtID": 679,
            "cityID": 63,
            "divisionCode": "230307",
            "districtName": "麻山区"
          },
          {
            "districtID": 680,
            "cityID": 63,
            "divisionCode": "230321",
            "districtName": "鸡东县"
          },
          {
            "districtID": 681,
            "cityID": 63,
            "divisionCode": "230381",
            "districtName": "虎林市"
          },
          {
            "districtID": 682,
            "cityID": 63,
            "divisionCode": "230382",
            "districtName": "密山市"
          },
          {
            "districtID": 683,
            "cityID": 64,
            "divisionCode": "230401",
            "districtName": "市辖区"
          },
          {
            "districtID": 684,
            "cityID": 64,
            "divisionCode": "230402",
            "districtName": "向阳区"
          },
          {
            "districtID": 685,
            "cityID": 64,
            "divisionCode": "230403",
            "districtName": "工农区"
          },
          {
            "districtID": 686,
            "cityID": 64,
            "divisionCode": "230404",
            "districtName": "南山区"
          },
          {
            "districtID": 687,
            "cityID": 64,
            "divisionCode": "230405",
            "districtName": "兴安区"
          },
          {
            "districtID": 688,
            "cityID": 64,
            "divisionCode": "230406",
            "districtName": "东山区"
          },
          {
            "districtID": 689,
            "cityID": 64,
            "divisionCode": "230407",
            "districtName": "兴山区"
          },
          {
            "districtID": 690,
            "cityID": 64,
            "divisionCode": "230421",
            "districtName": "萝北县"
          },
          {
            "districtID": 691,
            "cityID": 64,
            "divisionCode": "230422",
            "districtName": "绥滨县"
          },
          {
            "districtID": 692,
            "cityID": 65,
            "divisionCode": "230501",
            "districtName": "市辖区"
          },
          {
            "districtID": 693,
            "cityID": 65,
            "divisionCode": "230502",
            "districtName": "尖山区"
          },
          {
            "districtID": 694,
            "cityID": 65,
            "divisionCode": "230503",
            "districtName": "岭东区"
          },
          {
            "districtID": 695,
            "cityID": 65,
            "divisionCode": "230505",
            "districtName": "四方台区"
          },
          {
            "districtID": 696,
            "cityID": 65,
            "divisionCode": "230506",
            "districtName": "宝山区"
          },
          {
            "districtID": 697,
            "cityID": 65,
            "divisionCode": "230521",
            "districtName": "集贤县"
          },
          {
            "districtID": 698,
            "cityID": 65,
            "divisionCode": "230522",
            "districtName": "友谊县"
          },
          {
            "districtID": 699,
            "cityID": 65,
            "divisionCode": "230523",
            "districtName": "宝清县"
          },
          {
            "districtID": 700,
            "cityID": 65,
            "divisionCode": "230524",
            "districtName": "饶河县"
          },
          {
            "districtID": 701,
            "cityID": 66,
            "divisionCode": "230601",
            "districtName": "市辖区"
          },
          {
            "districtID": 702,
            "cityID": 66,
            "divisionCode": "230602",
            "districtName": "萨尔图区"
          },
          {
            "districtID": 703,
            "cityID": 66,
            "divisionCode": "230603",
            "districtName": "龙凤区"
          },
          {
            "districtID": 704,
            "cityID": 66,
            "divisionCode": "230604",
            "districtName": "让胡路区"
          },
          {
            "districtID": 705,
            "cityID": 66,
            "divisionCode": "230605",
            "districtName": "红岗区"
          },
          {
            "districtID": 706,
            "cityID": 66,
            "divisionCode": "230606",
            "districtName": "大同区"
          },
          {
            "districtID": 707,
            "cityID": 66,
            "divisionCode": "230621",
            "districtName": "肇州县"
          },
          {
            "districtID": 708,
            "cityID": 66,
            "divisionCode": "230622",
            "districtName": "肇源县"
          },
          {
            "districtID": 709,
            "cityID": 66,
            "divisionCode": "230623",
            "districtName": "林甸县"
          },
          {
            "districtID": 710,
            "cityID": 66,
            "divisionCode": "230624",
            "districtName": "杜尔伯特蒙古族自治县"
          },
          {
            "districtID": 711,
            "cityID": 67,
            "divisionCode": "230701",
            "districtName": "市辖区"
          },
          {
            "districtID": 712,
            "cityID": 67,
            "divisionCode": "230702",
            "districtName": "伊春区"
          },
          {
            "districtID": 713,
            "cityID": 67,
            "divisionCode": "230703",
            "districtName": "南岔区"
          },
          {
            "districtID": 714,
            "cityID": 67,
            "divisionCode": "230704",
            "districtName": "友好区"
          },
          {
            "districtID": 715,
            "cityID": 67,
            "divisionCode": "230705",
            "districtName": "西林区"
          },
          {
            "districtID": 716,
            "cityID": 67,
            "divisionCode": "230706",
            "districtName": "翠峦区"
          },
          {
            "districtID": 717,
            "cityID": 67,
            "divisionCode": "230707",
            "districtName": "新青区"
          },
          {
            "districtID": 718,
            "cityID": 67,
            "divisionCode": "230708",
            "districtName": "美溪区"
          },
          {
            "districtID": 719,
            "cityID": 67,
            "divisionCode": "230709",
            "districtName": "金山屯区"
          },
          {
            "districtID": 720,
            "cityID": 67,
            "divisionCode": "230710",
            "districtName": "五营区"
          },
          {
            "districtID": 721,
            "cityID": 67,
            "divisionCode": "230711",
            "districtName": "乌马河区"
          },
          {
            "districtID": 722,
            "cityID": 67,
            "divisionCode": "230712",
            "districtName": "汤旺河区"
          },
          {
            "districtID": 723,
            "cityID": 67,
            "divisionCode": "230713",
            "districtName": "带岭区"
          },
          {
            "districtID": 724,
            "cityID": 67,
            "divisionCode": "230714",
            "districtName": "乌伊岭区"
          },
          {
            "districtID": 725,
            "cityID": 67,
            "divisionCode": "230715",
            "districtName": "红星区"
          },
          {
            "districtID": 726,
            "cityID": 67,
            "divisionCode": "230716",
            "districtName": "上甘岭区"
          },
          {
            "districtID": 727,
            "cityID": 67,
            "divisionCode": "230722",
            "districtName": "嘉荫县"
          },
          {
            "districtID": 728,
            "cityID": 67,
            "divisionCode": "230781",
            "districtName": "铁力市"
          },
          {
            "districtID": 729,
            "cityID": 68,
            "divisionCode": "230801",
            "districtName": "市辖区"
          },
          {
            "districtID": 730,
            "cityID": 68,
            "divisionCode": "230803",
            "districtName": "向阳区"
          },
          {
            "districtID": 731,
            "cityID": 68,
            "divisionCode": "230804",
            "districtName": "前进区"
          },
          {
            "districtID": 732,
            "cityID": 68,
            "divisionCode": "230805",
            "districtName": "东风区"
          },
          {
            "districtID": 733,
            "cityID": 68,
            "divisionCode": "230811",
            "districtName": "郊区"
          },
          {
            "districtID": 734,
            "cityID": 68,
            "divisionCode": "230822",
            "districtName": "桦南县"
          },
          {
            "districtID": 735,
            "cityID": 68,
            "divisionCode": "230826",
            "districtName": "桦川县"
          },
          {
            "districtID": 736,
            "cityID": 68,
            "divisionCode": "230828",
            "districtName": "汤原县"
          },
          {
            "districtID": 737,
            "cityID": 68,
            "divisionCode": "230881",
            "districtName": "同江市"
          },
          {
            "districtID": 738,
            "cityID": 68,
            "divisionCode": "230882",
            "districtName": "富锦市"
          },
          {
            "districtID": 739,
            "cityID": 68,
            "divisionCode": "230883",
            "districtName": "抚远市"
          },
          {
            "districtID": 740,
            "cityID": 69,
            "divisionCode": "230901",
            "districtName": "市辖区"
          },
          {
            "districtID": 741,
            "cityID": 69,
            "divisionCode": "230902",
            "districtName": "新兴区"
          },
          {
            "districtID": 742,
            "cityID": 69,
            "divisionCode": "230903",
            "districtName": "桃山区"
          },
          {
            "districtID": 743,
            "cityID": 69,
            "divisionCode": "230904",
            "districtName": "茄子河区"
          },
          {
            "districtID": 744,
            "cityID": 69,
            "divisionCode": "230921",
            "districtName": "勃利县"
          },
          {
            "districtID": 745,
            "cityID": 70,
            "divisionCode": "231001",
            "districtName": "市辖区"
          },
          {
            "districtID": 746,
            "cityID": 70,
            "divisionCode": "231002",
            "districtName": "东安区"
          },
          {
            "districtID": 747,
            "cityID": 70,
            "divisionCode": "231003",
            "districtName": "阳明区"
          },
          {
            "districtID": 748,
            "cityID": 70,
            "divisionCode": "231004",
            "districtName": "爱民区"
          },
          {
            "districtID": 749,
            "cityID": 70,
            "divisionCode": "231005",
            "districtName": "西安区"
          },
          {
            "districtID": 750,
            "cityID": 70,
            "divisionCode": "231025",
            "districtName": "林口县"
          },
          {
            "districtID": 751,
            "cityID": 70,
            "divisionCode": "231081",
            "districtName": "绥芬河市"
          },
          {
            "districtID": 752,
            "cityID": 70,
            "divisionCode": "231083",
            "districtName": "海林市"
          },
          {
            "districtID": 753,
            "cityID": 70,
            "divisionCode": "231084",
            "districtName": "宁安市"
          },
          {
            "districtID": 754,
            "cityID": 70,
            "divisionCode": "231085",
            "districtName": "穆棱市"
          },
          {
            "districtID": 755,
            "cityID": 70,
            "divisionCode": "231086",
            "districtName": "东宁市"
          },
          {
            "districtID": 756,
            "cityID": 71,
            "divisionCode": "231101",
            "districtName": "市辖区"
          },
          {
            "districtID": 757,
            "cityID": 71,
            "divisionCode": "231102",
            "districtName": "爱辉区"
          },
          {
            "districtID": 758,
            "cityID": 71,
            "divisionCode": "231121",
            "districtName": "嫩江县"
          },
          {
            "districtID": 759,
            "cityID": 71,
            "divisionCode": "231123",
            "districtName": "逊克县"
          },
          {
            "districtID": 760,
            "cityID": 71,
            "divisionCode": "231124",
            "districtName": "孙吴县"
          },
          {
            "districtID": 761,
            "cityID": 71,
            "divisionCode": "231181",
            "districtName": "北安市"
          },
          {
            "districtID": 762,
            "cityID": 71,
            "divisionCode": "231182",
            "districtName": "五大连池市"
          },
          {
            "districtID": 763,
            "cityID": 72,
            "divisionCode": "231201",
            "districtName": "市辖区"
          },
          {
            "districtID": 764,
            "cityID": 72,
            "divisionCode": "231202",
            "districtName": "北林区"
          },
          {
            "districtID": 765,
            "cityID": 72,
            "divisionCode": "231221",
            "districtName": "望奎县"
          },
          {
            "districtID": 766,
            "cityID": 72,
            "divisionCode": "231222",
            "districtName": "兰西县"
          },
          {
            "districtID": 767,
            "cityID": 72,
            "divisionCode": "231223",
            "districtName": "青冈县"
          },
          {
            "districtID": 768,
            "cityID": 72,
            "divisionCode": "231224",
            "districtName": "庆安县"
          },
          {
            "districtID": 769,
            "cityID": 72,
            "divisionCode": "231225",
            "districtName": "明水县"
          },
          {
            "districtID": 770,
            "cityID": 72,
            "divisionCode": "231226",
            "districtName": "绥棱县"
          },
          {
            "districtID": 771,
            "cityID": 72,
            "divisionCode": "231281",
            "districtName": "安达市"
          },
          {
            "districtID": 772,
            "cityID": 72,
            "divisionCode": "231282",
            "districtName": "肇东市"
          },
          {
            "districtID": 773,
            "cityID": 72,
            "divisionCode": "231283",
            "districtName": "海伦市"
          },
          {
            "districtID": 774,
            "cityID": 73,
            "divisionCode": "232721",
            "districtName": "呼玛县"
          },
          {
            "districtID": 775,
            "cityID": 73,
            "divisionCode": "232722",
            "districtName": "塔河县"
          },
          {
            "districtID": 776,
            "cityID": 73,
            "divisionCode": "232723",
            "districtName": "漠河县"
          },
          {
            "districtID": 777,
            "cityID": 74,
            "divisionCode": "310101",
            "districtName": "黄浦区"
          },
          {
            "districtID": 778,
            "cityID": 74,
            "divisionCode": "310104",
            "districtName": "徐汇区"
          },
          {
            "districtID": 779,
            "cityID": 74,
            "divisionCode": "310105",
            "districtName": "长宁区"
          },
          {
            "districtID": 780,
            "cityID": 74,
            "divisionCode": "310106",
            "districtName": "静安区"
          },
          {
            "districtID": 781,
            "cityID": 74,
            "divisionCode": "310107",
            "districtName": "普陀区"
          },
          {
            "districtID": 782,
            "cityID": 74,
            "divisionCode": "310109",
            "districtName": "虹口区"
          },
          {
            "districtID": 783,
            "cityID": 74,
            "divisionCode": "310110",
            "districtName": "杨浦区"
          },
          {
            "districtID": 784,
            "cityID": 74,
            "divisionCode": "310112",
            "districtName": "闵行区"
          },
          {
            "districtID": 785,
            "cityID": 74,
            "divisionCode": "310113",
            "districtName": "宝山区"
          },
          {
            "districtID": 786,
            "cityID": 74,
            "divisionCode": "310114",
            "districtName": "嘉定区"
          },
          {
            "districtID": 787,
            "cityID": 74,
            "divisionCode": "310115",
            "districtName": "浦东新区"
          },
          {
            "districtID": 788,
            "cityID": 74,
            "divisionCode": "310116",
            "districtName": "金山区"
          },
          {
            "districtID": 789,
            "cityID": 74,
            "divisionCode": "310117",
            "districtName": "松江区"
          },
          {
            "districtID": 790,
            "cityID": 74,
            "divisionCode": "310118",
            "districtName": "青浦区"
          },
          {
            "districtID": 791,
            "cityID": 74,
            "divisionCode": "310120",
            "districtName": "奉贤区"
          },
          {
            "districtID": 792,
            "cityID": 74,
            "divisionCode": "310151",
            "districtName": "崇明区"
          },
          {
            "districtID": 793,
            "cityID": 75,
            "divisionCode": "320101",
            "districtName": "市辖区"
          },
          {
            "districtID": 794,
            "cityID": 75,
            "divisionCode": "320102",
            "districtName": "玄武区"
          },
          {
            "districtID": 795,
            "cityID": 75,
            "divisionCode": "320104",
            "districtName": "秦淮区"
          },
          {
            "districtID": 796,
            "cityID": 75,
            "divisionCode": "320105",
            "districtName": "建邺区"
          },
          {
            "districtID": 797,
            "cityID": 75,
            "divisionCode": "320106",
            "districtName": "鼓楼区"
          },
          {
            "districtID": 798,
            "cityID": 75,
            "divisionCode": "320111",
            "districtName": "浦口区"
          },
          {
            "districtID": 799,
            "cityID": 75,
            "divisionCode": "320113",
            "districtName": "栖霞区"
          },
          {
            "districtID": 800,
            "cityID": 75,
            "divisionCode": "320114",
            "districtName": "雨花台区"
          },
          {
            "districtID": 801,
            "cityID": 75,
            "divisionCode": "320115",
            "districtName": "江宁区"
          },
          {
            "districtID": 802,
            "cityID": 75,
            "divisionCode": "320116",
            "districtName": "六合区"
          },
          {
            "districtID": 803,
            "cityID": 75,
            "divisionCode": "320117",
            "districtName": "溧水区"
          },
          {
            "districtID": 804,
            "cityID": 75,
            "divisionCode": "320118",
            "districtName": "高淳区"
          },
          {
            "districtID": 805,
            "cityID": 76,
            "divisionCode": "320201",
            "districtName": "市辖区"
          },
          {
            "districtID": 806,
            "cityID": 76,
            "divisionCode": "320205",
            "districtName": "锡山区"
          },
          {
            "districtID": 807,
            "cityID": 76,
            "divisionCode": "320206",
            "districtName": "惠山区"
          },
          {
            "districtID": 808,
            "cityID": 76,
            "divisionCode": "320211",
            "districtName": "滨湖区"
          },
          {
            "districtID": 809,
            "cityID": 76,
            "divisionCode": "320213",
            "districtName": "梁溪区"
          },
          {
            "districtID": 810,
            "cityID": 76,
            "divisionCode": "320214",
            "districtName": "新吴区"
          },
          {
            "districtID": 811,
            "cityID": 76,
            "divisionCode": "320281",
            "districtName": "江阴市"
          },
          {
            "districtID": 812,
            "cityID": 76,
            "divisionCode": "320282",
            "districtName": "宜兴市"
          },
          {
            "districtID": 813,
            "cityID": 77,
            "divisionCode": "320301",
            "districtName": "市辖区"
          },
          {
            "districtID": 814,
            "cityID": 77,
            "divisionCode": "320302",
            "districtName": "鼓楼区"
          },
          {
            "districtID": 815,
            "cityID": 77,
            "divisionCode": "320303",
            "districtName": "云龙区"
          },
          {
            "districtID": 816,
            "cityID": 77,
            "divisionCode": "320305",
            "districtName": "贾汪区"
          },
          {
            "districtID": 817,
            "cityID": 77,
            "divisionCode": "320311",
            "districtName": "泉山区"
          },
          {
            "districtID": 818,
            "cityID": 77,
            "divisionCode": "320312",
            "districtName": "铜山区"
          },
          {
            "districtID": 819,
            "cityID": 77,
            "divisionCode": "320321",
            "districtName": "丰县"
          },
          {
            "districtID": 820,
            "cityID": 77,
            "divisionCode": "320322",
            "districtName": "沛县"
          },
          {
            "districtID": 821,
            "cityID": 77,
            "divisionCode": "320324",
            "districtName": "睢宁县"
          },
          {
            "districtID": 822,
            "cityID": 77,
            "divisionCode": "320381",
            "districtName": "新沂市"
          },
          {
            "districtID": 823,
            "cityID": 77,
            "divisionCode": "320382",
            "districtName": "邳州市"
          },
          {
            "districtID": 824,
            "cityID": 78,
            "divisionCode": "320401",
            "districtName": "市辖区"
          },
          {
            "districtID": 825,
            "cityID": 78,
            "divisionCode": "320402",
            "districtName": "天宁区"
          },
          {
            "districtID": 826,
            "cityID": 78,
            "divisionCode": "320404",
            "districtName": "钟楼区"
          },
          {
            "districtID": 827,
            "cityID": 78,
            "divisionCode": "320411",
            "districtName": "新北区"
          },
          {
            "districtID": 828,
            "cityID": 78,
            "divisionCode": "320412",
            "districtName": "武进区"
          },
          {
            "districtID": 829,
            "cityID": 78,
            "divisionCode": "320413",
            "districtName": "金坛区"
          },
          {
            "districtID": 830,
            "cityID": 78,
            "divisionCode": "320481",
            "districtName": "溧阳市"
          },
          {
            "districtID": 831,
            "cityID": 79,
            "divisionCode": "320501",
            "districtName": "市辖区"
          },
          {
            "districtID": 832,
            "cityID": 79,
            "divisionCode": "320505",
            "districtName": "虎丘区"
          },
          {
            "districtID": 833,
            "cityID": 79,
            "divisionCode": "320506",
            "districtName": "吴中区"
          },
          {
            "districtID": 834,
            "cityID": 79,
            "divisionCode": "320507",
            "districtName": "相城区"
          },
          {
            "districtID": 835,
            "cityID": 79,
            "divisionCode": "320508",
            "districtName": "姑苏区"
          },
          {
            "districtID": 836,
            "cityID": 79,
            "divisionCode": "320509",
            "districtName": "吴江区"
          },
          {
            "districtID": 837,
            "cityID": 79,
            "divisionCode": "320581",
            "districtName": "常熟市"
          },
          {
            "districtID": 838,
            "cityID": 79,
            "divisionCode": "320582",
            "districtName": "张家港市"
          },
          {
            "districtID": 839,
            "cityID": 79,
            "divisionCode": "320583",
            "districtName": "昆山市"
          },
          {
            "districtID": 840,
            "cityID": 79,
            "divisionCode": "320585",
            "districtName": "太仓市"
          },
          {
            "districtID": 841,
            "cityID": 80,
            "divisionCode": "320601",
            "districtName": "市辖区"
          },
          {
            "districtID": 842,
            "cityID": 80,
            "divisionCode": "320602",
            "districtName": "崇川区"
          },
          {
            "districtID": 843,
            "cityID": 80,
            "divisionCode": "320611",
            "districtName": "港闸区"
          },
          {
            "districtID": 844,
            "cityID": 80,
            "divisionCode": "320612",
            "districtName": "通州区"
          },
          {
            "districtID": 845,
            "cityID": 80,
            "divisionCode": "320621",
            "districtName": "海安县"
          },
          {
            "districtID": 846,
            "cityID": 80,
            "divisionCode": "320623",
            "districtName": "如东县"
          },
          {
            "districtID": 847,
            "cityID": 80,
            "divisionCode": "320681",
            "districtName": "启东市"
          },
          {
            "districtID": 848,
            "cityID": 80,
            "divisionCode": "320682",
            "districtName": "如皋市"
          },
          {
            "districtID": 849,
            "cityID": 80,
            "divisionCode": "320684",
            "districtName": "海门市"
          },
          {
            "districtID": 850,
            "cityID": 81,
            "divisionCode": "320701",
            "districtName": "市辖区"
          },
          {
            "districtID": 851,
            "cityID": 81,
            "divisionCode": "320703",
            "districtName": "连云区"
          },
          {
            "districtID": 852,
            "cityID": 81,
            "divisionCode": "320706",
            "districtName": "海州区"
          },
          {
            "districtID": 853,
            "cityID": 81,
            "divisionCode": "320707",
            "districtName": "赣榆区"
          },
          {
            "districtID": 854,
            "cityID": 81,
            "divisionCode": "320722",
            "districtName": "东海县"
          },
          {
            "districtID": 855,
            "cityID": 81,
            "divisionCode": "320723",
            "districtName": "灌云县"
          },
          {
            "districtID": 856,
            "cityID": 81,
            "divisionCode": "320724",
            "districtName": "灌南县"
          },
          {
            "districtID": 857,
            "cityID": 82,
            "divisionCode": "320801",
            "districtName": "市辖区"
          },
          {
            "districtID": 858,
            "cityID": 82,
            "divisionCode": "320803",
            "districtName": "淮安区"
          },
          {
            "districtID": 859,
            "cityID": 82,
            "divisionCode": "320804",
            "districtName": "淮阴区"
          },
          {
            "districtID": 860,
            "cityID": 82,
            "divisionCode": "320812",
            "districtName": "清江浦区"
          },
          {
            "districtID": 861,
            "cityID": 82,
            "divisionCode": "320813",
            "districtName": "洪泽区"
          },
          {
            "districtID": 862,
            "cityID": 82,
            "divisionCode": "320826",
            "districtName": "涟水县"
          },
          {
            "districtID": 863,
            "cityID": 82,
            "divisionCode": "320830",
            "districtName": "盱眙县"
          },
          {
            "districtID": 864,
            "cityID": 82,
            "divisionCode": "320831",
            "districtName": "金湖县"
          },
          {
            "districtID": 865,
            "cityID": 83,
            "divisionCode": "320901",
            "districtName": "市辖区"
          },
          {
            "districtID": 866,
            "cityID": 83,
            "divisionCode": "320902",
            "districtName": "亭湖区"
          },
          {
            "districtID": 867,
            "cityID": 83,
            "divisionCode": "320903",
            "districtName": "盐都区"
          },
          {
            "districtID": 868,
            "cityID": 83,
            "divisionCode": "320904",
            "districtName": "大丰区"
          },
          {
            "districtID": 869,
            "cityID": 83,
            "divisionCode": "320921",
            "districtName": "响水县"
          },
          {
            "districtID": 870,
            "cityID": 83,
            "divisionCode": "320922",
            "districtName": "滨海县"
          },
          {
            "districtID": 871,
            "cityID": 83,
            "divisionCode": "320923",
            "districtName": "阜宁县"
          },
          {
            "districtID": 872,
            "cityID": 83,
            "divisionCode": "320924",
            "districtName": "射阳县"
          },
          {
            "districtID": 873,
            "cityID": 83,
            "divisionCode": "320925",
            "districtName": "建湖县"
          },
          {
            "districtID": 874,
            "cityID": 83,
            "divisionCode": "320981",
            "districtName": "东台市"
          },
          {
            "districtID": 875,
            "cityID": 84,
            "divisionCode": "321001",
            "districtName": "市辖区"
          },
          {
            "districtID": 876,
            "cityID": 84,
            "divisionCode": "321002",
            "districtName": "广陵区"
          },
          {
            "districtID": 877,
            "cityID": 84,
            "divisionCode": "321003",
            "districtName": "邗江区"
          },
          {
            "districtID": 878,
            "cityID": 84,
            "divisionCode": "321012",
            "districtName": "江都区"
          },
          {
            "districtID": 879,
            "cityID": 84,
            "divisionCode": "321023",
            "districtName": "宝应县"
          },
          {
            "districtID": 880,
            "cityID": 84,
            "divisionCode": "321081",
            "districtName": "仪征市"
          },
          {
            "districtID": 881,
            "cityID": 84,
            "divisionCode": "321084",
            "districtName": "高邮市"
          },
          {
            "districtID": 882,
            "cityID": 85,
            "divisionCode": "321101",
            "districtName": "市辖区"
          },
          {
            "districtID": 883,
            "cityID": 85,
            "divisionCode": "321102",
            "districtName": "京口区"
          },
          {
            "districtID": 884,
            "cityID": 85,
            "divisionCode": "321111",
            "districtName": "润州区"
          },
          {
            "districtID": 885,
            "cityID": 85,
            "divisionCode": "321112",
            "districtName": "丹徒区"
          },
          {
            "districtID": 886,
            "cityID": 85,
            "divisionCode": "321181",
            "districtName": "丹阳市"
          },
          {
            "districtID": 887,
            "cityID": 85,
            "divisionCode": "321182",
            "districtName": "扬中市"
          },
          {
            "districtID": 888,
            "cityID": 85,
            "divisionCode": "321183",
            "districtName": "句容市"
          },
          {
            "districtID": 889,
            "cityID": 86,
            "divisionCode": "321201",
            "districtName": "市辖区"
          },
          {
            "districtID": 890,
            "cityID": 86,
            "divisionCode": "321202",
            "districtName": "海陵区"
          },
          {
            "districtID": 891,
            "cityID": 86,
            "divisionCode": "321203",
            "districtName": "高港区"
          },
          {
            "districtID": 892,
            "cityID": 86,
            "divisionCode": "321204",
            "districtName": "姜堰区"
          },
          {
            "districtID": 893,
            "cityID": 86,
            "divisionCode": "321281",
            "districtName": "兴化市"
          },
          {
            "districtID": 894,
            "cityID": 86,
            "divisionCode": "321282",
            "districtName": "靖江市"
          },
          {
            "districtID": 895,
            "cityID": 86,
            "divisionCode": "321283",
            "districtName": "泰兴市"
          },
          {
            "districtID": 896,
            "cityID": 87,
            "divisionCode": "321301",
            "districtName": "市辖区"
          },
          {
            "districtID": 897,
            "cityID": 87,
            "divisionCode": "321302",
            "districtName": "宿城区"
          },
          {
            "districtID": 898,
            "cityID": 87,
            "divisionCode": "321311",
            "districtName": "宿豫区"
          },
          {
            "districtID": 899,
            "cityID": 87,
            "divisionCode": "321322",
            "districtName": "沭阳县"
          },
          {
            "districtID": 900,
            "cityID": 87,
            "divisionCode": "321323",
            "districtName": "泗阳县"
          },
          {
            "districtID": 901,
            "cityID": 87,
            "divisionCode": "321324",
            "districtName": "泗洪县"
          },
          {
            "districtID": 902,
            "cityID": 88,
            "divisionCode": "330101",
            "districtName": "市辖区"
          },
          {
            "districtID": 903,
            "cityID": 88,
            "divisionCode": "330102",
            "districtName": "上城区"
          },
          {
            "districtID": 904,
            "cityID": 88,
            "divisionCode": "330103",
            "districtName": "下城区"
          },
          {
            "districtID": 905,
            "cityID": 88,
            "divisionCode": "330104",
            "districtName": "江干区"
          },
          {
            "districtID": 906,
            "cityID": 88,
            "divisionCode": "330105",
            "districtName": "拱墅区"
          },
          {
            "districtID": 907,
            "cityID": 88,
            "divisionCode": "330106",
            "districtName": "西湖区"
          },
          {
            "districtID": 908,
            "cityID": 88,
            "divisionCode": "330108",
            "districtName": "滨江区"
          },
          {
            "districtID": 909,
            "cityID": 88,
            "divisionCode": "330109",
            "districtName": "萧山区"
          },
          {
            "districtID": 910,
            "cityID": 88,
            "divisionCode": "330110",
            "districtName": "余杭区"
          },
          {
            "districtID": 911,
            "cityID": 88,
            "divisionCode": "330111",
            "districtName": "富阳区"
          },
          {
            "districtID": 912,
            "cityID": 88,
            "divisionCode": "330122",
            "districtName": "桐庐县"
          },
          {
            "districtID": 913,
            "cityID": 88,
            "divisionCode": "330127",
            "districtName": "淳安县"
          },
          {
            "districtID": 914,
            "cityID": 88,
            "divisionCode": "330182",
            "districtName": "建德市"
          },
          {
            "districtID": 915,
            "cityID": 88,
            "divisionCode": "330185",
            "districtName": "临安市"
          },
          {
            "districtID": 916,
            "cityID": 89,
            "divisionCode": "330201",
            "districtName": "市辖区"
          },
          {
            "districtID": 917,
            "cityID": 89,
            "divisionCode": "330203",
            "districtName": "海曙区"
          },
          {
            "districtID": 918,
            "cityID": 89,
            "divisionCode": "330204",
            "districtName": "江东区"
          },
          {
            "districtID": 919,
            "cityID": 89,
            "divisionCode": "330205",
            "districtName": "江北区"
          },
          {
            "districtID": 920,
            "cityID": 89,
            "divisionCode": "330206",
            "districtName": "北仑区"
          },
          {
            "districtID": 921,
            "cityID": 89,
            "divisionCode": "330211",
            "districtName": "镇海区"
          },
          {
            "districtID": 922,
            "cityID": 89,
            "divisionCode": "330212",
            "districtName": "鄞州区"
          },
          {
            "districtID": 923,
            "cityID": 89,
            "divisionCode": "330225",
            "districtName": "象山县"
          },
          {
            "districtID": 924,
            "cityID": 89,
            "divisionCode": "330226",
            "districtName": "宁海县"
          },
          {
            "districtID": 925,
            "cityID": 89,
            "divisionCode": "330281",
            "districtName": "余姚市"
          },
          {
            "districtID": 926,
            "cityID": 89,
            "divisionCode": "330282",
            "districtName": "慈溪市"
          },
          {
            "districtID": 927,
            "cityID": 89,
            "divisionCode": "330283",
            "districtName": "奉化市"
          },
          {
            "districtID": 928,
            "cityID": 90,
            "divisionCode": "330301",
            "districtName": "市辖区"
          },
          {
            "districtID": 929,
            "cityID": 90,
            "divisionCode": "330302",
            "districtName": "鹿城区"
          },
          {
            "districtID": 930,
            "cityID": 90,
            "divisionCode": "330303",
            "districtName": "龙湾区"
          },
          {
            "districtID": 931,
            "cityID": 90,
            "divisionCode": "330304",
            "districtName": "瓯海区"
          },
          {
            "districtID": 932,
            "cityID": 90,
            "divisionCode": "330305",
            "districtName": "洞头区"
          },
          {
            "districtID": 933,
            "cityID": 90,
            "divisionCode": "330324",
            "districtName": "永嘉县"
          },
          {
            "districtID": 934,
            "cityID": 90,
            "divisionCode": "330326",
            "districtName": "平阳县"
          },
          {
            "districtID": 935,
            "cityID": 90,
            "divisionCode": "330327",
            "districtName": "苍南县"
          },
          {
            "districtID": 936,
            "cityID": 90,
            "divisionCode": "330328",
            "districtName": "文成县"
          },
          {
            "districtID": 937,
            "cityID": 90,
            "divisionCode": "330329",
            "districtName": "泰顺县"
          },
          {
            "districtID": 938,
            "cityID": 90,
            "divisionCode": "330381",
            "districtName": "瑞安市"
          },
          {
            "districtID": 939,
            "cityID": 90,
            "divisionCode": "330382",
            "districtName": "乐清市"
          },
          {
            "districtID": 940,
            "cityID": 91,
            "divisionCode": "330401",
            "districtName": "市辖区"
          },
          {
            "districtID": 941,
            "cityID": 91,
            "divisionCode": "330402",
            "districtName": "南湖区"
          },
          {
            "districtID": 942,
            "cityID": 91,
            "divisionCode": "330411",
            "districtName": "秀洲区"
          },
          {
            "districtID": 943,
            "cityID": 91,
            "divisionCode": "330421",
            "districtName": "嘉善县"
          },
          {
            "districtID": 944,
            "cityID": 91,
            "divisionCode": "330424",
            "districtName": "海盐县"
          },
          {
            "districtID": 945,
            "cityID": 91,
            "divisionCode": "330481",
            "districtName": "海宁市"
          },
          {
            "districtID": 946,
            "cityID": 91,
            "divisionCode": "330482",
            "districtName": "平湖市"
          },
          {
            "districtID": 947,
            "cityID": 91,
            "divisionCode": "330483",
            "districtName": "桐乡市"
          },
          {
            "districtID": 948,
            "cityID": 92,
            "divisionCode": "330501",
            "districtName": "市辖区"
          },
          {
            "districtID": 949,
            "cityID": 92,
            "divisionCode": "330502",
            "districtName": "吴兴区"
          },
          {
            "districtID": 950,
            "cityID": 92,
            "divisionCode": "330503",
            "districtName": "南浔区"
          },
          {
            "districtID": 951,
            "cityID": 92,
            "divisionCode": "330521",
            "districtName": "德清县"
          },
          {
            "districtID": 952,
            "cityID": 92,
            "divisionCode": "330522",
            "districtName": "长兴县"
          },
          {
            "districtID": 953,
            "cityID": 92,
            "divisionCode": "330523",
            "districtName": "安吉县"
          },
          {
            "districtID": 954,
            "cityID": 93,
            "divisionCode": "330601",
            "districtName": "市辖区"
          },
          {
            "districtID": 955,
            "cityID": 93,
            "divisionCode": "330602",
            "districtName": "越城区"
          },
          {
            "districtID": 956,
            "cityID": 93,
            "divisionCode": "330603",
            "districtName": "柯桥区"
          },
          {
            "districtID": 957,
            "cityID": 93,
            "divisionCode": "330604",
            "districtName": "上虞区"
          },
          {
            "districtID": 958,
            "cityID": 93,
            "divisionCode": "330624",
            "districtName": "新昌县"
          },
          {
            "districtID": 959,
            "cityID": 93,
            "divisionCode": "330681",
            "districtName": "诸暨市"
          },
          {
            "districtID": 960,
            "cityID": 93,
            "divisionCode": "330683",
            "districtName": "嵊州市"
          },
          {
            "districtID": 961,
            "cityID": 94,
            "divisionCode": "330701",
            "districtName": "市辖区"
          },
          {
            "districtID": 962,
            "cityID": 94,
            "divisionCode": "330702",
            "districtName": "婺城区"
          },
          {
            "districtID": 963,
            "cityID": 94,
            "divisionCode": "330703",
            "districtName": "金东区"
          },
          {
            "districtID": 964,
            "cityID": 94,
            "divisionCode": "330723",
            "districtName": "武义县"
          },
          {
            "districtID": 965,
            "cityID": 94,
            "divisionCode": "330726",
            "districtName": "浦江县"
          },
          {
            "districtID": 966,
            "cityID": 94,
            "divisionCode": "330727",
            "districtName": "磐安县"
          },
          {
            "districtID": 967,
            "cityID": 94,
            "divisionCode": "330781",
            "districtName": "兰溪市"
          },
          {
            "districtID": 968,
            "cityID": 94,
            "divisionCode": "330782",
            "districtName": "义乌市"
          },
          {
            "districtID": 969,
            "cityID": 94,
            "divisionCode": "330783",
            "districtName": "东阳市"
          },
          {
            "districtID": 970,
            "cityID": 94,
            "divisionCode": "330784",
            "districtName": "永康市"
          },
          {
            "districtID": 971,
            "cityID": 95,
            "divisionCode": "330801",
            "districtName": "市辖区"
          },
          {
            "districtID": 972,
            "cityID": 95,
            "divisionCode": "330802",
            "districtName": "柯城区"
          },
          {
            "districtID": 973,
            "cityID": 95,
            "divisionCode": "330803",
            "districtName": "衢江区"
          },
          {
            "districtID": 974,
            "cityID": 95,
            "divisionCode": "330822",
            "districtName": "常山县"
          },
          {
            "districtID": 975,
            "cityID": 95,
            "divisionCode": "330824",
            "districtName": "开化县"
          },
          {
            "districtID": 976,
            "cityID": 95,
            "divisionCode": "330825",
            "districtName": "龙游县"
          },
          {
            "districtID": 977,
            "cityID": 95,
            "divisionCode": "330881",
            "districtName": "江山市"
          },
          {
            "districtID": 978,
            "cityID": 96,
            "divisionCode": "330901",
            "districtName": "市辖区"
          },
          {
            "districtID": 979,
            "cityID": 96,
            "divisionCode": "330902",
            "districtName": "定海区"
          },
          {
            "districtID": 980,
            "cityID": 96,
            "divisionCode": "330903",
            "districtName": "普陀区"
          },
          {
            "districtID": 981,
            "cityID": 96,
            "divisionCode": "330921",
            "districtName": "岱山县"
          },
          {
            "districtID": 982,
            "cityID": 96,
            "divisionCode": "330922",
            "districtName": "嵊泗县"
          },
          {
            "districtID": 983,
            "cityID": 97,
            "divisionCode": "331001",
            "districtName": "市辖区"
          },
          {
            "districtID": 984,
            "cityID": 97,
            "divisionCode": "331002",
            "districtName": "椒江区"
          },
          {
            "districtID": 985,
            "cityID": 97,
            "divisionCode": "331003",
            "districtName": "黄岩区"
          },
          {
            "districtID": 986,
            "cityID": 97,
            "divisionCode": "331004",
            "districtName": "路桥区"
          },
          {
            "districtID": 987,
            "cityID": 97,
            "divisionCode": "331021",
            "districtName": "玉环县"
          },
          {
            "districtID": 988,
            "cityID": 97,
            "divisionCode": "331022",
            "districtName": "三门县"
          },
          {
            "districtID": 989,
            "cityID": 97,
            "divisionCode": "331023",
            "districtName": "天台县"
          },
          {
            "districtID": 990,
            "cityID": 97,
            "divisionCode": "331024",
            "districtName": "仙居县"
          },
          {
            "districtID": 991,
            "cityID": 97,
            "divisionCode": "331081",
            "districtName": "温岭市"
          },
          {
            "districtID": 992,
            "cityID": 97,
            "divisionCode": "331082",
            "districtName": "临海市"
          },
          {
            "districtID": 993,
            "cityID": 98,
            "divisionCode": "331101",
            "districtName": "市辖区"
          },
          {
            "districtID": 994,
            "cityID": 98,
            "divisionCode": "331102",
            "districtName": "莲都区"
          },
          {
            "districtID": 995,
            "cityID": 98,
            "divisionCode": "331121",
            "districtName": "青田县"
          },
          {
            "districtID": 996,
            "cityID": 98,
            "divisionCode": "331122",
            "districtName": "缙云县"
          },
          {
            "districtID": 997,
            "cityID": 98,
            "divisionCode": "331123",
            "districtName": "遂昌县"
          },
          {
            "districtID": 998,
            "cityID": 98,
            "divisionCode": "331124",
            "districtName": "松阳县"
          },
          {
            "districtID": 999,
            "cityID": 98,
            "divisionCode": "331125",
            "districtName": "云和县"
          },
          {
            "districtID": 1000,
            "cityID": 98,
            "divisionCode": "331126",
            "districtName": "庆元县"
          },
          {
            "districtID": 1001,
            "cityID": 98,
            "divisionCode": "331127",
            "districtName": "景宁畲族自治县"
          },
          {
            "districtID": 1002,
            "cityID": 98,
            "divisionCode": "331181",
            "districtName": "龙泉市"
          },
          {
            "districtID": 1003,
            "cityID": 99,
            "divisionCode": "340101",
            "districtName": "市辖区"
          },
          {
            "districtID": 1004,
            "cityID": 99,
            "divisionCode": "340102",
            "districtName": "瑶海区"
          },
          {
            "districtID": 1005,
            "cityID": 99,
            "divisionCode": "340103",
            "districtName": "庐阳区"
          },
          {
            "districtID": 1006,
            "cityID": 99,
            "divisionCode": "340104",
            "districtName": "蜀山区"
          },
          {
            "districtID": 1007,
            "cityID": 99,
            "divisionCode": "340111",
            "districtName": "包河区"
          },
          {
            "districtID": 1008,
            "cityID": 99,
            "divisionCode": "340121",
            "districtName": "长丰县"
          },
          {
            "districtID": 1009,
            "cityID": 99,
            "divisionCode": "340122",
            "districtName": "肥东县"
          },
          {
            "districtID": 1010,
            "cityID": 99,
            "divisionCode": "340123",
            "districtName": "肥西县"
          },
          {
            "districtID": 1011,
            "cityID": 99,
            "divisionCode": "340124",
            "districtName": "庐江县"
          },
          {
            "districtID": 1012,
            "cityID": 99,
            "divisionCode": "340181",
            "districtName": "巢湖市"
          },
          {
            "districtID": 1013,
            "cityID": 100,
            "divisionCode": "340201",
            "districtName": "市辖区"
          },
          {
            "districtID": 1014,
            "cityID": 100,
            "divisionCode": "340202",
            "districtName": "镜湖区"
          },
          {
            "districtID": 1015,
            "cityID": 100,
            "divisionCode": "340203",
            "districtName": "弋江区"
          },
          {
            "districtID": 1016,
            "cityID": 100,
            "divisionCode": "340207",
            "districtName": "鸠江区"
          },
          {
            "districtID": 1017,
            "cityID": 100,
            "divisionCode": "340208",
            "districtName": "三山区"
          },
          {
            "districtID": 1018,
            "cityID": 100,
            "divisionCode": "340221",
            "districtName": "芜湖县"
          },
          {
            "districtID": 1019,
            "cityID": 100,
            "divisionCode": "340222",
            "districtName": "繁昌县"
          },
          {
            "districtID": 1020,
            "cityID": 100,
            "divisionCode": "340223",
            "districtName": "南陵县"
          },
          {
            "districtID": 1021,
            "cityID": 100,
            "divisionCode": "340225",
            "districtName": "无为县"
          },
          {
            "districtID": 1022,
            "cityID": 101,
            "divisionCode": "340301",
            "districtName": "市辖区"
          },
          {
            "districtID": 1023,
            "cityID": 101,
            "divisionCode": "340302",
            "districtName": "龙子湖区"
          },
          {
            "districtID": 1024,
            "cityID": 101,
            "divisionCode": "340303",
            "districtName": "蚌山区"
          },
          {
            "districtID": 1025,
            "cityID": 101,
            "divisionCode": "340304",
            "districtName": "禹会区"
          },
          {
            "districtID": 1026,
            "cityID": 101,
            "divisionCode": "340311",
            "districtName": "淮上区"
          },
          {
            "districtID": 1027,
            "cityID": 101,
            "divisionCode": "340321",
            "districtName": "怀远县"
          },
          {
            "districtID": 1028,
            "cityID": 101,
            "divisionCode": "340322",
            "districtName": "五河县"
          },
          {
            "districtID": 1029,
            "cityID": 101,
            "divisionCode": "340323",
            "districtName": "固镇县"
          },
          {
            "districtID": 1030,
            "cityID": 102,
            "divisionCode": "340401",
            "districtName": "市辖区"
          },
          {
            "districtID": 1031,
            "cityID": 102,
            "divisionCode": "340402",
            "districtName": "大通区"
          },
          {
            "districtID": 1032,
            "cityID": 102,
            "divisionCode": "340403",
            "districtName": "田家庵区"
          },
          {
            "districtID": 1033,
            "cityID": 102,
            "divisionCode": "340404",
            "districtName": "谢家集区"
          },
          {
            "districtID": 1034,
            "cityID": 102,
            "divisionCode": "340405",
            "districtName": "八公山区"
          },
          {
            "districtID": 1035,
            "cityID": 102,
            "divisionCode": "340406",
            "districtName": "潘集区"
          },
          {
            "districtID": 1036,
            "cityID": 102,
            "divisionCode": "340421",
            "districtName": "凤台县"
          },
          {
            "districtID": 1037,
            "cityID": 102,
            "divisionCode": "340422",
            "districtName": "寿县"
          },
          {
            "districtID": 1038,
            "cityID": 103,
            "divisionCode": "340501",
            "districtName": "市辖区"
          },
          {
            "districtID": 1039,
            "cityID": 103,
            "divisionCode": "340503",
            "districtName": "花山区"
          },
          {
            "districtID": 1040,
            "cityID": 103,
            "divisionCode": "340504",
            "districtName": "雨山区"
          },
          {
            "districtID": 1041,
            "cityID": 103,
            "divisionCode": "340506",
            "districtName": "博望区"
          },
          {
            "districtID": 1042,
            "cityID": 103,
            "divisionCode": "340521",
            "districtName": "当涂县"
          },
          {
            "districtID": 1043,
            "cityID": 103,
            "divisionCode": "340522",
            "districtName": "含山县"
          },
          {
            "districtID": 1044,
            "cityID": 103,
            "divisionCode": "340523",
            "districtName": "和县"
          },
          {
            "districtID": 1045,
            "cityID": 104,
            "divisionCode": "340601",
            "districtName": "市辖区"
          },
          {
            "districtID": 1046,
            "cityID": 104,
            "divisionCode": "340602",
            "districtName": "杜集区"
          },
          {
            "districtID": 1047,
            "cityID": 104,
            "divisionCode": "340603",
            "districtName": "相山区"
          },
          {
            "districtID": 1048,
            "cityID": 104,
            "divisionCode": "340604",
            "districtName": "烈山区"
          },
          {
            "districtID": 1049,
            "cityID": 104,
            "divisionCode": "340621",
            "districtName": "濉溪县"
          },
          {
            "districtID": 1050,
            "cityID": 105,
            "divisionCode": "340701",
            "districtName": "市辖区"
          },
          {
            "districtID": 1051,
            "cityID": 105,
            "divisionCode": "340705",
            "districtName": "铜官区"
          },
          {
            "districtID": 1052,
            "cityID": 105,
            "divisionCode": "340706",
            "districtName": "义安区"
          },
          {
            "districtID": 1053,
            "cityID": 105,
            "divisionCode": "340711",
            "districtName": "郊区"
          },
          {
            "districtID": 1054,
            "cityID": 105,
            "divisionCode": "340722",
            "districtName": "枞阳县"
          },
          {
            "districtID": 1055,
            "cityID": 106,
            "divisionCode": "340801",
            "districtName": "市辖区"
          },
          {
            "districtID": 1056,
            "cityID": 106,
            "divisionCode": "340802",
            "districtName": "迎江区"
          },
          {
            "districtID": 1057,
            "cityID": 106,
            "divisionCode": "340803",
            "districtName": "大观区"
          },
          {
            "districtID": 1058,
            "cityID": 106,
            "divisionCode": "340811",
            "districtName": "宜秀区"
          },
          {
            "districtID": 1059,
            "cityID": 106,
            "divisionCode": "340822",
            "districtName": "怀宁县"
          },
          {
            "districtID": 1060,
            "cityID": 106,
            "divisionCode": "340824",
            "districtName": "潜山县"
          },
          {
            "districtID": 1061,
            "cityID": 106,
            "divisionCode": "340825",
            "districtName": "太湖县"
          },
          {
            "districtID": 1062,
            "cityID": 106,
            "divisionCode": "340826",
            "districtName": "宿松县"
          },
          {
            "districtID": 1063,
            "cityID": 106,
            "divisionCode": "340827",
            "districtName": "望江县"
          },
          {
            "districtID": 1064,
            "cityID": 106,
            "divisionCode": "340828",
            "districtName": "岳西县"
          },
          {
            "districtID": 1065,
            "cityID": 106,
            "divisionCode": "340881",
            "districtName": "桐城市"
          },
          {
            "districtID": 1066,
            "cityID": 107,
            "divisionCode": "341001",
            "districtName": "市辖区"
          },
          {
            "districtID": 1067,
            "cityID": 107,
            "divisionCode": "341002",
            "districtName": "屯溪区"
          },
          {
            "districtID": 1068,
            "cityID": 107,
            "divisionCode": "341003",
            "districtName": "黄山区"
          },
          {
            "districtID": 1069,
            "cityID": 107,
            "divisionCode": "341004",
            "districtName": "徽州区"
          },
          {
            "districtID": 1070,
            "cityID": 107,
            "divisionCode": "341021",
            "districtName": "歙县"
          },
          {
            "districtID": 1071,
            "cityID": 107,
            "divisionCode": "341022",
            "districtName": "休宁县"
          },
          {
            "districtID": 1072,
            "cityID": 107,
            "divisionCode": "341023",
            "districtName": "黟县"
          },
          {
            "districtID": 1073,
            "cityID": 107,
            "divisionCode": "341024",
            "districtName": "祁门县"
          },
          {
            "districtID": 1074,
            "cityID": 108,
            "divisionCode": "341101",
            "districtName": "市辖区"
          },
          {
            "districtID": 1075,
            "cityID": 108,
            "divisionCode": "341102",
            "districtName": "琅琊区"
          },
          {
            "districtID": 1076,
            "cityID": 108,
            "divisionCode": "341103",
            "districtName": "南谯区"
          },
          {
            "districtID": 1077,
            "cityID": 108,
            "divisionCode": "341122",
            "districtName": "来安县"
          },
          {
            "districtID": 1078,
            "cityID": 108,
            "divisionCode": "341124",
            "districtName": "全椒县"
          },
          {
            "districtID": 1079,
            "cityID": 108,
            "divisionCode": "341125",
            "districtName": "定远县"
          },
          {
            "districtID": 1080,
            "cityID": 108,
            "divisionCode": "341126",
            "districtName": "凤阳县"
          },
          {
            "districtID": 1081,
            "cityID": 108,
            "divisionCode": "341181",
            "districtName": "天长市"
          },
          {
            "districtID": 1082,
            "cityID": 108,
            "divisionCode": "341182",
            "districtName": "明光市"
          },
          {
            "districtID": 1083,
            "cityID": 109,
            "divisionCode": "341201",
            "districtName": "市辖区"
          },
          {
            "districtID": 1084,
            "cityID": 109,
            "divisionCode": "341202",
            "districtName": "颍州区"
          },
          {
            "districtID": 1085,
            "cityID": 109,
            "divisionCode": "341203",
            "districtName": "颍东区"
          },
          {
            "districtID": 1086,
            "cityID": 109,
            "divisionCode": "341204",
            "districtName": "颍泉区"
          },
          {
            "districtID": 1087,
            "cityID": 109,
            "divisionCode": "341221",
            "districtName": "临泉县"
          },
          {
            "districtID": 1088,
            "cityID": 109,
            "divisionCode": "341222",
            "districtName": "太和县"
          },
          {
            "districtID": 1089,
            "cityID": 109,
            "divisionCode": "341225",
            "districtName": "阜南县"
          },
          {
            "districtID": 1090,
            "cityID": 109,
            "divisionCode": "341226",
            "districtName": "颍上县"
          },
          {
            "districtID": 1091,
            "cityID": 109,
            "divisionCode": "341282",
            "districtName": "界首市"
          },
          {
            "districtID": 1092,
            "cityID": 110,
            "divisionCode": "341301",
            "districtName": "市辖区"
          },
          {
            "districtID": 1093,
            "cityID": 110,
            "divisionCode": "341302",
            "districtName": "埇桥区"
          },
          {
            "districtID": 1094,
            "cityID": 110,
            "divisionCode": "341321",
            "districtName": "砀山县"
          },
          {
            "districtID": 1095,
            "cityID": 110,
            "divisionCode": "341322",
            "districtName": "萧县"
          },
          {
            "districtID": 1096,
            "cityID": 110,
            "divisionCode": "341323",
            "districtName": "灵璧县"
          },
          {
            "districtID": 1097,
            "cityID": 110,
            "divisionCode": "341324",
            "districtName": "泗县"
          },
          {
            "districtID": 1098,
            "cityID": 111,
            "divisionCode": "341501",
            "districtName": "市辖区"
          },
          {
            "districtID": 1099,
            "cityID": 111,
            "divisionCode": "341502",
            "districtName": "金安区"
          },
          {
            "districtID": 1100,
            "cityID": 111,
            "divisionCode": "341503",
            "districtName": "裕安区"
          },
          {
            "districtID": 1101,
            "cityID": 111,
            "divisionCode": "341504",
            "districtName": "叶集区"
          },
          {
            "districtID": 1102,
            "cityID": 111,
            "divisionCode": "341522",
            "districtName": "霍邱县"
          },
          {
            "districtID": 1103,
            "cityID": 111,
            "divisionCode": "341523",
            "districtName": "舒城县"
          },
          {
            "districtID": 1104,
            "cityID": 111,
            "divisionCode": "341524",
            "districtName": "金寨县"
          },
          {
            "districtID": 1105,
            "cityID": 111,
            "divisionCode": "341525",
            "districtName": "霍山县"
          },
          {
            "districtID": 1106,
            "cityID": 112,
            "divisionCode": "341601",
            "districtName": "市辖区"
          },
          {
            "districtID": 1107,
            "cityID": 112,
            "divisionCode": "341602",
            "districtName": "谯城区"
          },
          {
            "districtID": 1108,
            "cityID": 112,
            "divisionCode": "341621",
            "districtName": "涡阳县"
          },
          {
            "districtID": 1109,
            "cityID": 112,
            "divisionCode": "341622",
            "districtName": "蒙城县"
          },
          {
            "districtID": 1110,
            "cityID": 112,
            "divisionCode": "341623",
            "districtName": "利辛县"
          },
          {
            "districtID": 1111,
            "cityID": 113,
            "divisionCode": "341701",
            "districtName": "市辖区"
          },
          {
            "districtID": 1112,
            "cityID": 113,
            "divisionCode": "341702",
            "districtName": "贵池区"
          },
          {
            "districtID": 1113,
            "cityID": 113,
            "divisionCode": "341721",
            "districtName": "东至县"
          },
          {
            "districtID": 1114,
            "cityID": 113,
            "divisionCode": "341722",
            "districtName": "石台县"
          },
          {
            "districtID": 1115,
            "cityID": 113,
            "divisionCode": "341723",
            "districtName": "青阳县"
          },
          {
            "districtID": 1116,
            "cityID": 114,
            "divisionCode": "341801",
            "districtName": "市辖区"
          },
          {
            "districtID": 1117,
            "cityID": 114,
            "divisionCode": "341802",
            "districtName": "宣州区"
          },
          {
            "districtID": 1118,
            "cityID": 114,
            "divisionCode": "341821",
            "districtName": "郎溪县"
          },
          {
            "districtID": 1119,
            "cityID": 114,
            "divisionCode": "341822",
            "districtName": "广德县"
          },
          {
            "districtID": 1120,
            "cityID": 114,
            "divisionCode": "341823",
            "districtName": "泾县"
          },
          {
            "districtID": 1121,
            "cityID": 114,
            "divisionCode": "341824",
            "districtName": "绩溪县"
          },
          {
            "districtID": 1122,
            "cityID": 114,
            "divisionCode": "341825",
            "districtName": "旌德县"
          },
          {
            "districtID": 1123,
            "cityID": 114,
            "divisionCode": "341881",
            "districtName": "宁国市"
          },
          {
            "districtID": 1124,
            "cityID": 115,
            "divisionCode": "350101",
            "districtName": "市辖区"
          },
          {
            "districtID": 1125,
            "cityID": 115,
            "divisionCode": "350102",
            "districtName": "鼓楼区"
          },
          {
            "districtID": 1126,
            "cityID": 115,
            "divisionCode": "350103",
            "districtName": "台江区"
          },
          {
            "districtID": 1127,
            "cityID": 115,
            "divisionCode": "350104",
            "districtName": "仓山区"
          },
          {
            "districtID": 1128,
            "cityID": 115,
            "divisionCode": "350105",
            "districtName": "马尾区"
          },
          {
            "districtID": 1129,
            "cityID": 115,
            "divisionCode": "350111",
            "districtName": "晋安区"
          },
          {
            "districtID": 1130,
            "cityID": 115,
            "divisionCode": "350121",
            "districtName": "闽侯县"
          },
          {
            "districtID": 1131,
            "cityID": 115,
            "divisionCode": "350122",
            "districtName": "连江县"
          },
          {
            "districtID": 1132,
            "cityID": 115,
            "divisionCode": "350123",
            "districtName": "罗源县"
          },
          {
            "districtID": 1133,
            "cityID": 115,
            "divisionCode": "350124",
            "districtName": "闽清县"
          },
          {
            "districtID": 1134,
            "cityID": 115,
            "divisionCode": "350125",
            "districtName": "永泰县"
          },
          {
            "districtID": 1135,
            "cityID": 115,
            "divisionCode": "350128",
            "districtName": "平潭县"
          },
          {
            "districtID": 1136,
            "cityID": 115,
            "divisionCode": "350181",
            "districtName": "福清市"
          },
          {
            "districtID": 1137,
            "cityID": 115,
            "divisionCode": "350182",
            "districtName": "长乐市"
          },
          {
            "districtID": 1138,
            "cityID": 116,
            "divisionCode": "350201",
            "districtName": "市辖区"
          },
          {
            "districtID": 1139,
            "cityID": 116,
            "divisionCode": "350203",
            "districtName": "思明区"
          },
          {
            "districtID": 1140,
            "cityID": 116,
            "divisionCode": "350205",
            "districtName": "海沧区"
          },
          {
            "districtID": 1141,
            "cityID": 116,
            "divisionCode": "350206",
            "districtName": "湖里区"
          },
          {
            "districtID": 1142,
            "cityID": 116,
            "divisionCode": "350211",
            "districtName": "集美区"
          },
          {
            "districtID": 1143,
            "cityID": 116,
            "divisionCode": "350212",
            "districtName": "同安区"
          },
          {
            "districtID": 1144,
            "cityID": 116,
            "divisionCode": "350213",
            "districtName": "翔安区"
          },
          {
            "districtID": 1145,
            "cityID": 117,
            "divisionCode": "350301",
            "districtName": "市辖区"
          },
          {
            "districtID": 1146,
            "cityID": 117,
            "divisionCode": "350302",
            "districtName": "城厢区"
          },
          {
            "districtID": 1147,
            "cityID": 117,
            "divisionCode": "350303",
            "districtName": "涵江区"
          },
          {
            "districtID": 1148,
            "cityID": 117,
            "divisionCode": "350304",
            "districtName": "荔城区"
          },
          {
            "districtID": 1149,
            "cityID": 117,
            "divisionCode": "350305",
            "districtName": "秀屿区"
          },
          {
            "districtID": 1150,
            "cityID": 117,
            "divisionCode": "350322",
            "districtName": "仙游县"
          },
          {
            "districtID": 1151,
            "cityID": 118,
            "divisionCode": "350401",
            "districtName": "市辖区"
          },
          {
            "districtID": 1152,
            "cityID": 118,
            "divisionCode": "350402",
            "districtName": "梅列区"
          },
          {
            "districtID": 1153,
            "cityID": 118,
            "divisionCode": "350403",
            "districtName": "三元区"
          },
          {
            "districtID": 1154,
            "cityID": 118,
            "divisionCode": "350421",
            "districtName": "明溪县"
          },
          {
            "districtID": 1155,
            "cityID": 118,
            "divisionCode": "350423",
            "districtName": "清流县"
          },
          {
            "districtID": 1156,
            "cityID": 118,
            "divisionCode": "350424",
            "districtName": "宁化县"
          },
          {
            "districtID": 1157,
            "cityID": 118,
            "divisionCode": "350425",
            "districtName": "大田县"
          },
          {
            "districtID": 1158,
            "cityID": 118,
            "divisionCode": "350426",
            "districtName": "尤溪县"
          },
          {
            "districtID": 1159,
            "cityID": 118,
            "divisionCode": "350427",
            "districtName": "沙县"
          },
          {
            "districtID": 1160,
            "cityID": 118,
            "divisionCode": "350428",
            "districtName": "将乐县"
          },
          {
            "districtID": 1161,
            "cityID": 118,
            "divisionCode": "350429",
            "districtName": "泰宁县"
          },
          {
            "districtID": 1162,
            "cityID": 118,
            "divisionCode": "350430",
            "districtName": "建宁县"
          },
          {
            "districtID": 1163,
            "cityID": 118,
            "divisionCode": "350481",
            "districtName": "永安市"
          },
          {
            "districtID": 1164,
            "cityID": 119,
            "divisionCode": "350501",
            "districtName": "市辖区"
          },
          {
            "districtID": 1165,
            "cityID": 119,
            "divisionCode": "350502",
            "districtName": "鲤城区"
          },
          {
            "districtID": 1166,
            "cityID": 119,
            "divisionCode": "350503",
            "districtName": "丰泽区"
          },
          {
            "districtID": 1167,
            "cityID": 119,
            "divisionCode": "350504",
            "districtName": "洛江区"
          },
          {
            "districtID": 1168,
            "cityID": 119,
            "divisionCode": "350505",
            "districtName": "泉港区"
          },
          {
            "districtID": 1169,
            "cityID": 119,
            "divisionCode": "350521",
            "districtName": "惠安县"
          },
          {
            "districtID": 1170,
            "cityID": 119,
            "divisionCode": "350524",
            "districtName": "安溪县"
          },
          {
            "districtID": 1171,
            "cityID": 119,
            "divisionCode": "350525",
            "districtName": "永春县"
          },
          {
            "districtID": 1172,
            "cityID": 119,
            "divisionCode": "350526",
            "districtName": "德化县"
          },
          {
            "districtID": 1173,
            "cityID": 119,
            "divisionCode": "350527",
            "districtName": "金门县"
          },
          {
            "districtID": 1174,
            "cityID": 119,
            "divisionCode": "350581",
            "districtName": "石狮市"
          },
          {
            "districtID": 1175,
            "cityID": 119,
            "divisionCode": "350582",
            "districtName": "晋江市"
          },
          {
            "districtID": 1176,
            "cityID": 119,
            "divisionCode": "350583",
            "districtName": "南安市"
          },
          {
            "districtID": 1177,
            "cityID": 120,
            "divisionCode": "350601",
            "districtName": "市辖区"
          },
          {
            "districtID": 1178,
            "cityID": 120,
            "divisionCode": "350602",
            "districtName": "芗城区"
          },
          {
            "districtID": 1179,
            "cityID": 120,
            "divisionCode": "350603",
            "districtName": "龙文区"
          },
          {
            "districtID": 1180,
            "cityID": 120,
            "divisionCode": "350622",
            "districtName": "云霄县"
          },
          {
            "districtID": 1181,
            "cityID": 120,
            "divisionCode": "350623",
            "districtName": "漳浦县"
          },
          {
            "districtID": 1182,
            "cityID": 120,
            "divisionCode": "350624",
            "districtName": "诏安县"
          },
          {
            "districtID": 1183,
            "cityID": 120,
            "divisionCode": "350625",
            "districtName": "长泰县"
          },
          {
            "districtID": 1184,
            "cityID": 120,
            "divisionCode": "350626",
            "districtName": "东山县"
          },
          {
            "districtID": 1185,
            "cityID": 120,
            "divisionCode": "350627",
            "districtName": "南靖县"
          },
          {
            "districtID": 1186,
            "cityID": 120,
            "divisionCode": "350628",
            "districtName": "平和县"
          },
          {
            "districtID": 1187,
            "cityID": 120,
            "divisionCode": "350629",
            "districtName": "华安县"
          },
          {
            "districtID": 1188,
            "cityID": 120,
            "divisionCode": "350681",
            "districtName": "龙海市"
          },
          {
            "districtID": 1189,
            "cityID": 121,
            "divisionCode": "350701",
            "districtName": "市辖区"
          },
          {
            "districtID": 1190,
            "cityID": 121,
            "divisionCode": "350702",
            "districtName": "延平区"
          },
          {
            "districtID": 1191,
            "cityID": 121,
            "divisionCode": "350703",
            "districtName": "建阳区"
          },
          {
            "districtID": 1192,
            "cityID": 121,
            "divisionCode": "350721",
            "districtName": "顺昌县"
          },
          {
            "districtID": 1193,
            "cityID": 121,
            "divisionCode": "350722",
            "districtName": "浦城县"
          },
          {
            "districtID": 1194,
            "cityID": 121,
            "divisionCode": "350723",
            "districtName": "光泽县"
          },
          {
            "districtID": 1195,
            "cityID": 121,
            "divisionCode": "350724",
            "districtName": "松溪县"
          },
          {
            "districtID": 1196,
            "cityID": 121,
            "divisionCode": "350725",
            "districtName": "政和县"
          },
          {
            "districtID": 1197,
            "cityID": 121,
            "divisionCode": "350781",
            "districtName": "邵武市"
          },
          {
            "districtID": 1198,
            "cityID": 121,
            "divisionCode": "350782",
            "districtName": "武夷山市"
          },
          {
            "districtID": 1199,
            "cityID": 121,
            "divisionCode": "350783",
            "districtName": "建瓯市"
          },
          {
            "districtID": 1200,
            "cityID": 122,
            "divisionCode": "350801",
            "districtName": "市辖区"
          },
          {
            "districtID": 1201,
            "cityID": 122,
            "divisionCode": "350802",
            "districtName": "新罗区"
          },
          {
            "districtID": 1202,
            "cityID": 122,
            "divisionCode": "350803",
            "districtName": "永定区"
          },
          {
            "districtID": 1203,
            "cityID": 122,
            "divisionCode": "350821",
            "districtName": "长汀县"
          },
          {
            "districtID": 1204,
            "cityID": 122,
            "divisionCode": "350823",
            "districtName": "上杭县"
          },
          {
            "districtID": 1205,
            "cityID": 122,
            "divisionCode": "350824",
            "districtName": "武平县"
          },
          {
            "districtID": 1206,
            "cityID": 122,
            "divisionCode": "350825",
            "districtName": "连城县"
          },
          {
            "districtID": 1207,
            "cityID": 122,
            "divisionCode": "350881",
            "districtName": "漳平市"
          },
          {
            "districtID": 1208,
            "cityID": 123,
            "divisionCode": "350901",
            "districtName": "市辖区"
          },
          {
            "districtID": 1209,
            "cityID": 123,
            "divisionCode": "350902",
            "districtName": "蕉城区"
          },
          {
            "districtID": 1210,
            "cityID": 123,
            "divisionCode": "350921",
            "districtName": "霞浦县"
          },
          {
            "districtID": 1211,
            "cityID": 123,
            "divisionCode": "350922",
            "districtName": "古田县"
          },
          {
            "districtID": 1212,
            "cityID": 123,
            "divisionCode": "350923",
            "districtName": "屏南县"
          },
          {
            "districtID": 1213,
            "cityID": 123,
            "divisionCode": "350924",
            "districtName": "寿宁县"
          },
          {
            "districtID": 1214,
            "cityID": 123,
            "divisionCode": "350925",
            "districtName": "周宁县"
          },
          {
            "districtID": 1215,
            "cityID": 123,
            "divisionCode": "350926",
            "districtName": "柘荣县"
          },
          {
            "districtID": 1216,
            "cityID": 123,
            "divisionCode": "350981",
            "districtName": "福安市"
          },
          {
            "districtID": 1217,
            "cityID": 123,
            "divisionCode": "350982",
            "districtName": "福鼎市"
          },
          {
            "districtID": 1218,
            "cityID": 124,
            "divisionCode": "360101",
            "districtName": "市辖区"
          },
          {
            "districtID": 1219,
            "cityID": 124,
            "divisionCode": "360102",
            "districtName": "东湖区"
          },
          {
            "districtID": 1220,
            "cityID": 124,
            "divisionCode": "360103",
            "districtName": "西湖区"
          },
          {
            "districtID": 1221,
            "cityID": 124,
            "divisionCode": "360104",
            "districtName": "青云谱区"
          },
          {
            "districtID": 1222,
            "cityID": 124,
            "divisionCode": "360105",
            "districtName": "湾里区"
          },
          {
            "districtID": 1223,
            "cityID": 124,
            "divisionCode": "360111",
            "districtName": "青山湖区"
          },
          {
            "districtID": 1224,
            "cityID": 124,
            "divisionCode": "360112",
            "districtName": "新建区"
          },
          {
            "districtID": 1225,
            "cityID": 124,
            "divisionCode": "360121",
            "districtName": "南昌县"
          },
          {
            "districtID": 1226,
            "cityID": 124,
            "divisionCode": "360123",
            "districtName": "安义县"
          },
          {
            "districtID": 1227,
            "cityID": 124,
            "divisionCode": "360124",
            "districtName": "进贤县"
          },
          {
            "districtID": 1228,
            "cityID": 125,
            "divisionCode": "360201",
            "districtName": "市辖区"
          },
          {
            "districtID": 1229,
            "cityID": 125,
            "divisionCode": "360202",
            "districtName": "昌江区"
          },
          {
            "districtID": 1230,
            "cityID": 125,
            "divisionCode": "360203",
            "districtName": "珠山区"
          },
          {
            "districtID": 1231,
            "cityID": 125,
            "divisionCode": "360222",
            "districtName": "浮梁县"
          },
          {
            "districtID": 1232,
            "cityID": 125,
            "divisionCode": "360281",
            "districtName": "乐平市"
          },
          {
            "districtID": 1233,
            "cityID": 126,
            "divisionCode": "360301",
            "districtName": "市辖区"
          },
          {
            "districtID": 1234,
            "cityID": 126,
            "divisionCode": "360302",
            "districtName": "安源区"
          },
          {
            "districtID": 1235,
            "cityID": 126,
            "divisionCode": "360313",
            "districtName": "湘东区"
          },
          {
            "districtID": 1236,
            "cityID": 126,
            "divisionCode": "360321",
            "districtName": "莲花县"
          },
          {
            "districtID": 1237,
            "cityID": 126,
            "divisionCode": "360322",
            "districtName": "上栗县"
          },
          {
            "districtID": 1238,
            "cityID": 126,
            "divisionCode": "360323",
            "districtName": "芦溪县"
          },
          {
            "districtID": 1239,
            "cityID": 127,
            "divisionCode": "360401",
            "districtName": "市辖区"
          },
          {
            "districtID": 1240,
            "cityID": 127,
            "divisionCode": "360402",
            "districtName": "濂溪区"
          },
          {
            "districtID": 1241,
            "cityID": 127,
            "divisionCode": "360403",
            "districtName": "浔阳区"
          },
          {
            "districtID": 1242,
            "cityID": 127,
            "divisionCode": "360421",
            "districtName": "九江县"
          },
          {
            "districtID": 1243,
            "cityID": 127,
            "divisionCode": "360423",
            "districtName": "武宁县"
          },
          {
            "districtID": 1244,
            "cityID": 127,
            "divisionCode": "360424",
            "districtName": "修水县"
          },
          {
            "districtID": 1245,
            "cityID": 127,
            "divisionCode": "360425",
            "districtName": "永修县"
          },
          {
            "districtID": 1246,
            "cityID": 127,
            "divisionCode": "360426",
            "districtName": "德安县"
          },
          {
            "districtID": 1247,
            "cityID": 127,
            "divisionCode": "360428",
            "districtName": "都昌县"
          },
          {
            "districtID": 1248,
            "cityID": 127,
            "divisionCode": "360429",
            "districtName": "湖口县"
          },
          {
            "districtID": 1249,
            "cityID": 127,
            "divisionCode": "360430",
            "districtName": "彭泽县"
          },
          {
            "districtID": 1250,
            "cityID": 127,
            "divisionCode": "360481",
            "districtName": "瑞昌市"
          },
          {
            "districtID": 1251,
            "cityID": 127,
            "divisionCode": "360482",
            "districtName": "共青城市"
          },
          {
            "districtID": 1252,
            "cityID": 127,
            "divisionCode": "360483",
            "districtName": "庐山市"
          },
          {
            "districtID": 1253,
            "cityID": 128,
            "divisionCode": "360501",
            "districtName": "市辖区"
          },
          {
            "districtID": 1254,
            "cityID": 128,
            "divisionCode": "360502",
            "districtName": "渝水区"
          },
          {
            "districtID": 1255,
            "cityID": 128,
            "divisionCode": "360521",
            "districtName": "分宜县"
          },
          {
            "districtID": 1256,
            "cityID": 129,
            "divisionCode": "360601",
            "districtName": "市辖区"
          },
          {
            "districtID": 1257,
            "cityID": 129,
            "divisionCode": "360602",
            "districtName": "月湖区"
          },
          {
            "districtID": 1258,
            "cityID": 129,
            "divisionCode": "360622",
            "districtName": "余江县"
          },
          {
            "districtID": 1259,
            "cityID": 129,
            "divisionCode": "360681",
            "districtName": "贵溪市"
          },
          {
            "districtID": 1260,
            "cityID": 130,
            "divisionCode": "360701",
            "districtName": "市辖区"
          },
          {
            "districtID": 1261,
            "cityID": 130,
            "divisionCode": "360702",
            "districtName": "章贡区"
          },
          {
            "districtID": 1262,
            "cityID": 130,
            "divisionCode": "360703",
            "districtName": "南康区"
          },
          {
            "districtID": 1263,
            "cityID": 130,
            "divisionCode": "360721",
            "districtName": "赣县"
          },
          {
            "districtID": 1264,
            "cityID": 130,
            "divisionCode": "360722",
            "districtName": "信丰县"
          },
          {
            "districtID": 1265,
            "cityID": 130,
            "divisionCode": "360723",
            "districtName": "大余县"
          },
          {
            "districtID": 1266,
            "cityID": 130,
            "divisionCode": "360724",
            "districtName": "上犹县"
          },
          {
            "districtID": 1267,
            "cityID": 130,
            "divisionCode": "360725",
            "districtName": "崇义县"
          },
          {
            "districtID": 1268,
            "cityID": 130,
            "divisionCode": "360726",
            "districtName": "安远县"
          },
          {
            "districtID": 1269,
            "cityID": 130,
            "divisionCode": "360727",
            "districtName": "龙南县"
          },
          {
            "districtID": 1270,
            "cityID": 130,
            "divisionCode": "360728",
            "districtName": "定南县"
          },
          {
            "districtID": 1271,
            "cityID": 130,
            "divisionCode": "360729",
            "districtName": "全南县"
          },
          {
            "districtID": 1272,
            "cityID": 130,
            "divisionCode": "360730",
            "districtName": "宁都县"
          },
          {
            "districtID": 1273,
            "cityID": 130,
            "divisionCode": "360731",
            "districtName": "于都县"
          },
          {
            "districtID": 1274,
            "cityID": 130,
            "divisionCode": "360732",
            "districtName": "兴国县"
          },
          {
            "districtID": 1275,
            "cityID": 130,
            "divisionCode": "360733",
            "districtName": "会昌县"
          },
          {
            "districtID": 1276,
            "cityID": 130,
            "divisionCode": "360734",
            "districtName": "寻乌县"
          },
          {
            "districtID": 1277,
            "cityID": 130,
            "divisionCode": "360735",
            "districtName": "石城县"
          },
          {
            "districtID": 1278,
            "cityID": 130,
            "divisionCode": "360781",
            "districtName": "瑞金市"
          },
          {
            "districtID": 1279,
            "cityID": 131,
            "divisionCode": "360801",
            "districtName": "市辖区"
          },
          {
            "districtID": 1280,
            "cityID": 131,
            "divisionCode": "360802",
            "districtName": "吉州区"
          },
          {
            "districtID": 1281,
            "cityID": 131,
            "divisionCode": "360803",
            "districtName": "青原区"
          },
          {
            "districtID": 1282,
            "cityID": 131,
            "divisionCode": "360821",
            "districtName": "吉安县"
          },
          {
            "districtID": 1283,
            "cityID": 131,
            "divisionCode": "360822",
            "districtName": "吉水县"
          },
          {
            "districtID": 1284,
            "cityID": 131,
            "divisionCode": "360823",
            "districtName": "峡江县"
          },
          {
            "districtID": 1285,
            "cityID": 131,
            "divisionCode": "360824",
            "districtName": "新干县"
          },
          {
            "districtID": 1286,
            "cityID": 131,
            "divisionCode": "360825",
            "districtName": "永丰县"
          },
          {
            "districtID": 1287,
            "cityID": 131,
            "divisionCode": "360826",
            "districtName": "泰和县"
          },
          {
            "districtID": 1288,
            "cityID": 131,
            "divisionCode": "360827",
            "districtName": "遂川县"
          },
          {
            "districtID": 1289,
            "cityID": 131,
            "divisionCode": "360828",
            "districtName": "万安县"
          },
          {
            "districtID": 1290,
            "cityID": 131,
            "divisionCode": "360829",
            "districtName": "安福县"
          },
          {
            "districtID": 1291,
            "cityID": 131,
            "divisionCode": "360830",
            "districtName": "永新县"
          },
          {
            "districtID": 1292,
            "cityID": 131,
            "divisionCode": "360881",
            "districtName": "井冈山市"
          },
          {
            "districtID": 1293,
            "cityID": 132,
            "divisionCode": "360901",
            "districtName": "市辖区"
          },
          {
            "districtID": 1294,
            "cityID": 132,
            "divisionCode": "360902",
            "districtName": "袁州区"
          },
          {
            "districtID": 1295,
            "cityID": 132,
            "divisionCode": "360921",
            "districtName": "奉新县"
          },
          {
            "districtID": 1296,
            "cityID": 132,
            "divisionCode": "360922",
            "districtName": "万载县"
          },
          {
            "districtID": 1297,
            "cityID": 132,
            "divisionCode": "360923",
            "districtName": "上高县"
          },
          {
            "districtID": 1298,
            "cityID": 132,
            "divisionCode": "360924",
            "districtName": "宜丰县"
          },
          {
            "districtID": 1299,
            "cityID": 132,
            "divisionCode": "360925",
            "districtName": "靖安县"
          },
          {
            "districtID": 1300,
            "cityID": 132,
            "divisionCode": "360926",
            "districtName": "铜鼓县"
          },
          {
            "districtID": 1301,
            "cityID": 132,
            "divisionCode": "360981",
            "districtName": "丰城市"
          },
          {
            "districtID": 1302,
            "cityID": 132,
            "divisionCode": "360982",
            "districtName": "樟树市"
          },
          {
            "districtID": 1303,
            "cityID": 132,
            "divisionCode": "360983",
            "districtName": "高安市"
          },
          {
            "districtID": 1304,
            "cityID": 133,
            "divisionCode": "361001",
            "districtName": "市辖区"
          },
          {
            "districtID": 1305,
            "cityID": 133,
            "divisionCode": "361002",
            "districtName": "临川区"
          },
          {
            "districtID": 1306,
            "cityID": 133,
            "divisionCode": "361021",
            "districtName": "南城县"
          },
          {
            "districtID": 1307,
            "cityID": 133,
            "divisionCode": "361022",
            "districtName": "黎川县"
          },
          {
            "districtID": 1308,
            "cityID": 133,
            "divisionCode": "361023",
            "districtName": "南丰县"
          },
          {
            "districtID": 1309,
            "cityID": 133,
            "divisionCode": "361024",
            "districtName": "崇仁县"
          },
          {
            "districtID": 1310,
            "cityID": 133,
            "divisionCode": "361025",
            "districtName": "乐安县"
          },
          {
            "districtID": 1311,
            "cityID": 133,
            "divisionCode": "361026",
            "districtName": "宜黄县"
          },
          {
            "districtID": 1312,
            "cityID": 133,
            "divisionCode": "361027",
            "districtName": "金溪县"
          },
          {
            "districtID": 1313,
            "cityID": 133,
            "divisionCode": "361028",
            "districtName": "资溪县"
          },
          {
            "districtID": 1314,
            "cityID": 133,
            "divisionCode": "361029",
            "districtName": "东乡县"
          },
          {
            "districtID": 1315,
            "cityID": 133,
            "divisionCode": "361030",
            "districtName": "广昌县"
          },
          {
            "districtID": 1316,
            "cityID": 134,
            "divisionCode": "361101",
            "districtName": "市辖区"
          },
          {
            "districtID": 1317,
            "cityID": 134,
            "divisionCode": "361102",
            "districtName": "信州区"
          },
          {
            "districtID": 1318,
            "cityID": 134,
            "divisionCode": "361103",
            "districtName": "广丰区"
          },
          {
            "districtID": 1319,
            "cityID": 134,
            "divisionCode": "361121",
            "districtName": "上饶县"
          },
          {
            "districtID": 1320,
            "cityID": 134,
            "divisionCode": "361123",
            "districtName": "玉山县"
          },
          {
            "districtID": 1321,
            "cityID": 134,
            "divisionCode": "361124",
            "districtName": "铅山县"
          },
          {
            "districtID": 1322,
            "cityID": 134,
            "divisionCode": "361125",
            "districtName": "横峰县"
          },
          {
            "districtID": 1323,
            "cityID": 134,
            "divisionCode": "361126",
            "districtName": "弋阳县"
          },
          {
            "districtID": 1324,
            "cityID": 134,
            "divisionCode": "361127",
            "districtName": "余干县"
          },
          {
            "districtID": 1325,
            "cityID": 134,
            "divisionCode": "361128",
            "districtName": "鄱阳县"
          },
          {
            "districtID": 1326,
            "cityID": 134,
            "divisionCode": "361129",
            "districtName": "万年县"
          },
          {
            "districtID": 1327,
            "cityID": 134,
            "divisionCode": "361130",
            "districtName": "婺源县"
          },
          {
            "districtID": 1328,
            "cityID": 134,
            "divisionCode": "361181",
            "districtName": "德兴市"
          },
          {
            "districtID": 1329,
            "cityID": 135,
            "divisionCode": "370101",
            "districtName": "市辖区"
          },
          {
            "districtID": 1330,
            "cityID": 135,
            "divisionCode": "370102",
            "districtName": "历下区"
          },
          {
            "districtID": 1331,
            "cityID": 135,
            "divisionCode": "370103",
            "districtName": "市中区"
          },
          {
            "districtID": 1332,
            "cityID": 135,
            "divisionCode": "370104",
            "districtName": "槐荫区"
          },
          {
            "districtID": 1333,
            "cityID": 135,
            "divisionCode": "370105",
            "districtName": "天桥区"
          },
          {
            "districtID": 1334,
            "cityID": 135,
            "divisionCode": "370112",
            "districtName": "历城区"
          },
          {
            "districtID": 1335,
            "cityID": 135,
            "divisionCode": "370113",
            "districtName": "长清区"
          },
          {
            "districtID": 1336,
            "cityID": 135,
            "divisionCode": "370124",
            "districtName": "平阴县"
          },
          {
            "districtID": 1337,
            "cityID": 135,
            "divisionCode": "370125",
            "districtName": "济阳县"
          },
          {
            "districtID": 1338,
            "cityID": 135,
            "divisionCode": "370126",
            "districtName": "商河县"
          },
          {
            "districtID": 1339,
            "cityID": 135,
            "divisionCode": "370181",
            "districtName": "章丘市"
          },
          {
            "districtID": 1340,
            "cityID": 136,
            "divisionCode": "370201",
            "districtName": "市辖区"
          },
          {
            "districtID": 1341,
            "cityID": 136,
            "divisionCode": "370202",
            "districtName": "市南区"
          },
          {
            "districtID": 1342,
            "cityID": 136,
            "divisionCode": "370203",
            "districtName": "市北区"
          },
          {
            "districtID": 1343,
            "cityID": 136,
            "divisionCode": "370211",
            "districtName": "黄岛区"
          },
          {
            "districtID": 1344,
            "cityID": 136,
            "divisionCode": "370212",
            "districtName": "崂山区"
          },
          {
            "districtID": 1345,
            "cityID": 136,
            "divisionCode": "370213",
            "districtName": "李沧区"
          },
          {
            "districtID": 1346,
            "cityID": 136,
            "divisionCode": "370214",
            "districtName": "城阳区"
          },
          {
            "districtID": 1347,
            "cityID": 136,
            "divisionCode": "370281",
            "districtName": "胶州市"
          },
          {
            "districtID": 1348,
            "cityID": 136,
            "divisionCode": "370282",
            "districtName": "即墨市"
          },
          {
            "districtID": 1349,
            "cityID": 136,
            "divisionCode": "370283",
            "districtName": "平度市"
          },
          {
            "districtID": 1350,
            "cityID": 136,
            "divisionCode": "370285",
            "districtName": "莱西市"
          },
          {
            "districtID": 1351,
            "cityID": 137,
            "divisionCode": "370301",
            "districtName": "市辖区"
          },
          {
            "districtID": 1352,
            "cityID": 137,
            "divisionCode": "370302",
            "districtName": "淄川区"
          },
          {
            "districtID": 1353,
            "cityID": 137,
            "divisionCode": "370303",
            "districtName": "张店区"
          },
          {
            "districtID": 1354,
            "cityID": 137,
            "divisionCode": "370304",
            "districtName": "博山区"
          },
          {
            "districtID": 1355,
            "cityID": 137,
            "divisionCode": "370305",
            "districtName": "临淄区"
          },
          {
            "districtID": 1356,
            "cityID": 137,
            "divisionCode": "370306",
            "districtName": "周村区"
          },
          {
            "districtID": 1357,
            "cityID": 137,
            "divisionCode": "370321",
            "districtName": "桓台县"
          },
          {
            "districtID": 1358,
            "cityID": 137,
            "divisionCode": "370322",
            "districtName": "高青县"
          },
          {
            "districtID": 1359,
            "cityID": 137,
            "divisionCode": "370323",
            "districtName": "沂源县"
          },
          {
            "districtID": 1360,
            "cityID": 138,
            "divisionCode": "370401",
            "districtName": "市辖区"
          },
          {
            "districtID": 1361,
            "cityID": 138,
            "divisionCode": "370402",
            "districtName": "市中区"
          },
          {
            "districtID": 1362,
            "cityID": 138,
            "divisionCode": "370403",
            "districtName": "薛城区"
          },
          {
            "districtID": 1363,
            "cityID": 138,
            "divisionCode": "370404",
            "districtName": "峄城区"
          },
          {
            "districtID": 1364,
            "cityID": 138,
            "divisionCode": "370405",
            "districtName": "台儿庄区"
          },
          {
            "districtID": 1365,
            "cityID": 138,
            "divisionCode": "370406",
            "districtName": "山亭区"
          },
          {
            "districtID": 1366,
            "cityID": 138,
            "divisionCode": "370481",
            "districtName": "滕州市"
          },
          {
            "districtID": 1367,
            "cityID": 139,
            "divisionCode": "370501",
            "districtName": "市辖区"
          },
          {
            "districtID": 1368,
            "cityID": 139,
            "divisionCode": "370502",
            "districtName": "东营区"
          },
          {
            "districtID": 1369,
            "cityID": 139,
            "divisionCode": "370503",
            "districtName": "河口区"
          },
          {
            "districtID": 1370,
            "cityID": 139,
            "divisionCode": "370505",
            "districtName": "垦利区"
          },
          {
            "districtID": 1371,
            "cityID": 139,
            "divisionCode": "370522",
            "districtName": "利津县"
          },
          {
            "districtID": 1372,
            "cityID": 139,
            "divisionCode": "370523",
            "districtName": "广饶县"
          },
          {
            "districtID": 1373,
            "cityID": 140,
            "divisionCode": "370601",
            "districtName": "市辖区"
          },
          {
            "districtID": 1374,
            "cityID": 140,
            "divisionCode": "370602",
            "districtName": "芝罘区"
          },
          {
            "districtID": 1375,
            "cityID": 140,
            "divisionCode": "370611",
            "districtName": "福山区"
          },
          {
            "districtID": 1376,
            "cityID": 140,
            "divisionCode": "370612",
            "districtName": "牟平区"
          },
          {
            "districtID": 1377,
            "cityID": 140,
            "divisionCode": "370613",
            "districtName": "莱山区"
          },
          {
            "districtID": 1378,
            "cityID": 140,
            "divisionCode": "370634",
            "districtName": "长岛县"
          },
          {
            "districtID": 1379,
            "cityID": 140,
            "divisionCode": "370681",
            "districtName": "龙口市"
          },
          {
            "districtID": 1380,
            "cityID": 140,
            "divisionCode": "370682",
            "districtName": "莱阳市"
          },
          {
            "districtID": 1381,
            "cityID": 140,
            "divisionCode": "370683",
            "districtName": "莱州市"
          },
          {
            "districtID": 1382,
            "cityID": 140,
            "divisionCode": "370684",
            "districtName": "蓬莱市"
          },
          {
            "districtID": 1383,
            "cityID": 140,
            "divisionCode": "370685",
            "districtName": "招远市"
          },
          {
            "districtID": 1384,
            "cityID": 140,
            "divisionCode": "370686",
            "districtName": "栖霞市"
          },
          {
            "districtID": 1385,
            "cityID": 140,
            "divisionCode": "370687",
            "districtName": "海阳市"
          },
          {
            "districtID": 1386,
            "cityID": 141,
            "divisionCode": "370701",
            "districtName": "市辖区"
          },
          {
            "districtID": 1387,
            "cityID": 141,
            "divisionCode": "370702",
            "districtName": "潍城区"
          },
          {
            "districtID": 1388,
            "cityID": 141,
            "divisionCode": "370703",
            "districtName": "寒亭区"
          },
          {
            "districtID": 1389,
            "cityID": 141,
            "divisionCode": "370704",
            "districtName": "坊子区"
          },
          {
            "districtID": 1390,
            "cityID": 141,
            "divisionCode": "370705",
            "districtName": "奎文区"
          },
          {
            "districtID": 1391,
            "cityID": 141,
            "divisionCode": "370724",
            "districtName": "临朐县"
          },
          {
            "districtID": 1392,
            "cityID": 141,
            "divisionCode": "370725",
            "districtName": "昌乐县"
          },
          {
            "districtID": 1393,
            "cityID": 141,
            "divisionCode": "370781",
            "districtName": "青州市"
          },
          {
            "districtID": 1394,
            "cityID": 141,
            "divisionCode": "370782",
            "districtName": "诸城市"
          },
          {
            "districtID": 1395,
            "cityID": 141,
            "divisionCode": "370783",
            "districtName": "寿光市"
          },
          {
            "districtID": 1396,
            "cityID": 141,
            "divisionCode": "370784",
            "districtName": "安丘市"
          },
          {
            "districtID": 1397,
            "cityID": 141,
            "divisionCode": "370785",
            "districtName": "高密市"
          },
          {
            "districtID": 1398,
            "cityID": 141,
            "divisionCode": "370786",
            "districtName": "昌邑市"
          },
          {
            "districtID": 1399,
            "cityID": 142,
            "divisionCode": "370801",
            "districtName": "市辖区"
          },
          {
            "districtID": 1400,
            "cityID": 142,
            "divisionCode": "370811",
            "districtName": "任城区"
          },
          {
            "districtID": 1401,
            "cityID": 142,
            "divisionCode": "370812",
            "districtName": "兖州区"
          },
          {
            "districtID": 1402,
            "cityID": 142,
            "divisionCode": "370826",
            "districtName": "微山县"
          },
          {
            "districtID": 1403,
            "cityID": 142,
            "divisionCode": "370827",
            "districtName": "鱼台县"
          },
          {
            "districtID": 1404,
            "cityID": 142,
            "divisionCode": "370828",
            "districtName": "金乡县"
          },
          {
            "districtID": 1405,
            "cityID": 142,
            "divisionCode": "370829",
            "districtName": "嘉祥县"
          },
          {
            "districtID": 1406,
            "cityID": 142,
            "divisionCode": "370830",
            "districtName": "汶上县"
          },
          {
            "districtID": 1407,
            "cityID": 142,
            "divisionCode": "370831",
            "districtName": "泗水县"
          },
          {
            "districtID": 1408,
            "cityID": 142,
            "divisionCode": "370832",
            "districtName": "梁山县"
          },
          {
            "districtID": 1409,
            "cityID": 142,
            "divisionCode": "370881",
            "districtName": "曲阜市"
          },
          {
            "districtID": 1410,
            "cityID": 142,
            "divisionCode": "370883",
            "districtName": "邹城市"
          },
          {
            "districtID": 1411,
            "cityID": 143,
            "divisionCode": "370901",
            "districtName": "市辖区"
          },
          {
            "districtID": 1412,
            "cityID": 143,
            "divisionCode": "370902",
            "districtName": "泰山区"
          },
          {
            "districtID": 1413,
            "cityID": 143,
            "divisionCode": "370911",
            "districtName": "岱岳区"
          },
          {
            "districtID": 1414,
            "cityID": 143,
            "divisionCode": "370921",
            "districtName": "宁阳县"
          },
          {
            "districtID": 1415,
            "cityID": 143,
            "divisionCode": "370923",
            "districtName": "东平县"
          },
          {
            "districtID": 1416,
            "cityID": 143,
            "divisionCode": "370982",
            "districtName": "新泰市"
          },
          {
            "districtID": 1417,
            "cityID": 143,
            "divisionCode": "370983",
            "districtName": "肥城市"
          },
          {
            "districtID": 1418,
            "cityID": 144,
            "divisionCode": "371001",
            "districtName": "市辖区"
          },
          {
            "districtID": 1419,
            "cityID": 144,
            "divisionCode": "371002",
            "districtName": "环翠区"
          },
          {
            "districtID": 1420,
            "cityID": 144,
            "divisionCode": "371003",
            "districtName": "文登区"
          },
          {
            "districtID": 1421,
            "cityID": 144,
            "divisionCode": "371082",
            "districtName": "荣成市"
          },
          {
            "districtID": 1422,
            "cityID": 144,
            "divisionCode": "371083",
            "districtName": "乳山市"
          },
          {
            "districtID": 1423,
            "cityID": 145,
            "divisionCode": "371101",
            "districtName": "市辖区"
          },
          {
            "districtID": 1424,
            "cityID": 145,
            "divisionCode": "371102",
            "districtName": "东港区"
          },
          {
            "districtID": 1425,
            "cityID": 145,
            "divisionCode": "371103",
            "districtName": "岚山区"
          },
          {
            "districtID": 1426,
            "cityID": 145,
            "divisionCode": "371121",
            "districtName": "五莲县"
          },
          {
            "districtID": 1427,
            "cityID": 145,
            "divisionCode": "371122",
            "districtName": "莒县"
          },
          {
            "districtID": 1428,
            "cityID": 146,
            "divisionCode": "371201",
            "districtName": "市辖区"
          },
          {
            "districtID": 1429,
            "cityID": 146,
            "divisionCode": "371202",
            "districtName": "莱城区"
          },
          {
            "districtID": 1430,
            "cityID": 146,
            "divisionCode": "371203",
            "districtName": "钢城区"
          },
          {
            "districtID": 1431,
            "cityID": 147,
            "divisionCode": "371301",
            "districtName": "市辖区"
          },
          {
            "districtID": 1432,
            "cityID": 147,
            "divisionCode": "371302",
            "districtName": "兰山区"
          },
          {
            "districtID": 1433,
            "cityID": 147,
            "divisionCode": "371311",
            "districtName": "罗庄区"
          },
          {
            "districtID": 1434,
            "cityID": 147,
            "divisionCode": "371312",
            "districtName": "河东区"
          },
          {
            "districtID": 1435,
            "cityID": 147,
            "divisionCode": "371321",
            "districtName": "沂南县"
          },
          {
            "districtID": 1436,
            "cityID": 147,
            "divisionCode": "371322",
            "districtName": "郯城县"
          },
          {
            "districtID": 1437,
            "cityID": 147,
            "divisionCode": "371323",
            "districtName": "沂水县"
          },
          {
            "districtID": 1438,
            "cityID": 147,
            "divisionCode": "371324",
            "districtName": "兰陵县"
          },
          {
            "districtID": 1439,
            "cityID": 147,
            "divisionCode": "371325",
            "districtName": "费县"
          },
          {
            "districtID": 1440,
            "cityID": 147,
            "divisionCode": "371326",
            "districtName": "平邑县"
          },
          {
            "districtID": 1441,
            "cityID": 147,
            "divisionCode": "371327",
            "districtName": "莒南县"
          },
          {
            "districtID": 1442,
            "cityID": 147,
            "divisionCode": "371328",
            "districtName": "蒙阴县"
          },
          {
            "districtID": 1443,
            "cityID": 147,
            "divisionCode": "371329",
            "districtName": "临沭县"
          },
          {
            "districtID": 1444,
            "cityID": 148,
            "divisionCode": "371401",
            "districtName": "市辖区"
          },
          {
            "districtID": 1445,
            "cityID": 148,
            "divisionCode": "371402",
            "districtName": "德城区"
          },
          {
            "districtID": 1446,
            "cityID": 148,
            "divisionCode": "371403",
            "districtName": "陵城区"
          },
          {
            "districtID": 1447,
            "cityID": 148,
            "divisionCode": "371422",
            "districtName": "宁津县"
          },
          {
            "districtID": 1448,
            "cityID": 148,
            "divisionCode": "371423",
            "districtName": "庆云县"
          },
          {
            "districtID": 1449,
            "cityID": 148,
            "divisionCode": "371424",
            "districtName": "临邑县"
          },
          {
            "districtID": 1450,
            "cityID": 148,
            "divisionCode": "371425",
            "districtName": "齐河县"
          },
          {
            "districtID": 1451,
            "cityID": 148,
            "divisionCode": "371426",
            "districtName": "平原县"
          },
          {
            "districtID": 1452,
            "cityID": 148,
            "divisionCode": "371427",
            "districtName": "夏津县"
          },
          {
            "districtID": 1453,
            "cityID": 148,
            "divisionCode": "371428",
            "districtName": "武城县"
          },
          {
            "districtID": 1454,
            "cityID": 148,
            "divisionCode": "371481",
            "districtName": "乐陵市"
          },
          {
            "districtID": 1455,
            "cityID": 148,
            "divisionCode": "371482",
            "districtName": "禹城市"
          },
          {
            "districtID": 1456,
            "cityID": 149,
            "divisionCode": "371501",
            "districtName": "市辖区"
          },
          {
            "districtID": 1457,
            "cityID": 149,
            "divisionCode": "371502",
            "districtName": "东昌府区"
          },
          {
            "districtID": 1458,
            "cityID": 149,
            "divisionCode": "371521",
            "districtName": "阳谷县"
          },
          {
            "districtID": 1459,
            "cityID": 149,
            "divisionCode": "371522",
            "districtName": "莘县"
          },
          {
            "districtID": 1460,
            "cityID": 149,
            "divisionCode": "371523",
            "districtName": "茌平县"
          },
          {
            "districtID": 1461,
            "cityID": 149,
            "divisionCode": "371524",
            "districtName": "东阿县"
          },
          {
            "districtID": 1462,
            "cityID": 149,
            "divisionCode": "371525",
            "districtName": "冠县"
          },
          {
            "districtID": 1463,
            "cityID": 149,
            "divisionCode": "371526",
            "districtName": "高唐县"
          },
          {
            "districtID": 1464,
            "cityID": 149,
            "divisionCode": "371581",
            "districtName": "临清市"
          },
          {
            "districtID": 1465,
            "cityID": 150,
            "divisionCode": "371601",
            "districtName": "市辖区"
          },
          {
            "districtID": 1466,
            "cityID": 150,
            "divisionCode": "371602",
            "districtName": "滨城区"
          },
          {
            "districtID": 1467,
            "cityID": 150,
            "divisionCode": "371603",
            "districtName": "沾化区"
          },
          {
            "districtID": 1468,
            "cityID": 150,
            "divisionCode": "371621",
            "districtName": "惠民县"
          },
          {
            "districtID": 1469,
            "cityID": 150,
            "divisionCode": "371622",
            "districtName": "阳信县"
          },
          {
            "districtID": 1470,
            "cityID": 150,
            "divisionCode": "371623",
            "districtName": "无棣县"
          },
          {
            "districtID": 1471,
            "cityID": 150,
            "divisionCode": "371625",
            "districtName": "博兴县"
          },
          {
            "districtID": 1472,
            "cityID": 150,
            "divisionCode": "371626",
            "districtName": "邹平县"
          },
          {
            "districtID": 1473,
            "cityID": 151,
            "divisionCode": "371701",
            "districtName": "市辖区"
          },
          {
            "districtID": 1474,
            "cityID": 151,
            "divisionCode": "371702",
            "districtName": "牡丹区"
          },
          {
            "districtID": 1475,
            "cityID": 151,
            "divisionCode": "371703",
            "districtName": "定陶区"
          },
          {
            "districtID": 1476,
            "cityID": 151,
            "divisionCode": "371721",
            "districtName": "曹县"
          },
          {
            "districtID": 1477,
            "cityID": 151,
            "divisionCode": "371722",
            "districtName": "单县"
          },
          {
            "districtID": 1478,
            "cityID": 151,
            "divisionCode": "371723",
            "districtName": "成武县"
          },
          {
            "districtID": 1479,
            "cityID": 151,
            "divisionCode": "371724",
            "districtName": "巨野县"
          },
          {
            "districtID": 1480,
            "cityID": 151,
            "divisionCode": "371725",
            "districtName": "郓城县"
          },
          {
            "districtID": 1481,
            "cityID": 151,
            "divisionCode": "371726",
            "districtName": "鄄城县"
          },
          {
            "districtID": 1482,
            "cityID": 151,
            "divisionCode": "371728",
            "districtName": "东明县"
          },
          {
            "districtID": 1483,
            "cityID": 152,
            "divisionCode": "410101",
            "districtName": "市辖区"
          },
          {
            "districtID": 1484,
            "cityID": 152,
            "divisionCode": "410102",
            "districtName": "中原区"
          },
          {
            "districtID": 1485,
            "cityID": 152,
            "divisionCode": "410103",
            "districtName": "二七区"
          },
          {
            "districtID": 1486,
            "cityID": 152,
            "divisionCode": "410104",
            "districtName": "管城回族区"
          },
          {
            "districtID": 1487,
            "cityID": 152,
            "divisionCode": "410105",
            "districtName": "金水区"
          },
          {
            "districtID": 1488,
            "cityID": 152,
            "divisionCode": "410106",
            "districtName": "上街区"
          },
          {
            "districtID": 1489,
            "cityID": 152,
            "divisionCode": "410108",
            "districtName": "惠济区"
          },
          {
            "districtID": 1490,
            "cityID": 152,
            "divisionCode": "410122",
            "districtName": "中牟县"
          },
          {
            "districtID": 1491,
            "cityID": 152,
            "divisionCode": "410181",
            "districtName": "巩义市"
          },
          {
            "districtID": 1492,
            "cityID": 152,
            "divisionCode": "410182",
            "districtName": "荥阳市"
          },
          {
            "districtID": 1493,
            "cityID": 152,
            "divisionCode": "410183",
            "districtName": "新密市"
          },
          {
            "districtID": 1494,
            "cityID": 152,
            "divisionCode": "410184",
            "districtName": "新郑市"
          },
          {
            "districtID": 1495,
            "cityID": 152,
            "divisionCode": "410185",
            "districtName": "登封市"
          },
          {
            "districtID": 1496,
            "cityID": 153,
            "divisionCode": "410201",
            "districtName": "市辖区"
          },
          {
            "districtID": 1497,
            "cityID": 153,
            "divisionCode": "410202",
            "districtName": "龙亭区"
          },
          {
            "districtID": 1498,
            "cityID": 153,
            "divisionCode": "410203",
            "districtName": "顺河回族区"
          },
          {
            "districtID": 1499,
            "cityID": 153,
            "divisionCode": "410204",
            "districtName": "鼓楼区"
          },
          {
            "districtID": 1500,
            "cityID": 153,
            "divisionCode": "410205",
            "districtName": "禹王台区"
          },
          {
            "districtID": 1501,
            "cityID": 153,
            "divisionCode": "410211",
            "districtName": "金明区"
          },
          {
            "districtID": 1502,
            "cityID": 153,
            "divisionCode": "410212",
            "districtName": "祥符区"
          },
          {
            "districtID": 1503,
            "cityID": 153,
            "divisionCode": "410221",
            "districtName": "杞县"
          },
          {
            "districtID": 1504,
            "cityID": 153,
            "divisionCode": "410222",
            "districtName": "通许县"
          },
          {
            "districtID": 1505,
            "cityID": 153,
            "divisionCode": "410223",
            "districtName": "尉氏县"
          },
          {
            "districtID": 1506,
            "cityID": 153,
            "divisionCode": "410225",
            "districtName": "兰考县"
          },
          {
            "districtID": 1507,
            "cityID": 154,
            "divisionCode": "410301",
            "districtName": "市辖区"
          },
          {
            "districtID": 1508,
            "cityID": 154,
            "divisionCode": "410302",
            "districtName": "老城区"
          },
          {
            "districtID": 1509,
            "cityID": 154,
            "divisionCode": "410303",
            "districtName": "西工区"
          },
          {
            "districtID": 1510,
            "cityID": 154,
            "divisionCode": "410304",
            "districtName": "瀍河回族区"
          },
          {
            "districtID": 1511,
            "cityID": 154,
            "divisionCode": "410305",
            "districtName": "涧西区"
          },
          {
            "districtID": 1512,
            "cityID": 154,
            "divisionCode": "410306",
            "districtName": "吉利区"
          },
          {
            "districtID": 1513,
            "cityID": 154,
            "divisionCode": "410311",
            "districtName": "洛龙区"
          },
          {
            "districtID": 1514,
            "cityID": 154,
            "divisionCode": "410322",
            "districtName": "孟津县"
          },
          {
            "districtID": 1515,
            "cityID": 154,
            "divisionCode": "410323",
            "districtName": "新安县"
          },
          {
            "districtID": 1516,
            "cityID": 154,
            "divisionCode": "410324",
            "districtName": "栾川县"
          },
          {
            "districtID": 1517,
            "cityID": 154,
            "divisionCode": "410325",
            "districtName": "嵩县"
          },
          {
            "districtID": 1518,
            "cityID": 154,
            "divisionCode": "410326",
            "districtName": "汝阳县"
          },
          {
            "districtID": 1519,
            "cityID": 154,
            "divisionCode": "410327",
            "districtName": "宜阳县"
          },
          {
            "districtID": 1520,
            "cityID": 154,
            "divisionCode": "410328",
            "districtName": "洛宁县"
          },
          {
            "districtID": 1521,
            "cityID": 154,
            "divisionCode": "410329",
            "districtName": "伊川县"
          },
          {
            "districtID": 1522,
            "cityID": 154,
            "divisionCode": "410381",
            "districtName": "偃师市"
          },
          {
            "districtID": 1523,
            "cityID": 155,
            "divisionCode": "410401",
            "districtName": "市辖区"
          },
          {
            "districtID": 1524,
            "cityID": 155,
            "divisionCode": "410402",
            "districtName": "新华区"
          },
          {
            "districtID": 1525,
            "cityID": 155,
            "divisionCode": "410403",
            "districtName": "卫东区"
          },
          {
            "districtID": 1526,
            "cityID": 155,
            "divisionCode": "410404",
            "districtName": "石龙区"
          },
          {
            "districtID": 1527,
            "cityID": 155,
            "divisionCode": "410411",
            "districtName": "湛河区"
          },
          {
            "districtID": 1528,
            "cityID": 155,
            "divisionCode": "410421",
            "districtName": "宝丰县"
          },
          {
            "districtID": 1529,
            "cityID": 155,
            "divisionCode": "410422",
            "districtName": "叶县"
          },
          {
            "districtID": 1530,
            "cityID": 155,
            "divisionCode": "410423",
            "districtName": "鲁山县"
          },
          {
            "districtID": 1531,
            "cityID": 155,
            "divisionCode": "410425",
            "districtName": "郏县"
          },
          {
            "districtID": 1532,
            "cityID": 155,
            "divisionCode": "410481",
            "districtName": "舞钢市"
          },
          {
            "districtID": 1533,
            "cityID": 155,
            "divisionCode": "410482",
            "districtName": "汝州市"
          },
          {
            "districtID": 1534,
            "cityID": 156,
            "divisionCode": "410501",
            "districtName": "市辖区"
          },
          {
            "districtID": 1535,
            "cityID": 156,
            "divisionCode": "410502",
            "districtName": "文峰区"
          },
          {
            "districtID": 1536,
            "cityID": 156,
            "divisionCode": "410503",
            "districtName": "北关区"
          },
          {
            "districtID": 1537,
            "cityID": 156,
            "divisionCode": "410505",
            "districtName": "殷都区"
          },
          {
            "districtID": 1538,
            "cityID": 156,
            "divisionCode": "410506",
            "districtName": "龙安区"
          },
          {
            "districtID": 1539,
            "cityID": 156,
            "divisionCode": "410522",
            "districtName": "安阳县"
          },
          {
            "districtID": 1540,
            "cityID": 156,
            "divisionCode": "410523",
            "districtName": "汤阴县"
          },
          {
            "districtID": 1541,
            "cityID": 156,
            "divisionCode": "410526",
            "districtName": "滑县"
          },
          {
            "districtID": 1542,
            "cityID": 156,
            "divisionCode": "410527",
            "districtName": "内黄县"
          },
          {
            "districtID": 1543,
            "cityID": 156,
            "divisionCode": "410581",
            "districtName": "林州市"
          },
          {
            "districtID": 1544,
            "cityID": 157,
            "divisionCode": "410601",
            "districtName": "市辖区"
          },
          {
            "districtID": 1545,
            "cityID": 157,
            "divisionCode": "410602",
            "districtName": "鹤山区"
          },
          {
            "districtID": 1546,
            "cityID": 157,
            "divisionCode": "410603",
            "districtName": "山城区"
          },
          {
            "districtID": 1547,
            "cityID": 157,
            "divisionCode": "410611",
            "districtName": "淇滨区"
          },
          {
            "districtID": 1548,
            "cityID": 157,
            "divisionCode": "410621",
            "districtName": "浚县"
          },
          {
            "districtID": 1549,
            "cityID": 157,
            "divisionCode": "410622",
            "districtName": "淇县"
          },
          {
            "districtID": 1550,
            "cityID": 158,
            "divisionCode": "410701",
            "districtName": "市辖区"
          },
          {
            "districtID": 1551,
            "cityID": 158,
            "divisionCode": "410702",
            "districtName": "红旗区"
          },
          {
            "districtID": 1552,
            "cityID": 158,
            "divisionCode": "410703",
            "districtName": "卫滨区"
          },
          {
            "districtID": 1553,
            "cityID": 158,
            "divisionCode": "410704",
            "districtName": "凤泉区"
          },
          {
            "districtID": 1554,
            "cityID": 158,
            "divisionCode": "410711",
            "districtName": "牧野区"
          },
          {
            "districtID": 1555,
            "cityID": 158,
            "divisionCode": "410721",
            "districtName": "新乡县"
          },
          {
            "districtID": 1556,
            "cityID": 158,
            "divisionCode": "410724",
            "districtName": "获嘉县"
          },
          {
            "districtID": 1557,
            "cityID": 158,
            "divisionCode": "410725",
            "districtName": "原阳县"
          },
          {
            "districtID": 1558,
            "cityID": 158,
            "divisionCode": "410726",
            "districtName": "延津县"
          },
          {
            "districtID": 1559,
            "cityID": 158,
            "divisionCode": "410727",
            "districtName": "封丘县"
          },
          {
            "districtID": 1560,
            "cityID": 158,
            "divisionCode": "410728",
            "districtName": "长垣县"
          },
          {
            "districtID": 1561,
            "cityID": 158,
            "divisionCode": "410781",
            "districtName": "卫辉市"
          },
          {
            "districtID": 1562,
            "cityID": 158,
            "divisionCode": "410782",
            "districtName": "辉县市"
          },
          {
            "districtID": 1563,
            "cityID": 159,
            "divisionCode": "410801",
            "districtName": "市辖区"
          },
          {
            "districtID": 1564,
            "cityID": 159,
            "divisionCode": "410802",
            "districtName": "解放区"
          },
          {
            "districtID": 1565,
            "cityID": 159,
            "divisionCode": "410803",
            "districtName": "中站区"
          },
          {
            "districtID": 1566,
            "cityID": 159,
            "divisionCode": "410804",
            "districtName": "马村区"
          },
          {
            "districtID": 1567,
            "cityID": 159,
            "divisionCode": "410811",
            "districtName": "山阳区"
          },
          {
            "districtID": 1568,
            "cityID": 159,
            "divisionCode": "410821",
            "districtName": "修武县"
          },
          {
            "districtID": 1569,
            "cityID": 159,
            "divisionCode": "410822",
            "districtName": "博爱县"
          },
          {
            "districtID": 1570,
            "cityID": 159,
            "divisionCode": "410823",
            "districtName": "武陟县"
          },
          {
            "districtID": 1571,
            "cityID": 159,
            "divisionCode": "410825",
            "districtName": "温县"
          },
          {
            "districtID": 1572,
            "cityID": 159,
            "divisionCode": "410882",
            "districtName": "沁阳市"
          },
          {
            "districtID": 1573,
            "cityID": 159,
            "divisionCode": "410883",
            "districtName": "孟州市"
          },
          {
            "districtID": 1574,
            "cityID": 160,
            "divisionCode": "410901",
            "districtName": "市辖区"
          },
          {
            "districtID": 1575,
            "cityID": 160,
            "divisionCode": "410902",
            "districtName": "华龙区"
          },
          {
            "districtID": 1576,
            "cityID": 160,
            "divisionCode": "410922",
            "districtName": "清丰县"
          },
          {
            "districtID": 1577,
            "cityID": 160,
            "divisionCode": "410923",
            "districtName": "南乐县"
          },
          {
            "districtID": 1578,
            "cityID": 160,
            "divisionCode": "410926",
            "districtName": "范县"
          },
          {
            "districtID": 1579,
            "cityID": 160,
            "divisionCode": "410927",
            "districtName": "台前县"
          },
          {
            "districtID": 1580,
            "cityID": 160,
            "divisionCode": "410928",
            "districtName": "濮阳县"
          },
          {
            "districtID": 1581,
            "cityID": 161,
            "divisionCode": "411001",
            "districtName": "市辖区"
          },
          {
            "districtID": 1582,
            "cityID": 161,
            "divisionCode": "411002",
            "districtName": "魏都区"
          },
          {
            "districtID": 1583,
            "cityID": 161,
            "divisionCode": "411023",
            "districtName": "许昌县"
          },
          {
            "districtID": 1584,
            "cityID": 161,
            "divisionCode": "411024",
            "districtName": "鄢陵县"
          },
          {
            "districtID": 1585,
            "cityID": 161,
            "divisionCode": "411025",
            "districtName": "襄城县"
          },
          {
            "districtID": 1586,
            "cityID": 161,
            "divisionCode": "411081",
            "districtName": "禹州市"
          },
          {
            "districtID": 1587,
            "cityID": 161,
            "divisionCode": "411082",
            "districtName": "长葛市"
          },
          {
            "districtID": 1588,
            "cityID": 162,
            "divisionCode": "411101",
            "districtName": "市辖区"
          },
          {
            "districtID": 1589,
            "cityID": 162,
            "divisionCode": "411102",
            "districtName": "源汇区"
          },
          {
            "districtID": 1590,
            "cityID": 162,
            "divisionCode": "411103",
            "districtName": "郾城区"
          },
          {
            "districtID": 1591,
            "cityID": 162,
            "divisionCode": "411104",
            "districtName": "召陵区"
          },
          {
            "districtID": 1592,
            "cityID": 162,
            "divisionCode": "411121",
            "districtName": "舞阳县"
          },
          {
            "districtID": 1593,
            "cityID": 162,
            "divisionCode": "411122",
            "districtName": "临颍县"
          },
          {
            "districtID": 1594,
            "cityID": 163,
            "divisionCode": "411201",
            "districtName": "市辖区"
          },
          {
            "districtID": 1595,
            "cityID": 163,
            "divisionCode": "411202",
            "districtName": "湖滨区"
          },
          {
            "districtID": 1596,
            "cityID": 163,
            "divisionCode": "411203",
            "districtName": "陕州区"
          },
          {
            "districtID": 1597,
            "cityID": 163,
            "divisionCode": "411221",
            "districtName": "渑池县"
          },
          {
            "districtID": 1598,
            "cityID": 163,
            "divisionCode": "411224",
            "districtName": "卢氏县"
          },
          {
            "districtID": 1599,
            "cityID": 163,
            "divisionCode": "411281",
            "districtName": "义马市"
          },
          {
            "districtID": 1600,
            "cityID": 163,
            "divisionCode": "411282",
            "districtName": "灵宝市"
          },
          {
            "districtID": 1601,
            "cityID": 164,
            "divisionCode": "411301",
            "districtName": "市辖区"
          },
          {
            "districtID": 1602,
            "cityID": 164,
            "divisionCode": "411302",
            "districtName": "宛城区"
          },
          {
            "districtID": 1603,
            "cityID": 164,
            "divisionCode": "411303",
            "districtName": "卧龙区"
          },
          {
            "districtID": 1604,
            "cityID": 164,
            "divisionCode": "411321",
            "districtName": "南召县"
          },
          {
            "districtID": 1605,
            "cityID": 164,
            "divisionCode": "411322",
            "districtName": "方城县"
          },
          {
            "districtID": 1606,
            "cityID": 164,
            "divisionCode": "411323",
            "districtName": "西峡县"
          },
          {
            "districtID": 1607,
            "cityID": 164,
            "divisionCode": "411324",
            "districtName": "镇平县"
          },
          {
            "districtID": 1608,
            "cityID": 164,
            "divisionCode": "411325",
            "districtName": "内乡县"
          },
          {
            "districtID": 1609,
            "cityID": 164,
            "divisionCode": "411326",
            "districtName": "淅川县"
          },
          {
            "districtID": 1610,
            "cityID": 164,
            "divisionCode": "411327",
            "districtName": "社旗县"
          },
          {
            "districtID": 1611,
            "cityID": 164,
            "divisionCode": "411328",
            "districtName": "唐河县"
          },
          {
            "districtID": 1612,
            "cityID": 164,
            "divisionCode": "411329",
            "districtName": "新野县"
          },
          {
            "districtID": 1613,
            "cityID": 164,
            "divisionCode": "411330",
            "districtName": "桐柏县"
          },
          {
            "districtID": 1614,
            "cityID": 164,
            "divisionCode": "411381",
            "districtName": "邓州市"
          },
          {
            "districtID": 1615,
            "cityID": 165,
            "divisionCode": "411401",
            "districtName": "市辖区"
          },
          {
            "districtID": 1616,
            "cityID": 165,
            "divisionCode": "411402",
            "districtName": "梁园区"
          },
          {
            "districtID": 1617,
            "cityID": 165,
            "divisionCode": "411403",
            "districtName": "睢阳区"
          },
          {
            "districtID": 1618,
            "cityID": 165,
            "divisionCode": "411421",
            "districtName": "民权县"
          },
          {
            "districtID": 1619,
            "cityID": 165,
            "divisionCode": "411422",
            "districtName": "睢县"
          },
          {
            "districtID": 1620,
            "cityID": 165,
            "divisionCode": "411423",
            "districtName": "宁陵县"
          },
          {
            "districtID": 1621,
            "cityID": 165,
            "divisionCode": "411424",
            "districtName": "柘城县"
          },
          {
            "districtID": 1622,
            "cityID": 165,
            "divisionCode": "411425",
            "districtName": "虞城县"
          },
          {
            "districtID": 1623,
            "cityID": 165,
            "divisionCode": "411426",
            "districtName": "夏邑县"
          },
          {
            "districtID": 1624,
            "cityID": 165,
            "divisionCode": "411481",
            "districtName": "永城市"
          },
          {
            "districtID": 1625,
            "cityID": 166,
            "divisionCode": "411501",
            "districtName": "市辖区"
          },
          {
            "districtID": 1626,
            "cityID": 166,
            "divisionCode": "411502",
            "districtName": "浉河区"
          },
          {
            "districtID": 1627,
            "cityID": 166,
            "divisionCode": "411503",
            "districtName": "平桥区"
          },
          {
            "districtID": 1628,
            "cityID": 166,
            "divisionCode": "411521",
            "districtName": "罗山县"
          },
          {
            "districtID": 1629,
            "cityID": 166,
            "divisionCode": "411522",
            "districtName": "光山县"
          },
          {
            "districtID": 1630,
            "cityID": 166,
            "divisionCode": "411523",
            "districtName": "新县"
          },
          {
            "districtID": 1631,
            "cityID": 166,
            "divisionCode": "411524",
            "districtName": "商城县"
          },
          {
            "districtID": 1632,
            "cityID": 166,
            "divisionCode": "411525",
            "districtName": "固始县"
          },
          {
            "districtID": 1633,
            "cityID": 166,
            "divisionCode": "411526",
            "districtName": "潢川县"
          },
          {
            "districtID": 1634,
            "cityID": 166,
            "divisionCode": "411527",
            "districtName": "淮滨县"
          },
          {
            "districtID": 1635,
            "cityID": 166,
            "divisionCode": "411528",
            "districtName": "息县"
          },
          {
            "districtID": 1636,
            "cityID": 167,
            "divisionCode": "411601",
            "districtName": "市辖区"
          },
          {
            "districtID": 1637,
            "cityID": 167,
            "divisionCode": "411602",
            "districtName": "川汇区"
          },
          {
            "districtID": 1638,
            "cityID": 167,
            "divisionCode": "411621",
            "districtName": "扶沟县"
          },
          {
            "districtID": 1639,
            "cityID": 167,
            "divisionCode": "411622",
            "districtName": "西华县"
          },
          {
            "districtID": 1640,
            "cityID": 167,
            "divisionCode": "411623",
            "districtName": "商水县"
          },
          {
            "districtID": 1641,
            "cityID": 167,
            "divisionCode": "411624",
            "districtName": "沈丘县"
          },
          {
            "districtID": 1642,
            "cityID": 167,
            "divisionCode": "411625",
            "districtName": "郸城县"
          },
          {
            "districtID": 1643,
            "cityID": 167,
            "divisionCode": "411626",
            "districtName": "淮阳县"
          },
          {
            "districtID": 1644,
            "cityID": 167,
            "divisionCode": "411627",
            "districtName": "太康县"
          },
          {
            "districtID": 1645,
            "cityID": 167,
            "divisionCode": "411628",
            "districtName": "鹿邑县"
          },
          {
            "districtID": 1646,
            "cityID": 167,
            "divisionCode": "411681",
            "districtName": "项城市"
          },
          {
            "districtID": 1647,
            "cityID": 168,
            "divisionCode": "411701",
            "districtName": "市辖区"
          },
          {
            "districtID": 1648,
            "cityID": 168,
            "divisionCode": "411702",
            "districtName": "驿城区"
          },
          {
            "districtID": 1649,
            "cityID": 168,
            "divisionCode": "411721",
            "districtName": "西平县"
          },
          {
            "districtID": 1650,
            "cityID": 168,
            "divisionCode": "411722",
            "districtName": "上蔡县"
          },
          {
            "districtID": 1651,
            "cityID": 168,
            "divisionCode": "411723",
            "districtName": "平舆县"
          },
          {
            "districtID": 1652,
            "cityID": 168,
            "divisionCode": "411724",
            "districtName": "正阳县"
          },
          {
            "districtID": 1653,
            "cityID": 168,
            "divisionCode": "411725",
            "districtName": "确山县"
          },
          {
            "districtID": 1654,
            "cityID": 168,
            "divisionCode": "411726",
            "districtName": "泌阳县"
          },
          {
            "districtID": 1655,
            "cityID": 168,
            "divisionCode": "411727",
            "districtName": "汝南县"
          },
          {
            "districtID": 1656,
            "cityID": 168,
            "divisionCode": "411728",
            "districtName": "遂平县"
          },
          {
            "districtID": 1657,
            "cityID": 168,
            "divisionCode": "411729",
            "districtName": "新蔡县"
          },
          {
            "districtID": 1658,
            "cityID": 169,
            "divisionCode": "419001",
            "districtName": "济源市"
          },
          {
            "districtID": 1659,
            "cityID": 170,
            "divisionCode": "420101",
            "districtName": "市辖区"
          },
          {
            "districtID": 1660,
            "cityID": 170,
            "divisionCode": "420102",
            "districtName": "江岸区"
          },
          {
            "districtID": 1661,
            "cityID": 170,
            "divisionCode": "420103",
            "districtName": "江汉区"
          },
          {
            "districtID": 1662,
            "cityID": 170,
            "divisionCode": "420104",
            "districtName": "硚口区"
          },
          {
            "districtID": 1663,
            "cityID": 170,
            "divisionCode": "420105",
            "districtName": "汉阳区"
          },
          {
            "districtID": 1664,
            "cityID": 170,
            "divisionCode": "420106",
            "districtName": "武昌区"
          },
          {
            "districtID": 1665,
            "cityID": 170,
            "divisionCode": "420107",
            "districtName": "青山区"
          },
          {
            "districtID": 1666,
            "cityID": 170,
            "divisionCode": "420111",
            "districtName": "洪山区"
          },
          {
            "districtID": 1667,
            "cityID": 170,
            "divisionCode": "420112",
            "districtName": "东西湖区"
          },
          {
            "districtID": 1668,
            "cityID": 170,
            "divisionCode": "420113",
            "districtName": "汉南区"
          },
          {
            "districtID": 1669,
            "cityID": 170,
            "divisionCode": "420114",
            "districtName": "蔡甸区"
          },
          {
            "districtID": 1670,
            "cityID": 170,
            "divisionCode": "420115",
            "districtName": "江夏区"
          },
          {
            "districtID": 1671,
            "cityID": 170,
            "divisionCode": "420116",
            "districtName": "黄陂区"
          },
          {
            "districtID": 1672,
            "cityID": 170,
            "divisionCode": "420117",
            "districtName": "新洲区"
          },
          {
            "districtID": 1673,
            "cityID": 171,
            "divisionCode": "420201",
            "districtName": "市辖区"
          },
          {
            "districtID": 1674,
            "cityID": 171,
            "divisionCode": "420202",
            "districtName": "黄石港区"
          },
          {
            "districtID": 1675,
            "cityID": 171,
            "divisionCode": "420203",
            "districtName": "西塞山区"
          },
          {
            "districtID": 1676,
            "cityID": 171,
            "divisionCode": "420204",
            "districtName": "下陆区"
          },
          {
            "districtID": 1677,
            "cityID": 171,
            "divisionCode": "420205",
            "districtName": "铁山区"
          },
          {
            "districtID": 1678,
            "cityID": 171,
            "divisionCode": "420222",
            "districtName": "阳新县"
          },
          {
            "districtID": 1679,
            "cityID": 171,
            "divisionCode": "420281",
            "districtName": "大冶市"
          },
          {
            "districtID": 1680,
            "cityID": 172,
            "divisionCode": "420301",
            "districtName": "市辖区"
          },
          {
            "districtID": 1681,
            "cityID": 172,
            "divisionCode": "420302",
            "districtName": "茅箭区"
          },
          {
            "districtID": 1682,
            "cityID": 172,
            "divisionCode": "420303",
            "districtName": "张湾区"
          },
          {
            "districtID": 1683,
            "cityID": 172,
            "divisionCode": "420304",
            "districtName": "郧阳区"
          },
          {
            "districtID": 1684,
            "cityID": 172,
            "divisionCode": "420322",
            "districtName": "郧西县"
          },
          {
            "districtID": 1685,
            "cityID": 172,
            "divisionCode": "420323",
            "districtName": "竹山县"
          },
          {
            "districtID": 1686,
            "cityID": 172,
            "divisionCode": "420324",
            "districtName": "竹溪县"
          },
          {
            "districtID": 1687,
            "cityID": 172,
            "divisionCode": "420325",
            "districtName": "房县"
          },
          {
            "districtID": 1688,
            "cityID": 172,
            "divisionCode": "420381",
            "districtName": "丹江口市"
          },
          {
            "districtID": 1689,
            "cityID": 173,
            "divisionCode": "420501",
            "districtName": "市辖区"
          },
          {
            "districtID": 1690,
            "cityID": 173,
            "divisionCode": "420502",
            "districtName": "西陵区"
          },
          {
            "districtID": 1691,
            "cityID": 173,
            "divisionCode": "420503",
            "districtName": "伍家岗区"
          },
          {
            "districtID": 1692,
            "cityID": 173,
            "divisionCode": "420504",
            "districtName": "点军区"
          },
          {
            "districtID": 1693,
            "cityID": 173,
            "divisionCode": "420505",
            "districtName": "猇亭区"
          },
          {
            "districtID": 1694,
            "cityID": 173,
            "divisionCode": "420506",
            "districtName": "夷陵区"
          },
          {
            "districtID": 1695,
            "cityID": 173,
            "divisionCode": "420525",
            "districtName": "远安县"
          },
          {
            "districtID": 1696,
            "cityID": 173,
            "divisionCode": "420526",
            "districtName": "兴山县"
          },
          {
            "districtID": 1697,
            "cityID": 173,
            "divisionCode": "420527",
            "districtName": "秭归县"
          },
          {
            "districtID": 1698,
            "cityID": 173,
            "divisionCode": "420528",
            "districtName": "长阳土家族自治县"
          },
          {
            "districtID": 1699,
            "cityID": 173,
            "divisionCode": "420529",
            "districtName": "五峰土家族自治县"
          },
          {
            "districtID": 1700,
            "cityID": 173,
            "divisionCode": "420581",
            "districtName": "宜都市"
          },
          {
            "districtID": 1701,
            "cityID": 173,
            "divisionCode": "420582",
            "districtName": "当阳市"
          },
          {
            "districtID": 1702,
            "cityID": 173,
            "divisionCode": "420583",
            "districtName": "枝江市"
          },
          {
            "districtID": 1703,
            "cityID": 174,
            "divisionCode": "420601",
            "districtName": "市辖区"
          },
          {
            "districtID": 1704,
            "cityID": 174,
            "divisionCode": "420602",
            "districtName": "襄城区"
          },
          {
            "districtID": 1705,
            "cityID": 174,
            "divisionCode": "420606",
            "districtName": "樊城区"
          },
          {
            "districtID": 1706,
            "cityID": 174,
            "divisionCode": "420607",
            "districtName": "襄州区"
          },
          {
            "districtID": 1707,
            "cityID": 174,
            "divisionCode": "420624",
            "districtName": "南漳县"
          },
          {
            "districtID": 1708,
            "cityID": 174,
            "divisionCode": "420625",
            "districtName": "谷城县"
          },
          {
            "districtID": 1709,
            "cityID": 174,
            "divisionCode": "420626",
            "districtName": "保康县"
          },
          {
            "districtID": 1710,
            "cityID": 174,
            "divisionCode": "420682",
            "districtName": "老河口市"
          },
          {
            "districtID": 1711,
            "cityID": 174,
            "divisionCode": "420683",
            "districtName": "枣阳市"
          },
          {
            "districtID": 1712,
            "cityID": 174,
            "divisionCode": "420684",
            "districtName": "宜城市"
          },
          {
            "districtID": 1713,
            "cityID": 175,
            "divisionCode": "420701",
            "districtName": "市辖区"
          },
          {
            "districtID": 1714,
            "cityID": 175,
            "divisionCode": "420702",
            "districtName": "梁子湖区"
          },
          {
            "districtID": 1715,
            "cityID": 175,
            "divisionCode": "420703",
            "districtName": "华容区"
          },
          {
            "districtID": 1716,
            "cityID": 175,
            "divisionCode": "420704",
            "districtName": "鄂城区"
          },
          {
            "districtID": 1717,
            "cityID": 176,
            "divisionCode": "420801",
            "districtName": "市辖区"
          },
          {
            "districtID": 1718,
            "cityID": 176,
            "divisionCode": "420802",
            "districtName": "东宝区"
          },
          {
            "districtID": 1719,
            "cityID": 176,
            "divisionCode": "420804",
            "districtName": "掇刀区"
          },
          {
            "districtID": 1720,
            "cityID": 176,
            "divisionCode": "420821",
            "districtName": "京山县"
          },
          {
            "districtID": 1721,
            "cityID": 176,
            "divisionCode": "420822",
            "districtName": "沙洋县"
          },
          {
            "districtID": 1722,
            "cityID": 176,
            "divisionCode": "420881",
            "districtName": "钟祥市"
          },
          {
            "districtID": 1723,
            "cityID": 177,
            "divisionCode": "420901",
            "districtName": "市辖区"
          },
          {
            "districtID": 1724,
            "cityID": 177,
            "divisionCode": "420902",
            "districtName": "孝南区"
          },
          {
            "districtID": 1725,
            "cityID": 177,
            "divisionCode": "420921",
            "districtName": "孝昌县"
          },
          {
            "districtID": 1726,
            "cityID": 177,
            "divisionCode": "420922",
            "districtName": "大悟县"
          },
          {
            "districtID": 1727,
            "cityID": 177,
            "divisionCode": "420923",
            "districtName": "云梦县"
          },
          {
            "districtID": 1728,
            "cityID": 177,
            "divisionCode": "420981",
            "districtName": "应城市"
          },
          {
            "districtID": 1729,
            "cityID": 177,
            "divisionCode": "420982",
            "districtName": "安陆市"
          },
          {
            "districtID": 1730,
            "cityID": 177,
            "divisionCode": "420984",
            "districtName": "汉川市"
          },
          {
            "districtID": 1731,
            "cityID": 178,
            "divisionCode": "421001",
            "districtName": "市辖区"
          },
          {
            "districtID": 1732,
            "cityID": 178,
            "divisionCode": "421002",
            "districtName": "沙市区"
          },
          {
            "districtID": 1733,
            "cityID": 178,
            "divisionCode": "421003",
            "districtName": "荆州区"
          },
          {
            "districtID": 1734,
            "cityID": 178,
            "divisionCode": "421022",
            "districtName": "公安县"
          },
          {
            "districtID": 1735,
            "cityID": 178,
            "divisionCode": "421023",
            "districtName": "监利县"
          },
          {
            "districtID": 1736,
            "cityID": 178,
            "divisionCode": "421024",
            "districtName": "江陵县"
          },
          {
            "districtID": 1737,
            "cityID": 178,
            "divisionCode": "421081",
            "districtName": "石首市"
          },
          {
            "districtID": 1738,
            "cityID": 178,
            "divisionCode": "421083",
            "districtName": "洪湖市"
          },
          {
            "districtID": 1739,
            "cityID": 178,
            "divisionCode": "421087",
            "districtName": "松滋市"
          },
          {
            "districtID": 1740,
            "cityID": 179,
            "divisionCode": "421101",
            "districtName": "市辖区"
          },
          {
            "districtID": 1741,
            "cityID": 179,
            "divisionCode": "421102",
            "districtName": "黄州区"
          },
          {
            "districtID": 1742,
            "cityID": 179,
            "divisionCode": "421121",
            "districtName": "团风县"
          },
          {
            "districtID": 1743,
            "cityID": 179,
            "divisionCode": "421122",
            "districtName": "红安县"
          },
          {
            "districtID": 1744,
            "cityID": 179,
            "divisionCode": "421123",
            "districtName": "罗田县"
          },
          {
            "districtID": 1745,
            "cityID": 179,
            "divisionCode": "421124",
            "districtName": "英山县"
          },
          {
            "districtID": 1746,
            "cityID": 179,
            "divisionCode": "421125",
            "districtName": "浠水县"
          },
          {
            "districtID": 1747,
            "cityID": 179,
            "divisionCode": "421126",
            "districtName": "蕲春县"
          },
          {
            "districtID": 1748,
            "cityID": 179,
            "divisionCode": "421127",
            "districtName": "黄梅县"
          },
          {
            "districtID": 1749,
            "cityID": 179,
            "divisionCode": "421181",
            "districtName": "麻城市"
          },
          {
            "districtID": 1750,
            "cityID": 179,
            "divisionCode": "421182",
            "districtName": "武穴市"
          },
          {
            "districtID": 1751,
            "cityID": 180,
            "divisionCode": "421201",
            "districtName": "市辖区"
          },
          {
            "districtID": 1752,
            "cityID": 180,
            "divisionCode": "421202",
            "districtName": "咸安区"
          },
          {
            "districtID": 1753,
            "cityID": 180,
            "divisionCode": "421221",
            "districtName": "嘉鱼县"
          },
          {
            "districtID": 1754,
            "cityID": 180,
            "divisionCode": "421222",
            "districtName": "通城县"
          },
          {
            "districtID": 1755,
            "cityID": 180,
            "divisionCode": "421223",
            "districtName": "崇阳县"
          },
          {
            "districtID": 1756,
            "cityID": 180,
            "divisionCode": "421224",
            "districtName": "通山县"
          },
          {
            "districtID": 1757,
            "cityID": 180,
            "divisionCode": "421281",
            "districtName": "赤壁市"
          },
          {
            "districtID": 1758,
            "cityID": 181,
            "divisionCode": "421301",
            "districtName": "市辖区"
          },
          {
            "districtID": 1759,
            "cityID": 181,
            "divisionCode": "421303",
            "districtName": "曾都区"
          },
          {
            "districtID": 1760,
            "cityID": 181,
            "divisionCode": "421321",
            "districtName": "随县"
          },
          {
            "districtID": 1761,
            "cityID": 181,
            "divisionCode": "421381",
            "districtName": "广水市"
          },
          {
            "districtID": 1762,
            "cityID": 182,
            "divisionCode": "422801",
            "districtName": "恩施市"
          },
          {
            "districtID": 1763,
            "cityID": 182,
            "divisionCode": "422802",
            "districtName": "利川市"
          },
          {
            "districtID": 1764,
            "cityID": 182,
            "divisionCode": "422822",
            "districtName": "建始县"
          },
          {
            "districtID": 1765,
            "cityID": 182,
            "divisionCode": "422823",
            "districtName": "巴东县"
          },
          {
            "districtID": 1766,
            "cityID": 182,
            "divisionCode": "422825",
            "districtName": "宣恩县"
          },
          {
            "districtID": 1767,
            "cityID": 182,
            "divisionCode": "422826",
            "districtName": "咸丰县"
          },
          {
            "districtID": 1768,
            "cityID": 182,
            "divisionCode": "422827",
            "districtName": "来凤县"
          },
          {
            "districtID": 1769,
            "cityID": 182,
            "divisionCode": "422828",
            "districtName": "鹤峰县"
          },
          {
            "districtID": 1770,
            "cityID": 183,
            "divisionCode": "429004",
            "districtName": "仙桃市"
          },
          {
            "districtID": 1771,
            "cityID": 183,
            "divisionCode": "429005",
            "districtName": "潜江市"
          },
          {
            "districtID": 1772,
            "cityID": 183,
            "divisionCode": "429006",
            "districtName": "天门市"
          },
          {
            "districtID": 1773,
            "cityID": 183,
            "divisionCode": "429021",
            "districtName": "神农架林区"
          },
          {
            "districtID": 1774,
            "cityID": 184,
            "divisionCode": "430101",
            "districtName": "市辖区"
          },
          {
            "districtID": 1775,
            "cityID": 184,
            "divisionCode": "430102",
            "districtName": "芙蓉区"
          },
          {
            "districtID": 1776,
            "cityID": 184,
            "divisionCode": "430103",
            "districtName": "天心区"
          },
          {
            "districtID": 1777,
            "cityID": 184,
            "divisionCode": "430104",
            "districtName": "岳麓区"
          },
          {
            "districtID": 1778,
            "cityID": 184,
            "divisionCode": "430105",
            "districtName": "开福区"
          },
          {
            "districtID": 1779,
            "cityID": 184,
            "divisionCode": "430111",
            "districtName": "雨花区"
          },
          {
            "districtID": 1780,
            "cityID": 184,
            "divisionCode": "430112",
            "districtName": "望城区"
          },
          {
            "districtID": 1781,
            "cityID": 184,
            "divisionCode": "430121",
            "districtName": "长沙县"
          },
          {
            "districtID": 1782,
            "cityID": 184,
            "divisionCode": "430124",
            "districtName": "宁乡县"
          },
          {
            "districtID": 1783,
            "cityID": 184,
            "divisionCode": "430181",
            "districtName": "浏阳市"
          },
          {
            "districtID": 1784,
            "cityID": 185,
            "divisionCode": "430201",
            "districtName": "市辖区"
          },
          {
            "districtID": 1785,
            "cityID": 185,
            "divisionCode": "430202",
            "districtName": "荷塘区"
          },
          {
            "districtID": 1786,
            "cityID": 185,
            "divisionCode": "430203",
            "districtName": "芦淞区"
          },
          {
            "districtID": 1787,
            "cityID": 185,
            "divisionCode": "430204",
            "districtName": "石峰区"
          },
          {
            "districtID": 1788,
            "cityID": 185,
            "divisionCode": "430211",
            "districtName": "天元区"
          },
          {
            "districtID": 1789,
            "cityID": 185,
            "divisionCode": "430221",
            "districtName": "株洲县"
          },
          {
            "districtID": 1790,
            "cityID": 185,
            "divisionCode": "430223",
            "districtName": "攸县"
          },
          {
            "districtID": 1791,
            "cityID": 185,
            "divisionCode": "430224",
            "districtName": "茶陵县"
          },
          {
            "districtID": 1792,
            "cityID": 185,
            "divisionCode": "430225",
            "districtName": "炎陵县"
          },
          {
            "districtID": 1793,
            "cityID": 185,
            "divisionCode": "430281",
            "districtName": "醴陵市"
          },
          {
            "districtID": 1794,
            "cityID": 186,
            "divisionCode": "430301",
            "districtName": "市辖区"
          },
          {
            "districtID": 1795,
            "cityID": 186,
            "divisionCode": "430302",
            "districtName": "雨湖区"
          },
          {
            "districtID": 1796,
            "cityID": 186,
            "divisionCode": "430304",
            "districtName": "岳塘区"
          },
          {
            "districtID": 1797,
            "cityID": 186,
            "divisionCode": "430321",
            "districtName": "湘潭县"
          },
          {
            "districtID": 1798,
            "cityID": 186,
            "divisionCode": "430381",
            "districtName": "湘乡市"
          },
          {
            "districtID": 1799,
            "cityID": 186,
            "divisionCode": "430382",
            "districtName": "韶山市"
          },
          {
            "districtID": 1800,
            "cityID": 187,
            "divisionCode": "430401",
            "districtName": "市辖区"
          },
          {
            "districtID": 1801,
            "cityID": 187,
            "divisionCode": "430405",
            "districtName": "珠晖区"
          },
          {
            "districtID": 1802,
            "cityID": 187,
            "divisionCode": "430406",
            "districtName": "雁峰区"
          },
          {
            "districtID": 1803,
            "cityID": 187,
            "divisionCode": "430407",
            "districtName": "石鼓区"
          },
          {
            "districtID": 1804,
            "cityID": 187,
            "divisionCode": "430408",
            "districtName": "蒸湘区"
          },
          {
            "districtID": 1805,
            "cityID": 187,
            "divisionCode": "430412",
            "districtName": "南岳区"
          },
          {
            "districtID": 1806,
            "cityID": 187,
            "divisionCode": "430421",
            "districtName": "衡阳县"
          },
          {
            "districtID": 1807,
            "cityID": 187,
            "divisionCode": "430422",
            "districtName": "衡南县"
          },
          {
            "districtID": 1808,
            "cityID": 187,
            "divisionCode": "430423",
            "districtName": "衡山县"
          },
          {
            "districtID": 1809,
            "cityID": 187,
            "divisionCode": "430424",
            "districtName": "衡东县"
          },
          {
            "districtID": 1810,
            "cityID": 187,
            "divisionCode": "430426",
            "districtName": "祁东县"
          },
          {
            "districtID": 1811,
            "cityID": 187,
            "divisionCode": "430481",
            "districtName": "耒阳市"
          },
          {
            "districtID": 1812,
            "cityID": 187,
            "divisionCode": "430482",
            "districtName": "常宁市"
          },
          {
            "districtID": 1813,
            "cityID": 188,
            "divisionCode": "430501",
            "districtName": "市辖区"
          },
          {
            "districtID": 1814,
            "cityID": 188,
            "divisionCode": "430502",
            "districtName": "双清区"
          },
          {
            "districtID": 1815,
            "cityID": 188,
            "divisionCode": "430503",
            "districtName": "大祥区"
          },
          {
            "districtID": 1816,
            "cityID": 188,
            "divisionCode": "430511",
            "districtName": "北塔区"
          },
          {
            "districtID": 1817,
            "cityID": 188,
            "divisionCode": "430521",
            "districtName": "邵东县"
          },
          {
            "districtID": 1818,
            "cityID": 188,
            "divisionCode": "430522",
            "districtName": "新邵县"
          },
          {
            "districtID": 1819,
            "cityID": 188,
            "divisionCode": "430523",
            "districtName": "邵阳县"
          },
          {
            "districtID": 1820,
            "cityID": 188,
            "divisionCode": "430524",
            "districtName": "隆回县"
          },
          {
            "districtID": 1821,
            "cityID": 188,
            "divisionCode": "430525",
            "districtName": "洞口县"
          },
          {
            "districtID": 1822,
            "cityID": 188,
            "divisionCode": "430527",
            "districtName": "绥宁县"
          },
          {
            "districtID": 1823,
            "cityID": 188,
            "divisionCode": "430528",
            "districtName": "新宁县"
          },
          {
            "districtID": 1824,
            "cityID": 188,
            "divisionCode": "430529",
            "districtName": "城步苗族自治县"
          },
          {
            "districtID": 1825,
            "cityID": 188,
            "divisionCode": "430581",
            "districtName": "武冈市"
          },
          {
            "districtID": 1826,
            "cityID": 189,
            "divisionCode": "430601",
            "districtName": "市辖区"
          },
          {
            "districtID": 1827,
            "cityID": 189,
            "divisionCode": "430602",
            "districtName": "岳阳楼区"
          },
          {
            "districtID": 1828,
            "cityID": 189,
            "divisionCode": "430603",
            "districtName": "云溪区"
          },
          {
            "districtID": 1829,
            "cityID": 189,
            "divisionCode": "430611",
            "districtName": "君山区"
          },
          {
            "districtID": 1830,
            "cityID": 189,
            "divisionCode": "430621",
            "districtName": "岳阳县"
          },
          {
            "districtID": 1831,
            "cityID": 189,
            "divisionCode": "430623",
            "districtName": "华容县"
          },
          {
            "districtID": 1832,
            "cityID": 189,
            "divisionCode": "430624",
            "districtName": "湘阴县"
          },
          {
            "districtID": 1833,
            "cityID": 189,
            "divisionCode": "430626",
            "districtName": "平江县"
          },
          {
            "districtID": 1834,
            "cityID": 189,
            "divisionCode": "430681",
            "districtName": "汨罗市"
          },
          {
            "districtID": 1835,
            "cityID": 189,
            "divisionCode": "430682",
            "districtName": "临湘市"
          },
          {
            "districtID": 1836,
            "cityID": 190,
            "divisionCode": "430701",
            "districtName": "市辖区"
          },
          {
            "districtID": 1837,
            "cityID": 190,
            "divisionCode": "430702",
            "districtName": "武陵区"
          },
          {
            "districtID": 1838,
            "cityID": 190,
            "divisionCode": "430703",
            "districtName": "鼎城区"
          },
          {
            "districtID": 1839,
            "cityID": 190,
            "divisionCode": "430721",
            "districtName": "安乡县"
          },
          {
            "districtID": 1840,
            "cityID": 190,
            "divisionCode": "430722",
            "districtName": "汉寿县"
          },
          {
            "districtID": 1841,
            "cityID": 190,
            "divisionCode": "430723",
            "districtName": "澧县"
          },
          {
            "districtID": 1842,
            "cityID": 190,
            "divisionCode": "430724",
            "districtName": "临澧县"
          },
          {
            "districtID": 1843,
            "cityID": 190,
            "divisionCode": "430725",
            "districtName": "桃源县"
          },
          {
            "districtID": 1844,
            "cityID": 190,
            "divisionCode": "430726",
            "districtName": "石门县"
          },
          {
            "districtID": 1845,
            "cityID": 190,
            "divisionCode": "430781",
            "districtName": "津市市"
          },
          {
            "districtID": 1846,
            "cityID": 191,
            "divisionCode": "430801",
            "districtName": "市辖区"
          },
          {
            "districtID": 1847,
            "cityID": 191,
            "divisionCode": "430802",
            "districtName": "永定区"
          },
          {
            "districtID": 1848,
            "cityID": 191,
            "divisionCode": "430811",
            "districtName": "武陵源区"
          },
          {
            "districtID": 1849,
            "cityID": 191,
            "divisionCode": "430821",
            "districtName": "慈利县"
          },
          {
            "districtID": 1850,
            "cityID": 191,
            "divisionCode": "430822",
            "districtName": "桑植县"
          },
          {
            "districtID": 1851,
            "cityID": 192,
            "divisionCode": "430901",
            "districtName": "市辖区"
          },
          {
            "districtID": 1852,
            "cityID": 192,
            "divisionCode": "430902",
            "districtName": "资阳区"
          },
          {
            "districtID": 1853,
            "cityID": 192,
            "divisionCode": "430903",
            "districtName": "赫山区"
          },
          {
            "districtID": 1854,
            "cityID": 192,
            "divisionCode": "430921",
            "districtName": "南县"
          },
          {
            "districtID": 1855,
            "cityID": 192,
            "divisionCode": "430922",
            "districtName": "桃江县"
          },
          {
            "districtID": 1856,
            "cityID": 192,
            "divisionCode": "430923",
            "districtName": "安化县"
          },
          {
            "districtID": 1857,
            "cityID": 192,
            "divisionCode": "430981",
            "districtName": "沅江市"
          },
          {
            "districtID": 1858,
            "cityID": 193,
            "divisionCode": "431001",
            "districtName": "市辖区"
          },
          {
            "districtID": 1859,
            "cityID": 193,
            "divisionCode": "431002",
            "districtName": "北湖区"
          },
          {
            "districtID": 1860,
            "cityID": 193,
            "divisionCode": "431003",
            "districtName": "苏仙区"
          },
          {
            "districtID": 1861,
            "cityID": 193,
            "divisionCode": "431021",
            "districtName": "桂阳县"
          },
          {
            "districtID": 1862,
            "cityID": 193,
            "divisionCode": "431022",
            "districtName": "宜章县"
          },
          {
            "districtID": 1863,
            "cityID": 193,
            "divisionCode": "431023",
            "districtName": "永兴县"
          },
          {
            "districtID": 1864,
            "cityID": 193,
            "divisionCode": "431024",
            "districtName": "嘉禾县"
          },
          {
            "districtID": 1865,
            "cityID": 193,
            "divisionCode": "431025",
            "districtName": "临武县"
          },
          {
            "districtID": 1866,
            "cityID": 193,
            "divisionCode": "431026",
            "districtName": "汝城县"
          },
          {
            "districtID": 1867,
            "cityID": 193,
            "divisionCode": "431027",
            "districtName": "桂东县"
          },
          {
            "districtID": 1868,
            "cityID": 193,
            "divisionCode": "431028",
            "districtName": "安仁县"
          },
          {
            "districtID": 1869,
            "cityID": 193,
            "divisionCode": "431081",
            "districtName": "资兴市"
          },
          {
            "districtID": 1870,
            "cityID": 194,
            "divisionCode": "431101",
            "districtName": "市辖区"
          },
          {
            "districtID": 1871,
            "cityID": 194,
            "divisionCode": "431102",
            "districtName": "零陵区"
          },
          {
            "districtID": 1872,
            "cityID": 194,
            "divisionCode": "431103",
            "districtName": "冷水滩区"
          },
          {
            "districtID": 1873,
            "cityID": 194,
            "divisionCode": "431121",
            "districtName": "祁阳县"
          },
          {
            "districtID": 1874,
            "cityID": 194,
            "divisionCode": "431122",
            "districtName": "东安县"
          },
          {
            "districtID": 1875,
            "cityID": 194,
            "divisionCode": "431123",
            "districtName": "双牌县"
          },
          {
            "districtID": 1876,
            "cityID": 194,
            "divisionCode": "431124",
            "districtName": "道县"
          },
          {
            "districtID": 1877,
            "cityID": 194,
            "divisionCode": "431125",
            "districtName": "江永县"
          },
          {
            "districtID": 1878,
            "cityID": 194,
            "divisionCode": "431126",
            "districtName": "宁远县"
          },
          {
            "districtID": 1879,
            "cityID": 194,
            "divisionCode": "431127",
            "districtName": "蓝山县"
          },
          {
            "districtID": 1880,
            "cityID": 194,
            "divisionCode": "431128",
            "districtName": "新田县"
          },
          {
            "districtID": 1881,
            "cityID": 194,
            "divisionCode": "431129",
            "districtName": "江华瑶族自治县"
          },
          {
            "districtID": 1882,
            "cityID": 195,
            "divisionCode": "431201",
            "districtName": "市辖区"
          },
          {
            "districtID": 1883,
            "cityID": 195,
            "divisionCode": "431202",
            "districtName": "鹤城区"
          },
          {
            "districtID": 1884,
            "cityID": 195,
            "divisionCode": "431221",
            "districtName": "中方县"
          },
          {
            "districtID": 1885,
            "cityID": 195,
            "divisionCode": "431222",
            "districtName": "沅陵县"
          },
          {
            "districtID": 1886,
            "cityID": 195,
            "divisionCode": "431223",
            "districtName": "辰溪县"
          },
          {
            "districtID": 1887,
            "cityID": 195,
            "divisionCode": "431224",
            "districtName": "溆浦县"
          },
          {
            "districtID": 1888,
            "cityID": 195,
            "divisionCode": "431225",
            "districtName": "会同县"
          },
          {
            "districtID": 1889,
            "cityID": 195,
            "divisionCode": "431226",
            "districtName": "麻阳苗族自治县"
          },
          {
            "districtID": 1890,
            "cityID": 195,
            "divisionCode": "431227",
            "districtName": "新晃侗族自治县"
          },
          {
            "districtID": 1891,
            "cityID": 195,
            "divisionCode": "431228",
            "districtName": "芷江侗族自治县"
          },
          {
            "districtID": 1892,
            "cityID": 195,
            "divisionCode": "431229",
            "districtName": "靖州苗族侗族自治县"
          },
          {
            "districtID": 1893,
            "cityID": 195,
            "divisionCode": "431230",
            "districtName": "通道侗族自治县"
          },
          {
            "districtID": 1894,
            "cityID": 195,
            "divisionCode": "431281",
            "districtName": "洪江市"
          },
          {
            "districtID": 1895,
            "cityID": 196,
            "divisionCode": "431301",
            "districtName": "市辖区"
          },
          {
            "districtID": 1896,
            "cityID": 196,
            "divisionCode": "431302",
            "districtName": "娄星区"
          },
          {
            "districtID": 1897,
            "cityID": 196,
            "divisionCode": "431321",
            "districtName": "双峰县"
          },
          {
            "districtID": 1898,
            "cityID": 196,
            "divisionCode": "431322",
            "districtName": "新化县"
          },
          {
            "districtID": 1899,
            "cityID": 196,
            "divisionCode": "431381",
            "districtName": "冷水江市"
          },
          {
            "districtID": 1900,
            "cityID": 196,
            "divisionCode": "431382",
            "districtName": "涟源市"
          },
          {
            "districtID": 1901,
            "cityID": 197,
            "divisionCode": "433101",
            "districtName": "吉首市"
          },
          {
            "districtID": 1902,
            "cityID": 197,
            "divisionCode": "433122",
            "districtName": "泸溪县"
          },
          {
            "districtID": 1903,
            "cityID": 197,
            "divisionCode": "433123",
            "districtName": "凤凰县"
          },
          {
            "districtID": 1904,
            "cityID": 197,
            "divisionCode": "433124",
            "districtName": "花垣县"
          },
          {
            "districtID": 1905,
            "cityID": 197,
            "divisionCode": "433125",
            "districtName": "保靖县"
          },
          {
            "districtID": 1906,
            "cityID": 197,
            "divisionCode": "433126",
            "districtName": "古丈县"
          },
          {
            "districtID": 1907,
            "cityID": 197,
            "divisionCode": "433127",
            "districtName": "永顺县"
          },
          {
            "districtID": 1908,
            "cityID": 197,
            "divisionCode": "433130",
            "districtName": "龙山县"
          },
          {
            "districtID": 1909,
            "cityID": 198,
            "divisionCode": "440101",
            "districtName": "市辖区"
          },
          {
            "districtID": 1910,
            "cityID": 198,
            "divisionCode": "440103",
            "districtName": "荔湾区"
          },
          {
            "districtID": 1911,
            "cityID": 198,
            "divisionCode": "440104",
            "districtName": "越秀区"
          },
          {
            "districtID": 1912,
            "cityID": 198,
            "divisionCode": "440105",
            "districtName": "海珠区"
          },
          {
            "districtID": 1913,
            "cityID": 198,
            "divisionCode": "440106",
            "districtName": "天河区"
          },
          {
            "districtID": 1914,
            "cityID": 198,
            "divisionCode": "440111",
            "districtName": "白云区"
          },
          {
            "districtID": 1915,
            "cityID": 198,
            "divisionCode": "440112",
            "districtName": "黄埔区"
          },
          {
            "districtID": 1916,
            "cityID": 198,
            "divisionCode": "440113",
            "districtName": "番禺区"
          },
          {
            "districtID": 1917,
            "cityID": 198,
            "divisionCode": "440114",
            "districtName": "花都区"
          },
          {
            "districtID": 1918,
            "cityID": 198,
            "divisionCode": "440115",
            "districtName": "南沙区"
          },
          {
            "districtID": 1919,
            "cityID": 198,
            "divisionCode": "440117",
            "districtName": "从化区"
          },
          {
            "districtID": 1920,
            "cityID": 198,
            "divisionCode": "440118",
            "districtName": "增城区"
          },
          {
            "districtID": 1921,
            "cityID": 199,
            "divisionCode": "440201",
            "districtName": "市辖区"
          },
          {
            "districtID": 1922,
            "cityID": 199,
            "divisionCode": "440203",
            "districtName": "武江区"
          },
          {
            "districtID": 1923,
            "cityID": 199,
            "divisionCode": "440204",
            "districtName": "浈江区"
          },
          {
            "districtID": 1924,
            "cityID": 199,
            "divisionCode": "440205",
            "districtName": "曲江区"
          },
          {
            "districtID": 1925,
            "cityID": 199,
            "divisionCode": "440222",
            "districtName": "始兴县"
          },
          {
            "districtID": 1926,
            "cityID": 199,
            "divisionCode": "440224",
            "districtName": "仁化县"
          },
          {
            "districtID": 1927,
            "cityID": 199,
            "divisionCode": "440229",
            "districtName": "翁源县"
          },
          {
            "districtID": 1928,
            "cityID": 199,
            "divisionCode": "440232",
            "districtName": "乳源瑶族自治县"
          },
          {
            "districtID": 1929,
            "cityID": 199,
            "divisionCode": "440233",
            "districtName": "新丰县"
          },
          {
            "districtID": 1930,
            "cityID": 199,
            "divisionCode": "440281",
            "districtName": "乐昌市"
          },
          {
            "districtID": 1931,
            "cityID": 199,
            "divisionCode": "440282",
            "districtName": "南雄市"
          },
          {
            "districtID": 1932,
            "cityID": 200,
            "divisionCode": "440301",
            "districtName": "市辖区"
          },
          {
            "districtID": 1933,
            "cityID": 200,
            "divisionCode": "440303",
            "districtName": "罗湖区"
          },
          {
            "districtID": 1934,
            "cityID": 200,
            "divisionCode": "440304",
            "districtName": "福田区"
          },
          {
            "districtID": 1935,
            "cityID": 200,
            "divisionCode": "440305",
            "districtName": "南山区"
          },
          {
            "districtID": 1936,
            "cityID": 200,
            "divisionCode": "440306",
            "districtName": "宝安区"
          },
          {
            "districtID": 1937,
            "cityID": 200,
            "divisionCode": "440307",
            "districtName": "龙岗区"
          },
          {
            "districtID": 1938,
            "cityID": 200,
            "divisionCode": "440308",
            "districtName": "盐田区"
          },
          {
            "districtID": 1939,
            "cityID": 201,
            "divisionCode": "440401",
            "districtName": "市辖区"
          },
          {
            "districtID": 1940,
            "cityID": 201,
            "divisionCode": "440402",
            "districtName": "香洲区"
          },
          {
            "districtID": 1941,
            "cityID": 201,
            "divisionCode": "440403",
            "districtName": "斗门区"
          },
          {
            "districtID": 1942,
            "cityID": 201,
            "divisionCode": "440404",
            "districtName": "金湾区"
          },
          {
            "districtID": 1943,
            "cityID": 202,
            "divisionCode": "440501",
            "districtName": "市辖区"
          },
          {
            "districtID": 1944,
            "cityID": 202,
            "divisionCode": "440507",
            "districtName": "龙湖区"
          },
          {
            "districtID": 1945,
            "cityID": 202,
            "divisionCode": "440511",
            "districtName": "金平区"
          },
          {
            "districtID": 1946,
            "cityID": 202,
            "divisionCode": "440512",
            "districtName": "濠江区"
          },
          {
            "districtID": 1947,
            "cityID": 202,
            "divisionCode": "440513",
            "districtName": "潮阳区"
          },
          {
            "districtID": 1948,
            "cityID": 202,
            "divisionCode": "440514",
            "districtName": "潮南区"
          },
          {
            "districtID": 1949,
            "cityID": 202,
            "divisionCode": "440515",
            "districtName": "澄海区"
          },
          {
            "districtID": 1950,
            "cityID": 202,
            "divisionCode": "440523",
            "districtName": "南澳县"
          },
          {
            "districtID": 1951,
            "cityID": 203,
            "divisionCode": "440601",
            "districtName": "市辖区"
          },
          {
            "districtID": 1952,
            "cityID": 203,
            "divisionCode": "440604",
            "districtName": "禅城区"
          },
          {
            "districtID": 1953,
            "cityID": 203,
            "divisionCode": "440605",
            "districtName": "南海区"
          },
          {
            "districtID": 1954,
            "cityID": 203,
            "divisionCode": "440606",
            "districtName": "顺德区"
          },
          {
            "districtID": 1955,
            "cityID": 203,
            "divisionCode": "440607",
            "districtName": "三水区"
          },
          {
            "districtID": 1956,
            "cityID": 203,
            "divisionCode": "440608",
            "districtName": "高明区"
          },
          {
            "districtID": 1957,
            "cityID": 204,
            "divisionCode": "440701",
            "districtName": "市辖区"
          },
          {
            "districtID": 1958,
            "cityID": 204,
            "divisionCode": "440703",
            "districtName": "蓬江区"
          },
          {
            "districtID": 1959,
            "cityID": 204,
            "divisionCode": "440704",
            "districtName": "江海区"
          },
          {
            "districtID": 1960,
            "cityID": 204,
            "divisionCode": "440705",
            "districtName": "新会区"
          },
          {
            "districtID": 1961,
            "cityID": 204,
            "divisionCode": "440781",
            "districtName": "台山市"
          },
          {
            "districtID": 1962,
            "cityID": 204,
            "divisionCode": "440783",
            "districtName": "开平市"
          },
          {
            "districtID": 1963,
            "cityID": 204,
            "divisionCode": "440784",
            "districtName": "鹤山市"
          },
          {
            "districtID": 1964,
            "cityID": 204,
            "divisionCode": "440785",
            "districtName": "恩平市"
          },
          {
            "districtID": 1965,
            "cityID": 205,
            "divisionCode": "440801",
            "districtName": "市辖区"
          },
          {
            "districtID": 1966,
            "cityID": 205,
            "divisionCode": "440802",
            "districtName": "赤坎区"
          },
          {
            "districtID": 1967,
            "cityID": 205,
            "divisionCode": "440803",
            "districtName": "霞山区"
          },
          {
            "districtID": 1968,
            "cityID": 205,
            "divisionCode": "440804",
            "districtName": "坡头区"
          },
          {
            "districtID": 1969,
            "cityID": 205,
            "divisionCode": "440811",
            "districtName": "麻章区"
          },
          {
            "districtID": 1970,
            "cityID": 205,
            "divisionCode": "440823",
            "districtName": "遂溪县"
          },
          {
            "districtID": 1971,
            "cityID": 205,
            "divisionCode": "440825",
            "districtName": "徐闻县"
          },
          {
            "districtID": 1972,
            "cityID": 205,
            "divisionCode": "440881",
            "districtName": "廉江市"
          },
          {
            "districtID": 1973,
            "cityID": 205,
            "divisionCode": "440882",
            "districtName": "雷州市"
          },
          {
            "districtID": 1974,
            "cityID": 205,
            "divisionCode": "440883",
            "districtName": "吴川市"
          },
          {
            "districtID": 1975,
            "cityID": 206,
            "divisionCode": "440901",
            "districtName": "市辖区"
          },
          {
            "districtID": 1976,
            "cityID": 206,
            "divisionCode": "440902",
            "districtName": "茂南区"
          },
          {
            "districtID": 1977,
            "cityID": 206,
            "divisionCode": "440904",
            "districtName": "电白区"
          },
          {
            "districtID": 1978,
            "cityID": 206,
            "divisionCode": "440981",
            "districtName": "高州市"
          },
          {
            "districtID": 1979,
            "cityID": 206,
            "divisionCode": "440982",
            "districtName": "化州市"
          },
          {
            "districtID": 1980,
            "cityID": 206,
            "divisionCode": "440983",
            "districtName": "信宜市"
          },
          {
            "districtID": 1981,
            "cityID": 207,
            "divisionCode": "441201",
            "districtName": "市辖区"
          },
          {
            "districtID": 1982,
            "cityID": 207,
            "divisionCode": "441202",
            "districtName": "端州区"
          },
          {
            "districtID": 1983,
            "cityID": 207,
            "divisionCode": "441203",
            "districtName": "鼎湖区"
          },
          {
            "districtID": 1984,
            "cityID": 207,
            "divisionCode": "441204",
            "districtName": "高要区"
          },
          {
            "districtID": 1985,
            "cityID": 207,
            "divisionCode": "441223",
            "districtName": "广宁县"
          },
          {
            "districtID": 1986,
            "cityID": 207,
            "divisionCode": "441224",
            "districtName": "怀集县"
          },
          {
            "districtID": 1987,
            "cityID": 207,
            "divisionCode": "441225",
            "districtName": "封开县"
          },
          {
            "districtID": 1988,
            "cityID": 207,
            "divisionCode": "441226",
            "districtName": "德庆县"
          },
          {
            "districtID": 1989,
            "cityID": 207,
            "divisionCode": "441284",
            "districtName": "四会市"
          },
          {
            "districtID": 1990,
            "cityID": 208,
            "divisionCode": "441301",
            "districtName": "市辖区"
          },
          {
            "districtID": 1991,
            "cityID": 208,
            "divisionCode": "441302",
            "districtName": "惠城区"
          },
          {
            "districtID": 1992,
            "cityID": 208,
            "divisionCode": "441303",
            "districtName": "惠阳区"
          },
          {
            "districtID": 1993,
            "cityID": 208,
            "divisionCode": "441322",
            "districtName": "博罗县"
          },
          {
            "districtID": 1994,
            "cityID": 208,
            "divisionCode": "441323",
            "districtName": "惠东县"
          },
          {
            "districtID": 1995,
            "cityID": 208,
            "divisionCode": "441324",
            "districtName": "龙门县"
          },
          {
            "districtID": 1996,
            "cityID": 209,
            "divisionCode": "441401",
            "districtName": "市辖区"
          },
          {
            "districtID": 1997,
            "cityID": 209,
            "divisionCode": "441402",
            "districtName": "梅江区"
          },
          {
            "districtID": 1998,
            "cityID": 209,
            "divisionCode": "441403",
            "districtName": "梅县区"
          },
          {
            "districtID": 1999,
            "cityID": 209,
            "divisionCode": "441422",
            "districtName": "大埔县"
          },
          {
            "districtID": 2000,
            "cityID": 209,
            "divisionCode": "441423",
            "districtName": "丰顺县"
          },
          {
            "districtID": 2001,
            "cityID": 209,
            "divisionCode": "441424",
            "districtName": "五华县"
          },
          {
            "districtID": 2002,
            "cityID": 209,
            "divisionCode": "441426",
            "districtName": "平远县"
          },
          {
            "districtID": 2003,
            "cityID": 209,
            "divisionCode": "441427",
            "districtName": "蕉岭县"
          },
          {
            "districtID": 2004,
            "cityID": 209,
            "divisionCode": "441481",
            "districtName": "兴宁市"
          },
          {
            "districtID": 2005,
            "cityID": 210,
            "divisionCode": "441501",
            "districtName": "市辖区"
          },
          {
            "districtID": 2006,
            "cityID": 210,
            "divisionCode": "441502",
            "districtName": "城区"
          },
          {
            "districtID": 2007,
            "cityID": 210,
            "divisionCode": "441521",
            "districtName": "海丰县"
          },
          {
            "districtID": 2008,
            "cityID": 210,
            "divisionCode": "441523",
            "districtName": "陆河县"
          },
          {
            "districtID": 2009,
            "cityID": 210,
            "divisionCode": "441581",
            "districtName": "陆丰市"
          },
          {
            "districtID": 2010,
            "cityID": 211,
            "divisionCode": "441601",
            "districtName": "市辖区"
          },
          {
            "districtID": 2011,
            "cityID": 211,
            "divisionCode": "441602",
            "districtName": "源城区"
          },
          {
            "districtID": 2012,
            "cityID": 211,
            "divisionCode": "441621",
            "districtName": "紫金县"
          },
          {
            "districtID": 2013,
            "cityID": 211,
            "divisionCode": "441622",
            "districtName": "龙川县"
          },
          {
            "districtID": 2014,
            "cityID": 211,
            "divisionCode": "441623",
            "districtName": "连平县"
          },
          {
            "districtID": 2015,
            "cityID": 211,
            "divisionCode": "441624",
            "districtName": "和平县"
          },
          {
            "districtID": 2016,
            "cityID": 211,
            "divisionCode": "441625",
            "districtName": "东源县"
          },
          {
            "districtID": 2017,
            "cityID": 212,
            "divisionCode": "441701",
            "districtName": "市辖区"
          },
          {
            "districtID": 2018,
            "cityID": 212,
            "divisionCode": "441702",
            "districtName": "江城区"
          },
          {
            "districtID": 2019,
            "cityID": 212,
            "divisionCode": "441704",
            "districtName": "阳东区"
          },
          {
            "districtID": 2020,
            "cityID": 212,
            "divisionCode": "441721",
            "districtName": "阳西县"
          },
          {
            "districtID": 2021,
            "cityID": 212,
            "divisionCode": "441781",
            "districtName": "阳春市"
          },
          {
            "districtID": 2022,
            "cityID": 213,
            "divisionCode": "441801",
            "districtName": "市辖区"
          },
          {
            "districtID": 2023,
            "cityID": 213,
            "divisionCode": "441802",
            "districtName": "清城区"
          },
          {
            "districtID": 2024,
            "cityID": 213,
            "divisionCode": "441803",
            "districtName": "清新区"
          },
          {
            "districtID": 2025,
            "cityID": 213,
            "divisionCode": "441821",
            "districtName": "佛冈县"
          },
          {
            "districtID": 2026,
            "cityID": 213,
            "divisionCode": "441823",
            "districtName": "阳山县"
          },
          {
            "districtID": 2027,
            "cityID": 213,
            "divisionCode": "441825",
            "districtName": "连山壮族瑶族自治县"
          },
          {
            "districtID": 2028,
            "cityID": 213,
            "divisionCode": "441826",
            "districtName": "连南瑶族自治县"
          },
          {
            "districtID": 2029,
            "cityID": 213,
            "divisionCode": "441881",
            "districtName": "英德市"
          },
          {
            "districtID": 2030,
            "cityID": 213,
            "divisionCode": "441882",
            "districtName": "连州市"
          },
          {
            "districtID": 2031,
            "cityID": 216,
            "divisionCode": "445101",
            "districtName": "市辖区"
          },
          {
            "districtID": 2032,
            "cityID": 216,
            "divisionCode": "445102",
            "districtName": "湘桥区"
          },
          {
            "districtID": 2033,
            "cityID": 216,
            "divisionCode": "445103",
            "districtName": "潮安区"
          },
          {
            "districtID": 2034,
            "cityID": 216,
            "divisionCode": "445122",
            "districtName": "饶平县"
          },
          {
            "districtID": 2035,
            "cityID": 217,
            "divisionCode": "445201",
            "districtName": "市辖区"
          },
          {
            "districtID": 2036,
            "cityID": 217,
            "divisionCode": "445202",
            "districtName": "榕城区"
          },
          {
            "districtID": 2037,
            "cityID": 217,
            "divisionCode": "445203",
            "districtName": "揭东区"
          },
          {
            "districtID": 2038,
            "cityID": 217,
            "divisionCode": "445222",
            "districtName": "揭西县"
          },
          {
            "districtID": 2039,
            "cityID": 217,
            "divisionCode": "445224",
            "districtName": "惠来县"
          },
          {
            "districtID": 2040,
            "cityID": 217,
            "divisionCode": "445281",
            "districtName": "普宁市"
          },
          {
            "districtID": 2041,
            "cityID": 218,
            "divisionCode": "445301",
            "districtName": "市辖区"
          },
          {
            "districtID": 2042,
            "cityID": 218,
            "divisionCode": "445302",
            "districtName": "云城区"
          },
          {
            "districtID": 2043,
            "cityID": 218,
            "divisionCode": "445303",
            "districtName": "云安区"
          },
          {
            "districtID": 2044,
            "cityID": 218,
            "divisionCode": "445321",
            "districtName": "新兴县"
          },
          {
            "districtID": 2045,
            "cityID": 218,
            "divisionCode": "445322",
            "districtName": "郁南县"
          },
          {
            "districtID": 2046,
            "cityID": 218,
            "divisionCode": "445381",
            "districtName": "罗定市"
          },
          {
            "districtID": 2047,
            "cityID": 219,
            "divisionCode": "450101",
            "districtName": "市辖区"
          },
          {
            "districtID": 2048,
            "cityID": 219,
            "divisionCode": "450102",
            "districtName": "兴宁区"
          },
          {
            "districtID": 2049,
            "cityID": 219,
            "divisionCode": "450103",
            "districtName": "青秀区"
          },
          {
            "districtID": 2050,
            "cityID": 219,
            "divisionCode": "450105",
            "districtName": "江南区"
          },
          {
            "districtID": 2051,
            "cityID": 219,
            "divisionCode": "450107",
            "districtName": "西乡塘区"
          },
          {
            "districtID": 2052,
            "cityID": 219,
            "divisionCode": "450108",
            "districtName": "良庆区"
          },
          {
            "districtID": 2053,
            "cityID": 219,
            "divisionCode": "450109",
            "districtName": "邕宁区"
          },
          {
            "districtID": 2054,
            "cityID": 219,
            "divisionCode": "450110",
            "districtName": "武鸣区"
          },
          {
            "districtID": 2055,
            "cityID": 219,
            "divisionCode": "450123",
            "districtName": "隆安县"
          },
          {
            "districtID": 2056,
            "cityID": 219,
            "divisionCode": "450124",
            "districtName": "马山县"
          },
          {
            "districtID": 2057,
            "cityID": 219,
            "divisionCode": "450125",
            "districtName": "上林县"
          },
          {
            "districtID": 2058,
            "cityID": 219,
            "divisionCode": "450126",
            "districtName": "宾阳县"
          },
          {
            "districtID": 2059,
            "cityID": 219,
            "divisionCode": "450127",
            "districtName": "横县"
          },
          {
            "districtID": 2060,
            "cityID": 220,
            "divisionCode": "450201",
            "districtName": "市辖区"
          },
          {
            "districtID": 2061,
            "cityID": 220,
            "divisionCode": "450202",
            "districtName": "城中区"
          },
          {
            "districtID": 2062,
            "cityID": 220,
            "divisionCode": "450203",
            "districtName": "鱼峰区"
          },
          {
            "districtID": 2063,
            "cityID": 220,
            "divisionCode": "450204",
            "districtName": "柳南区"
          },
          {
            "districtID": 2064,
            "cityID": 220,
            "divisionCode": "450205",
            "districtName": "柳北区"
          },
          {
            "districtID": 2065,
            "cityID": 220,
            "divisionCode": "450206",
            "districtName": "柳江区"
          },
          {
            "districtID": 2066,
            "cityID": 220,
            "divisionCode": "450222",
            "districtName": "柳城县"
          },
          {
            "districtID": 2067,
            "cityID": 220,
            "divisionCode": "450223",
            "districtName": "鹿寨县"
          },
          {
            "districtID": 2068,
            "cityID": 220,
            "divisionCode": "450224",
            "districtName": "融安县"
          },
          {
            "districtID": 2069,
            "cityID": 220,
            "divisionCode": "450225",
            "districtName": "融水苗族自治县"
          },
          {
            "districtID": 2070,
            "cityID": 220,
            "divisionCode": "450226",
            "districtName": "三江侗族自治县"
          },
          {
            "districtID": 2071,
            "cityID": 221,
            "divisionCode": "450301",
            "districtName": "市辖区"
          },
          {
            "districtID": 2072,
            "cityID": 221,
            "divisionCode": "450302",
            "districtName": "秀峰区"
          },
          {
            "districtID": 2073,
            "cityID": 221,
            "divisionCode": "450303",
            "districtName": "叠彩区"
          },
          {
            "districtID": 2074,
            "cityID": 221,
            "divisionCode": "450304",
            "districtName": "象山区"
          },
          {
            "districtID": 2075,
            "cityID": 221,
            "divisionCode": "450305",
            "districtName": "七星区"
          },
          {
            "districtID": 2076,
            "cityID": 221,
            "divisionCode": "450311",
            "districtName": "雁山区"
          },
          {
            "districtID": 2077,
            "cityID": 221,
            "divisionCode": "450312",
            "districtName": "临桂区"
          },
          {
            "districtID": 2078,
            "cityID": 221,
            "divisionCode": "450321",
            "districtName": "阳朔县"
          },
          {
            "districtID": 2079,
            "cityID": 221,
            "divisionCode": "450323",
            "districtName": "灵川县"
          },
          {
            "districtID": 2080,
            "cityID": 221,
            "divisionCode": "450324",
            "districtName": "全州县"
          },
          {
            "districtID": 2081,
            "cityID": 221,
            "divisionCode": "450325",
            "districtName": "兴安县"
          },
          {
            "districtID": 2082,
            "cityID": 221,
            "divisionCode": "450326",
            "districtName": "永福县"
          },
          {
            "districtID": 2083,
            "cityID": 221,
            "divisionCode": "450327",
            "districtName": "灌阳县"
          },
          {
            "districtID": 2084,
            "cityID": 221,
            "divisionCode": "450328",
            "districtName": "龙胜各族自治县"
          },
          {
            "districtID": 2085,
            "cityID": 221,
            "divisionCode": "450329",
            "districtName": "资源县"
          },
          {
            "districtID": 2086,
            "cityID": 221,
            "divisionCode": "450330",
            "districtName": "平乐县"
          },
          {
            "districtID": 2087,
            "cityID": 221,
            "divisionCode": "450331",
            "districtName": "荔浦县"
          },
          {
            "districtID": 2088,
            "cityID": 221,
            "divisionCode": "450332",
            "districtName": "恭城瑶族自治县"
          },
          {
            "districtID": 2089,
            "cityID": 222,
            "divisionCode": "450401",
            "districtName": "市辖区"
          },
          {
            "districtID": 2090,
            "cityID": 222,
            "divisionCode": "450403",
            "districtName": "万秀区"
          },
          {
            "districtID": 2091,
            "cityID": 222,
            "divisionCode": "450405",
            "districtName": "长洲区"
          },
          {
            "districtID": 2092,
            "cityID": 222,
            "divisionCode": "450406",
            "districtName": "龙圩区"
          },
          {
            "districtID": 2093,
            "cityID": 222,
            "divisionCode": "450421",
            "districtName": "苍梧县"
          },
          {
            "districtID": 2094,
            "cityID": 222,
            "divisionCode": "450422",
            "districtName": "藤县"
          },
          {
            "districtID": 2095,
            "cityID": 222,
            "divisionCode": "450423",
            "districtName": "蒙山县"
          },
          {
            "districtID": 2096,
            "cityID": 222,
            "divisionCode": "450481",
            "districtName": "岑溪市"
          },
          {
            "districtID": 2097,
            "cityID": 223,
            "divisionCode": "450501",
            "districtName": "市辖区"
          },
          {
            "districtID": 2098,
            "cityID": 223,
            "divisionCode": "450502",
            "districtName": "海城区"
          },
          {
            "districtID": 2099,
            "cityID": 223,
            "divisionCode": "450503",
            "districtName": "银海区"
          },
          {
            "districtID": 2100,
            "cityID": 223,
            "divisionCode": "450512",
            "districtName": "铁山港区"
          },
          {
            "districtID": 2101,
            "cityID": 223,
            "divisionCode": "450521",
            "districtName": "合浦县"
          },
          {
            "districtID": 2102,
            "cityID": 224,
            "divisionCode": "450601",
            "districtName": "市辖区"
          },
          {
            "districtID": 2103,
            "cityID": 224,
            "divisionCode": "450602",
            "districtName": "港口区"
          },
          {
            "districtID": 2104,
            "cityID": 224,
            "divisionCode": "450603",
            "districtName": "防城区"
          },
          {
            "districtID": 2105,
            "cityID": 224,
            "divisionCode": "450621",
            "districtName": "上思县"
          },
          {
            "districtID": 2106,
            "cityID": 224,
            "divisionCode": "450681",
            "districtName": "东兴市"
          },
          {
            "districtID": 2107,
            "cityID": 225,
            "divisionCode": "450701",
            "districtName": "市辖区"
          },
          {
            "districtID": 2108,
            "cityID": 225,
            "divisionCode": "450702",
            "districtName": "钦南区"
          },
          {
            "districtID": 2109,
            "cityID": 225,
            "divisionCode": "450703",
            "districtName": "钦北区"
          },
          {
            "districtID": 2110,
            "cityID": 225,
            "divisionCode": "450721",
            "districtName": "灵山县"
          },
          {
            "districtID": 2111,
            "cityID": 225,
            "divisionCode": "450722",
            "districtName": "浦北县"
          },
          {
            "districtID": 2112,
            "cityID": 226,
            "divisionCode": "450801",
            "districtName": "市辖区"
          },
          {
            "districtID": 2113,
            "cityID": 226,
            "divisionCode": "450802",
            "districtName": "港北区"
          },
          {
            "districtID": 2114,
            "cityID": 226,
            "divisionCode": "450803",
            "districtName": "港南区"
          },
          {
            "districtID": 2115,
            "cityID": 226,
            "divisionCode": "450804",
            "districtName": "覃塘区"
          },
          {
            "districtID": 2116,
            "cityID": 226,
            "divisionCode": "450821",
            "districtName": "平南县"
          },
          {
            "districtID": 2117,
            "cityID": 226,
            "divisionCode": "450881",
            "districtName": "桂平市"
          },
          {
            "districtID": 2118,
            "cityID": 227,
            "divisionCode": "450901",
            "districtName": "市辖区"
          },
          {
            "districtID": 2119,
            "cityID": 227,
            "divisionCode": "450902",
            "districtName": "玉州区"
          },
          {
            "districtID": 2120,
            "cityID": 227,
            "divisionCode": "450903",
            "districtName": "福绵区"
          },
          {
            "districtID": 2121,
            "cityID": 227,
            "divisionCode": "450921",
            "districtName": "容县"
          },
          {
            "districtID": 2122,
            "cityID": 227,
            "divisionCode": "450922",
            "districtName": "陆川县"
          },
          {
            "districtID": 2123,
            "cityID": 227,
            "divisionCode": "450923",
            "districtName": "博白县"
          },
          {
            "districtID": 2124,
            "cityID": 227,
            "divisionCode": "450924",
            "districtName": "兴业县"
          },
          {
            "districtID": 2125,
            "cityID": 227,
            "divisionCode": "450981",
            "districtName": "北流市"
          },
          {
            "districtID": 2126,
            "cityID": 228,
            "divisionCode": "451001",
            "districtName": "市辖区"
          },
          {
            "districtID": 2127,
            "cityID": 228,
            "divisionCode": "451002",
            "districtName": "右江区"
          },
          {
            "districtID": 2128,
            "cityID": 228,
            "divisionCode": "451021",
            "districtName": "田阳县"
          },
          {
            "districtID": 2129,
            "cityID": 228,
            "divisionCode": "451022",
            "districtName": "田东县"
          },
          {
            "districtID": 2130,
            "cityID": 228,
            "divisionCode": "451023",
            "districtName": "平果县"
          },
          {
            "districtID": 2131,
            "cityID": 228,
            "divisionCode": "451024",
            "districtName": "德保县"
          },
          {
            "districtID": 2132,
            "cityID": 228,
            "divisionCode": "451026",
            "districtName": "那坡县"
          },
          {
            "districtID": 2133,
            "cityID": 228,
            "divisionCode": "451027",
            "districtName": "凌云县"
          },
          {
            "districtID": 2134,
            "cityID": 228,
            "divisionCode": "451028",
            "districtName": "乐业县"
          },
          {
            "districtID": 2135,
            "cityID": 228,
            "divisionCode": "451029",
            "districtName": "田林县"
          },
          {
            "districtID": 2136,
            "cityID": 228,
            "divisionCode": "451030",
            "districtName": "西林县"
          },
          {
            "districtID": 2137,
            "cityID": 228,
            "divisionCode": "451031",
            "districtName": "隆林各族自治县"
          },
          {
            "districtID": 2138,
            "cityID": 228,
            "divisionCode": "451081",
            "districtName": "靖西市"
          },
          {
            "districtID": 2139,
            "cityID": 229,
            "divisionCode": "451101",
            "districtName": "市辖区"
          },
          {
            "districtID": 2140,
            "cityID": 229,
            "divisionCode": "451102",
            "districtName": "八步区"
          },
          {
            "districtID": 2141,
            "cityID": 229,
            "divisionCode": "451103",
            "districtName": "平桂区"
          },
          {
            "districtID": 2142,
            "cityID": 229,
            "divisionCode": "451121",
            "districtName": "昭平县"
          },
          {
            "districtID": 2143,
            "cityID": 229,
            "divisionCode": "451122",
            "districtName": "钟山县"
          },
          {
            "districtID": 2144,
            "cityID": 229,
            "divisionCode": "451123",
            "districtName": "富川瑶族自治县"
          },
          {
            "districtID": 2145,
            "cityID": 230,
            "divisionCode": "451201",
            "districtName": "市辖区"
          },
          {
            "districtID": 2146,
            "cityID": 230,
            "divisionCode": "451202",
            "districtName": "金城江区"
          },
          {
            "districtID": 2147,
            "cityID": 230,
            "divisionCode": "451221",
            "districtName": "南丹县"
          },
          {
            "districtID": 2148,
            "cityID": 230,
            "divisionCode": "451222",
            "districtName": "天峨县"
          },
          {
            "districtID": 2149,
            "cityID": 230,
            "divisionCode": "451223",
            "districtName": "凤山县"
          },
          {
            "districtID": 2150,
            "cityID": 230,
            "divisionCode": "451224",
            "districtName": "东兰县"
          },
          {
            "districtID": 2151,
            "cityID": 230,
            "divisionCode": "451225",
            "districtName": "罗城仫佬族自治县"
          },
          {
            "districtID": 2152,
            "cityID": 230,
            "divisionCode": "451226",
            "districtName": "环江毛南族自治县"
          },
          {
            "districtID": 2153,
            "cityID": 230,
            "divisionCode": "451227",
            "districtName": "巴马瑶族自治县"
          },
          {
            "districtID": 2154,
            "cityID": 230,
            "divisionCode": "451228",
            "districtName": "都安瑶族自治县"
          },
          {
            "districtID": 2155,
            "cityID": 230,
            "divisionCode": "451229",
            "districtName": "大化瑶族自治县"
          },
          {
            "districtID": 2156,
            "cityID": 230,
            "divisionCode": "451281",
            "districtName": "宜州市"
          },
          {
            "districtID": 2157,
            "cityID": 231,
            "divisionCode": "451301",
            "districtName": "市辖区"
          },
          {
            "districtID": 2158,
            "cityID": 231,
            "divisionCode": "451302",
            "districtName": "兴宾区"
          },
          {
            "districtID": 2159,
            "cityID": 231,
            "divisionCode": "451321",
            "districtName": "忻城县"
          },
          {
            "districtID": 2160,
            "cityID": 231,
            "divisionCode": "451322",
            "districtName": "象州县"
          },
          {
            "districtID": 2161,
            "cityID": 231,
            "divisionCode": "451323",
            "districtName": "武宣县"
          },
          {
            "districtID": 2162,
            "cityID": 231,
            "divisionCode": "451324",
            "districtName": "金秀瑶族自治县"
          },
          {
            "districtID": 2163,
            "cityID": 231,
            "divisionCode": "451381",
            "districtName": "合山市"
          },
          {
            "districtID": 2164,
            "cityID": 232,
            "divisionCode": "451401",
            "districtName": "市辖区"
          },
          {
            "districtID": 2165,
            "cityID": 232,
            "divisionCode": "451402",
            "districtName": "江州区"
          },
          {
            "districtID": 2166,
            "cityID": 232,
            "divisionCode": "451421",
            "districtName": "扶绥县"
          },
          {
            "districtID": 2167,
            "cityID": 232,
            "divisionCode": "451422",
            "districtName": "宁明县"
          },
          {
            "districtID": 2168,
            "cityID": 232,
            "divisionCode": "451423",
            "districtName": "龙州县"
          },
          {
            "districtID": 2169,
            "cityID": 232,
            "divisionCode": "451424",
            "districtName": "大新县"
          },
          {
            "districtID": 2170,
            "cityID": 232,
            "divisionCode": "451425",
            "districtName": "天等县"
          },
          {
            "districtID": 2171,
            "cityID": 232,
            "divisionCode": "451481",
            "districtName": "凭祥市"
          },
          {
            "districtID": 2172,
            "cityID": 233,
            "divisionCode": "460101",
            "districtName": "市辖区"
          },
          {
            "districtID": 2173,
            "cityID": 233,
            "divisionCode": "460105",
            "districtName": "秀英区"
          },
          {
            "districtID": 2174,
            "cityID": 233,
            "divisionCode": "460106",
            "districtName": "龙华区"
          },
          {
            "districtID": 2175,
            "cityID": 233,
            "divisionCode": "460107",
            "districtName": "琼山区"
          },
          {
            "districtID": 2176,
            "cityID": 233,
            "divisionCode": "460108",
            "districtName": "美兰区"
          },
          {
            "districtID": 2177,
            "cityID": 234,
            "divisionCode": "460201",
            "districtName": "市辖区"
          },
          {
            "districtID": 2178,
            "cityID": 234,
            "divisionCode": "460202",
            "districtName": "海棠区"
          },
          {
            "districtID": 2179,
            "cityID": 234,
            "divisionCode": "460203",
            "districtName": "吉阳区"
          },
          {
            "districtID": 2180,
            "cityID": 234,
            "divisionCode": "460204",
            "districtName": "天涯区"
          },
          {
            "districtID": 2181,
            "cityID": 234,
            "divisionCode": "460205",
            "districtName": "崖州区"
          },
          {
            "districtID": 2182,
            "cityID": 237,
            "divisionCode": "469001",
            "districtName": "五指山市"
          },
          {
            "districtID": 2183,
            "cityID": 237,
            "divisionCode": "469002",
            "districtName": "琼海市"
          },
          {
            "districtID": 2184,
            "cityID": 237,
            "divisionCode": "469005",
            "districtName": "文昌市"
          },
          {
            "districtID": 2185,
            "cityID": 237,
            "divisionCode": "469006",
            "districtName": "万宁市"
          },
          {
            "districtID": 2186,
            "cityID": 237,
            "divisionCode": "469007",
            "districtName": "东方市"
          },
          {
            "districtID": 2187,
            "cityID": 237,
            "divisionCode": "469021",
            "districtName": "定安县"
          },
          {
            "districtID": 2188,
            "cityID": 237,
            "divisionCode": "469022",
            "districtName": "屯昌县"
          },
          {
            "districtID": 2189,
            "cityID": 237,
            "divisionCode": "469023",
            "districtName": "澄迈县"
          },
          {
            "districtID": 2190,
            "cityID": 237,
            "divisionCode": "469024",
            "districtName": "临高县"
          },
          {
            "districtID": 2191,
            "cityID": 237,
            "divisionCode": "469025",
            "districtName": "白沙黎族自治县"
          },
          {
            "districtID": 2192,
            "cityID": 237,
            "divisionCode": "469026",
            "districtName": "昌江黎族自治县"
          },
          {
            "districtID": 2193,
            "cityID": 237,
            "divisionCode": "469027",
            "districtName": "乐东黎族自治县"
          },
          {
            "districtID": 2194,
            "cityID": 237,
            "divisionCode": "469028",
            "districtName": "陵水黎族自治县"
          },
          {
            "districtID": 2195,
            "cityID": 237,
            "divisionCode": "469029",
            "districtName": "保亭黎族苗族自治县"
          },
          {
            "districtID": 2196,
            "cityID": 237,
            "divisionCode": "469030",
            "districtName": "琼中黎族苗族自治县"
          },
          {
            "districtID": 2197,
            "cityID": 238,
            "divisionCode": "500101",
            "districtName": "万州区"
          },
          {
            "districtID": 2198,
            "cityID": 238,
            "divisionCode": "500102",
            "districtName": "涪陵区"
          },
          {
            "districtID": 2199,
            "cityID": 238,
            "divisionCode": "500103",
            "districtName": "渝中区"
          },
          {
            "districtID": 2200,
            "cityID": 238,
            "divisionCode": "500104",
            "districtName": "大渡口区"
          },
          {
            "districtID": 2201,
            "cityID": 238,
            "divisionCode": "500105",
            "districtName": "江北区"
          },
          {
            "districtID": 2202,
            "cityID": 238,
            "divisionCode": "500106",
            "districtName": "沙坪坝区"
          },
          {
            "districtID": 2203,
            "cityID": 238,
            "divisionCode": "500107",
            "districtName": "九龙坡区"
          },
          {
            "districtID": 2204,
            "cityID": 238,
            "divisionCode": "500108",
            "districtName": "南岸区"
          },
          {
            "districtID": 2205,
            "cityID": 238,
            "divisionCode": "500109",
            "districtName": "北碚区"
          },
          {
            "districtID": 2206,
            "cityID": 238,
            "divisionCode": "500110",
            "districtName": "綦江区"
          },
          {
            "districtID": 2207,
            "cityID": 238,
            "divisionCode": "500111",
            "districtName": "大足区"
          },
          {
            "districtID": 2208,
            "cityID": 238,
            "divisionCode": "500112",
            "districtName": "渝北区"
          },
          {
            "districtID": 2209,
            "cityID": 238,
            "divisionCode": "500113",
            "districtName": "巴南区"
          },
          {
            "districtID": 2210,
            "cityID": 238,
            "divisionCode": "500114",
            "districtName": "黔江区"
          },
          {
            "districtID": 2211,
            "cityID": 238,
            "divisionCode": "500115",
            "districtName": "长寿区"
          },
          {
            "districtID": 2212,
            "cityID": 238,
            "divisionCode": "500116",
            "districtName": "江津区"
          },
          {
            "districtID": 2213,
            "cityID": 238,
            "divisionCode": "500117",
            "districtName": "合川区"
          },
          {
            "districtID": 2214,
            "cityID": 238,
            "divisionCode": "500118",
            "districtName": "永川区"
          },
          {
            "districtID": 2215,
            "cityID": 238,
            "divisionCode": "500119",
            "districtName": "南川区"
          },
          {
            "districtID": 2216,
            "cityID": 238,
            "divisionCode": "500120",
            "districtName": "璧山区"
          },
          {
            "districtID": 2217,
            "cityID": 238,
            "divisionCode": "500151",
            "districtName": "铜梁区"
          },
          {
            "districtID": 2218,
            "cityID": 238,
            "divisionCode": "500152",
            "districtName": "潼南区"
          },
          {
            "districtID": 2219,
            "cityID": 238,
            "divisionCode": "500153",
            "districtName": "荣昌区"
          },
          {
            "districtID": 2220,
            "cityID": 238,
            "divisionCode": "500154",
            "districtName": "开州区"
          },
          {
            "districtID": 2221,
            "cityID": 239,
            "divisionCode": "500228",
            "districtName": "梁平县"
          },
          {
            "districtID": 2222,
            "cityID": 239,
            "divisionCode": "500229",
            "districtName": "城口县"
          },
          {
            "districtID": 2223,
            "cityID": 239,
            "divisionCode": "500230",
            "districtName": "丰都县"
          },
          {
            "districtID": 2224,
            "cityID": 239,
            "divisionCode": "500231",
            "districtName": "垫江县"
          },
          {
            "districtID": 2225,
            "cityID": 239,
            "divisionCode": "500232",
            "districtName": "武隆县"
          },
          {
            "districtID": 2226,
            "cityID": 239,
            "divisionCode": "500233",
            "districtName": "忠县"
          },
          {
            "districtID": 2227,
            "cityID": 239,
            "divisionCode": "500235",
            "districtName": "云阳县"
          },
          {
            "districtID": 2228,
            "cityID": 239,
            "divisionCode": "500236",
            "districtName": "奉节县"
          },
          {
            "districtID": 2229,
            "cityID": 239,
            "divisionCode": "500237",
            "districtName": "巫山县"
          },
          {
            "districtID": 2230,
            "cityID": 239,
            "divisionCode": "500238",
            "districtName": "巫溪县"
          },
          {
            "districtID": 2231,
            "cityID": 239,
            "divisionCode": "500240",
            "districtName": "石柱土家族自治县"
          },
          {
            "districtID": 2232,
            "cityID": 239,
            "divisionCode": "500241",
            "districtName": "秀山土家族苗族自治县"
          },
          {
            "districtID": 2233,
            "cityID": 239,
            "divisionCode": "500242",
            "districtName": "酉阳土家族苗族自治县"
          },
          {
            "districtID": 2234,
            "cityID": 239,
            "divisionCode": "500243",
            "districtName": "彭水苗族土家族自治县"
          },
          {
            "districtID": 2235,
            "cityID": 240,
            "divisionCode": "510101",
            "districtName": "市辖区"
          },
          {
            "districtID": 2236,
            "cityID": 240,
            "divisionCode": "510104",
            "districtName": "锦江区"
          },
          {
            "districtID": 2237,
            "cityID": 240,
            "divisionCode": "510105",
            "districtName": "青羊区"
          },
          {
            "districtID": 2238,
            "cityID": 240,
            "divisionCode": "510106",
            "districtName": "金牛区"
          },
          {
            "districtID": 2239,
            "cityID": 240,
            "divisionCode": "510107",
            "districtName": "武侯区"
          },
          {
            "districtID": 2240,
            "cityID": 240,
            "divisionCode": "510108",
            "districtName": "成华区"
          },
          {
            "districtID": 2241,
            "cityID": 240,
            "divisionCode": "510112",
            "districtName": "龙泉驿区"
          },
          {
            "districtID": 2242,
            "cityID": 240,
            "divisionCode": "510113",
            "districtName": "青白江区"
          },
          {
            "districtID": 2243,
            "cityID": 240,
            "divisionCode": "510114",
            "districtName": "新都区"
          },
          {
            "districtID": 2244,
            "cityID": 240,
            "divisionCode": "510115",
            "districtName": "温江区"
          },
          {
            "districtID": 2245,
            "cityID": 240,
            "divisionCode": "510116",
            "districtName": "双流区"
          },
          {
            "districtID": 2246,
            "cityID": 240,
            "divisionCode": "510121",
            "districtName": "金堂县"
          },
          {
            "districtID": 2247,
            "cityID": 240,
            "divisionCode": "510124",
            "districtName": "郫县"
          },
          {
            "districtID": 2248,
            "cityID": 240,
            "divisionCode": "510129",
            "districtName": "大邑县"
          },
          {
            "districtID": 2249,
            "cityID": 240,
            "divisionCode": "510131",
            "districtName": "蒲江县"
          },
          {
            "districtID": 2250,
            "cityID": 240,
            "divisionCode": "510132",
            "districtName": "新津县"
          },
          {
            "districtID": 2251,
            "cityID": 240,
            "divisionCode": "510181",
            "districtName": "都江堰市"
          },
          {
            "districtID": 2252,
            "cityID": 240,
            "divisionCode": "510182",
            "districtName": "彭州市"
          },
          {
            "districtID": 2253,
            "cityID": 240,
            "divisionCode": "510183",
            "districtName": "邛崃市"
          },
          {
            "districtID": 2254,
            "cityID": 240,
            "divisionCode": "510184",
            "districtName": "崇州市"
          },
          {
            "districtID": 2255,
            "cityID": 240,
            "divisionCode": "510185",
            "districtName": "简阳市"
          },
          {
            "districtID": 2256,
            "cityID": 241,
            "divisionCode": "510301",
            "districtName": "市辖区"
          },
          {
            "districtID": 2257,
            "cityID": 241,
            "divisionCode": "510302",
            "districtName": "自流井区"
          },
          {
            "districtID": 2258,
            "cityID": 241,
            "divisionCode": "510303",
            "districtName": "贡井区"
          },
          {
            "districtID": 2259,
            "cityID": 241,
            "divisionCode": "510304",
            "districtName": "大安区"
          },
          {
            "districtID": 2260,
            "cityID": 241,
            "divisionCode": "510311",
            "districtName": "沿滩区"
          },
          {
            "districtID": 2261,
            "cityID": 241,
            "divisionCode": "510321",
            "districtName": "荣县"
          },
          {
            "districtID": 2262,
            "cityID": 241,
            "divisionCode": "510322",
            "districtName": "富顺县"
          },
          {
            "districtID": 2263,
            "cityID": 242,
            "divisionCode": "510401",
            "districtName": "市辖区"
          },
          {
            "districtID": 2264,
            "cityID": 242,
            "divisionCode": "510402",
            "districtName": "东区"
          },
          {
            "districtID": 2265,
            "cityID": 242,
            "divisionCode": "510403",
            "districtName": "西区"
          },
          {
            "districtID": 2266,
            "cityID": 242,
            "divisionCode": "510411",
            "districtName": "仁和区"
          },
          {
            "districtID": 2267,
            "cityID": 242,
            "divisionCode": "510421",
            "districtName": "米易县"
          },
          {
            "districtID": 2268,
            "cityID": 242,
            "divisionCode": "510422",
            "districtName": "盐边县"
          },
          {
            "districtID": 2269,
            "cityID": 243,
            "divisionCode": "510501",
            "districtName": "市辖区"
          },
          {
            "districtID": 2270,
            "cityID": 243,
            "divisionCode": "510502",
            "districtName": "江阳区"
          },
          {
            "districtID": 2271,
            "cityID": 243,
            "divisionCode": "510503",
            "districtName": "纳溪区"
          },
          {
            "districtID": 2272,
            "cityID": 243,
            "divisionCode": "510504",
            "districtName": "龙马潭区"
          },
          {
            "districtID": 2273,
            "cityID": 243,
            "divisionCode": "510521",
            "districtName": "泸县"
          },
          {
            "districtID": 2274,
            "cityID": 243,
            "divisionCode": "510522",
            "districtName": "合江县"
          },
          {
            "districtID": 2275,
            "cityID": 243,
            "divisionCode": "510524",
            "districtName": "叙永县"
          },
          {
            "districtID": 2276,
            "cityID": 243,
            "divisionCode": "510525",
            "districtName": "古蔺县"
          },
          {
            "districtID": 2277,
            "cityID": 244,
            "divisionCode": "510601",
            "districtName": "市辖区"
          },
          {
            "districtID": 2278,
            "cityID": 244,
            "divisionCode": "510603",
            "districtName": "旌阳区"
          },
          {
            "districtID": 2279,
            "cityID": 244,
            "divisionCode": "510623",
            "districtName": "中江县"
          },
          {
            "districtID": 2280,
            "cityID": 244,
            "divisionCode": "510626",
            "districtName": "罗江县"
          },
          {
            "districtID": 2281,
            "cityID": 244,
            "divisionCode": "510681",
            "districtName": "广汉市"
          },
          {
            "districtID": 2282,
            "cityID": 244,
            "divisionCode": "510682",
            "districtName": "什邡市"
          },
          {
            "districtID": 2283,
            "cityID": 244,
            "divisionCode": "510683",
            "districtName": "绵竹市"
          },
          {
            "districtID": 2284,
            "cityID": 245,
            "divisionCode": "510701",
            "districtName": "市辖区"
          },
          {
            "districtID": 2285,
            "cityID": 245,
            "divisionCode": "510703",
            "districtName": "涪城区"
          },
          {
            "districtID": 2286,
            "cityID": 245,
            "divisionCode": "510704",
            "districtName": "游仙区"
          },
          {
            "districtID": 2287,
            "cityID": 245,
            "divisionCode": "510705",
            "districtName": "安州区"
          },
          {
            "districtID": 2288,
            "cityID": 245,
            "divisionCode": "510722",
            "districtName": "三台县"
          },
          {
            "districtID": 2289,
            "cityID": 245,
            "divisionCode": "510723",
            "districtName": "盐亭县"
          },
          {
            "districtID": 2290,
            "cityID": 245,
            "divisionCode": "510725",
            "districtName": "梓潼县"
          },
          {
            "districtID": 2291,
            "cityID": 245,
            "divisionCode": "510726",
            "districtName": "北川羌族自治县"
          },
          {
            "districtID": 2292,
            "cityID": 245,
            "divisionCode": "510727",
            "districtName": "平武县"
          },
          {
            "districtID": 2293,
            "cityID": 245,
            "divisionCode": "510781",
            "districtName": "江油市"
          },
          {
            "districtID": 2294,
            "cityID": 246,
            "divisionCode": "510801",
            "districtName": "市辖区"
          },
          {
            "districtID": 2295,
            "cityID": 246,
            "divisionCode": "510802",
            "districtName": "利州区"
          },
          {
            "districtID": 2296,
            "cityID": 246,
            "divisionCode": "510811",
            "districtName": "昭化区"
          },
          {
            "districtID": 2297,
            "cityID": 246,
            "divisionCode": "510812",
            "districtName": "朝天区"
          },
          {
            "districtID": 2298,
            "cityID": 246,
            "divisionCode": "510821",
            "districtName": "旺苍县"
          },
          {
            "districtID": 2299,
            "cityID": 246,
            "divisionCode": "510822",
            "districtName": "青川县"
          },
          {
            "districtID": 2300,
            "cityID": 246,
            "divisionCode": "510823",
            "districtName": "剑阁县"
          },
          {
            "districtID": 2301,
            "cityID": 246,
            "divisionCode": "510824",
            "districtName": "苍溪县"
          },
          {
            "districtID": 2302,
            "cityID": 247,
            "divisionCode": "510901",
            "districtName": "市辖区"
          },
          {
            "districtID": 2303,
            "cityID": 247,
            "divisionCode": "510903",
            "districtName": "船山区"
          },
          {
            "districtID": 2304,
            "cityID": 247,
            "divisionCode": "510904",
            "districtName": "安居区"
          },
          {
            "districtID": 2305,
            "cityID": 247,
            "divisionCode": "510921",
            "districtName": "蓬溪县"
          },
          {
            "districtID": 2306,
            "cityID": 247,
            "divisionCode": "510922",
            "districtName": "射洪县"
          },
          {
            "districtID": 2307,
            "cityID": 247,
            "divisionCode": "510923",
            "districtName": "大英县"
          },
          {
            "districtID": 2308,
            "cityID": 248,
            "divisionCode": "511001",
            "districtName": "市辖区"
          },
          {
            "districtID": 2309,
            "cityID": 248,
            "divisionCode": "511002",
            "districtName": "市中区"
          },
          {
            "districtID": 2310,
            "cityID": 248,
            "divisionCode": "511011",
            "districtName": "东兴区"
          },
          {
            "districtID": 2311,
            "cityID": 248,
            "divisionCode": "511024",
            "districtName": "威远县"
          },
          {
            "districtID": 2312,
            "cityID": 248,
            "divisionCode": "511025",
            "districtName": "资中县"
          },
          {
            "districtID": 2313,
            "cityID": 248,
            "divisionCode": "511028",
            "districtName": "隆昌县"
          },
          {
            "districtID": 2314,
            "cityID": 249,
            "divisionCode": "511101",
            "districtName": "市辖区"
          },
          {
            "districtID": 2315,
            "cityID": 249,
            "divisionCode": "511102",
            "districtName": "市中区"
          },
          {
            "districtID": 2316,
            "cityID": 249,
            "divisionCode": "511111",
            "districtName": "沙湾区"
          },
          {
            "districtID": 2317,
            "cityID": 249,
            "divisionCode": "511112",
            "districtName": "五通桥区"
          },
          {
            "districtID": 2318,
            "cityID": 249,
            "divisionCode": "511113",
            "districtName": "金口河区"
          },
          {
            "districtID": 2319,
            "cityID": 249,
            "divisionCode": "511123",
            "districtName": "犍为县"
          },
          {
            "districtID": 2320,
            "cityID": 249,
            "divisionCode": "511124",
            "districtName": "井研县"
          },
          {
            "districtID": 2321,
            "cityID": 249,
            "divisionCode": "511126",
            "districtName": "夹江县"
          },
          {
            "districtID": 2322,
            "cityID": 249,
            "divisionCode": "511129",
            "districtName": "沐川县"
          },
          {
            "districtID": 2323,
            "cityID": 249,
            "divisionCode": "511132",
            "districtName": "峨边彝族自治县"
          },
          {
            "districtID": 2324,
            "cityID": 249,
            "divisionCode": "511133",
            "districtName": "马边彝族自治县"
          },
          {
            "districtID": 2325,
            "cityID": 249,
            "divisionCode": "511181",
            "districtName": "峨眉山市"
          },
          {
            "districtID": 2326,
            "cityID": 250,
            "divisionCode": "511301",
            "districtName": "市辖区"
          },
          {
            "districtID": 2327,
            "cityID": 250,
            "divisionCode": "511302",
            "districtName": "顺庆区"
          },
          {
            "districtID": 2328,
            "cityID": 250,
            "divisionCode": "511303",
            "districtName": "高坪区"
          },
          {
            "districtID": 2329,
            "cityID": 250,
            "divisionCode": "511304",
            "districtName": "嘉陵区"
          },
          {
            "districtID": 2330,
            "cityID": 250,
            "divisionCode": "511321",
            "districtName": "南部县"
          },
          {
            "districtID": 2331,
            "cityID": 250,
            "divisionCode": "511322",
            "districtName": "营山县"
          },
          {
            "districtID": 2332,
            "cityID": 250,
            "divisionCode": "511323",
            "districtName": "蓬安县"
          },
          {
            "districtID": 2333,
            "cityID": 250,
            "divisionCode": "511324",
            "districtName": "仪陇县"
          },
          {
            "districtID": 2334,
            "cityID": 250,
            "divisionCode": "511325",
            "districtName": "西充县"
          },
          {
            "districtID": 2335,
            "cityID": 250,
            "divisionCode": "511381",
            "districtName": "阆中市"
          },
          {
            "districtID": 2336,
            "cityID": 251,
            "divisionCode": "511401",
            "districtName": "市辖区"
          },
          {
            "districtID": 2337,
            "cityID": 251,
            "divisionCode": "511402",
            "districtName": "东坡区"
          },
          {
            "districtID": 2338,
            "cityID": 251,
            "divisionCode": "511403",
            "districtName": "彭山区"
          },
          {
            "districtID": 2339,
            "cityID": 251,
            "divisionCode": "511421",
            "districtName": "仁寿县"
          },
          {
            "districtID": 2340,
            "cityID": 251,
            "divisionCode": "511423",
            "districtName": "洪雅县"
          },
          {
            "districtID": 2341,
            "cityID": 251,
            "divisionCode": "511424",
            "districtName": "丹棱县"
          },
          {
            "districtID": 2342,
            "cityID": 251,
            "divisionCode": "511425",
            "districtName": "青神县"
          },
          {
            "districtID": 2343,
            "cityID": 252,
            "divisionCode": "511501",
            "districtName": "市辖区"
          },
          {
            "districtID": 2344,
            "cityID": 252,
            "divisionCode": "511502",
            "districtName": "翠屏区"
          },
          {
            "districtID": 2345,
            "cityID": 252,
            "divisionCode": "511503",
            "districtName": "南溪区"
          },
          {
            "districtID": 2346,
            "cityID": 252,
            "divisionCode": "511521",
            "districtName": "宜宾县"
          },
          {
            "districtID": 2347,
            "cityID": 252,
            "divisionCode": "511523",
            "districtName": "江安县"
          },
          {
            "districtID": 2348,
            "cityID": 252,
            "divisionCode": "511524",
            "districtName": "长宁县"
          },
          {
            "districtID": 2349,
            "cityID": 252,
            "divisionCode": "511525",
            "districtName": "高县"
          },
          {
            "districtID": 2350,
            "cityID": 252,
            "divisionCode": "511526",
            "districtName": "珙县"
          },
          {
            "districtID": 2351,
            "cityID": 252,
            "divisionCode": "511527",
            "districtName": "筠连县"
          },
          {
            "districtID": 2352,
            "cityID": 252,
            "divisionCode": "511528",
            "districtName": "兴文县"
          },
          {
            "districtID": 2353,
            "cityID": 252,
            "divisionCode": "511529",
            "districtName": "屏山县"
          },
          {
            "districtID": 2354,
            "cityID": 253,
            "divisionCode": "511601",
            "districtName": "市辖区"
          },
          {
            "districtID": 2355,
            "cityID": 253,
            "divisionCode": "511602",
            "districtName": "广安区"
          },
          {
            "districtID": 2356,
            "cityID": 253,
            "divisionCode": "511603",
            "districtName": "前锋区"
          },
          {
            "districtID": 2357,
            "cityID": 253,
            "divisionCode": "511621",
            "districtName": "岳池县"
          },
          {
            "districtID": 2358,
            "cityID": 253,
            "divisionCode": "511622",
            "districtName": "武胜县"
          },
          {
            "districtID": 2359,
            "cityID": 253,
            "divisionCode": "511623",
            "districtName": "邻水县"
          },
          {
            "districtID": 2360,
            "cityID": 253,
            "divisionCode": "511681",
            "districtName": "华蓥市"
          },
          {
            "districtID": 2361,
            "cityID": 254,
            "divisionCode": "511701",
            "districtName": "市辖区"
          },
          {
            "districtID": 2362,
            "cityID": 254,
            "divisionCode": "511702",
            "districtName": "通川区"
          },
          {
            "districtID": 2363,
            "cityID": 254,
            "divisionCode": "511703",
            "districtName": "达川区"
          },
          {
            "districtID": 2364,
            "cityID": 254,
            "divisionCode": "511722",
            "districtName": "宣汉县"
          },
          {
            "districtID": 2365,
            "cityID": 254,
            "divisionCode": "511723",
            "districtName": "开江县"
          },
          {
            "districtID": 2366,
            "cityID": 254,
            "divisionCode": "511724",
            "districtName": "大竹县"
          },
          {
            "districtID": 2367,
            "cityID": 254,
            "divisionCode": "511725",
            "districtName": "渠县"
          },
          {
            "districtID": 2368,
            "cityID": 254,
            "divisionCode": "511781",
            "districtName": "万源市"
          },
          {
            "districtID": 2369,
            "cityID": 255,
            "divisionCode": "511801",
            "districtName": "市辖区"
          },
          {
            "districtID": 2370,
            "cityID": 255,
            "divisionCode": "511802",
            "districtName": "雨城区"
          },
          {
            "districtID": 2371,
            "cityID": 255,
            "divisionCode": "511803",
            "districtName": "名山区"
          },
          {
            "districtID": 2372,
            "cityID": 255,
            "divisionCode": "511822",
            "districtName": "荥经县"
          },
          {
            "districtID": 2373,
            "cityID": 255,
            "divisionCode": "511823",
            "districtName": "汉源县"
          },
          {
            "districtID": 2374,
            "cityID": 255,
            "divisionCode": "511824",
            "districtName": "石棉县"
          },
          {
            "districtID": 2375,
            "cityID": 255,
            "divisionCode": "511825",
            "districtName": "天全县"
          },
          {
            "districtID": 2376,
            "cityID": 255,
            "divisionCode": "511826",
            "districtName": "芦山县"
          },
          {
            "districtID": 2377,
            "cityID": 255,
            "divisionCode": "511827",
            "districtName": "宝兴县"
          },
          {
            "districtID": 2378,
            "cityID": 256,
            "divisionCode": "511901",
            "districtName": "市辖区"
          },
          {
            "districtID": 2379,
            "cityID": 256,
            "divisionCode": "511902",
            "districtName": "巴州区"
          },
          {
            "districtID": 2380,
            "cityID": 256,
            "divisionCode": "511903",
            "districtName": "恩阳区"
          },
          {
            "districtID": 2381,
            "cityID": 256,
            "divisionCode": "511921",
            "districtName": "通江县"
          },
          {
            "districtID": 2382,
            "cityID": 256,
            "divisionCode": "511922",
            "districtName": "南江县"
          },
          {
            "districtID": 2383,
            "cityID": 256,
            "divisionCode": "511923",
            "districtName": "平昌县"
          },
          {
            "districtID": 2384,
            "cityID": 257,
            "divisionCode": "512001",
            "districtName": "市辖区"
          },
          {
            "districtID": 2385,
            "cityID": 257,
            "divisionCode": "512002",
            "districtName": "雁江区"
          },
          {
            "districtID": 2386,
            "cityID": 257,
            "divisionCode": "512021",
            "districtName": "安岳县"
          },
          {
            "districtID": 2387,
            "cityID": 257,
            "divisionCode": "512022",
            "districtName": "乐至县"
          },
          {
            "districtID": 2388,
            "cityID": 258,
            "divisionCode": "513201",
            "districtName": "马尔康市"
          },
          {
            "districtID": 2389,
            "cityID": 258,
            "divisionCode": "513221",
            "districtName": "汶川县"
          },
          {
            "districtID": 2390,
            "cityID": 258,
            "divisionCode": "513222",
            "districtName": "理县"
          },
          {
            "districtID": 2391,
            "cityID": 258,
            "divisionCode": "513223",
            "districtName": "茂县"
          },
          {
            "districtID": 2392,
            "cityID": 258,
            "divisionCode": "513224",
            "districtName": "松潘县"
          },
          {
            "districtID": 2393,
            "cityID": 258,
            "divisionCode": "513225",
            "districtName": "九寨沟县"
          },
          {
            "districtID": 2394,
            "cityID": 258,
            "divisionCode": "513226",
            "districtName": "金川县"
          },
          {
            "districtID": 2395,
            "cityID": 258,
            "divisionCode": "513227",
            "districtName": "小金县"
          },
          {
            "districtID": 2396,
            "cityID": 258,
            "divisionCode": "513228",
            "districtName": "黑水县"
          },
          {
            "districtID": 2397,
            "cityID": 258,
            "divisionCode": "513230",
            "districtName": "壤塘县"
          },
          {
            "districtID": 2398,
            "cityID": 258,
            "divisionCode": "513231",
            "districtName": "阿坝县"
          },
          {
            "districtID": 2399,
            "cityID": 258,
            "divisionCode": "513232",
            "districtName": "若尔盖县"
          },
          {
            "districtID": 2400,
            "cityID": 258,
            "divisionCode": "513233",
            "districtName": "红原县"
          },
          {
            "districtID": 2401,
            "cityID": 259,
            "divisionCode": "513301",
            "districtName": "康定市"
          },
          {
            "districtID": 2402,
            "cityID": 259,
            "divisionCode": "513322",
            "districtName": "泸定县"
          },
          {
            "districtID": 2403,
            "cityID": 259,
            "divisionCode": "513323",
            "districtName": "丹巴县"
          },
          {
            "districtID": 2404,
            "cityID": 259,
            "divisionCode": "513324",
            "districtName": "九龙县"
          },
          {
            "districtID": 2405,
            "cityID": 259,
            "divisionCode": "513325",
            "districtName": "雅江县"
          },
          {
            "districtID": 2406,
            "cityID": 259,
            "divisionCode": "513326",
            "districtName": "道孚县"
          },
          {
            "districtID": 2407,
            "cityID": 259,
            "divisionCode": "513327",
            "districtName": "炉霍县"
          },
          {
            "districtID": 2408,
            "cityID": 259,
            "divisionCode": "513328",
            "districtName": "甘孜县"
          },
          {
            "districtID": 2409,
            "cityID": 259,
            "divisionCode": "513329",
            "districtName": "新龙县"
          },
          {
            "districtID": 2410,
            "cityID": 259,
            "divisionCode": "513330",
            "districtName": "德格县"
          },
          {
            "districtID": 2411,
            "cityID": 259,
            "divisionCode": "513331",
            "districtName": "白玉县"
          },
          {
            "districtID": 2412,
            "cityID": 259,
            "divisionCode": "513332",
            "districtName": "石渠县"
          },
          {
            "districtID": 2413,
            "cityID": 259,
            "divisionCode": "513333",
            "districtName": "色达县"
          },
          {
            "districtID": 2414,
            "cityID": 259,
            "divisionCode": "513334",
            "districtName": "理塘县"
          },
          {
            "districtID": 2415,
            "cityID": 259,
            "divisionCode": "513335",
            "districtName": "巴塘县"
          },
          {
            "districtID": 2416,
            "cityID": 259,
            "divisionCode": "513336",
            "districtName": "乡城县"
          },
          {
            "districtID": 2417,
            "cityID": 259,
            "divisionCode": "513337",
            "districtName": "稻城县"
          },
          {
            "districtID": 2418,
            "cityID": 259,
            "divisionCode": "513338",
            "districtName": "得荣县"
          },
          {
            "districtID": 2419,
            "cityID": 260,
            "divisionCode": "513401",
            "districtName": "西昌市"
          },
          {
            "districtID": 2420,
            "cityID": 260,
            "divisionCode": "513422",
            "districtName": "木里藏族自治县"
          },
          {
            "districtID": 2421,
            "cityID": 260,
            "divisionCode": "513423",
            "districtName": "盐源县"
          },
          {
            "districtID": 2422,
            "cityID": 260,
            "divisionCode": "513424",
            "districtName": "德昌县"
          },
          {
            "districtID": 2423,
            "cityID": 260,
            "divisionCode": "513425",
            "districtName": "会理县"
          },
          {
            "districtID": 2424,
            "cityID": 260,
            "divisionCode": "513426",
            "districtName": "会东县"
          },
          {
            "districtID": 2425,
            "cityID": 260,
            "divisionCode": "513427",
            "districtName": "宁南县"
          },
          {
            "districtID": 2426,
            "cityID": 260,
            "divisionCode": "513428",
            "districtName": "普格县"
          },
          {
            "districtID": 2427,
            "cityID": 260,
            "divisionCode": "513429",
            "districtName": "布拖县"
          },
          {
            "districtID": 2428,
            "cityID": 260,
            "divisionCode": "513430",
            "districtName": "金阳县"
          },
          {
            "districtID": 2429,
            "cityID": 260,
            "divisionCode": "513431",
            "districtName": "昭觉县"
          },
          {
            "districtID": 2430,
            "cityID": 260,
            "divisionCode": "513432",
            "districtName": "喜德县"
          },
          {
            "districtID": 2431,
            "cityID": 260,
            "divisionCode": "513433",
            "districtName": "冕宁县"
          },
          {
            "districtID": 2432,
            "cityID": 260,
            "divisionCode": "513434",
            "districtName": "越西县"
          },
          {
            "districtID": 2433,
            "cityID": 260,
            "divisionCode": "513435",
            "districtName": "甘洛县"
          },
          {
            "districtID": 2434,
            "cityID": 260,
            "divisionCode": "513436",
            "districtName": "美姑县"
          },
          {
            "districtID": 2435,
            "cityID": 260,
            "divisionCode": "513437",
            "districtName": "雷波县"
          },
          {
            "districtID": 2436,
            "cityID": 261,
            "divisionCode": "520101",
            "districtName": "市辖区"
          },
          {
            "districtID": 2437,
            "cityID": 261,
            "divisionCode": "520102",
            "districtName": "南明区"
          },
          {
            "districtID": 2438,
            "cityID": 261,
            "divisionCode": "520103",
            "districtName": "云岩区"
          },
          {
            "districtID": 2439,
            "cityID": 261,
            "divisionCode": "520111",
            "districtName": "花溪区"
          },
          {
            "districtID": 2440,
            "cityID": 261,
            "divisionCode": "520112",
            "districtName": "乌当区"
          },
          {
            "districtID": 2441,
            "cityID": 261,
            "divisionCode": "520113",
            "districtName": "白云区"
          },
          {
            "districtID": 2442,
            "cityID": 261,
            "divisionCode": "520115",
            "districtName": "观山湖区"
          },
          {
            "districtID": 2443,
            "cityID": 261,
            "divisionCode": "520121",
            "districtName": "开阳县"
          },
          {
            "districtID": 2444,
            "cityID": 261,
            "divisionCode": "520122",
            "districtName": "息烽县"
          },
          {
            "districtID": 2445,
            "cityID": 261,
            "divisionCode": "520123",
            "districtName": "修文县"
          },
          {
            "districtID": 2446,
            "cityID": 261,
            "divisionCode": "520181",
            "districtName": "清镇市"
          },
          {
            "districtID": 2447,
            "cityID": 262,
            "divisionCode": "520201",
            "districtName": "钟山区"
          },
          {
            "districtID": 2448,
            "cityID": 262,
            "divisionCode": "520203",
            "districtName": "六枝特区"
          },
          {
            "districtID": 2449,
            "cityID": 262,
            "divisionCode": "520221",
            "districtName": "水城县"
          },
          {
            "districtID": 2450,
            "cityID": 262,
            "divisionCode": "520222",
            "districtName": "盘县"
          },
          {
            "districtID": 2451,
            "cityID": 263,
            "divisionCode": "520301",
            "districtName": "市辖区"
          },
          {
            "districtID": 2452,
            "cityID": 263,
            "divisionCode": "520302",
            "districtName": "红花岗区"
          },
          {
            "districtID": 2453,
            "cityID": 263,
            "divisionCode": "520303",
            "districtName": "汇川区"
          },
          {
            "districtID": 2454,
            "cityID": 263,
            "divisionCode": "520304",
            "districtName": "播州区"
          },
          {
            "districtID": 2455,
            "cityID": 263,
            "divisionCode": "520322",
            "districtName": "桐梓县"
          },
          {
            "districtID": 2456,
            "cityID": 263,
            "divisionCode": "520323",
            "districtName": "绥阳县"
          },
          {
            "districtID": 2457,
            "cityID": 263,
            "divisionCode": "520324",
            "districtName": "正安县"
          },
          {
            "districtID": 2458,
            "cityID": 263,
            "divisionCode": "520325",
            "districtName": "道真仡佬族苗族自治县"
          },
          {
            "districtID": 2459,
            "cityID": 263,
            "divisionCode": "520326",
            "districtName": "务川仡佬族苗族自治县"
          },
          {
            "districtID": 2460,
            "cityID": 263,
            "divisionCode": "520327",
            "districtName": "凤冈县"
          },
          {
            "districtID": 2461,
            "cityID": 263,
            "divisionCode": "520328",
            "districtName": "湄潭县"
          },
          {
            "districtID": 2462,
            "cityID": 263,
            "divisionCode": "520329",
            "districtName": "余庆县"
          },
          {
            "districtID": 2463,
            "cityID": 263,
            "divisionCode": "520330",
            "districtName": "习水县"
          },
          {
            "districtID": 2464,
            "cityID": 263,
            "divisionCode": "520381",
            "districtName": "赤水市"
          },
          {
            "districtID": 2465,
            "cityID": 263,
            "divisionCode": "520382",
            "districtName": "仁怀市"
          },
          {
            "districtID": 2466,
            "cityID": 264,
            "divisionCode": "520401",
            "districtName": "市辖区"
          },
          {
            "districtID": 2467,
            "cityID": 264,
            "divisionCode": "520402",
            "districtName": "西秀区"
          },
          {
            "districtID": 2468,
            "cityID": 264,
            "divisionCode": "520403",
            "districtName": "平坝区"
          },
          {
            "districtID": 2469,
            "cityID": 264,
            "divisionCode": "520422",
            "districtName": "普定县"
          },
          {
            "districtID": 2470,
            "cityID": 264,
            "divisionCode": "520423",
            "districtName": "镇宁布依族苗族自治县"
          },
          {
            "districtID": 2471,
            "cityID": 264,
            "divisionCode": "520424",
            "districtName": "关岭布依族苗族自治县"
          },
          {
            "districtID": 2472,
            "cityID": 264,
            "divisionCode": "520425",
            "districtName": "紫云苗族布依族自治县"
          },
          {
            "districtID": 2473,
            "cityID": 265,
            "divisionCode": "520501",
            "districtName": "市辖区"
          },
          {
            "districtID": 2474,
            "cityID": 265,
            "divisionCode": "520502",
            "districtName": "七星关区"
          },
          {
            "districtID": 2475,
            "cityID": 265,
            "divisionCode": "520521",
            "districtName": "大方县"
          },
          {
            "districtID": 2476,
            "cityID": 265,
            "divisionCode": "520522",
            "districtName": "黔西县"
          },
          {
            "districtID": 2477,
            "cityID": 265,
            "divisionCode": "520523",
            "districtName": "金沙县"
          },
          {
            "districtID": 2478,
            "cityID": 265,
            "divisionCode": "520524",
            "districtName": "织金县"
          },
          {
            "districtID": 2479,
            "cityID": 265,
            "divisionCode": "520525",
            "districtName": "纳雍县"
          },
          {
            "districtID": 2480,
            "cityID": 265,
            "divisionCode": "520526",
            "districtName": "威宁彝族回族苗族自治县"
          },
          {
            "districtID": 2481,
            "cityID": 265,
            "divisionCode": "520527",
            "districtName": "赫章县"
          },
          {
            "districtID": 2482,
            "cityID": 266,
            "divisionCode": "520601",
            "districtName": "市辖区"
          },
          {
            "districtID": 2483,
            "cityID": 266,
            "divisionCode": "520602",
            "districtName": "碧江区"
          },
          {
            "districtID": 2484,
            "cityID": 266,
            "divisionCode": "520603",
            "districtName": "万山区"
          },
          {
            "districtID": 2485,
            "cityID": 266,
            "divisionCode": "520621",
            "districtName": "江口县"
          },
          {
            "districtID": 2486,
            "cityID": 266,
            "divisionCode": "520622",
            "districtName": "玉屏侗族自治县"
          },
          {
            "districtID": 2487,
            "cityID": 266,
            "divisionCode": "520623",
            "districtName": "石阡县"
          },
          {
            "districtID": 2488,
            "cityID": 266,
            "divisionCode": "520624",
            "districtName": "思南县"
          },
          {
            "districtID": 2489,
            "cityID": 266,
            "divisionCode": "520625",
            "districtName": "印江土家族苗族自治县"
          },
          {
            "districtID": 2490,
            "cityID": 266,
            "divisionCode": "520626",
            "districtName": "德江县"
          },
          {
            "districtID": 2491,
            "cityID": 266,
            "divisionCode": "520627",
            "districtName": "沿河土家族自治县"
          },
          {
            "districtID": 2492,
            "cityID": 266,
            "divisionCode": "520628",
            "districtName": "松桃苗族自治县"
          },
          {
            "districtID": 2493,
            "cityID": 267,
            "divisionCode": "522301",
            "districtName": "兴义市"
          },
          {
            "districtID": 2494,
            "cityID": 267,
            "divisionCode": "522322",
            "districtName": "兴仁县"
          },
          {
            "districtID": 2495,
            "cityID": 267,
            "divisionCode": "522323",
            "districtName": "普安县"
          },
          {
            "districtID": 2496,
            "cityID": 267,
            "divisionCode": "522324",
            "districtName": "晴隆县"
          },
          {
            "districtID": 2497,
            "cityID": 267,
            "divisionCode": "522325",
            "districtName": "贞丰县"
          },
          {
            "districtID": 2498,
            "cityID": 267,
            "divisionCode": "522326",
            "districtName": "望谟县"
          },
          {
            "districtID": 2499,
            "cityID": 267,
            "divisionCode": "522327",
            "districtName": "册亨县"
          },
          {
            "districtID": 2500,
            "cityID": 267,
            "divisionCode": "522328",
            "districtName": "安龙县"
          },
          {
            "districtID": 2501,
            "cityID": 268,
            "divisionCode": "522601",
            "districtName": "凯里市"
          },
          {
            "districtID": 2502,
            "cityID": 268,
            "divisionCode": "522622",
            "districtName": "黄平县"
          },
          {
            "districtID": 2503,
            "cityID": 268,
            "divisionCode": "522623",
            "districtName": "施秉县"
          },
          {
            "districtID": 2504,
            "cityID": 268,
            "divisionCode": "522624",
            "districtName": "三穗县"
          },
          {
            "districtID": 2505,
            "cityID": 268,
            "divisionCode": "522625",
            "districtName": "镇远县"
          },
          {
            "districtID": 2506,
            "cityID": 268,
            "divisionCode": "522626",
            "districtName": "岑巩县"
          },
          {
            "districtID": 2507,
            "cityID": 268,
            "divisionCode": "522627",
            "districtName": "天柱县"
          },
          {
            "districtID": 2508,
            "cityID": 268,
            "divisionCode": "522628",
            "districtName": "锦屏县"
          },
          {
            "districtID": 2509,
            "cityID": 268,
            "divisionCode": "522629",
            "districtName": "剑河县"
          },
          {
            "districtID": 2510,
            "cityID": 268,
            "divisionCode": "522630",
            "districtName": "台江县"
          },
          {
            "districtID": 2511,
            "cityID": 268,
            "divisionCode": "522631",
            "districtName": "黎平县"
          },
          {
            "districtID": 2512,
            "cityID": 268,
            "divisionCode": "522632",
            "districtName": "榕江县"
          },
          {
            "districtID": 2513,
            "cityID": 268,
            "divisionCode": "522633",
            "districtName": "从江县"
          },
          {
            "districtID": 2514,
            "cityID": 268,
            "divisionCode": "522634",
            "districtName": "雷山县"
          },
          {
            "districtID": 2515,
            "cityID": 268,
            "divisionCode": "522635",
            "districtName": "麻江县"
          },
          {
            "districtID": 2516,
            "cityID": 268,
            "divisionCode": "522636",
            "districtName": "丹寨县"
          },
          {
            "districtID": 2517,
            "cityID": 269,
            "divisionCode": "522701",
            "districtName": "都匀市"
          },
          {
            "districtID": 2518,
            "cityID": 269,
            "divisionCode": "522702",
            "districtName": "福泉市"
          },
          {
            "districtID": 2519,
            "cityID": 269,
            "divisionCode": "522722",
            "districtName": "荔波县"
          },
          {
            "districtID": 2520,
            "cityID": 269,
            "divisionCode": "522723",
            "districtName": "贵定县"
          },
          {
            "districtID": 2521,
            "cityID": 269,
            "divisionCode": "522725",
            "districtName": "瓮安县"
          },
          {
            "districtID": 2522,
            "cityID": 269,
            "divisionCode": "522726",
            "districtName": "独山县"
          },
          {
            "districtID": 2523,
            "cityID": 269,
            "divisionCode": "522727",
            "districtName": "平塘县"
          },
          {
            "districtID": 2524,
            "cityID": 269,
            "divisionCode": "522728",
            "districtName": "罗甸县"
          },
          {
            "districtID": 2525,
            "cityID": 269,
            "divisionCode": "522729",
            "districtName": "长顺县"
          },
          {
            "districtID": 2526,
            "cityID": 269,
            "divisionCode": "522730",
            "districtName": "龙里县"
          },
          {
            "districtID": 2527,
            "cityID": 269,
            "divisionCode": "522731",
            "districtName": "惠水县"
          },
          {
            "districtID": 2528,
            "cityID": 269,
            "divisionCode": "522732",
            "districtName": "三都水族自治县"
          },
          {
            "districtID": 2529,
            "cityID": 270,
            "divisionCode": "530101",
            "districtName": "市辖区"
          },
          {
            "districtID": 2530,
            "cityID": 270,
            "divisionCode": "530102",
            "districtName": "五华区"
          },
          {
            "districtID": 2531,
            "cityID": 270,
            "divisionCode": "530103",
            "districtName": "盘龙区"
          },
          {
            "districtID": 2532,
            "cityID": 270,
            "divisionCode": "530111",
            "districtName": "官渡区"
          },
          {
            "districtID": 2533,
            "cityID": 270,
            "divisionCode": "530112",
            "districtName": "西山区"
          },
          {
            "districtID": 2534,
            "cityID": 270,
            "divisionCode": "530113",
            "districtName": "东川区"
          },
          {
            "districtID": 2535,
            "cityID": 270,
            "divisionCode": "530114",
            "districtName": "呈贡区"
          },
          {
            "districtID": 2536,
            "cityID": 270,
            "divisionCode": "530122",
            "districtName": "晋宁县"
          },
          {
            "districtID": 2537,
            "cityID": 270,
            "divisionCode": "530124",
            "districtName": "富民县"
          },
          {
            "districtID": 2538,
            "cityID": 270,
            "divisionCode": "530125",
            "districtName": "宜良县"
          },
          {
            "districtID": 2539,
            "cityID": 270,
            "divisionCode": "530126",
            "districtName": "石林彝族自治县"
          },
          {
            "districtID": 2540,
            "cityID": 270,
            "divisionCode": "530127",
            "districtName": "嵩明县"
          },
          {
            "districtID": 2541,
            "cityID": 270,
            "divisionCode": "530128",
            "districtName": "禄劝彝族苗族自治县"
          },
          {
            "districtID": 2542,
            "cityID": 270,
            "divisionCode": "530129",
            "districtName": "寻甸回族彝族自治县"
          },
          {
            "districtID": 2543,
            "cityID": 270,
            "divisionCode": "530181",
            "districtName": "安宁市"
          },
          {
            "districtID": 2544,
            "cityID": 271,
            "divisionCode": "530301",
            "districtName": "市辖区"
          },
          {
            "districtID": 2545,
            "cityID": 271,
            "divisionCode": "530302",
            "districtName": "麒麟区"
          },
          {
            "districtID": 2546,
            "cityID": 271,
            "divisionCode": "530303",
            "districtName": "沾益区"
          },
          {
            "districtID": 2547,
            "cityID": 271,
            "divisionCode": "530321",
            "districtName": "马龙县"
          },
          {
            "districtID": 2548,
            "cityID": 271,
            "divisionCode": "530322",
            "districtName": "陆良县"
          },
          {
            "districtID": 2549,
            "cityID": 271,
            "divisionCode": "530323",
            "districtName": "师宗县"
          },
          {
            "districtID": 2550,
            "cityID": 271,
            "divisionCode": "530324",
            "districtName": "罗平县"
          },
          {
            "districtID": 2551,
            "cityID": 271,
            "divisionCode": "530325",
            "districtName": "富源县"
          },
          {
            "districtID": 2552,
            "cityID": 271,
            "divisionCode": "530326",
            "districtName": "会泽县"
          },
          {
            "districtID": 2553,
            "cityID": 271,
            "divisionCode": "530381",
            "districtName": "宣威市"
          },
          {
            "districtID": 2554,
            "cityID": 272,
            "divisionCode": "530401",
            "districtName": "市辖区"
          },
          {
            "districtID": 2555,
            "cityID": 272,
            "divisionCode": "530402",
            "districtName": "红塔区"
          },
          {
            "districtID": 2556,
            "cityID": 272,
            "divisionCode": "530403",
            "districtName": "江川区"
          },
          {
            "districtID": 2557,
            "cityID": 272,
            "divisionCode": "530422",
            "districtName": "澄江县"
          },
          {
            "districtID": 2558,
            "cityID": 272,
            "divisionCode": "530423",
            "districtName": "通海县"
          },
          {
            "districtID": 2559,
            "cityID": 272,
            "divisionCode": "530424",
            "districtName": "华宁县"
          },
          {
            "districtID": 2560,
            "cityID": 272,
            "divisionCode": "530425",
            "districtName": "易门县"
          },
          {
            "districtID": 2561,
            "cityID": 272,
            "divisionCode": "530426",
            "districtName": "峨山彝族自治县"
          },
          {
            "districtID": 2562,
            "cityID": 272,
            "divisionCode": "530427",
            "districtName": "新平彝族傣族自治县"
          },
          {
            "districtID": 2563,
            "cityID": 272,
            "divisionCode": "530428",
            "districtName": "元江哈尼族彝族傣族自治县"
          },
          {
            "districtID": 2564,
            "cityID": 273,
            "divisionCode": "530501",
            "districtName": "市辖区"
          },
          {
            "districtID": 2565,
            "cityID": 273,
            "divisionCode": "530502",
            "districtName": "隆阳区"
          },
          {
            "districtID": 2566,
            "cityID": 273,
            "divisionCode": "530521",
            "districtName": "施甸县"
          },
          {
            "districtID": 2567,
            "cityID": 273,
            "divisionCode": "530523",
            "districtName": "龙陵县"
          },
          {
            "districtID": 2568,
            "cityID": 273,
            "divisionCode": "530524",
            "districtName": "昌宁县"
          },
          {
            "districtID": 2569,
            "cityID": 273,
            "divisionCode": "530581",
            "districtName": "腾冲市"
          },
          {
            "districtID": 2570,
            "cityID": 274,
            "divisionCode": "530601",
            "districtName": "市辖区"
          },
          {
            "districtID": 2571,
            "cityID": 274,
            "divisionCode": "530602",
            "districtName": "昭阳区"
          },
          {
            "districtID": 2572,
            "cityID": 274,
            "divisionCode": "530621",
            "districtName": "鲁甸县"
          },
          {
            "districtID": 2573,
            "cityID": 274,
            "divisionCode": "530622",
            "districtName": "巧家县"
          },
          {
            "districtID": 2574,
            "cityID": 274,
            "divisionCode": "530623",
            "districtName": "盐津县"
          },
          {
            "districtID": 2575,
            "cityID": 274,
            "divisionCode": "530624",
            "districtName": "大关县"
          },
          {
            "districtID": 2576,
            "cityID": 274,
            "divisionCode": "530625",
            "districtName": "永善县"
          },
          {
            "districtID": 2577,
            "cityID": 274,
            "divisionCode": "530626",
            "districtName": "绥江县"
          },
          {
            "districtID": 2578,
            "cityID": 274,
            "divisionCode": "530627",
            "districtName": "镇雄县"
          },
          {
            "districtID": 2579,
            "cityID": 274,
            "divisionCode": "530628",
            "districtName": "彝良县"
          },
          {
            "districtID": 2580,
            "cityID": 274,
            "divisionCode": "530629",
            "districtName": "威信县"
          },
          {
            "districtID": 2581,
            "cityID": 274,
            "divisionCode": "530630",
            "districtName": "水富县"
          },
          {
            "districtID": 2582,
            "cityID": 275,
            "divisionCode": "530701",
            "districtName": "市辖区"
          },
          {
            "districtID": 2583,
            "cityID": 275,
            "divisionCode": "530702",
            "districtName": "古城区"
          },
          {
            "districtID": 2584,
            "cityID": 275,
            "divisionCode": "530721",
            "districtName": "玉龙纳西族自治县"
          },
          {
            "districtID": 2585,
            "cityID": 275,
            "divisionCode": "530722",
            "districtName": "永胜县"
          },
          {
            "districtID": 2586,
            "cityID": 275,
            "divisionCode": "530723",
            "districtName": "华坪县"
          },
          {
            "districtID": 2587,
            "cityID": 275,
            "divisionCode": "530724",
            "districtName": "宁蒗彝族自治县"
          },
          {
            "districtID": 2588,
            "cityID": 276,
            "divisionCode": "530801",
            "districtName": "市辖区"
          },
          {
            "districtID": 2589,
            "cityID": 276,
            "divisionCode": "530802",
            "districtName": "思茅区"
          },
          {
            "districtID": 2590,
            "cityID": 276,
            "divisionCode": "530821",
            "districtName": "宁洱哈尼族彝族自治县"
          },
          {
            "districtID": 2591,
            "cityID": 276,
            "divisionCode": "530822",
            "districtName": "墨江哈尼族自治县"
          },
          {
            "districtID": 2592,
            "cityID": 276,
            "divisionCode": "530823",
            "districtName": "景东彝族自治县"
          },
          {
            "districtID": 2593,
            "cityID": 276,
            "divisionCode": "530824",
            "districtName": "景谷傣族彝族自治县"
          },
          {
            "districtID": 2594,
            "cityID": 276,
            "divisionCode": "530825",
            "districtName": "镇沅彝族哈尼族拉祜族自治县"
          },
          {
            "districtID": 2595,
            "cityID": 276,
            "divisionCode": "530826",
            "districtName": "江城哈尼族彝族自治县"
          },
          {
            "districtID": 2596,
            "cityID": 276,
            "divisionCode": "530827",
            "districtName": "孟连傣族拉祜族佤族自治县"
          },
          {
            "districtID": 2597,
            "cityID": 276,
            "divisionCode": "530828",
            "districtName": "澜沧拉祜族自治县"
          },
          {
            "districtID": 2598,
            "cityID": 276,
            "divisionCode": "530829",
            "districtName": "西盟佤族自治县"
          },
          {
            "districtID": 2599,
            "cityID": 277,
            "divisionCode": "530901",
            "districtName": "市辖区"
          },
          {
            "districtID": 2600,
            "cityID": 277,
            "divisionCode": "530902",
            "districtName": "临翔区"
          },
          {
            "districtID": 2601,
            "cityID": 277,
            "divisionCode": "530921",
            "districtName": "凤庆县"
          },
          {
            "districtID": 2602,
            "cityID": 277,
            "divisionCode": "530922",
            "districtName": "云县"
          },
          {
            "districtID": 2603,
            "cityID": 277,
            "divisionCode": "530923",
            "districtName": "永德县"
          },
          {
            "districtID": 2604,
            "cityID": 277,
            "divisionCode": "530924",
            "districtName": "镇康县"
          },
          {
            "districtID": 2605,
            "cityID": 277,
            "divisionCode": "530925",
            "districtName": "双江拉祜族佤族布朗族傣族自治县"
          },
          {
            "districtID": 2606,
            "cityID": 277,
            "divisionCode": "530926",
            "districtName": "耿马傣族佤族自治县"
          },
          {
            "districtID": 2607,
            "cityID": 277,
            "divisionCode": "530927",
            "districtName": "沧源佤族自治县"
          },
          {
            "districtID": 2608,
            "cityID": 278,
            "divisionCode": "532301",
            "districtName": "楚雄市"
          },
          {
            "districtID": 2609,
            "cityID": 278,
            "divisionCode": "532322",
            "districtName": "双柏县"
          },
          {
            "districtID": 2610,
            "cityID": 278,
            "divisionCode": "532323",
            "districtName": "牟定县"
          },
          {
            "districtID": 2611,
            "cityID": 278,
            "divisionCode": "532324",
            "districtName": "南华县"
          },
          {
            "districtID": 2612,
            "cityID": 278,
            "divisionCode": "532325",
            "districtName": "姚安县"
          },
          {
            "districtID": 2613,
            "cityID": 278,
            "divisionCode": "532326",
            "districtName": "大姚县"
          },
          {
            "districtID": 2614,
            "cityID": 278,
            "divisionCode": "532327",
            "districtName": "永仁县"
          },
          {
            "districtID": 2615,
            "cityID": 278,
            "divisionCode": "532328",
            "districtName": "元谋县"
          },
          {
            "districtID": 2616,
            "cityID": 278,
            "divisionCode": "532329",
            "districtName": "武定县"
          },
          {
            "districtID": 2617,
            "cityID": 278,
            "divisionCode": "532331",
            "districtName": "禄丰县"
          },
          {
            "districtID": 2618,
            "cityID": 279,
            "divisionCode": "532501",
            "districtName": "个旧市"
          },
          {
            "districtID": 2619,
            "cityID": 279,
            "divisionCode": "532502",
            "districtName": "开远市"
          },
          {
            "districtID": 2620,
            "cityID": 279,
            "divisionCode": "532503",
            "districtName": "蒙自市"
          },
          {
            "districtID": 2621,
            "cityID": 279,
            "divisionCode": "532504",
            "districtName": "弥勒市"
          },
          {
            "districtID": 2622,
            "cityID": 279,
            "divisionCode": "532523",
            "districtName": "屏边苗族自治县"
          },
          {
            "districtID": 2623,
            "cityID": 279,
            "divisionCode": "532524",
            "districtName": "建水县"
          },
          {
            "districtID": 2624,
            "cityID": 279,
            "divisionCode": "532525",
            "districtName": "石屏县"
          },
          {
            "districtID": 2625,
            "cityID": 279,
            "divisionCode": "532527",
            "districtName": "泸西县"
          },
          {
            "districtID": 2626,
            "cityID": 279,
            "divisionCode": "532528",
            "districtName": "元阳县"
          },
          {
            "districtID": 2627,
            "cityID": 279,
            "divisionCode": "532529",
            "districtName": "红河县"
          },
          {
            "districtID": 2628,
            "cityID": 279,
            "divisionCode": "532530",
            "districtName": "金平苗族瑶族傣族自治县"
          },
          {
            "districtID": 2629,
            "cityID": 279,
            "divisionCode": "532531",
            "districtName": "绿春县"
          },
          {
            "districtID": 2630,
            "cityID": 279,
            "divisionCode": "532532",
            "districtName": "河口瑶族自治县"
          },
          {
            "districtID": 2631,
            "cityID": 280,
            "divisionCode": "532601",
            "districtName": "文山市"
          },
          {
            "districtID": 2632,
            "cityID": 280,
            "divisionCode": "532622",
            "districtName": "砚山县"
          },
          {
            "districtID": 2633,
            "cityID": 280,
            "divisionCode": "532623",
            "districtName": "西畴县"
          },
          {
            "districtID": 2634,
            "cityID": 280,
            "divisionCode": "532624",
            "districtName": "麻栗坡县"
          },
          {
            "districtID": 2635,
            "cityID": 280,
            "divisionCode": "532625",
            "districtName": "马关县"
          },
          {
            "districtID": 2636,
            "cityID": 280,
            "divisionCode": "532626",
            "districtName": "丘北县"
          },
          {
            "districtID": 2637,
            "cityID": 280,
            "divisionCode": "532627",
            "districtName": "广南县"
          },
          {
            "districtID": 2638,
            "cityID": 280,
            "divisionCode": "532628",
            "districtName": "富宁县"
          },
          {
            "districtID": 2639,
            "cityID": 281,
            "divisionCode": "532801",
            "districtName": "景洪市"
          },
          {
            "districtID": 2640,
            "cityID": 281,
            "divisionCode": "532822",
            "districtName": "勐海县"
          },
          {
            "districtID": 2641,
            "cityID": 281,
            "divisionCode": "532823",
            "districtName": "勐腊县"
          },
          {
            "districtID": 2642,
            "cityID": 282,
            "divisionCode": "532901",
            "districtName": "大理市"
          },
          {
            "districtID": 2643,
            "cityID": 282,
            "divisionCode": "532922",
            "districtName": "漾濞彝族自治县"
          },
          {
            "districtID": 2644,
            "cityID": 282,
            "divisionCode": "532923",
            "districtName": "祥云县"
          },
          {
            "districtID": 2645,
            "cityID": 282,
            "divisionCode": "532924",
            "districtName": "宾川县"
          },
          {
            "districtID": 2646,
            "cityID": 282,
            "divisionCode": "532925",
            "districtName": "弥渡县"
          },
          {
            "districtID": 2647,
            "cityID": 282,
            "divisionCode": "532926",
            "districtName": "南涧彝族自治县"
          },
          {
            "districtID": 2648,
            "cityID": 282,
            "divisionCode": "532927",
            "districtName": "巍山彝族回族自治县"
          },
          {
            "districtID": 2649,
            "cityID": 282,
            "divisionCode": "532928",
            "districtName": "永平县"
          },
          {
            "districtID": 2650,
            "cityID": 282,
            "divisionCode": "532929",
            "districtName": "云龙县"
          },
          {
            "districtID": 2651,
            "cityID": 282,
            "divisionCode": "532930",
            "districtName": "洱源县"
          },
          {
            "districtID": 2652,
            "cityID": 282,
            "divisionCode": "532931",
            "districtName": "剑川县"
          },
          {
            "districtID": 2653,
            "cityID": 282,
            "divisionCode": "532932",
            "districtName": "鹤庆县"
          },
          {
            "districtID": 2654,
            "cityID": 283,
            "divisionCode": "533102",
            "districtName": "瑞丽市"
          },
          {
            "districtID": 2655,
            "cityID": 283,
            "divisionCode": "533103",
            "districtName": "芒市"
          },
          {
            "districtID": 2656,
            "cityID": 283,
            "divisionCode": "533122",
            "districtName": "梁河县"
          },
          {
            "districtID": 2657,
            "cityID": 283,
            "divisionCode": "533123",
            "districtName": "盈江县"
          },
          {
            "districtID": 2658,
            "cityID": 283,
            "divisionCode": "533124",
            "districtName": "陇川县"
          },
          {
            "districtID": 2659,
            "cityID": 284,
            "divisionCode": "533301",
            "districtName": "泸水市"
          },
          {
            "districtID": 2660,
            "cityID": 284,
            "divisionCode": "533323",
            "districtName": "福贡县"
          },
          {
            "districtID": 2661,
            "cityID": 284,
            "divisionCode": "533324",
            "districtName": "贡山独龙族怒族自治县"
          },
          {
            "districtID": 2662,
            "cityID": 284,
            "divisionCode": "533325",
            "districtName": "兰坪白族普米族自治县"
          },
          {
            "districtID": 2663,
            "cityID": 285,
            "divisionCode": "533401",
            "districtName": "香格里拉市"
          },
          {
            "districtID": 2664,
            "cityID": 285,
            "divisionCode": "533422",
            "districtName": "德钦县"
          },
          {
            "districtID": 2665,
            "cityID": 285,
            "divisionCode": "533423",
            "districtName": "维西傈僳族自治县"
          },
          {
            "districtID": 2666,
            "cityID": 286,
            "divisionCode": "540101",
            "districtName": "市辖区"
          },
          {
            "districtID": 2667,
            "cityID": 286,
            "divisionCode": "540102",
            "districtName": "城关区"
          },
          {
            "districtID": 2668,
            "cityID": 286,
            "divisionCode": "540103",
            "districtName": "堆龙德庆区"
          },
          {
            "districtID": 2669,
            "cityID": 286,
            "divisionCode": "540121",
            "districtName": "林周县"
          },
          {
            "districtID": 2670,
            "cityID": 286,
            "divisionCode": "540122",
            "districtName": "当雄县"
          },
          {
            "districtID": 2671,
            "cityID": 286,
            "divisionCode": "540123",
            "districtName": "尼木县"
          },
          {
            "districtID": 2672,
            "cityID": 286,
            "divisionCode": "540124",
            "districtName": "曲水县"
          },
          {
            "districtID": 2673,
            "cityID": 286,
            "divisionCode": "540126",
            "districtName": "达孜县"
          },
          {
            "districtID": 2674,
            "cityID": 286,
            "divisionCode": "540127",
            "districtName": "墨竹工卡县"
          },
          {
            "districtID": 2675,
            "cityID": 287,
            "divisionCode": "540202",
            "districtName": "桑珠孜区"
          },
          {
            "districtID": 2676,
            "cityID": 287,
            "divisionCode": "540221",
            "districtName": "南木林县"
          },
          {
            "districtID": 2677,
            "cityID": 287,
            "divisionCode": "540222",
            "districtName": "江孜县"
          },
          {
            "districtID": 2678,
            "cityID": 287,
            "divisionCode": "540223",
            "districtName": "定日县"
          },
          {
            "districtID": 2679,
            "cityID": 287,
            "divisionCode": "540224",
            "districtName": "萨迦县"
          },
          {
            "districtID": 2680,
            "cityID": 287,
            "divisionCode": "540225",
            "districtName": "拉孜县"
          },
          {
            "districtID": 2681,
            "cityID": 287,
            "divisionCode": "540226",
            "districtName": "昂仁县"
          },
          {
            "districtID": 2682,
            "cityID": 287,
            "divisionCode": "540227",
            "districtName": "谢通门县"
          },
          {
            "districtID": 2683,
            "cityID": 287,
            "divisionCode": "540228",
            "districtName": "白朗县"
          },
          {
            "districtID": 2684,
            "cityID": 287,
            "divisionCode": "540229",
            "districtName": "仁布县"
          },
          {
            "districtID": 2685,
            "cityID": 287,
            "divisionCode": "540230",
            "districtName": "康马县"
          },
          {
            "districtID": 2686,
            "cityID": 287,
            "divisionCode": "540231",
            "districtName": "定结县"
          },
          {
            "districtID": 2687,
            "cityID": 287,
            "divisionCode": "540232",
            "districtName": "仲巴县"
          },
          {
            "districtID": 2688,
            "cityID": 287,
            "divisionCode": "540233",
            "districtName": "亚东县"
          },
          {
            "districtID": 2689,
            "cityID": 287,
            "divisionCode": "540234",
            "districtName": "吉隆县"
          },
          {
            "districtID": 2690,
            "cityID": 287,
            "divisionCode": "540235",
            "districtName": "聂拉木县"
          },
          {
            "districtID": 2691,
            "cityID": 287,
            "divisionCode": "540236",
            "districtName": "萨嘎县"
          },
          {
            "districtID": 2692,
            "cityID": 287,
            "divisionCode": "540237",
            "districtName": "岗巴县"
          },
          {
            "districtID": 2693,
            "cityID": 288,
            "divisionCode": "540302",
            "districtName": "卡若区"
          },
          {
            "districtID": 2694,
            "cityID": 288,
            "divisionCode": "540321",
            "districtName": "江达县"
          },
          {
            "districtID": 2695,
            "cityID": 288,
            "divisionCode": "540322",
            "districtName": "贡觉县"
          },
          {
            "districtID": 2696,
            "cityID": 288,
            "divisionCode": "540323",
            "districtName": "类乌齐县"
          },
          {
            "districtID": 2697,
            "cityID": 288,
            "divisionCode": "540324",
            "districtName": "丁青县"
          },
          {
            "districtID": 2698,
            "cityID": 288,
            "divisionCode": "540325",
            "districtName": "察雅县"
          },
          {
            "districtID": 2699,
            "cityID": 288,
            "divisionCode": "540326",
            "districtName": "八宿县"
          },
          {
            "districtID": 2700,
            "cityID": 288,
            "divisionCode": "540327",
            "districtName": "左贡县"
          },
          {
            "districtID": 2701,
            "cityID": 288,
            "divisionCode": "540328",
            "districtName": "芒康县"
          },
          {
            "districtID": 2702,
            "cityID": 288,
            "divisionCode": "540329",
            "districtName": "洛隆县"
          },
          {
            "districtID": 2703,
            "cityID": 288,
            "divisionCode": "540330",
            "districtName": "边坝县"
          },
          {
            "districtID": 2704,
            "cityID": 289,
            "divisionCode": "540402",
            "districtName": "巴宜区"
          },
          {
            "districtID": 2705,
            "cityID": 289,
            "divisionCode": "540421",
            "districtName": "工布江达县"
          },
          {
            "districtID": 2706,
            "cityID": 289,
            "divisionCode": "540422",
            "districtName": "米林县"
          },
          {
            "districtID": 2707,
            "cityID": 289,
            "divisionCode": "540423",
            "districtName": "墨脱县"
          },
          {
            "districtID": 2708,
            "cityID": 289,
            "divisionCode": "540424",
            "districtName": "波密县"
          },
          {
            "districtID": 2709,
            "cityID": 289,
            "divisionCode": "540425",
            "districtName": "察隅县"
          },
          {
            "districtID": 2710,
            "cityID": 289,
            "divisionCode": "540426",
            "districtName": "朗县"
          },
          {
            "districtID": 2711,
            "cityID": 290,
            "divisionCode": "540501",
            "districtName": "市辖区"
          },
          {
            "districtID": 2712,
            "cityID": 290,
            "divisionCode": "540502",
            "districtName": "乃东区"
          },
          {
            "districtID": 2713,
            "cityID": 290,
            "divisionCode": "540521",
            "districtName": "扎囊县"
          },
          {
            "districtID": 2714,
            "cityID": 290,
            "divisionCode": "540522",
            "districtName": "贡嘎县"
          },
          {
            "districtID": 2715,
            "cityID": 290,
            "divisionCode": "540523",
            "districtName": "桑日县"
          },
          {
            "districtID": 2716,
            "cityID": 290,
            "divisionCode": "540524",
            "districtName": "琼结县"
          },
          {
            "districtID": 2717,
            "cityID": 290,
            "divisionCode": "540525",
            "districtName": "曲松县"
          },
          {
            "districtID": 2718,
            "cityID": 290,
            "divisionCode": "540526",
            "districtName": "措美县"
          },
          {
            "districtID": 2719,
            "cityID": 290,
            "divisionCode": "540527",
            "districtName": "洛扎县"
          },
          {
            "districtID": 2720,
            "cityID": 290,
            "divisionCode": "540528",
            "districtName": "加查县"
          },
          {
            "districtID": 2721,
            "cityID": 290,
            "divisionCode": "540529",
            "districtName": "隆子县"
          },
          {
            "districtID": 2722,
            "cityID": 290,
            "divisionCode": "540530",
            "districtName": "错那县"
          },
          {
            "districtID": 2723,
            "cityID": 290,
            "divisionCode": "540531",
            "districtName": "浪卡子县"
          },
          {
            "districtID": 2724,
            "cityID": 291,
            "divisionCode": "542421",
            "districtName": "那曲县"
          },
          {
            "districtID": 2725,
            "cityID": 291,
            "divisionCode": "542422",
            "districtName": "嘉黎县"
          },
          {
            "districtID": 2726,
            "cityID": 291,
            "divisionCode": "542423",
            "districtName": "比如县"
          },
          {
            "districtID": 2727,
            "cityID": 291,
            "divisionCode": "542424",
            "districtName": "聂荣县"
          },
          {
            "districtID": 2728,
            "cityID": 291,
            "divisionCode": "542425",
            "districtName": "安多县"
          },
          {
            "districtID": 2729,
            "cityID": 291,
            "divisionCode": "542426",
            "districtName": "申扎县"
          },
          {
            "districtID": 2730,
            "cityID": 291,
            "divisionCode": "542427",
            "districtName": "索县"
          },
          {
            "districtID": 2731,
            "cityID": 291,
            "divisionCode": "542428",
            "districtName": "班戈县"
          },
          {
            "districtID": 2732,
            "cityID": 291,
            "divisionCode": "542429",
            "districtName": "巴青县"
          },
          {
            "districtID": 2733,
            "cityID": 291,
            "divisionCode": "542430",
            "districtName": "尼玛县"
          },
          {
            "districtID": 2734,
            "cityID": 291,
            "divisionCode": "542431",
            "districtName": "双湖县"
          },
          {
            "districtID": 2735,
            "cityID": 292,
            "divisionCode": "542521",
            "districtName": "普兰县"
          },
          {
            "districtID": 2736,
            "cityID": 292,
            "divisionCode": "542522",
            "districtName": "札达县"
          },
          {
            "districtID": 2737,
            "cityID": 292,
            "divisionCode": "542523",
            "districtName": "噶尔县"
          },
          {
            "districtID": 2738,
            "cityID": 292,
            "divisionCode": "542524",
            "districtName": "日土县"
          },
          {
            "districtID": 2739,
            "cityID": 292,
            "divisionCode": "542525",
            "districtName": "革吉县"
          },
          {
            "districtID": 2740,
            "cityID": 292,
            "divisionCode": "542526",
            "districtName": "改则县"
          },
          {
            "districtID": 2741,
            "cityID": 292,
            "divisionCode": "542527",
            "districtName": "措勤县"
          },
          {
            "districtID": 2742,
            "cityID": 293,
            "divisionCode": "610101",
            "districtName": "市辖区"
          },
          {
            "districtID": 2743,
            "cityID": 293,
            "divisionCode": "610102",
            "districtName": "新城区"
          },
          {
            "districtID": 2744,
            "cityID": 293,
            "divisionCode": "610103",
            "districtName": "碑林区"
          },
          {
            "districtID": 2745,
            "cityID": 293,
            "divisionCode": "610104",
            "districtName": "莲湖区"
          },
          {
            "districtID": 2746,
            "cityID": 293,
            "divisionCode": "610111",
            "districtName": "灞桥区"
          },
          {
            "districtID": 2747,
            "cityID": 293,
            "divisionCode": "610112",
            "districtName": "未央区"
          },
          {
            "districtID": 2748,
            "cityID": 293,
            "divisionCode": "610113",
            "districtName": "雁塔区"
          },
          {
            "districtID": 2749,
            "cityID": 293,
            "divisionCode": "610114",
            "districtName": "阎良区"
          },
          {
            "districtID": 2750,
            "cityID": 293,
            "divisionCode": "610115",
            "districtName": "临潼区"
          },
          {
            "districtID": 2751,
            "cityID": 293,
            "divisionCode": "610116",
            "districtName": "长安区"
          },
          {
            "districtID": 2752,
            "cityID": 293,
            "divisionCode": "610117",
            "districtName": "高陵区"
          },
          {
            "districtID": 2753,
            "cityID": 293,
            "divisionCode": "610122",
            "districtName": "蓝田县"
          },
          {
            "districtID": 2754,
            "cityID": 293,
            "divisionCode": "610124",
            "districtName": "周至县"
          },
          {
            "districtID": 2755,
            "cityID": 293,
            "divisionCode": "610125",
            "districtName": "户县"
          },
          {
            "districtID": 2756,
            "cityID": 294,
            "divisionCode": "610201",
            "districtName": "市辖区"
          },
          {
            "districtID": 2757,
            "cityID": 294,
            "divisionCode": "610202",
            "districtName": "王益区"
          },
          {
            "districtID": 2758,
            "cityID": 294,
            "divisionCode": "610203",
            "districtName": "印台区"
          },
          {
            "districtID": 2759,
            "cityID": 294,
            "divisionCode": "610204",
            "districtName": "耀州区"
          },
          {
            "districtID": 2760,
            "cityID": 294,
            "divisionCode": "610222",
            "districtName": "宜君县"
          },
          {
            "districtID": 2761,
            "cityID": 295,
            "divisionCode": "610301",
            "districtName": "市辖区"
          },
          {
            "districtID": 2762,
            "cityID": 295,
            "divisionCode": "610302",
            "districtName": "渭滨区"
          },
          {
            "districtID": 2763,
            "cityID": 295,
            "divisionCode": "610303",
            "districtName": "金台区"
          },
          {
            "districtID": 2764,
            "cityID": 295,
            "divisionCode": "610304",
            "districtName": "陈仓区"
          },
          {
            "districtID": 2765,
            "cityID": 295,
            "divisionCode": "610322",
            "districtName": "凤翔县"
          },
          {
            "districtID": 2766,
            "cityID": 295,
            "divisionCode": "610323",
            "districtName": "岐山县"
          },
          {
            "districtID": 2767,
            "cityID": 295,
            "divisionCode": "610324",
            "districtName": "扶风县"
          },
          {
            "districtID": 2768,
            "cityID": 295,
            "divisionCode": "610326",
            "districtName": "眉县"
          },
          {
            "districtID": 2769,
            "cityID": 295,
            "divisionCode": "610327",
            "districtName": "陇县"
          },
          {
            "districtID": 2770,
            "cityID": 295,
            "divisionCode": "610328",
            "districtName": "千阳县"
          },
          {
            "districtID": 2771,
            "cityID": 295,
            "divisionCode": "610329",
            "districtName": "麟游县"
          },
          {
            "districtID": 2772,
            "cityID": 295,
            "divisionCode": "610330",
            "districtName": "凤县"
          },
          {
            "districtID": 2773,
            "cityID": 295,
            "divisionCode": "610331",
            "districtName": "太白县"
          },
          {
            "districtID": 2774,
            "cityID": 296,
            "divisionCode": "610401",
            "districtName": "市辖区"
          },
          {
            "districtID": 2775,
            "cityID": 296,
            "divisionCode": "610402",
            "districtName": "秦都区"
          },
          {
            "districtID": 2776,
            "cityID": 296,
            "divisionCode": "610403",
            "districtName": "杨陵区"
          },
          {
            "districtID": 2777,
            "cityID": 296,
            "divisionCode": "610404",
            "districtName": "渭城区"
          },
          {
            "districtID": 2778,
            "cityID": 296,
            "divisionCode": "610422",
            "districtName": "三原县"
          },
          {
            "districtID": 2779,
            "cityID": 296,
            "divisionCode": "610423",
            "districtName": "泾阳县"
          },
          {
            "districtID": 2780,
            "cityID": 296,
            "divisionCode": "610424",
            "districtName": "乾县"
          },
          {
            "districtID": 2781,
            "cityID": 296,
            "divisionCode": "610425",
            "districtName": "礼泉县"
          },
          {
            "districtID": 2782,
            "cityID": 296,
            "divisionCode": "610426",
            "districtName": "永寿县"
          },
          {
            "districtID": 2783,
            "cityID": 296,
            "divisionCode": "610427",
            "districtName": "彬县"
          },
          {
            "districtID": 2784,
            "cityID": 296,
            "divisionCode": "610428",
            "districtName": "长武县"
          },
          {
            "districtID": 2785,
            "cityID": 296,
            "divisionCode": "610429",
            "districtName": "旬邑县"
          },
          {
            "districtID": 2786,
            "cityID": 296,
            "divisionCode": "610430",
            "districtName": "淳化县"
          },
          {
            "districtID": 2787,
            "cityID": 296,
            "divisionCode": "610431",
            "districtName": "武功县"
          },
          {
            "districtID": 2788,
            "cityID": 296,
            "divisionCode": "610481",
            "districtName": "兴平市"
          },
          {
            "districtID": 2789,
            "cityID": 297,
            "divisionCode": "610501",
            "districtName": "市辖区"
          },
          {
            "districtID": 2790,
            "cityID": 297,
            "divisionCode": "610502",
            "districtName": "临渭区"
          },
          {
            "districtID": 2791,
            "cityID": 297,
            "divisionCode": "610503",
            "districtName": "华州区"
          },
          {
            "districtID": 2792,
            "cityID": 297,
            "divisionCode": "610522",
            "districtName": "潼关县"
          },
          {
            "districtID": 2793,
            "cityID": 297,
            "divisionCode": "610523",
            "districtName": "大荔县"
          },
          {
            "districtID": 2794,
            "cityID": 297,
            "divisionCode": "610524",
            "districtName": "合阳县"
          },
          {
            "districtID": 2795,
            "cityID": 297,
            "divisionCode": "610525",
            "districtName": "澄城县"
          },
          {
            "districtID": 2796,
            "cityID": 297,
            "divisionCode": "610526",
            "districtName": "蒲城县"
          },
          {
            "districtID": 2797,
            "cityID": 297,
            "divisionCode": "610527",
            "districtName": "白水县"
          },
          {
            "districtID": 2798,
            "cityID": 297,
            "divisionCode": "610528",
            "districtName": "富平县"
          },
          {
            "districtID": 2799,
            "cityID": 297,
            "divisionCode": "610581",
            "districtName": "韩城市"
          },
          {
            "districtID": 2800,
            "cityID": 297,
            "divisionCode": "610582",
            "districtName": "华阴市"
          },
          {
            "districtID": 2801,
            "cityID": 298,
            "divisionCode": "610601",
            "districtName": "市辖区"
          },
          {
            "districtID": 2802,
            "cityID": 298,
            "divisionCode": "610602",
            "districtName": "宝塔区"
          },
          {
            "districtID": 2803,
            "cityID": 298,
            "divisionCode": "610603",
            "districtName": "安塞区"
          },
          {
            "districtID": 2804,
            "cityID": 298,
            "divisionCode": "610621",
            "districtName": "延长县"
          },
          {
            "districtID": 2805,
            "cityID": 298,
            "divisionCode": "610622",
            "districtName": "延川县"
          },
          {
            "districtID": 2806,
            "cityID": 298,
            "divisionCode": "610623",
            "districtName": "子长县"
          },
          {
            "districtID": 2807,
            "cityID": 298,
            "divisionCode": "610625",
            "districtName": "志丹县"
          },
          {
            "districtID": 2808,
            "cityID": 298,
            "divisionCode": "610626",
            "districtName": "吴起县"
          },
          {
            "districtID": 2809,
            "cityID": 298,
            "divisionCode": "610627",
            "districtName": "甘泉县"
          },
          {
            "districtID": 2810,
            "cityID": 298,
            "divisionCode": "610628",
            "districtName": "富县"
          },
          {
            "districtID": 2811,
            "cityID": 298,
            "divisionCode": "610629",
            "districtName": "洛川县"
          },
          {
            "districtID": 2812,
            "cityID": 298,
            "divisionCode": "610630",
            "districtName": "宜川县"
          },
          {
            "districtID": 2813,
            "cityID": 298,
            "divisionCode": "610631",
            "districtName": "黄龙县"
          },
          {
            "districtID": 2814,
            "cityID": 298,
            "divisionCode": "610632",
            "districtName": "黄陵县"
          },
          {
            "districtID": 2815,
            "cityID": 299,
            "divisionCode": "610701",
            "districtName": "市辖区"
          },
          {
            "districtID": 2816,
            "cityID": 299,
            "divisionCode": "610702",
            "districtName": "汉台区"
          },
          {
            "districtID": 2817,
            "cityID": 299,
            "divisionCode": "610721",
            "districtName": "南郑县"
          },
          {
            "districtID": 2818,
            "cityID": 299,
            "divisionCode": "610722",
            "districtName": "城固县"
          },
          {
            "districtID": 2819,
            "cityID": 299,
            "divisionCode": "610723",
            "districtName": "洋县"
          },
          {
            "districtID": 2820,
            "cityID": 299,
            "divisionCode": "610724",
            "districtName": "西乡县"
          },
          {
            "districtID": 2821,
            "cityID": 299,
            "divisionCode": "610725",
            "districtName": "勉县"
          },
          {
            "districtID": 2822,
            "cityID": 299,
            "divisionCode": "610726",
            "districtName": "宁强县"
          },
          {
            "districtID": 2823,
            "cityID": 299,
            "divisionCode": "610727",
            "districtName": "略阳县"
          },
          {
            "districtID": 2824,
            "cityID": 299,
            "divisionCode": "610728",
            "districtName": "镇巴县"
          },
          {
            "districtID": 2825,
            "cityID": 299,
            "divisionCode": "610729",
            "districtName": "留坝县"
          },
          {
            "districtID": 2826,
            "cityID": 299,
            "divisionCode": "610730",
            "districtName": "佛坪县"
          },
          {
            "districtID": 2827,
            "cityID": 300,
            "divisionCode": "610801",
            "districtName": "市辖区"
          },
          {
            "districtID": 2828,
            "cityID": 300,
            "divisionCode": "610802",
            "districtName": "榆阳区"
          },
          {
            "districtID": 2829,
            "cityID": 300,
            "divisionCode": "610803",
            "districtName": "横山区"
          },
          {
            "districtID": 2830,
            "cityID": 300,
            "divisionCode": "610821",
            "districtName": "神木县"
          },
          {
            "districtID": 2831,
            "cityID": 300,
            "divisionCode": "610822",
            "districtName": "府谷县"
          },
          {
            "districtID": 2832,
            "cityID": 300,
            "divisionCode": "610824",
            "districtName": "靖边县"
          },
          {
            "districtID": 2833,
            "cityID": 300,
            "divisionCode": "610825",
            "districtName": "定边县"
          },
          {
            "districtID": 2834,
            "cityID": 300,
            "divisionCode": "610826",
            "districtName": "绥德县"
          },
          {
            "districtID": 2835,
            "cityID": 300,
            "divisionCode": "610827",
            "districtName": "米脂县"
          },
          {
            "districtID": 2836,
            "cityID": 300,
            "divisionCode": "610828",
            "districtName": "佳县"
          },
          {
            "districtID": 2837,
            "cityID": 300,
            "divisionCode": "610829",
            "districtName": "吴堡县"
          },
          {
            "districtID": 2838,
            "cityID": 300,
            "divisionCode": "610830",
            "districtName": "清涧县"
          },
          {
            "districtID": 2839,
            "cityID": 300,
            "divisionCode": "610831",
            "districtName": "子洲县"
          },
          {
            "districtID": 2840,
            "cityID": 301,
            "divisionCode": "610901",
            "districtName": "市辖区"
          },
          {
            "districtID": 2841,
            "cityID": 301,
            "divisionCode": "610902",
            "districtName": "汉滨区"
          },
          {
            "districtID": 2842,
            "cityID": 301,
            "divisionCode": "610921",
            "districtName": "汉阴县"
          },
          {
            "districtID": 2843,
            "cityID": 301,
            "divisionCode": "610922",
            "districtName": "石泉县"
          },
          {
            "districtID": 2844,
            "cityID": 301,
            "divisionCode": "610923",
            "districtName": "宁陕县"
          },
          {
            "districtID": 2845,
            "cityID": 301,
            "divisionCode": "610924",
            "districtName": "紫阳县"
          },
          {
            "districtID": 2846,
            "cityID": 301,
            "divisionCode": "610925",
            "districtName": "岚皋县"
          },
          {
            "districtID": 2847,
            "cityID": 301,
            "divisionCode": "610926",
            "districtName": "平利县"
          },
          {
            "districtID": 2848,
            "cityID": 301,
            "divisionCode": "610927",
            "districtName": "镇坪县"
          },
          {
            "districtID": 2849,
            "cityID": 301,
            "divisionCode": "610928",
            "districtName": "旬阳县"
          },
          {
            "districtID": 2850,
            "cityID": 301,
            "divisionCode": "610929",
            "districtName": "白河县"
          },
          {
            "districtID": 2851,
            "cityID": 302,
            "divisionCode": "611001",
            "districtName": "市辖区"
          },
          {
            "districtID": 2852,
            "cityID": 302,
            "divisionCode": "611002",
            "districtName": "商州区"
          },
          {
            "districtID": 2853,
            "cityID": 302,
            "divisionCode": "611021",
            "districtName": "洛南县"
          },
          {
            "districtID": 2854,
            "cityID": 302,
            "divisionCode": "611022",
            "districtName": "丹凤县"
          },
          {
            "districtID": 2855,
            "cityID": 302,
            "divisionCode": "611023",
            "districtName": "商南县"
          },
          {
            "districtID": 2856,
            "cityID": 302,
            "divisionCode": "611024",
            "districtName": "山阳县"
          },
          {
            "districtID": 2857,
            "cityID": 302,
            "divisionCode": "611025",
            "districtName": "镇安县"
          },
          {
            "districtID": 2858,
            "cityID": 302,
            "divisionCode": "611026",
            "districtName": "柞水县"
          },
          {
            "districtID": 2859,
            "cityID": 303,
            "divisionCode": "620101",
            "districtName": "市辖区"
          },
          {
            "districtID": 2860,
            "cityID": 303,
            "divisionCode": "620102",
            "districtName": "城关区"
          },
          {
            "districtID": 2861,
            "cityID": 303,
            "divisionCode": "620103",
            "districtName": "七里河区"
          },
          {
            "districtID": 2862,
            "cityID": 303,
            "divisionCode": "620104",
            "districtName": "西固区"
          },
          {
            "districtID": 2863,
            "cityID": 303,
            "divisionCode": "620105",
            "districtName": "安宁区"
          },
          {
            "districtID": 2864,
            "cityID": 303,
            "divisionCode": "620111",
            "districtName": "红古区"
          },
          {
            "districtID": 2865,
            "cityID": 303,
            "divisionCode": "620121",
            "districtName": "永登县"
          },
          {
            "districtID": 2866,
            "cityID": 303,
            "divisionCode": "620122",
            "districtName": "皋兰县"
          },
          {
            "districtID": 2867,
            "cityID": 303,
            "divisionCode": "620123",
            "districtName": "榆中县"
          },
          {
            "districtID": 2868,
            "cityID": 304,
            "divisionCode": "620201",
            "districtName": "市辖区"
          },
          {
            "districtID": 2869,
            "cityID": 305,
            "divisionCode": "620301",
            "districtName": "市辖区"
          },
          {
            "districtID": 2870,
            "cityID": 305,
            "divisionCode": "620302",
            "districtName": "金川区"
          },
          {
            "districtID": 2871,
            "cityID": 305,
            "divisionCode": "620321",
            "districtName": "永昌县"
          },
          {
            "districtID": 2872,
            "cityID": 306,
            "divisionCode": "620401",
            "districtName": "市辖区"
          },
          {
            "districtID": 2873,
            "cityID": 306,
            "divisionCode": "620402",
            "districtName": "白银区"
          },
          {
            "districtID": 2874,
            "cityID": 306,
            "divisionCode": "620403",
            "districtName": "平川区"
          },
          {
            "districtID": 2875,
            "cityID": 306,
            "divisionCode": "620421",
            "districtName": "靖远县"
          },
          {
            "districtID": 2876,
            "cityID": 306,
            "divisionCode": "620422",
            "districtName": "会宁县"
          },
          {
            "districtID": 2877,
            "cityID": 306,
            "divisionCode": "620423",
            "districtName": "景泰县"
          },
          {
            "districtID": 2878,
            "cityID": 307,
            "divisionCode": "620501",
            "districtName": "市辖区"
          },
          {
            "districtID": 2879,
            "cityID": 307,
            "divisionCode": "620502",
            "districtName": "秦州区"
          },
          {
            "districtID": 2880,
            "cityID": 307,
            "divisionCode": "620503",
            "districtName": "麦积区"
          },
          {
            "districtID": 2881,
            "cityID": 307,
            "divisionCode": "620521",
            "districtName": "清水县"
          },
          {
            "districtID": 2882,
            "cityID": 307,
            "divisionCode": "620522",
            "districtName": "秦安县"
          },
          {
            "districtID": 2883,
            "cityID": 307,
            "divisionCode": "620523",
            "districtName": "甘谷县"
          },
          {
            "districtID": 2884,
            "cityID": 307,
            "divisionCode": "620524",
            "districtName": "武山县"
          },
          {
            "districtID": 2885,
            "cityID": 307,
            "divisionCode": "620525",
            "districtName": "张家川回族自治县"
          },
          {
            "districtID": 2886,
            "cityID": 308,
            "divisionCode": "620601",
            "districtName": "市辖区"
          },
          {
            "districtID": 2887,
            "cityID": 308,
            "divisionCode": "620602",
            "districtName": "凉州区"
          },
          {
            "districtID": 2888,
            "cityID": 308,
            "divisionCode": "620621",
            "districtName": "民勤县"
          },
          {
            "districtID": 2889,
            "cityID": 308,
            "divisionCode": "620622",
            "districtName": "古浪县"
          },
          {
            "districtID": 2890,
            "cityID": 308,
            "divisionCode": "620623",
            "districtName": "天祝藏族自治县"
          },
          {
            "districtID": 2891,
            "cityID": 309,
            "divisionCode": "620701",
            "districtName": "市辖区"
          },
          {
            "districtID": 2892,
            "cityID": 309,
            "divisionCode": "620702",
            "districtName": "甘州区"
          },
          {
            "districtID": 2893,
            "cityID": 309,
            "divisionCode": "620721",
            "districtName": "肃南裕固族自治县"
          },
          {
            "districtID": 2894,
            "cityID": 309,
            "divisionCode": "620722",
            "districtName": "民乐县"
          },
          {
            "districtID": 2895,
            "cityID": 309,
            "divisionCode": "620723",
            "districtName": "临泽县"
          },
          {
            "districtID": 2896,
            "cityID": 309,
            "divisionCode": "620724",
            "districtName": "高台县"
          },
          {
            "districtID": 2897,
            "cityID": 309,
            "divisionCode": "620725",
            "districtName": "山丹县"
          },
          {
            "districtID": 2898,
            "cityID": 310,
            "divisionCode": "620801",
            "districtName": "市辖区"
          },
          {
            "districtID": 2899,
            "cityID": 310,
            "divisionCode": "620802",
            "districtName": "崆峒区"
          },
          {
            "districtID": 2900,
            "cityID": 310,
            "divisionCode": "620821",
            "districtName": "泾川县"
          },
          {
            "districtID": 2901,
            "cityID": 310,
            "divisionCode": "620822",
            "districtName": "灵台县"
          },
          {
            "districtID": 2902,
            "cityID": 310,
            "divisionCode": "620823",
            "districtName": "崇信县"
          },
          {
            "districtID": 2903,
            "cityID": 310,
            "divisionCode": "620824",
            "districtName": "华亭县"
          },
          {
            "districtID": 2904,
            "cityID": 310,
            "divisionCode": "620825",
            "districtName": "庄浪县"
          },
          {
            "districtID": 2905,
            "cityID": 310,
            "divisionCode": "620826",
            "districtName": "静宁县"
          },
          {
            "districtID": 2906,
            "cityID": 311,
            "divisionCode": "620901",
            "districtName": "市辖区"
          },
          {
            "districtID": 2907,
            "cityID": 311,
            "divisionCode": "620902",
            "districtName": "肃州区"
          },
          {
            "districtID": 2908,
            "cityID": 311,
            "divisionCode": "620921",
            "districtName": "金塔县"
          },
          {
            "districtID": 2909,
            "cityID": 311,
            "divisionCode": "620922",
            "districtName": "瓜州县"
          },
          {
            "districtID": 2910,
            "cityID": 311,
            "divisionCode": "620923",
            "districtName": "肃北蒙古族自治县"
          },
          {
            "districtID": 2911,
            "cityID": 311,
            "divisionCode": "620924",
            "districtName": "阿克塞哈萨克族自治县"
          },
          {
            "districtID": 2912,
            "cityID": 311,
            "divisionCode": "620981",
            "districtName": "玉门市"
          },
          {
            "districtID": 2913,
            "cityID": 311,
            "divisionCode": "620982",
            "districtName": "敦煌市"
          },
          {
            "districtID": 2914,
            "cityID": 312,
            "divisionCode": "621001",
            "districtName": "市辖区"
          },
          {
            "districtID": 2915,
            "cityID": 312,
            "divisionCode": "621002",
            "districtName": "西峰区"
          },
          {
            "districtID": 2916,
            "cityID": 312,
            "divisionCode": "621021",
            "districtName": "庆城县"
          },
          {
            "districtID": 2917,
            "cityID": 312,
            "divisionCode": "621022",
            "districtName": "环县"
          },
          {
            "districtID": 2918,
            "cityID": 312,
            "divisionCode": "621023",
            "districtName": "华池县"
          },
          {
            "districtID": 2919,
            "cityID": 312,
            "divisionCode": "621024",
            "districtName": "合水县"
          },
          {
            "districtID": 2920,
            "cityID": 312,
            "divisionCode": "621025",
            "districtName": "正宁县"
          },
          {
            "districtID": 2921,
            "cityID": 312,
            "divisionCode": "621026",
            "districtName": "宁县"
          },
          {
            "districtID": 2922,
            "cityID": 312,
            "divisionCode": "621027",
            "districtName": "镇原县"
          },
          {
            "districtID": 2923,
            "cityID": 313,
            "divisionCode": "621101",
            "districtName": "市辖区"
          },
          {
            "districtID": 2924,
            "cityID": 313,
            "divisionCode": "621102",
            "districtName": "安定区"
          },
          {
            "districtID": 2925,
            "cityID": 313,
            "divisionCode": "621121",
            "districtName": "通渭县"
          },
          {
            "districtID": 2926,
            "cityID": 313,
            "divisionCode": "621122",
            "districtName": "陇西县"
          },
          {
            "districtID": 2927,
            "cityID": 313,
            "divisionCode": "621123",
            "districtName": "渭源县"
          },
          {
            "districtID": 2928,
            "cityID": 313,
            "divisionCode": "621124",
            "districtName": "临洮县"
          },
          {
            "districtID": 2929,
            "cityID": 313,
            "divisionCode": "621125",
            "districtName": "漳县"
          },
          {
            "districtID": 2930,
            "cityID": 313,
            "divisionCode": "621126",
            "districtName": "岷县"
          },
          {
            "districtID": 2931,
            "cityID": 314,
            "divisionCode": "621201",
            "districtName": "市辖区"
          },
          {
            "districtID": 2932,
            "cityID": 314,
            "divisionCode": "621202",
            "districtName": "武都区"
          },
          {
            "districtID": 2933,
            "cityID": 314,
            "divisionCode": "621221",
            "districtName": "成县"
          },
          {
            "districtID": 2934,
            "cityID": 314,
            "divisionCode": "621222",
            "districtName": "文县"
          },
          {
            "districtID": 2935,
            "cityID": 314,
            "divisionCode": "621223",
            "districtName": "宕昌县"
          },
          {
            "districtID": 2936,
            "cityID": 314,
            "divisionCode": "621224",
            "districtName": "康县"
          },
          {
            "districtID": 2937,
            "cityID": 314,
            "divisionCode": "621225",
            "districtName": "西和县"
          },
          {
            "districtID": 2938,
            "cityID": 314,
            "divisionCode": "621226",
            "districtName": "礼县"
          },
          {
            "districtID": 2939,
            "cityID": 314,
            "divisionCode": "621227",
            "districtName": "徽县"
          },
          {
            "districtID": 2940,
            "cityID": 314,
            "divisionCode": "621228",
            "districtName": "两当县"
          },
          {
            "districtID": 2941,
            "cityID": 315,
            "divisionCode": "622901",
            "districtName": "临夏市"
          },
          {
            "districtID": 2942,
            "cityID": 315,
            "divisionCode": "622921",
            "districtName": "临夏县"
          },
          {
            "districtID": 2943,
            "cityID": 315,
            "divisionCode": "622922",
            "districtName": "康乐县"
          },
          {
            "districtID": 2944,
            "cityID": 315,
            "divisionCode": "622923",
            "districtName": "永靖县"
          },
          {
            "districtID": 2945,
            "cityID": 315,
            "divisionCode": "622924",
            "districtName": "广河县"
          },
          {
            "districtID": 2946,
            "cityID": 315,
            "divisionCode": "622925",
            "districtName": "和政县"
          },
          {
            "districtID": 2947,
            "cityID": 315,
            "divisionCode": "622926",
            "districtName": "东乡族自治县"
          },
          {
            "districtID": 2948,
            "cityID": 315,
            "divisionCode": "622927",
            "districtName": "积石山保安族东乡族撒拉族自治县"
          },
          {
            "districtID": 2949,
            "cityID": 316,
            "divisionCode": "623001",
            "districtName": "合作市"
          },
          {
            "districtID": 2950,
            "cityID": 316,
            "divisionCode": "623021",
            "districtName": "临潭县"
          },
          {
            "districtID": 2951,
            "cityID": 316,
            "divisionCode": "623022",
            "districtName": "卓尼县"
          },
          {
            "districtID": 2952,
            "cityID": 316,
            "divisionCode": "623023",
            "districtName": "舟曲县"
          },
          {
            "districtID": 2953,
            "cityID": 316,
            "divisionCode": "623024",
            "districtName": "迭部县"
          },
          {
            "districtID": 2954,
            "cityID": 316,
            "divisionCode": "623025",
            "districtName": "玛曲县"
          },
          {
            "districtID": 2955,
            "cityID": 316,
            "divisionCode": "623026",
            "districtName": "碌曲县"
          },
          {
            "districtID": 2956,
            "cityID": 316,
            "divisionCode": "623027",
            "districtName": "夏河县"
          },
          {
            "districtID": 2957,
            "cityID": 317,
            "divisionCode": "630101",
            "districtName": "市辖区"
          },
          {
            "districtID": 2958,
            "cityID": 317,
            "divisionCode": "630102",
            "districtName": "城东区"
          },
          {
            "districtID": 2959,
            "cityID": 317,
            "divisionCode": "630103",
            "districtName": "城中区"
          },
          {
            "districtID": 2960,
            "cityID": 317,
            "divisionCode": "630104",
            "districtName": "城西区"
          },
          {
            "districtID": 2961,
            "cityID": 317,
            "divisionCode": "630105",
            "districtName": "城北区"
          },
          {
            "districtID": 2962,
            "cityID": 317,
            "divisionCode": "630121",
            "districtName": "大通回族土族自治县"
          },
          {
            "districtID": 2963,
            "cityID": 317,
            "divisionCode": "630122",
            "districtName": "湟中县"
          },
          {
            "districtID": 2964,
            "cityID": 317,
            "divisionCode": "630123",
            "districtName": "湟源县"
          },
          {
            "districtID": 2965,
            "cityID": 318,
            "divisionCode": "630202",
            "districtName": "乐都区"
          },
          {
            "districtID": 2966,
            "cityID": 318,
            "divisionCode": "630203",
            "districtName": "平安区"
          },
          {
            "districtID": 2967,
            "cityID": 318,
            "divisionCode": "630222",
            "districtName": "民和回族土族自治县"
          },
          {
            "districtID": 2968,
            "cityID": 318,
            "divisionCode": "630223",
            "districtName": "互助土族自治县"
          },
          {
            "districtID": 2969,
            "cityID": 318,
            "divisionCode": "630224",
            "districtName": "化隆回族自治县"
          },
          {
            "districtID": 2970,
            "cityID": 318,
            "divisionCode": "630225",
            "districtName": "循化撒拉族自治县"
          },
          {
            "districtID": 2971,
            "cityID": 319,
            "divisionCode": "632221",
            "districtName": "门源回族自治县"
          },
          {
            "districtID": 2972,
            "cityID": 319,
            "divisionCode": "632222",
            "districtName": "祁连县"
          },
          {
            "districtID": 2973,
            "cityID": 319,
            "divisionCode": "632223",
            "districtName": "海晏县"
          },
          {
            "districtID": 2974,
            "cityID": 319,
            "divisionCode": "632224",
            "districtName": "刚察县"
          },
          {
            "districtID": 2975,
            "cityID": 320,
            "divisionCode": "632321",
            "districtName": "同仁县"
          },
          {
            "districtID": 2976,
            "cityID": 320,
            "divisionCode": "632322",
            "districtName": "尖扎县"
          },
          {
            "districtID": 2977,
            "cityID": 320,
            "divisionCode": "632323",
            "districtName": "泽库县"
          },
          {
            "districtID": 2978,
            "cityID": 320,
            "divisionCode": "632324",
            "districtName": "河南蒙古族自治县"
          },
          {
            "districtID": 2979,
            "cityID": 321,
            "divisionCode": "632521",
            "districtName": "共和县"
          },
          {
            "districtID": 2980,
            "cityID": 321,
            "divisionCode": "632522",
            "districtName": "同德县"
          },
          {
            "districtID": 2981,
            "cityID": 321,
            "divisionCode": "632523",
            "districtName": "贵德县"
          },
          {
            "districtID": 2982,
            "cityID": 321,
            "divisionCode": "632524",
            "districtName": "兴海县"
          },
          {
            "districtID": 2983,
            "cityID": 321,
            "divisionCode": "632525",
            "districtName": "贵南县"
          },
          {
            "districtID": 2984,
            "cityID": 322,
            "divisionCode": "632621",
            "districtName": "玛沁县"
          },
          {
            "districtID": 2985,
            "cityID": 322,
            "divisionCode": "632622",
            "districtName": "班玛县"
          },
          {
            "districtID": 2986,
            "cityID": 322,
            "divisionCode": "632623",
            "districtName": "甘德县"
          },
          {
            "districtID": 2987,
            "cityID": 322,
            "divisionCode": "632624",
            "districtName": "达日县"
          },
          {
            "districtID": 2988,
            "cityID": 322,
            "divisionCode": "632625",
            "districtName": "久治县"
          },
          {
            "districtID": 2989,
            "cityID": 322,
            "divisionCode": "632626",
            "districtName": "玛多县"
          },
          {
            "districtID": 2990,
            "cityID": 323,
            "divisionCode": "632701",
            "districtName": "玉树市"
          },
          {
            "districtID": 2991,
            "cityID": 323,
            "divisionCode": "632722",
            "districtName": "杂多县"
          },
          {
            "districtID": 2992,
            "cityID": 323,
            "divisionCode": "632723",
            "districtName": "称多县"
          },
          {
            "districtID": 2993,
            "cityID": 323,
            "divisionCode": "632724",
            "districtName": "治多县"
          },
          {
            "districtID": 2994,
            "cityID": 323,
            "divisionCode": "632725",
            "districtName": "囊谦县"
          },
          {
            "districtID": 2995,
            "cityID": 323,
            "divisionCode": "632726",
            "districtName": "曲麻莱县"
          },
          {
            "districtID": 2996,
            "cityID": 324,
            "divisionCode": "632801",
            "districtName": "格尔木市"
          },
          {
            "districtID": 2997,
            "cityID": 324,
            "divisionCode": "632802",
            "districtName": "德令哈市"
          },
          {
            "districtID": 2998,
            "cityID": 324,
            "divisionCode": "632821",
            "districtName": "乌兰县"
          },
          {
            "districtID": 2999,
            "cityID": 324,
            "divisionCode": "632822",
            "districtName": "都兰县"
          },
          {
            "districtID": 3000,
            "cityID": 324,
            "divisionCode": "632823",
            "districtName": "天峻县"
          },
          {
            "districtID": 3001,
            "cityID": 325,
            "divisionCode": "640101",
            "districtName": "市辖区"
          },
          {
            "districtID": 3002,
            "cityID": 325,
            "divisionCode": "640104",
            "districtName": "兴庆区"
          },
          {
            "districtID": 3003,
            "cityID": 325,
            "divisionCode": "640105",
            "districtName": "西夏区"
          },
          {
            "districtID": 3004,
            "cityID": 325,
            "divisionCode": "640106",
            "districtName": "金凤区"
          },
          {
            "districtID": 3005,
            "cityID": 325,
            "divisionCode": "640121",
            "districtName": "永宁县"
          },
          {
            "districtID": 3006,
            "cityID": 325,
            "divisionCode": "640122",
            "districtName": "贺兰县"
          },
          {
            "districtID": 3007,
            "cityID": 325,
            "divisionCode": "640181",
            "districtName": "灵武市"
          },
          {
            "districtID": 3008,
            "cityID": 326,
            "divisionCode": "640201",
            "districtName": "市辖区"
          },
          {
            "districtID": 3009,
            "cityID": 326,
            "divisionCode": "640202",
            "districtName": "大武口区"
          },
          {
            "districtID": 3010,
            "cityID": 326,
            "divisionCode": "640205",
            "districtName": "惠农区"
          },
          {
            "districtID": 3011,
            "cityID": 326,
            "divisionCode": "640221",
            "districtName": "平罗县"
          },
          {
            "districtID": 3012,
            "cityID": 327,
            "divisionCode": "640301",
            "districtName": "市辖区"
          },
          {
            "districtID": 3013,
            "cityID": 327,
            "divisionCode": "640302",
            "districtName": "利通区"
          },
          {
            "districtID": 3014,
            "cityID": 327,
            "divisionCode": "640303",
            "districtName": "红寺堡区"
          },
          {
            "districtID": 3015,
            "cityID": 327,
            "divisionCode": "640323",
            "districtName": "盐池县"
          },
          {
            "districtID": 3016,
            "cityID": 327,
            "divisionCode": "640324",
            "districtName": "同心县"
          },
          {
            "districtID": 3017,
            "cityID": 327,
            "divisionCode": "640381",
            "districtName": "青铜峡市"
          },
          {
            "districtID": 3018,
            "cityID": 328,
            "divisionCode": "640401",
            "districtName": "市辖区"
          },
          {
            "districtID": 3019,
            "cityID": 328,
            "divisionCode": "640402",
            "districtName": "原州区"
          },
          {
            "districtID": 3020,
            "cityID": 328,
            "divisionCode": "640422",
            "districtName": "西吉县"
          },
          {
            "districtID": 3021,
            "cityID": 328,
            "divisionCode": "640423",
            "districtName": "隆德县"
          },
          {
            "districtID": 3022,
            "cityID": 328,
            "divisionCode": "640424",
            "districtName": "泾源县"
          },
          {
            "districtID": 3023,
            "cityID": 328,
            "divisionCode": "640425",
            "districtName": "彭阳县"
          },
          {
            "districtID": 3024,
            "cityID": 329,
            "divisionCode": "640501",
            "districtName": "市辖区"
          },
          {
            "districtID": 3025,
            "cityID": 329,
            "divisionCode": "640502",
            "districtName": "沙坡头区"
          },
          {
            "districtID": 3026,
            "cityID": 329,
            "divisionCode": "640521",
            "districtName": "中宁县"
          },
          {
            "districtID": 3027,
            "cityID": 329,
            "divisionCode": "640522",
            "districtName": "海原县"
          },
          {
            "districtID": 3028,
            "cityID": 330,
            "divisionCode": "650101",
            "districtName": "市辖区"
          },
          {
            "districtID": 3029,
            "cityID": 330,
            "divisionCode": "650102",
            "districtName": "天山区"
          },
          {
            "districtID": 3030,
            "cityID": 330,
            "divisionCode": "650103",
            "districtName": "沙依巴克区"
          },
          {
            "districtID": 3031,
            "cityID": 330,
            "divisionCode": "650104",
            "districtName": "新市区"
          },
          {
            "districtID": 3032,
            "cityID": 330,
            "divisionCode": "650105",
            "districtName": "水磨沟区"
          },
          {
            "districtID": 3033,
            "cityID": 330,
            "divisionCode": "650106",
            "districtName": "头屯河区"
          },
          {
            "districtID": 3034,
            "cityID": 330,
            "divisionCode": "650107",
            "districtName": "达坂城区"
          },
          {
            "districtID": 3035,
            "cityID": 330,
            "divisionCode": "650109",
            "districtName": "米东区"
          },
          {
            "districtID": 3036,
            "cityID": 330,
            "divisionCode": "650121",
            "districtName": "乌鲁木齐县"
          },
          {
            "districtID": 3037,
            "cityID": 331,
            "divisionCode": "650201",
            "districtName": "市辖区"
          },
          {
            "districtID": 3038,
            "cityID": 331,
            "divisionCode": "650202",
            "districtName": "独山子区"
          },
          {
            "districtID": 3039,
            "cityID": 331,
            "divisionCode": "650203",
            "districtName": "克拉玛依区"
          },
          {
            "districtID": 3040,
            "cityID": 331,
            "divisionCode": "650204",
            "districtName": "白碱滩区"
          },
          {
            "districtID": 3041,
            "cityID": 331,
            "divisionCode": "650205",
            "districtName": "乌尔禾区"
          },
          {
            "districtID": 3042,
            "cityID": 332,
            "divisionCode": "650402",
            "districtName": "高昌区"
          },
          {
            "districtID": 3043,
            "cityID": 332,
            "divisionCode": "650421",
            "districtName": "鄯善县"
          },
          {
            "districtID": 3044,
            "cityID": 332,
            "divisionCode": "650422",
            "districtName": "托克逊县"
          },
          {
            "districtID": 3045,
            "cityID": 333,
            "divisionCode": "650502",
            "districtName": "伊州区"
          },
          {
            "districtID": 3046,
            "cityID": 333,
            "divisionCode": "650521",
            "districtName": "巴里坤哈萨克自治县"
          },
          {
            "districtID": 3047,
            "cityID": 333,
            "divisionCode": "650522",
            "districtName": "伊吾县"
          },
          {
            "districtID": 3048,
            "cityID": 334,
            "divisionCode": "652301",
            "districtName": "昌吉市"
          },
          {
            "districtID": 3049,
            "cityID": 334,
            "divisionCode": "652302",
            "districtName": "阜康市"
          },
          {
            "districtID": 3050,
            "cityID": 334,
            "divisionCode": "652323",
            "districtName": "呼图壁县"
          },
          {
            "districtID": 3051,
            "cityID": 334,
            "divisionCode": "652324",
            "districtName": "玛纳斯县"
          },
          {
            "districtID": 3052,
            "cityID": 334,
            "divisionCode": "652325",
            "districtName": "奇台县"
          },
          {
            "districtID": 3053,
            "cityID": 334,
            "divisionCode": "652327",
            "districtName": "吉木萨尔县"
          },
          {
            "districtID": 3054,
            "cityID": 334,
            "divisionCode": "652328",
            "districtName": "木垒哈萨克自治县"
          },
          {
            "districtID": 3055,
            "cityID": 335,
            "divisionCode": "652701",
            "districtName": "博乐市"
          },
          {
            "districtID": 3056,
            "cityID": 335,
            "divisionCode": "652702",
            "districtName": "阿拉山口市"
          },
          {
            "districtID": 3057,
            "cityID": 335,
            "divisionCode": "652722",
            "districtName": "精河县"
          },
          {
            "districtID": 3058,
            "cityID": 335,
            "divisionCode": "652723",
            "districtName": "温泉县"
          },
          {
            "districtID": 3059,
            "cityID": 336,
            "divisionCode": "652801",
            "districtName": "库尔勒市"
          },
          {
            "districtID": 3060,
            "cityID": 336,
            "divisionCode": "652822",
            "districtName": "轮台县"
          },
          {
            "districtID": 3061,
            "cityID": 336,
            "divisionCode": "652823",
            "districtName": "尉犁县"
          },
          {
            "districtID": 3062,
            "cityID": 336,
            "divisionCode": "652824",
            "districtName": "若羌县"
          },
          {
            "districtID": 3063,
            "cityID": 336,
            "divisionCode": "652825",
            "districtName": "且末县"
          },
          {
            "districtID": 3064,
            "cityID": 336,
            "divisionCode": "652826",
            "districtName": "焉耆回族自治县"
          },
          {
            "districtID": 3065,
            "cityID": 336,
            "divisionCode": "652827",
            "districtName": "和静县"
          },
          {
            "districtID": 3066,
            "cityID": 336,
            "divisionCode": "652828",
            "districtName": "和硕县"
          },
          {
            "districtID": 3067,
            "cityID": 336,
            "divisionCode": "652829",
            "districtName": "博湖县"
          },
          {
            "districtID": 3068,
            "cityID": 337,
            "divisionCode": "652901",
            "districtName": "阿克苏市"
          },
          {
            "districtID": 3069,
            "cityID": 337,
            "divisionCode": "652922",
            "districtName": "温宿县"
          },
          {
            "districtID": 3070,
            "cityID": 337,
            "divisionCode": "652923",
            "districtName": "库车县"
          },
          {
            "districtID": 3071,
            "cityID": 337,
            "divisionCode": "652924",
            "districtName": "沙雅县"
          },
          {
            "districtID": 3072,
            "cityID": 337,
            "divisionCode": "652925",
            "districtName": "新和县"
          },
          {
            "districtID": 3073,
            "cityID": 337,
            "divisionCode": "652926",
            "districtName": "拜城县"
          },
          {
            "districtID": 3074,
            "cityID": 337,
            "divisionCode": "652927",
            "districtName": "乌什县"
          },
          {
            "districtID": 3075,
            "cityID": 337,
            "divisionCode": "652928",
            "districtName": "阿瓦提县"
          },
          {
            "districtID": 3076,
            "cityID": 337,
            "divisionCode": "652929",
            "districtName": "柯坪县"
          },
          {
            "districtID": 3077,
            "cityID": 338,
            "divisionCode": "653001",
            "districtName": "阿图什市"
          },
          {
            "districtID": 3078,
            "cityID": 338,
            "divisionCode": "653022",
            "districtName": "阿克陶县"
          },
          {
            "districtID": 3079,
            "cityID": 338,
            "divisionCode": "653023",
            "districtName": "阿合奇县"
          },
          {
            "districtID": 3080,
            "cityID": 338,
            "divisionCode": "653024",
            "districtName": "乌恰县"
          },
          {
            "districtID": 3081,
            "cityID": 339,
            "divisionCode": "653101",
            "districtName": "喀什市"
          },
          {
            "districtID": 3082,
            "cityID": 339,
            "divisionCode": "653121",
            "districtName": "疏附县"
          },
          {
            "districtID": 3083,
            "cityID": 339,
            "divisionCode": "653122",
            "districtName": "疏勒县"
          },
          {
            "districtID": 3084,
            "cityID": 339,
            "divisionCode": "653123",
            "districtName": "英吉沙县"
          },
          {
            "districtID": 3085,
            "cityID": 339,
            "divisionCode": "653124",
            "districtName": "泽普县"
          },
          {
            "districtID": 3086,
            "cityID": 339,
            "divisionCode": "653125",
            "districtName": "莎车县"
          },
          {
            "districtID": 3087,
            "cityID": 339,
            "divisionCode": "653126",
            "districtName": "叶城县"
          },
          {
            "districtID": 3088,
            "cityID": 339,
            "divisionCode": "653127",
            "districtName": "麦盖提县"
          },
          {
            "districtID": 3089,
            "cityID": 339,
            "divisionCode": "653128",
            "districtName": "岳普湖县"
          },
          {
            "districtID": 3090,
            "cityID": 339,
            "divisionCode": "653129",
            "districtName": "伽师县"
          },
          {
            "districtID": 3091,
            "cityID": 339,
            "divisionCode": "653130",
            "districtName": "巴楚县"
          },
          {
            "districtID": 3092,
            "cityID": 339,
            "divisionCode": "653131",
            "districtName": "塔什库尔干塔吉克自治县"
          },
          {
            "districtID": 3093,
            "cityID": 340,
            "divisionCode": "653201",
            "districtName": "和田市"
          },
          {
            "districtID": 3094,
            "cityID": 340,
            "divisionCode": "653221",
            "districtName": "和田县"
          },
          {
            "districtID": 3095,
            "cityID": 340,
            "divisionCode": "653222",
            "districtName": "墨玉县"
          },
          {
            "districtID": 3096,
            "cityID": 340,
            "divisionCode": "653223",
            "districtName": "皮山县"
          },
          {
            "districtID": 3097,
            "cityID": 340,
            "divisionCode": "653224",
            "districtName": "洛浦县"
          },
          {
            "districtID": 3098,
            "cityID": 340,
            "divisionCode": "653225",
            "districtName": "策勒县"
          },
          {
            "districtID": 3099,
            "cityID": 340,
            "divisionCode": "653226",
            "districtName": "于田县"
          },
          {
            "districtID": 3100,
            "cityID": 340,
            "divisionCode": "653227",
            "districtName": "民丰县"
          },
          {
            "districtID": 3101,
            "cityID": 341,
            "divisionCode": "654002",
            "districtName": "伊宁市"
          },
          {
            "districtID": 3102,
            "cityID": 341,
            "divisionCode": "654003",
            "districtName": "奎屯市"
          },
          {
            "districtID": 3103,
            "cityID": 341,
            "divisionCode": "654004",
            "districtName": "霍尔果斯市"
          },
          {
            "districtID": 3104,
            "cityID": 341,
            "divisionCode": "654021",
            "districtName": "伊宁县"
          },
          {
            "districtID": 3105,
            "cityID": 341,
            "divisionCode": "654022",
            "districtName": "察布查尔锡伯自治县"
          },
          {
            "districtID": 3106,
            "cityID": 341,
            "divisionCode": "654023",
            "districtName": "霍城县"
          },
          {
            "districtID": 3107,
            "cityID": 341,
            "divisionCode": "654024",
            "districtName": "巩留县"
          },
          {
            "districtID": 3108,
            "cityID": 341,
            "divisionCode": "654025",
            "districtName": "新源县"
          },
          {
            "districtID": 3109,
            "cityID": 341,
            "divisionCode": "654026",
            "districtName": "昭苏县"
          },
          {
            "districtID": 3110,
            "cityID": 341,
            "divisionCode": "654027",
            "districtName": "特克斯县"
          },
          {
            "districtID": 3111,
            "cityID": 341,
            "divisionCode": "654028",
            "districtName": "尼勒克县"
          },
          {
            "districtID": 3112,
            "cityID": 342,
            "divisionCode": "654201",
            "districtName": "塔城市"
          },
          {
            "districtID": 3113,
            "cityID": 342,
            "divisionCode": "654202",
            "districtName": "乌苏市"
          },
          {
            "districtID": 3114,
            "cityID": 342,
            "divisionCode": "654221",
            "districtName": "额敏县"
          },
          {
            "districtID": 3115,
            "cityID": 342,
            "divisionCode": "654223",
            "districtName": "沙湾县"
          },
          {
            "districtID": 3116,
            "cityID": 342,
            "divisionCode": "654224",
            "districtName": "托里县"
          },
          {
            "districtID": 3117,
            "cityID": 342,
            "divisionCode": "654225",
            "districtName": "裕民县"
          },
          {
            "districtID": 3118,
            "cityID": 342,
            "divisionCode": "654226",
            "districtName": "和布克赛尔蒙古自治县"
          },
          {
            "districtID": 3119,
            "cityID": 343,
            "divisionCode": "654301",
            "districtName": "阿勒泰市"
          },
          {
            "districtID": 3120,
            "cityID": 343,
            "divisionCode": "654321",
            "districtName": "布尔津县"
          },
          {
            "districtID": 3121,
            "cityID": 343,
            "divisionCode": "654322",
            "districtName": "富蕴县"
          },
          {
            "districtID": 3122,
            "cityID": 343,
            "divisionCode": "654323",
            "districtName": "福海县"
          },
          {
            "districtID": 3123,
            "cityID": 343,
            "divisionCode": "654324",
            "districtName": "哈巴河县"
          },
          {
            "districtID": 3124,
            "cityID": 343,
            "divisionCode": "654325",
            "districtName": "青河县"
          },
          {
            "districtID": 3125,
            "cityID": 343,
            "divisionCode": "654326",
            "districtName": "吉木乃县"
          },
          {
            "districtID": 3126,
            "cityID": 344,
            "divisionCode": "659001",
            "districtName": "石河子市"
          },
          {
            "districtID": 3127,
            "cityID": 344,
            "divisionCode": "659002",
            "districtName": "阿拉尔市"
          },
          {
            "districtID": 3128,
            "cityID": 344,
            "divisionCode": "659003",
            "districtName": "图木舒克市"
          },
          {
            "districtID": 3129,
            "cityID": 344,
            "divisionCode": "659004",
            "districtName": "五家渠市"
          },
          {
            "districtID": 3130,
            "cityID": 344,
            "divisionCode": "659006",
            "districtName": "铁门关市"
          }
        ],
        off:true,
        locationData:null,
        url:'/dist/static/json',//打包时候在前面加一个dist
//        url:'/static/json'//测试环境下使用 使用用dev-server 但是不知道打包后如何访问;

      }
    },
    methods: {
      getarea(id, fn){
        if (id) {
          let vm = this;
          let getcity = new RemoteCall();
          getcity.init({
            router: '/base/area/idname/get',
            async: false,
            session: this.session,
            data: {
              id: id,
              pageInfo: {
                pageSize: 100,
                pageNum: 1
              }
            },
            callback: fn
          })
        }
      },
      getLocation(){
          var vm=this;
          vm.$set(vm, 'area', vm.locationData.districtName);
          vm.cityData.forEach(function(item){
            if(vm.locationData.cityID==item.cityID){
              vm.$set(vm, 'city', item.cityName);
              vm.locationData.provinceID=item.provinceID;
            }
          })

          vm.provinceData.forEach(function(item){
            if(vm.locationData.provinceID==item.provinceID){
              vm.$set(vm, 'province', item.provinceName);
            }
          })
          if(vm.areaData){
            vm.areaData=vm.locationData;
          }
    },
      addall(){//添加全部地市全部区县
        var vm=this;
        vm.provinceData.forEach(function(item){
          var options={};
          options.cityID=vm.cityData.length-1;
          options.provinceID=item.provinceID;
          options.divisionCode=null;
          options.cityName='全部城市';
          vm.cityData.push(options);
        })
        vm.cityData.forEach(function(item){
          var options={};
          options.districtID=vm.dirtrictData.length-1;
          options.cityID=item.cityID;
          options.divisionCode=null;
          options.districtName='全部区县';
          vm.dirtrictData.push(options);
        })
      }
    },
    mounted: function () {
      var vm = this;
      this.addall();
      if (this.areaId) {
        vm.dirtrictData.forEach(function(item){
          if(vm.areaId==item.divisionCode){
            vm.off=false;
            vm.locationData=item;
          }
        });
          if(this.off){
            this.getarea(this.areaId, function (data) {
              if (data.ret.errorCode === 0) {
                if(data.rows.length>0){
                    console.log(data.rows[0].name);
                  vm.$set(vm, 'area', data.rows[0].name);
                  vm.getarea(data.rows[0].parentAreaId, function (datas) {
                    if (datas.ret.errorCode === 0) {
                      vm.$set(vm, 'city', datas.rows[0].name);
                      vm.getarea(datas.rows[0].parentAreaId, function (str) {
                        vm.$set(vm, 'province', str.rows[0].name);
                      })
                    }
                  })
                }
              }
            })
          }else{
             this.getLocation();
          }
      }else if(this.startAreaData){
            if(vm.startAreaData.provinceId){
            vm.provinceData.forEach(function(item){
              if(vm.startAreaData.provinceId==item.divisionCode){

                vm.$set(vm, 'province', item.provinceName);
              }
            })
              vm.cityData.forEach(function(item){
                if(vm.startAreaData.cityId==item.divisionCode){
                  vm.$set(vm, 'city', item.cityName);
                 }
               })
              vm.dirtrictData.forEach(function(item){
                if(vm.startAreaData.areaId==item.divisionCode){
                  vm.$set(vm, 'area', item.districtName);
                }
              })
            }
      } else {
        vm.province = null;
        vm.city = null;
        vm.area = null;
      }
    },
    watch: {
      areaId(){
        var vm = this;
        if (this.areaId) {
          vm.dirtrictData.forEach(function(item){
            if(vm.areaId==item.divisionCode){
              vm.off=false;
              vm.locationData=item;
            }
          });
          if(this.off){
            this.getarea(this.areaId, function (data) {
              if (data.ret.errorCode === 0) {
                if(data.rows.length>0){
                  vm.$set(vm, 'area', data.rows[0].name);
                  vm.getarea(data.rows[0].parentAreaId, function (datas) {
                    if (datas.ret.errorCode === 0) {
                      vm.$set(vm, 'city', datas.rows[0].name);
                      vm.getarea(datas.rows[0].parentAreaId, function (str) {
                        vm.$set(vm, 'province', str.rows[0].name);
                      })
                    }
                  })
                }
              }
            })
          }else{
            this.getLocation();
          }
        }else if(this.startAreaData){
          if(vm.startAreaData.provinceId){
            vm.provinceData.forEach(function(item){
              if(vm.startAreaData.provinceId==item.divisionCode){
                vm.$set(vm, 'province', item.provinceName);
              }
            })
            vm.cityData.forEach(function(item){
              if(vm.startAreaData.cityId==item.divisionCode){
                vm.$set(vm, 'city', item.cityName);
              }
            })
            vm.dirtrictData.forEach(function(item){
              if(vm.startAreaData.areaId==item.divisionCode){
                vm.$set(vm, 'area', item.districtName);
              }
            })
          }
        } else {
          vm.province = null;
          vm.city = null;
          vm.area = null;
        }

      },
      startAreaData(){
          var vm=this;
        if(this.startAreaData){
          if(vm.startAreaData.provinceId){
            vm.provinceData.forEach(function(item){
              if(vm.startAreaData.provinceId==item.divisionCode){

                vm.$set(vm, 'province', item.provinceName);
              }
            })
            vm.cityData.forEach(function(item){
              if(vm.startAreaData.cityId==item.divisionCode){
                vm.$set(vm, 'city', item.cityName);
              }
            })
            vm.dirtrictData.forEach(function(item){
              if(vm.startAreaData.areaId==item.divisionCode){
                vm.$set(vm, 'area', item.districtName);
              }
            })
          }else{
            vm.province = null;
            vm.city = null;
            vm.area = null;
          }
        }
      }
    }
  }
</script>
<style>

</style>
