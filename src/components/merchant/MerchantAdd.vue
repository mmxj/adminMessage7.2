  ...
  <div id="MerchantAdd">
    <div class="breadcrumb">
      ...
      <el-breadcrumb separatorClass="el-icon-arrow-right">
        <el-breadcrumb-item :to="{path:'/merchant'}">商户申请列表</el-breadcrumb-item>
        <el-breadcrumb-item>新商户申请</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="form-wrap">
      <el-form class="form" :inline="true" :model="formData" :rules="rules"  ref="ruleForm" >
        <el-form-item label="商户编号：" label-width="120px" prop="no">
          <el-input size="small" v-model="formData.no"></el-input>
        </el-form-item>
        <el-form-item label="商户名称：" label-width="120px" prop="name">
          <el-input size="small" v-model="formData.name"></el-input>
        </el-form-item>
        <el-form-item label="商户联系人：" label-width="120px" prop="leaderName">
          <el-input size="small" v-model="formData.leaderName"></el-input>
        </el-form-item>
        <el-form-item label="联系电话：" label-width="120px" prop="telephone">
          <el-input size="small" v-model="formData.telephone"></el-input>
        </el-form-item>
        <el-form-item label="银行账号名：" label-width="120px" prop="accountName">
          <el-input size="small" v-model="formData.accountName"></el-input>
        </el-form-item>
        <el-form-item label="银行账号：" label-width="120px" prop="account">
        ...
        <el-form-item label="商户法人：" label-width="120px" prop="corporation">
          <el-input size="small" v-model="formData.corporation" ></el-input>
        </el-form-item>
        <el-form-item label="商户终端：" label-width="120px">
          <el-input size="small" v-model="formData.terminalNo"></el-input>
        </el-form-item>
        <el-form-item label="身份证号码：" label-width="120px" prop="certificateNo">
          <el-input size="small" v-model="formData.certificateNo"></el-input>
        </el-form-item>

        <el-form-item label="所属区域：" label-width="120px" prop="area">
          <div class="area">
            <Province :propdata="formData" :noall="true"></Province>
          </div>
        </el-form-item>
        <el-form-item label="商户地址：" label-width="120px" prop="address">
    ...
    <div class="submit-button">
      ...
      <el-button size="small" class="other-button" @click="$router.push('/merchant')">关闭</el-button>
    </div>
  ...
  export default{
      ...
      data(){
          return {
            ...
            rules:{
              no:[{required:true,message: '商户编号不能为空',trigger:'blur'}],
              ...
              companyTypeId:[{required:true,message: '商户类型不能为空',trigger:'blur'}],
              corporation:[{required:true,message: '商户法人不能为空',trigger:'blur'}],
...
//          console.log(this.imgUrl);
        ...
        submit(){
          ...
          if(vm.formData.no==null){this.$alert('商户编号不能为空');return}
          if(vm.formData.name==null){this.$alert('商户名称不能为空');return}
          if(vm.formData.leaderName==null){this.$alert('商户联系人不能为空');return}
          if(vm.formData.telephone==null){this.$alert('商户联系电话不能为空');return}
          if(vm.formData.accountName==null){this.$alert('银行账号名不能为空');return}
          if(vm.formData.account==null){this.$alert('银行账号不能为空');return}
          if(vm.formData.companyTypeId==null){this.$alert('请选择商户类型');return}
          if(vm.formData.corporation==null){this.$alert('商户法人不能为空');return}
          if(vm.formData.certificateNo==null){this.$alert('身份证号码不能为空');return}
          if(vm.formData.areaId==null){this.$alert('请选择归属区域');return}
          if(vm.formData.address==null){this.$alert('商户地址不能为空');return}
