...
RemoteCall.prototype.init = function(opt){
	...
	if(!this.setting.session){
		...
		this.url=this.setting.openUrl+this.setting.router+"?appid=" +this.setting.appid + "&version=" + this.version + "&nonce=" + this.nonce + "&session="+ this.setting.session +"&timestamp=" +
              this.timestamp + "&signature=" + this.makeSignature(this.setting.session, this.data);
	}
   	var _this=this;
    /*
    *contentType设置请求头
    ...
    $.ajax({
      ...
      async: this.setting.async,
        ...
        error:function (resp) {
          ...
          if(resp.responseText=='session null'){
            alert('身份认证失效,请重新登录'+'session null');
...
RemoteCall.prototype.makeSignature = function(session, biz_content){
	this.md5Content = $.md5(biz_content).toUpperCase();
		if(!session){
			...
			var signature="appid="+this.setting.appid +"&biz_content="+this.md5Content + "&nonce="+ this.nonce + "&session="+session+"&timestamp=" +this.timestamp + "&version=" + this.version +
                    "&key=" + this.key;
		}
